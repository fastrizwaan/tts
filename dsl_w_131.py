...
class DictionaryIndexer:
    ...
    def build_index(self, on_progress=None):
        ...
        ngram_index = {}  # NEW multi-length substring index
            ...
            while True:
                ...
                while True:
                    ...
                    if word and word not in offsets_seen:
                        key = word.lower()
                        entries.append({"word": word, "key": key, "offset": pos - len(line_bytes)})
                        offsets_seen.add(word)

                        # --- Multi-length n-grams (1–4), with normalized spaces ---
                        lower = re.sub(r"\s+", " ", key)
                        for n in range(1, 5):
                            if len(lower) < n:
                                continue
                            for i in range(len(lower) - n + 1):
                                ngram = lower[i:i + n]
                                ngram_index.setdefault(ngram, []).append(word)
        ...
        # Save n-gram index
        if ngram_index:
            ngram_path = self.index_path.with_suffix(".ngrams.json")
            with open(ngram_path, "w", encoding="utf-8") as f:
                json.dump(ngram_index, f, ensure_ascii=False, separators=(",", ":"))
            print(f"[DEBUG] Saved n-gram index → {ngram_path} ({len(ngram_index)} keys)")
...
class DictionaryManager:
    ...
    def search(self, q, exact_only=True, max_preview=300, with_defs=False):
        ...
        for dict_path, idx_data in self.index_cache.items():
            ...
            # --- Load n-gram index from correct directory ---
            INDEX_DIR = Path(GLib.get_user_config_dir()) / "dsl-dictionary" / "indexes"
            ngram_path = INDEX_DIR / (Path(idx_data["file"]).name + ".index.ngrams.json")
            ngrams = None
            if ngram_path.exists():
                try:
                    with open(ngram_path, "r", encoding="utf-8") as f:
                        ngrams = json.load(f)
                    print(f"[DEBUG] Loaded n-gram index: {ngram_path}")
                except Exception as e:
                    print(f"[WARN] Failed to load n-gram index: {e}")
            ...
            if ngrams:
                ...
                variants = {sub, " " + sub, sub + " ", sub.rstrip(), sub.strip() + " "}
                ...
                if q_lower == "as":
                    print("[DEBUG] Sample candidates:", sorted(list(candidate_words))[:15])
            else:
            ...
            for e in idx_data.get("entries", []):
                ...
                if word not in candidate_words or word.lower() in seen:
                    continue
                seen.add(word.lower())

                # ---- tier detection (punctuation + spacing tolerant) ----
                def normalized_start(s: str):
                    return s.lstrip(string.punctuation + " ")
                ...
                if norm_key.lower().startswith(q_lower):
                    ...
                    f" {q_lower} " in norm_key
                    or norm_key.endswith(f" {q_lower}")
                    or norm_key.startswith(f"{q_lower} ")
                ):
                    tier = 2
        ...
        candidate_hits.sort(
            key=lambda x: (
                x[0],
                0 if q_lower in x[1].lower() else 1,
                x[1].lower(),
            )
        )
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def _search_worker_loop(self):
        while True:
            ...
            def async_subsearch(local_search_id=search_id, local_query=query):
                ...
                if (
                    ...
                    GLib.idle_add(self._on_search_results, local_query, results2, False)
