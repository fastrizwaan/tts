...
class DictionaryManager:
    ...
    def load_dictionary(self, path, parent_window=None, *args):
        """Load a DSL or DSL.DZ dictionary efficiently (with fast exact lookup map)."""
        ...
        # Try to load existing index if valid
        if idx_path.exists():
            try:
                ...
                stat = path.stat()
                if (
                    abs(idx_data.get("mtime", 0) - stat.st_mtime) < 1
                    and idx_data.get("size") == stat.st_size
                    and idx_data.get("encoding")
                ):
                    rebuild_index = False
            except Exception as e:
                print(f"[WARN] Failed to read existing index: {e}")

        # Rebuild if missing or outdated
        if rebuild_index:
            transient_target = (
                parent_window if isinstance(parent_window, Gtk.Window) else None
            )
            progress_dialog = Adw.MessageDialog(
                transient_for=transient_target,
                modal=True,
                heading=f"Indexing {path.stem}",
                body="Building word index, please wait...",
            )
            ...
            progress_bar.set_margin_start(24)
            progress_bar.set_margin_end(24)
            progress_dialog.set_extra_child(progress_bar)
            ...
            print(
                f"[DEBUG] Indexed dictionary: {idx_data['file']} "
                f"with {len(idx_data['entries'])} headwords "
                f"in {time.time() - start_time:.2f}s."
            )

        # --- Build fast exact lookup map for instant exact searches ---
        exact_map = {}
        word_map = {}  # For n-gram lookups
        for e in idx_data.get("entries", []):
            k = e.get("key") or e.get("word", "").lower()
            if k:
                exact_map[k] = e
            # Also build lowercase word->entry map for fast n-gram lookups
            word_map[e["word"].lower()] = e
        idx_data["_exact_map"] = exact_map
        idx_data["_word_map"] = word_map
        print(f"[DEBUG] Built lookup maps: exact={len(exact_map)}, word={len(word_map)}")

        # Cache for later searches
        self.index_cache[str(path)] = idx_data

        # Extract friendly dictionary name
        denc = idx_data.get("encoding", "utf-8")
        dnname = path.stem
        try:
            with open(path, "rb") as f:
                head = f.read(512)
                text = head.decode(denc, "ignore")
        ...
        self.loaded_dicts[path] = {
            "path": path,
            "name": dnname,
            "encoding": denc,
            "entries": len(idx_data.get("entries", [])),
        }

        print(
            f"[INFO] Loaded dictionary: {dnname} "
            f"({len(idx_data['entries'])} entries, exact-map {len(exact_map)} keys)"
        )
    
    # ------------------------------------------------------------
    def search(self, q, exact_only=True, max_preview=300, with_defs=False):
        """Debug-heavy search that shows file detection and candidate tracing."""
        ...
        print(f"[DEBUG] Starting search for '{q_lower}' (exact_only={exact_only})")

        # --- Tier 0: Exact match ---
        for dict_path, idx_data in self.index_cache.items():
            exact_map = idx_data.get("_exact_map")
            if not exact_map:
                continue
            entry = exact_map.get(q_lower)
            if entry:
                dict_name = Path(idx_data["file"]).stem
                for ld in self.loaded_dicts.values():
                    if Path(ld["path"]) == Path(idx_data["file"]):
                        dict_name = ld.get("name", dict_name)
                        break
                candidate_hits.append((0, entry["word"], dict_name, entry, idx_data))
                seen.add(entry["word"].lower())
                print(f"[DEBUG] Exact hit: {entry['word']} from {dict_name}")

        if exact_only:
            results = [
                (word, [(dname, self._read_entry(Path(idx["file"]), e, idx.get("encoding", "utf-8")) if with_defs else [], "default")])
        ...
        for dict_path, idx_data in self.index_cache.items():
            ...
            file_stem = Path(idx_data["file"]).name
            candidates = [
                INDEX_DIR / f"{file_stem}.ngrams.json",
                INDEX_DIR / f"{file_stem}.index.ngrams.json",
                INDEX_DIR / f"{file_stem}.index.index.ngrams.json",
            ]
            existing = [p for p in candidates if p.exists()]
            if not existing:
                print(f"[CHECK] {dict_name}: No n-gram index found (will fallback)")
                ngram_path = None
            else:
                ngram_path = existing[0]
                print(f"[CHECK] {dict_name}: Using n-gram index {ngram_path}")
            ...
            if ngram_path:
                try:
                    with open(ngram_path, "r", encoding="utf-8") as f:
                        ngrams = json.load(f)
                    print(f"[CHECK] {dict_name}: loaded {len(ngrams)} n-gram keys")
                    print(f"[CHECK] has '^as':", "^as" in ngrams, "has 'as':", "as" in ngrams)
                except Exception as e:
                    print(f"[WARN] Failed to load {ngram_path}: {e}")

            # Use cached word->entry lookup (built once at dictionary load time)
            word_to_entry = idx_data.get("_word_map", {})
            if not word_to_entry:
                # Fallback: build it now if not cached (shouldn't happen with new code)
                print(f"[WARN] {dict_name}: word_map not cached, building now (this is slow!)")
                word_to_entry = {e["word"].lower(): e for e in idx_data.get("entries", [])}

            candidate_words = set()
            if ngrams:
                sub = q_lower[:min(len(q_lower), 4)]
                variants = {sub, " " + sub, sub + " ", " " + sub + " ", "^" + sub}
                found_keys = [v for v in variants if v in ngrams]
                print(f"[CHECK] {dict_name}: variant keys present:", found_keys)
                for v in found_keys:
                    candidate_words.update(ngrams[v])
                if len(candidate_words) < 50:
                    for k in ngrams.keys():
                        if sub in k:
                            candidate_words.update(ngrams[k])
                print(f"[DEBUG] {dict_name}: '{q_lower}' → {len(candidate_words)} candidates via ngrams")
                if q_lower == "as":
                    probes = ["as a formality", "as a group", "as a matter of fact"]
                    print("[DEBUG] Probe membership:", {p: (p in candidate_words) for p in probes})
                    for probe in probes:
                        if probe in candidate_words:
                            probe_entry = word_to_entry.get(probe.lower())
                            print(f"[DEBUG] Probe '{probe}' lookup result: {probe_entry is not None}")
            else:
                for e in idx_data.get("entries", []):
                    key = e.get("key") or e.get("word", "").lower()
                    if pattern.search(key):
                        candidate_words.add(e["word"])
                print(f"[DEBUG] {dict_name}: '{q_lower}' → {len(candidate_words)} candidates via fallback scan")


            matched_count = 0
            # Iterate through n-gram candidates (much faster than scanning all entries!)
            for word in candidate_words:
                word_lower = word.lower()
                if word_lower in seen:
                    continue
                    
                # Look up the entry for this candidate (case-insensitive)
                e = word_to_entry.get(word_lower)
                if not e:
                    continue
                    
                seen.add(word_lower)
                matched_count += 1
                # Use the original word from the entry for display
                original_word = e["word"]
                key = e.get("key") or word_lower

                def normalized_start(s): return s.lstrip(string.punctuation + " ")
                norm_key = normalized_start(key)
                tier = 4
                if norm_key.lower().startswith(q_lower):
                    tier = 1
                elif f" {q_lower} " in norm_key or norm_key.endswith(f" {q_lower}") or norm_key.startswith(f"{q_lower} "):
                    tier = 2
                elif q_lower in norm_key:
                    tier = 3
                candidate_hits.append((tier, original_word, dict_name, e, idx_data))
            
            print(f"[DEBUG] {dict_name}: matched {matched_count} entries from n-gram candidates")
            search_time = time.time() - search_start
            print(f"[TIMING] {dict_name}: search completed in {search_time*1000:.1f}ms")

        candidate_hits.sort(key=lambda x: (x[0], 0 if q_lower in x[1].lower() else 1, x[1].lower()))
        candidate_hits = candidate_hits[:max_preview]
        print(f"[DEBUG] Final candidate count: {len(candidate_hits)}")
        
        # Debug: show tier distribution and sample results
        tier_counts = {}
        phrase_results = []
        for tier, word, dname, _, _ in candidate_hits:
            tier_counts[tier] = tier_counts.get(tier, 0) + 1
            # Track multi-word entries (phrases)
            if ' ' in word and q_lower in word.lower():
                phrase_results.append(f"{word} (tier {tier}, {dname})")
        
        print(f"[DEBUG] Tier distribution: {tier_counts}")
        print(f"[DEBUG] Found {len(phrase_results)} phrase results in final 300")
        if phrase_results and q_lower == "as":
            print(f"[DEBUG] Sample phrases: {phrase_results[:5]}")

        
        # Show first 10 and last 10 results
        sample_first = [f"{word} (tier {tier})" for tier, word, _, _, _ in candidate_hits[:10]]
        sample_last = [f"{word} (tier {tier})" for tier, word, _, _, _ in candidate_hits[-10:]]
        print(f"[DEBUG] First 10 results: {sample_first}")
        print(f"[DEBUG] Last 10 results: {sample_last}")


        for tier, word, dname, e, idx in candidate_hits:
            enc = idx.get("encoding", "utf-8")
            fpath = Path(idx["file"])
            defs = self._read_entry(fpath, e, enc) if with_defs else []
            results.append((word, [(dname, defs, "default")]))

        print(f"[DEBUG] Returning {len(results)} results for '{q_lower}'")
        return results
