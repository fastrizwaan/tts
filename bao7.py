...
class Win(Adw.ApplicationWindow):
    ...
    def apply_column_layout(self, selected_column_index):
        ...
        # Add JavaScript for mouse wheel handling
        js_script = """
        document.addEventListener('wheel', function(e) {
            if (e.deltaX !== 0) return; // Already horizontal scroll
            if (Math.abs(e.deltaY) < 10) return; // Ignore small scrolls
            
            // Calculate column width based on current layout
            const container = document.querySelector('.content-container');
            const computedStyle = window.getComputedStyle(container);
            const containerWidth = container.offsetWidth;
            const colCount = parseInt(computedStyle.columnCount) || 1;
    ...
    def on_size_changed(self, *args):
        # Reapply current layout when window size changes
        # Use idle_add to ensure the UI has finished updating
    ...
    def on_key_pressed(self, controller, keyval, keycode, state):
        ...
        if state & Gdk.ModifierType.CONTROL_MASK:
            return False  # Don't interfere with browser shortcuts
        ...
        if keyval in [Gdk.KEY_Left, Gdk.KEY_Right, Gdk.KEY_Page_Up, Gdk.KEY_Page_Down, Gdk.KEY_Home, Gdk.KEY_End]:
            ...
            # Calculate the width of one column (approximate)
            webview_width = self.webview.get_allocated_width()
            column_width = webview_width / self.current_columns

            if keyval == Gdk.KEY_Left or keyval == Gdk.KEY_Page_Up:
                # Scroll left (previous page)
                scroll_distance = -column_width
                ...
                # Scroll to end
                js_code = "window.scrollTo(document.body.scrollWidth, 0);"
                self.webview.run_javascript(js_code, None, None, None)
                return True
            else:
                return False

            # Execute JavaScript to scroll horizontally
            js_code = f"window.scrollBy({scroll_distance}, 0);"
            self.webview.run_javascript(js_code, None, None, None)
