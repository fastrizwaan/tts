...
def load_file_with_encoding(file_path, encoding=None, max_line_length=10000):
    ...
    Returns tuple: (lines, encoding_used, detection_info, has_line_terminators)
    
    Args:
        file_path: Path to the file
        encoding: Encoding to use (None for auto-detect)
        max_line_length: Maximum line length before auto-wrapping (default 10000)
    """
    if encoding is None:
    ...
    # Detect if file has line terminators and if it's HTML-like
    has_line_terminators = False
    is_html = False
    try:
        with open(file_path, 'rb') as f:
            sample = f.read(100000)  # Read first 100KB
            has_line_terminators = b'\n' in sample or b'\r' in sample
            # Check for HTML/XML content in the actual data
            sample_lower = sample.lower()
            is_html = (b'<html' in sample_lower or 
                      b'<!doctype' in sample_lower or 
                      b'<?xml' in sample_lower or
                      b'<body' in sample_lower or
                      b'<head' in sample_lower or
                      b'<div' in sample_lower or
                      b'<span' in sample_lower or
                      file_path.lower().endswith(('.html', '.htm', '.xml', '.dict', '.xhtml')))
    except Exception as e:
        print(f"Error detecting file properties: {e}")
        has_line_terminators = True  # Assume normal file
        is_html = False
    ...
    try:
        ...
        # If file has line terminators, process normally
        if has_line_terminators:
            lines = content.split('\n')
            lines = [line.rstrip('\r') for line in lines]
            return (lines, encoding, detection_info, True)
        
        # File has no line terminators - need to split
        lines = []
        long_line_count = 0
        
        if is_html and len(content) > max_line_length:
            # Smart HTML splitting at tag boundaries
            detection_info += " [No line terminators - HTML mode]"
            pos = 0
            while pos < len(content):
                # Look for a good break point within max_line_length
                end_pos = min(pos + max_line_length, len(content))
                
                if end_pos < len(content):
                    # Try to find a closing tag near the boundary
                    search_start = max(pos, end_pos - 500)  # Look back up to 500 chars
                    search_text = content[search_start:min(len(content), end_pos + 500)]  # Look ahead up to 500 chars
                    
                    # Find last closing tag before boundary
                    last_close = search_text.rfind('>', 0, end_pos - search_start)
                    if last_close != -1 and last_close > 100:  # Don't split too short
                        end_pos = search_start + last_close + 1
                    else:
                        # Try to find tag boundary after current position
                        next_close = search_text.find('>', end_pos - search_start)
                        if next_close != -1 and next_close < 500:
                            end_pos = search_start + next_close + 1
                
                lines.append(content[pos:end_pos])
                pos = end_pos
                long_line_count += 1
        else:
            # Regular splitting for non-HTML files
            detection_info += f" [No line terminators]"
            for i in range(0, len(content), max_line_length):
                lines.append(content[i:i+max_line_length])
                long_line_count += 1
        
        if long_line_count > 1:
            detection_info += f" [Split into {long_line_count} segments]"
        
        return (lines, encoding, detection_info, False)
        
    except Exception as e:
        error_msg = str(e)
        print(f"Failed to load with {encoding}: {error_msg}. Falling back to UTF-8.")
        # Try again with UTF-8
        try:
            with open(file_path, 'r', encoding='utf-8', errors='replace') as f:
                content = f.read()
            
            # Check for line terminators in the content
            has_line_terminators_utf8 = '\n' in content or '\r' in content
            
            if has_line_terminators_utf8:
                lines = content.split('\n')
                lines = [line.rstrip('\r') for line in lines]
                return (lines, 'utf-8', f"Fallback: utf-8 (error: {error_msg[:50]})", True)
            
            # No line terminators in fallback either
            lines = []
            for i in range(0, len(content), max_line_length):
                lines.append(content[i:i+max_line_length])
            
            return (lines, 'utf-8', f"Fallback: utf-8 [No line terminators, split into {len(lines)} segments]", False)
        except Exception as e2:
            error_msg2 = str(e2)
            print(f"Fallback also failed: {error_msg2}")
            return (["Error loading file"], 'utf-8', f"Error: {error_msg2[:100]}", True)
...
class VirtualTextBuffer:
    ...
    def __init__(self):
        ...
        # Track if file had line terminators originally
        self.has_line_terminators = True  # Default to normal files
    ...
    def save_to_file(self, file_path=None, encoding=None):
        ...
        try:
            with open(path, 'w', encoding=encoding, errors='replace') as f:
                if self.has_line_terminators:
                    # Normal file - save with newlines
                    f.write('\n'.join(self.lines))
                else:
                    # File originally had no line terminators - join without newlines
...
class VirtualTextView(Gtk.DrawingArea):
    ...
    def _on_draw(self, area, cr, width, height):
        ...
        # -------------------------------------------------------------
        # FIRST PASS – LINE NUMBERS WITH BACKGROUND HIGHLIGHTING
        # -------------------------------------------------------------
        y_offset = -(self.scroll_y % self.line_height)
        vis_line = 0
        ...
        for idx, segments in enumerate(wrapped_data):
            ...
            # Draw line number background highlighting for cursor line
            if logical_line == self.cursor_line and not self.has_selection:
                ...
                cr.rectangle(0, y, line_num_width, self.line_height * max(1, len(segments)))
                cr.fill()
                cr.restore()
            ...
            # display line number
            cr.set_source_rgb(0.4, 0.4, 0.4) if not is_dark else cr.set_source_rgb(0.4, 0.4, 0.4)
        ...
        # -------------------------------------------------------------
        # Gutter separator (same color as background - effectively hidden)
        # -------------------------------------------------------------
        cr.set_line_width(1)
        if is_dark:
            cr.set_source_rgb(0.10, 0.10, 0.10)
...
class TextEditorWindow(Adw.ApplicationWindow):
    ...
    def _load_file(self, filepath):
        def load_file():
            try:
                start_time = time.time()
                # Use 100 character line limit as specified
                result = load_file_with_encoding(filepath, max_line_length=100)
                lines, encoding, detection_info, has_line_terminators = result
                load_time = time.time() - start_time
                GLib.idle_add(self._on_file_loaded, lines, load_time, filepath, encoding, detection_info, has_line_terminators)
            except Exception as ex:
                error_message = str(ex)
                GLib.idle_add(self._on_file_error, error_message)
    ...
    def _on_file_loaded(
            ...
            filepath,
            encoding="utf-8",
            detection_info="UTF-8",
            has_line_terminators=True,   # NEW
            *unused                     # idle_add always passes one extra arg
    ):
        buffer = VirtualTextBuffer()
        buffer.load_lines(lines)
        ...
        self.status_bar.set_text(
            f"Loaded {filename} - {len(lines):,} lines in {load_time:.2f}s - {detection_info}"
        )
