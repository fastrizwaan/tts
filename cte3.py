...
class LazyLineIndex:
    ...
    def _run(self):
        ...
        while pos < size:
            ...
            # Find newlines in block
            temp_newlines = []
            for i, byte in enumerate(block):
                if byte == 10:  # \n
                    temp_newlines.append(pos + i)

            # Update with lock
            with self.lock:
                for nl in temp_newlines:
                    self.line_offsets.append(nl + 1)  # Start of next line
                self.indexed_up_to = end
    ...
    def line_count(self):
        """Get actual line count (blocking until indexing is complete)"""
...
class VirtualTextBuffer:
    ...
    def save_to_file(self, path):
        """Save buffer to file (simplified implementation)"""
        # For now, just write all lines to the file
        with open(path, 'w', encoding='utf-8') as f:
            ...
            while True:
                ...
                if line == "":
                    break
                f.write(line)
                if line_no < self.line_count() - 1:  # Don't add newline after last line
                    f.write('\n')
                line_no += 1
...
class EditorWindow(Adw.ApplicationWindow):
    ...
    def load_file(self, path):
        ...
        self.view.set_content_height = lambda h: self.view.set_size_request(-1, h)
