...
#        menu_button.set_size_request(16, 20)
    ...
    def on_tab_move_new_window(self, action, parameter):
        ...
        # Check if modified by looking at tab CSS class
        is_modified = False
        for tab in self.tab_bar.tabs:
            if hasattr(tab, '_page') and tab._page == page:
        ...
        # Create new window (it will have an initial empty "Untitled N" tab)
        app = self.get_application()
        new_window = EditorWindow(app)
        new_window.present()
        
        # Get the initial tab that was created automatically
        new_page = new_window.tab_view.get_selected_page()
        new_editor = new_page.get_child()._editor
        
        # IMPORTANT: Decrement the global counter because we're reusing the initial tab
        # instead of creating a new one. The initial tab incremented the counter,
        # but we're replacing its content with the moved tab's content.
        if app and isinstance(app, VirtualTextEditor):
            app._global_untitled_counter -= 1
        
        # Transfer state to the existing initial tab
        new_editor.set_text(content)
        new_editor.current_file_path = file_path
        new_editor.set_title(title)
        
        # Update title in the tab
        new_page.set_title(title)
        
        # Find the corresponding ChromeTab and set modified state
...
class VirtualTextEditor(Adw.Application):
    # Global untitled counter shared across all windows
    _global_untitled_counter = 0
    
    @classmethod
    def get_next_global_untitled_number(cls):
        """Get the next global untitled number and increment the counter"""
        cls._global_untitled_counter += 1
        return cls._global_untitled_counter
