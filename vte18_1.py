        ...
        if x > line_num_width:
            ...
            if logical_line_num is not None:
                ...
                # ---------------------------
                    ...
                    # Only create viewport selection if clicking significantly beyond text (not just on last char)
                    # and the line is empty or we're clearly past all text
                    should_select_to_viewport = False
                    
                    if len(line_text) == 0:
                        # Empty line - don't create flashing selection, just place cursor
                        self.cursor_col = 0
                        self.has_selection = False
                    elif clicked_on_newline_visual and self.cursor_col >= len(line_text):
                        # Click is clearly beyond the text (not on the last character)
                        # Only create viewport selection if click is significantly past the text
                        text_pixel_width = self._get_text_width(line_text)
                        # Require click to be at least a full character width beyond text
                        if click_x_local > text_pixel_width + self.char_width:
                    ...
                    if should_select_to_viewport:
                        ...
                        # Real double-click on text (including last glyph): select a word or whitespace block
                        ...
                        # If character at effective_col is whitespace -> select whitespace block
