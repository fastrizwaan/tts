...
class MainWindow(Adw.ApplicationWindow):
    ...
    def build_html(self, results):
            ...
            for dname, defs, color in dict_data:
                ...
                for i, d in enumerate(defs):
                    ...
                    if m_content:
                        ...
                        # Remove formatting tags temporarily to check content
                        inner_plain = re.sub(r'\[/?[biu]\]', '', inner)
                        inner_plain = re.sub(r'\[sup\].*?\[/sup\]', '', inner_plain)
                        inner_plain = inner_plain.strip()
                        
                        # Check if it's a list item (but not just the headword marker ~)
                        is_numbered_m = (
                            (re.match(r'^\d+[\.\)》]\s', inner_plain) or 
                             '■' in inner[:10]) and 
                            inner_plain != '~'
                        )
                    else:
                    ...
                    if is_numbered_m or is_numbered:
                        ...
                        if not in_list:
                            defs_html += "<ul style='list-style-type: none; padding-left: 0;'>"
                            in_list = True
                        
                        # Add the numbered definition to current item
                        current_li_content.append(self.render_dsl_text(d, is_main=False, headword=word))
                    else:
                        ...
                        # This is a sub-item or standalone content
                        if current_li_content:
                            ...
                            # Standalone content (like "noun", headword markers, etc.)
        ...
        ul { margin: 0.5em 0; padding-left: 0; line-height: 1.6; list-style-type: none; }
    ...
    def render_dsl_text(self, raw, is_main=False, headword=""):
        ...
        # Don't remove list markers - keep them as is (1》, 2》, ■, etc.)
