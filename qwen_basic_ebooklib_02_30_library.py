...
class EPubViewer(Adw.ApplicationWindow):
    ...
    def show_library(self):
        ...
        for entry in list(reversed(self.library)):
            ...
            if cover and os.path.exists(cover):
                try:
                    pix = GdkPixbuf.Pixbuf.new_from_file_at_scale(cover, 160, 240, True)
                    tex = Gdk.Texture.new_for_pixbuf(pix)
                    img.set_from_paintable(tex)
                except Exception:
                    pass
            else:
                pb = GdkPixbuf.Pixbuf.new(GdkPixbuf.Colorspace.RGB, False, 8, 160, 240)
                pb.fill(0xddddddff)
    ...
    def cleanup(self):
        ...
        self.cover_image.set_from_paintable(Gdk.Texture.new_for_pixbuf(GdkPixbuf.Pixbuf.new(GdkPixbuf.Colorspace.RGB, False, 8, COVER_W, COVER_H)))

    # ---------- Library helpers ----------
    def _update_library_entry(self):
        # create or update library entry for current book
        ...
        if cover_src and os.path.exists(cover_src):
            try:
                # create deterministic filename based on book path
