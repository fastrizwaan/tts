...
class DictionaryManager:
    ...
    def search(self, q, exact_only=True, max_preview=100, with_defs=False):
        """
        Tiered, case-insensitive search:
          0. Exact (O(1))
          1. Prefix (startswith)
          2. Proximity (query appears early in word, like 'immu' → 'autoimmune')
          3. Substring (fallback, anywhere)
        """
        q = q.strip()
        if not q:
            return []
        ...
        q_lower = q.lower()
        results, seen_words = [], set()

        # --- Tier 0: Exact lookup (O(1)) ---
        for dict_path, idx_data in self.index_cache.items():
            ...
            if entry:
                ...
                seen_words.add(q_lower)

        if exact_only:
            return results

        # --- Prepare regex once for proximity & substring ---
        pattern = re.compile(re.escape(q_lower), re.IGNORECASE)

        prefix_hits, proximity_hits, substring_hits = [], [], []
        ...
        for dict_path, idx_data in self.index_cache.items():
            ...
            for e in idx_data.get("entries", []):
                ...
                # Tier 2: Proximity (query appears near start, within first 3 chars)
                m = pattern.search(key)
                if m:
                    start_pos = m.start()
                    if start_pos <= 3:
                        defs = self._read_entry(fpath, e, enc) if with_defs else []
                        proximity_hits.append((e["word"], [(dict_name, defs, "default")]))
                        seen_words.add(key)
                        continue

                    # Tier 3: Substring (anywhere else, slower)
                    if start_pos > 3:
                        defs = self._read_entry(fpath, e, enc) if with_defs else []
                        substring_hits.append((e["word"], [(dict_name, defs, "default")]))
                        seen_words.add(key)

                # Stop if too many
                if len(results) + len(prefix_hits) + len(proximity_hits) + len(substring_hits) >= max_preview:
                    break

        # Sort within tiers alphabetically for predictability
        prefix_hits.sort(key=lambda x: x[0])
        proximity_hits.sort(key=lambda x: x[0])
        substring_hits.sort(key=lambda x: x[0])

        # Merge tiers: exact → prefix → proximity → substring
        results.extend(prefix_hits)
        results.extend(proximity_hits)
        results.extend(substring_hits)
