...
class VirtualTextView(Gtk.DrawingArea):
    ...
    def _delete_selection(self):
        ...
        # Commit any pending edits so deletion is tracked properly
        self._commit_word_to_undo()
        if self.editing:
            self._finish_editing()

        start_line, start_col = self.selection_start_line, self.selection_start_col
        end_line, end_col = self.selection_end_line, self.selection_end_col
        if start_line > end_line or (start_line == end_line and start_col > end_col):
            start_line, end_line = end_line, start_line
            start_col, end_col = end_col, start_col

        # Gather text for undo stack
        deleted_parts = []
        for line in range(start_line, end_line + 1):
            line_text = self.buffer.get_line(line)
            if line == start_line and line == end_line:
                deleted_parts.append(line_text[start_col:end_col])
            elif line == start_line:
                deleted_parts.append(line_text[start_col:])
            elif line == end_line:
                deleted_parts.append(line_text[:end_col])
            else:
                deleted_parts.append(line_text)
        deleted_text = '\\n'.join(deleted_parts)

        # Store delete action for undo
        self.buffer.add_undo_action('delete', {
            'line': start_line,
            'pos': start_col,
            'end': end_col,
        ...
        if start_line == end_line:
            ...
            start_text = self.buffer.get_line(start_line)[:start_col]
            end_text = self.buffer.get_line(end_line)[end_col:]
            self.buffer.lines[start_line:end_line + 1] = [start_text + end_text]
            self.buffer.total_lines = len(self.buffer.lines)

        # Reset selection and update view
        self.has_selection = False
        self.selection_start_line = self.selection_start_col = -1
        self.selection_end_line = self.selection_end_col = -1
    ...
    def _cut_to_clipboard(self):
        """Cut selection with undo support."""
        if not self.has_selection:
            return
        ...
        # Copy to clipboard
        self.clipboard.set_text(deleted_text)
