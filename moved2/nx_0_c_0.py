...
class EPUBWindow(Adw.ApplicationWindow):
    ...
    def on_open_clicked(self, button):
        ...
        # 1. Create the EPUB filter
        epub_filter = Gtk.FileFilter()
        epub_filter.set_name("EPUB files")
        epub_filter.add_pattern("*.epub")
        
        # 2. Create the Gio.ListStore container for filters
        filter_store = Gio.ListStore.new(Gtk.FileFilter)
        
        # 3. Add the filter to the ListStore using 'append'
        filter_store.append(epub_filter)
        
        # Optional: Add an "All Files" filter
        all_filter = Gtk.FileFilter()
        all_filter.set_name("All files")
        all_filter.add_pattern("*")
        filter_store.append(all_filter)
        
        # 4. Set the ListStore on the dialog
        dialog.set_filters(filter_store)
        
        # Set the default filter (optional but good practice)
    ...
    def apply_layout(self):
        ...
        # Determine the total width of the content when using columns
        # This is VITAL for horizontal scrolling. WebKit's body width must be
        # large enough to contain all virtual columns.
        viewport_width = self.scrolled_window.get_width()
        
        # Calculate the actual column count visible in the viewport.
        # This helps in sizing the HTML body to match the scrolled width correctly.
        # When columns > 1, we set the body width to match the columns required
        # for the content height, but we need the content to be wider than the viewport
        # to enable horizontal scrolling.
        
        # When column-count is specified as an integer, WebKit calculates the content
        # width based on the content. When column-width is specified, the content
        # width is potentially infinite, which we want to leverage for horizontal scrolling.

        # We will use JavaScript to calculate the required scroll width post-CSS
        # application and then use the CSS below to force column layout.

        # Build robust CSS with !important
        # The key change is forcing overflow-y: hidden and applying column-fill: auto
        ...
        # Manually ensure WebView is wide enough for the ScrolledWindow's viewport
        # in single-column mode, and potentially wider in multi-column.
        # This is mostly handled by the CSS 'width: max-content' but an explicit
        # request might help. We rely primarily on the CSS.
