...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # Library button to return to library view
        ...
        self.library_btn.connect("clicked", lambda *_: self.show_library())
        self.content_header.pack_start(self.library_btn)
    ...
    def show_library(self):
        # Build FlowBox grid with covers + titles (display using LIB_DISPLAY_* size)
        flow = Gtk.FlowBox()
        ...
        flow.set_margin_top(20); flow.set_margin_bottom(20)
        flow.set_row_spacing(18); flow.set_column_spacing(18)
        ...
        for entry in list(reversed(self.library)):  # most recent first
            ...
            cover_box.set_margin_bottom(0)
            cover_box.set_halign(Gtk.Align.CENTER)
            img = Gtk.Image()
            img.set_size_request(LIB_DISPLAY_W, LIB_DISPLAY_H)  # Add this line
            if cover and os.path.exists(cover):
                try:
                    pix = GdkPixbuf.Pixbuf.new_from_file_at_scale(cover, LIB_DISPLAY_W, LIB_DISPLAY_H, True)
                    tex = Gdk.Texture.new_for_pixbuf(pix)
                    img.set_from_paintable(tex)
                except Exception:
                    pb = GdkPixbuf.Pixbuf.new(GdkPixbuf.Colorspace.RGB, False, 8, LIB_DISPLAY_W, LIB_DISPLAY_H)
                    pb.fill(0xddddddff)
            ...
            # meta row: progress text + small spacer
            ...
            meta.append(prog_lbl)
            card.append(meta)

            # clickable: make entire card a flat button so it has click affordance
            btn = Gtk.Button()
            ...
            def _on_open(_b, p=path, resume_idx=idx):
        ...
        # put grid inside a centred container with some extra padding
        container = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        container.append(scroll)
        container.set_margin_start(8); container.set_margin_end(8)
