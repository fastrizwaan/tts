...
class MainWindow(Adw.ApplicationWindow):
    ...
    def generate_audio(self, text):
        try:
            ...
            # Split audio into word segments
            word_samples = self.split_audio_by_words(samples, sample_rate)
            
            # Put segments in queue
            for i, segment in enumerate(word_samples):
                # Create temporary WAV file for each segment
    ...
    def split_audio_by_words(self, samples, sample_rate):
        # Simple approximation: split evenly based on word count
        word_count = len(self.app.word_positions)
        if word_count == 0:
            return [samples]
            
        samples_per_word = len(samples) // word_count
        segments = []
        
        for i in range(word_count):
            start = i * samples_per_word
            end = start + samples_per_word if i < word_count - 1 else len(samples)
    ...
    def audio_playback_worker(self):
        while True:
            try:
                index, temp_file, sample_rate = self.app.playback_queue.get(timeout=0.1)
                
                if index is None:  # End of stream
                    break
                    
                # Highlight current word
                GLib.idle_add(self.highlight_word, index)
