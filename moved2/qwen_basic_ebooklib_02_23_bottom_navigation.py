...
THEME_INJECTION_CSS = """
/* user-provided theme snippet: light/dark switch */
...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # Book cover + metadata: top-aligned to avoid wasted space
        book_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
        book_box.set_valign(Gtk.Align.START)
        book_box.set_margin_top(4)
        book_box.set_margin_bottom(6)
        book_box.set_margin_start(8)
        book_box.set_margin_end(8)

        # placeholder cover paintable (no undefined variables in __init__)
        ...
        md_v = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4)
        md_v.set_valign(Gtk.Align.START)
        ...
        self.content_sidebar_toggle = Gtk.Button(icon_name="sidebar-show-symbolic")
        ...
        # pack_end places it on the right side of header; keep that
        ...
        # --- window blur handling: hide sidebar when unpinned and window loses focus ---
        # Use EventControllerFocus attached to the window to detect leave events (window losing focus)
        try:
            ec_focus = Gtk.EventControllerFocus()
            ec_focus.connect("leave", lambda *_: self._on_window_blur())
            self.add_controller(ec_focus)
        except Exception:
            # fallback: listen to display focus-changed notifications and test is_active()
            try:
                disp = Gdk.Display.get_default()
                if disp:
                    disp.connect("notify::focus-window", lambda *_: self._on_window_blur())
            except Exception:
                pass
    ...
    def _on_pin_toggled(self, btn):
        ...
        # prefer set_pin_sidebar API, otherwise fallback to show_sidebar behavior
        try:
            if hasattr(self.split, "set_pin_sidebar"):
                self.split.set_pin_sidebar(pinned)
            else:
                # pinned => ensure sidebar shown and overlay disabled (keep visible)
            ...
            try:
                ...
                pass
        # when unpinned, if window not active immediately hide sidebar
        if not pinned:
            # hide immediately when unpinned and window is not active
            try:
                # if the window is not active => hide sidebar
    ...
    def _on_window_blur(self):
        # called when the window lost focus (via EventControllerFocus.leave or display notify)
        try:
            if getattr(self, "pin_btn", None) and not self.pin_btn.get_active():
                # unpinned => hide sidebar immediately
                self.split.set_show_sidebar(False)
        except Exception:
            pass
    ...
    def _on_sidebar_toggle(self, btn):
        ...
        try:
            ...
            pass
        icon = "sidebar-hide-symbolic" if new else "sidebar-show-symbolic"
        try:
    ...
    def _filter_toc(self, text):
        self.tab_toc.set_active(True)
        # TODO: implement filtering of Gio.ListStore
        return
    ...
    def load_epub(self, path):
        try:
            ...
            # cover loading: prefer items with "cover" in name; fallback to first image
            try:
                cov = None
                ...
                for it in images:
                    ...
                    if "cover" in name:
                        cov = it
                        break
                if cov is None and images:
                    cov = images[0]

                if cov:
                    fname = cov.get_name() or f"image_{os.urandom(6).hex()}.img"
                    p = os.path.join(self.temp_dir, os.path.basename(fname))
                    with open(p, "wb") as fh:
                        fh.write(cov.get_content())
                    pix = GdkPixbuf.Pixbuf.new_from_file_at_scale(p, COVER_W, COVER_H, True)
    ...
    def display_page(self, fragment=None):
        ...
        try:
            ...
            content = str(soup)
            # Inject extracted CSS and theme CSS
