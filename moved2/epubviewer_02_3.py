...
#####################
    ...
    def _create_rounded_cover_texture(self, cover_path, width, height, radius=10):
        ...
        try:
            pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(cover_path, width, height, True)
            
            # Create a surface with alpha channel (transparency)
            ...
            # Paint the image into the rounded clip
    ...
    def show_library(self):
        ...
        for entry in entries:
            ...
            img = Gtk.Picture()
            img.set_size_request(160, 200)
            img.set_can_shrink(True)

            if cover and os.path.exists(cover):
                texture = self._create_rounded_cover_texture(cover, 160, 200, radius=10)
                if texture:
            ...
            t.set_margin_bottom(2)  # Reduce bottom margin
            t.set_markup(highlight_markup(title, self.library_search_text))
            card.append(t)
            ...
            meta_row.set_margin_bottom(4)  # Small bottom margin
