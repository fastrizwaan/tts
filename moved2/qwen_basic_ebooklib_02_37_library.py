...
# sidebar small CSS (rounded hover band, margin) + author dim for library card
_css = """
...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # create open button and put it in content header (swapped)
    ...
    def show_library(self):
        ...
        for entry in list(reversed(self.library)):  # most recent first
            ...
            # author (center) - expand to take available space and center
            ...
            # Connect remove with confirmation
            def confirm_and_remove(btn, p=path, coverp=cover):
                dlg = Adw.MessageDialog.new(self, "Confirm", f"Remove \"{os.path.basename(p)}\" from library?")
                    ...
                    try:
                        if resp == "ok":
                            try:
                                ...
                                # delete cover if inside COVERS_DIR
                                try:
            ...
            rem_btn.connect("clicked", confirm_and_remove)
    ...
    def on_library_clicked(self):
        # when EPUB loaded and library clicked -> close EPUB, hide sidebar, show library
        try:
            if self.book:
                try:
                    self.cleanup()
    ...
    def load_epub(self, path, resume=False, resume_index=None):
        try:
            ...
            # If TOC exists, auto-show sidebar (requested)
            try:
