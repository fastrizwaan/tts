...
class EPubViewer(Adw.ApplicationWindow):
    ...
    def load_epub(self, path):
        try:
            ...
            # ---------- robust cover loading (prioritize OPS/cover.xhtml) ----------
            try:
                ...
                # prefer explicit OPS/cover.xhtml or OEBPS/cover.xhtml then generic cover documents
                cover_doc = None
                # added titlepage variants as requested
                priority_names = (
                ...
                for pn in priority_names:
                    if pn in lower_map:
                        ...
                        break
                if not cover_doc:
                    # fallback: any doc name that contains '/ops/cover' or endswith 'cover.xhtml'
                    for d in docs_list:
                        n = (d.get_name() or "").lower()
                        if "/ops/cover" in n or n.endswith("cover.xhtml") or n.endswith("cover.html") or "cover.xhtml" in n:
                            cover_doc = d
                            break
                # if still not found, try class-based detection
                if not cover_doc:
                    for d in docs_list:
                        try:
                ...
                # parse cover_doc for <img src> and svg <image xlink:href|href>
                if cover_doc:
                    try:
                ...
                # ---------- NEW FALLBACK: inspect content.opf (package) for cover metadata ----------
                if not cover_path_to_use and not cover_item_obj:
                    # common OPF locations relative to extracted temp_dir
                    opf_candidates = [
                        os.path.join(self.temp_dir, "OEBPS", "content.opf"),
                        os.path.join(self.temp_dir, "content.opf"),
                        os.path.join(self.temp_dir, "package.opf"),
                        os.path.join(self.temp_dir, "OPS", "content.opf"),
                    ]
                    opf_found = None
                    ...
                    if opf_found:
                        try:
                            with open(opf_found, "rb") as fh:
                                opf_data = fh.read()
                            opf_soup = BeautifulSoup(opf_data, "xml")
                            # EPUB2 style: <meta name="cover" content="cover-image"/>
                            meta = opf_soup.find(lambda tag: tag.name == "meta" and tag.has_attr("name") and tag["name"].lower() == "cover")
                            cover_id = None
                            if meta and meta.has_attr("content"):
                                cover_id = meta["content"]
                            # EPUB3 style: manifest item with properties="cover-image"
                            if not cover_id:
                                it_prop = opf_soup.find(lambda tag: tag.name == "item" and tag.has_attr("properties") and "cover-image" in tag["properties"])
                                if it_prop and it_prop.has_attr("id"):
                                    cover_id = it_prop["id"]
                            # fallback: item whose id or href contains 'cover'
                            if not cover_id:
                                it_guess = opf_soup.find(lambda tag: tag.name == "item" and (tag.has_attr("id") and "cover" in tag["id"].lower() or tag.has_attr("href") and "cover" in tag["href"].lower()))
                                if it_guess and it_guess.has_attr("id"):
                                    cover_id = it_guess["id"]
                            if cover_id:
                                # find corresponding item element in manifest to get href
                                item_tag = opf_soup.find(lambda tag: tag.name == "item" and tag.has_attr("id") and tag["id"] == cover_id)
                                href = None
                                if item_tag and item_tag.has_attr("href"):
                                    href = item_tag["href"]
                                # if no manifest match, try searching by id in other ways
                                if not href:
                                    maybe = opf_soup.find(lambda tag: tag.name == "item" and tag.has_attr("href") and (tag["href"].endswith(".jpg") or tag["href"].endswith(".png") or "cover" in tag.get("href","").lower()))
                                    if maybe and maybe.has_attr("href"):
                                        href = maybe["href"]
                                if href:
                                    # candidate relative to opf directory
                                    opf_dir = os.path.dirname(opf_found) or ""
                                    candidate_rel = os.path.normpath(os.path.join(opf_dir.replace(self.temp_dir + os.sep, ""), href)).replace("\\", "/")
                                    # candidate_rel may be relative to OEBPS/ or OPS/, construct variants
                                    for v in variants_for(candidate_rel):
                                        if v in extracted_paths:
                                            cover_path_to_use = os.path.join(self.temp_dir, v)
                                            break
                ...
                # fallback: try any image item whose name contains 'cover'
                if not cover_path_to_use and not cover_item_obj:
                ...
                # final fallback: any extracted image file
                if not cover_path_to_use and not cover_item_obj:
    ...
    def handle_internal_link(self, uri):
        ...
        # Attempt to match to spine documents first
        ...
        for cand in candidates:
            # match in spine items
            for i, it in enumerate(self.items):
                try:
                    name = it.get_name()
                except Exception:
                    name = None
                if name == cand or (name and name.endswith(cand)) or name in candidates:
                    self.current_index = i
                    self.update_navigation()
                    # when fragment present, pass it to display_page to scroll after load
                    self.display_page(fragment=fragment)
                    # mark toc selection if available
        ...
        # If not a spine doc: check if it's an extracted file on disk (images, standalone html, etc.)
        # Prefer temp_dir-relative path
        possible_paths = []
        if self.temp_dir:
        ...
        for p in possible_paths:
            if not p:
                continue
            if os.path.exists(p):
                # If it's an image or other non-html asset, wrap it in HTML and inject extracted CSS so styles apply
                ext = os.path.splitext(p)[1].lower()
                if ext in (".png", ".jpg", ".jpeg", ".gif", ".webp", ".svg"):
                    # Build an HTML wrapper that references the actual file via file:// and includes extracted CSS
                    ...
                    try:
                        ...
                        pass
                    return True
                # If it's an HTML/XHTML file not part of spine, load it but inject CSS
                if ext in (".html", ".xhtml", ".htm"):
                    try:
                        ...
                        pass
                # Otherwise let webkit handle it (e.g. CSS file or unknown): return False to allow default handling
                return False
