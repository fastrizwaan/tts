        ...
        def after(webview, result, user_data=None):
            try:
                ...
                try:
                    ...
                    d = safe_json_parse(json_str_raw)

                    
                    # Check if the parsed value is a dictionary (object) as expected
                    if not isinstance(d, dict):
                        print(f"Warning: Expected object from JS, got {type(d).__name__}: {d}", file=sys.stderr)
                        return
    ...
    def update_nav_buttons(self, *args):
        ...
        def finish(webview, result, user_data=None):
            try:
                ...
                self.prev_btn.set_sensitive(self._has_prev_spine())
                self.next_btn.set_sensitive(self._has_next_spine())
        def safe_json_parse(json_str_raw):
            """Safely handle WebKit JSON results (sometimes double-encoded)."""
            if not json_str_raw:
                return None
            try:
                data = json.loads(json_str_raw)
                # If still a string (double encoding), decode once more
                if isinstance(data, str):
                    data = json.loads(data)
                return data
            except Exception as e:
                print(f"JSON parse error: {e}, Raw: {json_str_raw}", file=sys.stderr)
                return None
