...
class OverlayText:
    def __init__(self, x, y, text='New Text', font_size=12, font_family='Sans', bold=False):
        self.x = x
        self.y = y
        self.text = text
...
class PDFEditor(Adw.Application):
    ...
    def on_add_text(self, button):
        ...
        overlays.append(OverlayText(100, 100, text_content, font_size, font_family, bold))
        self.drawing_area.queue_draw()
    ...
    def on_add_image_finish(self, dialog, result):
        try:
            ...
            if file:
                ...
                overlays.append(OverlayImage(50, 50, path, width, height))
                self.drawing_area.queue_draw()
        except GLib.Error as e:
    ...
    def on_drag_update(self, gesture, dx, dy):
        if self.selected_overlay:
            ...
            self.selected_overlay.x += adj_dx
            self.selected_overlay.y += adj_dy
            self.drawing_area.queue_draw()
