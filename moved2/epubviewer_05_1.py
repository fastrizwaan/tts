...
@media (prefers-color-scheme: dark) {
    body { background-color:#242424; color:#e3e3e3; }
    blockquote { border-left-color:#62a0ea; }
    .tts-highlight { background:rgba(100,200,100,0.4) !important; box-shadow:0 0 0 2px rgba(100,200,100,0.6) !important; border-radius:3px; transition:background 0.3s ease; }
}
@media (prefers-color-scheme: light) {
    .tts-highlight { background:rgba(255,255,100,0.5) !important; box-shadow:0 0 0 2px rgba(255,200,0,0.6) !important; border-radius:3px; transition:background 0.3s ease; }
}
"""
_dark_override_css = """
.epub-sidebar .book-author { color: rgba(255,255,255,0.6); }
...
class EPubViewer(Adw.ApplicationWindow):
    ...
    def _load_file_with_css(self, file_path, fragment=None):
        """Load a file (HTML or image) with CSS applied"""
        ...
        if ext in (".html", ".xhtml", ".htm"):
            try:
                ...
                # Wrap text with sentence markers
                soup = self._wrap_text_with_sentence_markers(soup)
    ...
    def _wrap_html(self, raw_html, base_uri):
        ...
        # Enhanced link intercept script with TTS highlighting support
        # Enhanced link intercept script with TTS highlighting support
        ...
        (function(){
            ...
            window.tts_highlightSentence = function(sentenceIndex) {
                console.log('[TTS-JS] Highlighting sentence:', sentenceIndex);
                
                // Clear previous highlights
                window.tts_clearHighlight();
                
                // Find and highlight the sentence
                var selector = '[data-tts-sentence="' + sentenceIndex + '"]';
                var elements = document.querySelectorAll(selector);
                
                console.log('[TTS-JS] Found elements:', elements.length);
                ...
                if (elements.length > 0) {
                    ...
                    // Scroll first element into view smoothly
                    if (firstElement) {
                ...
                console.log('[TTS-JS] No elements found for sentence', sentenceIndex);
                return false;
            };
        })();
        </script>
    ...
    # ============================================================================
    # PATCH 2: Add method to wrap text nodes with sentence markers
    # ============================================================================
    # Add this new method to the EPubViewer class:

    def _wrap_text_with_sentence_markers(self, soup):
        """Wrap text nodes with span elements that have sentence IDs for TTS highlighting"""
        if not self.tts:
            return soup
        ...
        try:
            ...
            sentences = self.tts.split_sentences(text_content)
            
            if not sentences:
                return soup
            
            # Build a character-to-sentence-index map
            char_to_sent = []
            current_pos = 0
            for idx, sent in enumerate(sentences):
                sent_len = len(sent)
                char_to_sent.extend([idx] * sent_len)
                current_pos += sent_len
            
            # Get all text content with positions
            all_text = soup.get_text()
            
            # Function to find sentence index for a text fragment
            def find_sentence_idx(text_fragment):
                if not text_fragment.strip():
                    return None
                # Find where this fragment appears in the full text
                pos = all_text.find(text_fragment.strip())
                if pos >= 0 and pos < len(char_to_sent):
                    return char_to_sent[pos]
                return None
            ...
            def wrap_text_nodes(element):
                for child in list(element.children):
                    if isinstance(child, NavigableString):
                        ...
                        sent_idx = find_sentence_idx(str(child))
                        
                        if sent_idx is not None:
                            # Create a span with the sentence ID
                            span = Tag(name='span')
                            span['data-tts-sentence'] = str(sent_idx)
                            span.string = str(child)
                            child.replace_with(span)
                    elif child.name:  # It's a tag
                        # Skip script and style tags
                        if child.name not in ['script', 'style']:
                            wrap_text_nodes(child)
            
            # Start wrapping from body
            body = soup.find('body')
            if body:
                wrap_text_nodes(body)
            
            return soup
        except Exception as e:
            print(f"[error] Failed to wrap text with sentence markers: {e}")
            import traceback
            traceback.print_exc()
            return soup
 ...
 or "", parent_store)
    ...
    def display_page(self, fragment=None):
        ...
        try:
            ...
            # Wrap text with sentence markers for TTS
            soup = self._wrap_text_with_sentence_markers(soup)
...
, "r", encoding="utf-8", errors="ignore") as fh:
    ...
    def _on_tts_highlight(self, idx, meta):
        """Highlight callback for TTS - visual feedback in WebView"""
        if idx < 0:
        ...
        if meta:
            ...
            if self.webview:
                js_code = f"""
                (function() {{
                    console.log('[TTS-PY] Attempting to highlight sentence {idx}');
                    if(window.tts_highlightSentence) {{ 
                        var result = window.tts_highlightSentence({idx});
                        console.log('[TTS-PY] Highlight result:', result);
                        return result;
                    }} else {{
                        console.log('[TTS-PY] tts_highlightSentence function not found!');
                        return false;
                    }}
                }})();
                """
                try:
                    ...
                    try:
                        ...
                        print(f"[error] Failed to highlight sentence {idx}: {e}")
