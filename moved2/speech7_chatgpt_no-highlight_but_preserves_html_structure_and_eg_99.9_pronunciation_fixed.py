...
DEFAULT_HTML = 'here e.g., is the latest 99.9 true message.'
LANG = "en-us"
VOICE = "af_sarah"
SPEED = 1.0
SR = 24000
...
def tokenize_words(sentence: str) -> list:
    """Tokenize a sentence into words, preserving punctuation"""
    # Split on whitespace but keep delimiters
    parts = re.split(r'(\s+)', sentence)
    words = []
    for part in parts:
        if part.strip():  # Non-whitespace
            words.append(part)
    return words
...
<head>
  ...
  <script>
    // Wrap text nodes in-place with word spans, preserving markup.
    // Sentence index increments on .,!,? not between digits.
    function prepareHighlighting(){
      ...
      let s = 1, w = 0;
      const reSplit = /((?<!\\d)[.!?](?!\\d)|\\s+)/g; // keep delimiters
      const nodes = [];
      ...
      for (const textNode of nodes){
        ...
        for (const part of parts){
          ...
          if (/^(?<!\\d)[.!?](?!\\d)$/.test(part)){ // sentence end punct
            // keep the punctuation as normal text to preserve look
            frag.appendChild(document.createTextNode(part));
            s += 1; w = 0;
            continue;
          }
          ...
          frag.appendChild(document.createTextNode(' ')); // keep spacing visual
          w += 1;
        }
        ...
        try:
            while not ctrl.stop.is_set():
                ...
                if current_playing in buf and not ctrl.stop.is_set():
                    ...
                    bytes_per_word = n // max(word_count, 1) if word_count > 0 else n
                    ...
                    while off < n and not ctrl.stop.is_set():
                        ...
                        # Calculate current word based on bytes played
                        if word_count > 0:
                            current_word = min((off // bytes_per_word), word_count - 1)
                            if current_word != ctrl.current_word:
                                ctrl.current_word = current_word
