...
from gi.repository import Gtk, Adw, Gio, GtkSource, GLib
...
class TTSApplication(Adw.Application):
    ...
    def on_toggle_syntax(self, btn):
        ...
        if enabled and not self.buffer.get_language():
            # fallback guess from buffer content if no language set
            text = self.buffer.get_text(
                self.buffer.get_start_iter(),
                self.buffer.get_end_iter(),
                True
            )
            lang = self.language_manager.guess_language(None, text)
            self.buffer.set_language(lang)
