...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # larger cover size (96x140)
        COVER_W, COVER_H = 96, 140
        self.cover_image = Gtk.Image()
        pb = GdkPixbuf.Pixbuf.new(GdkPixbuf.Colorspace.RGB, False, 8, COVER_W, COVER_H)
        pb.fill(0xddddddff)  # light gray placeholder
        ...
        self.tab_bm = make_icon_tab("bookmark-symbolic", "Bookmarks", "bookmarks")
    ...
    def load_epub(self, path):
        try:
            ...
            try:
                cov = None
                # first prefer known cover filename
                for it in self.book.get_items_of_type(ebooklib.ITEM_IMAGE):
                    name = (it.get_name() or "").lower()
                    if "cover" in name:
                        cov = it
                        break
                # fallback: if none found, pick the first image available (helps EPUB2)
                if cov is None:
                    imgs = list(self.book.get_items_of_type(ebooklib.ITEM_IMAGE))
                    if imgs:
                        cov = imgs[0]

                if cov:
                    # write to temp and load pixbuf at COVER_WxCOVER_H
