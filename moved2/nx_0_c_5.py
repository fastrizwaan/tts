...
class EPUBWindow(Adw.ApplicationWindow):
    ...
    def setup_toolbar(self):
        ...
        # Column width (only affects single column or as a guide)
        cw_label = Gtk.Label(label="Max Width:", halign=Gtk.Align.START)
        cw_adj = Gtk.Adjustment(value=self.app.column_width, lower=100, upper=800, step_increment=10)
        ...
        self.cw_spin.set_tooltip_text("Maximum content width (informational)")
        ...
        grid.attach(gap_label, 0, 5, 1, 1)
        grid.attach(self.gap_spin, 1, 5, 1, 1)
    ...
    def on_scroll_event(self, controller, dx, dy):
        # Handle mouse wheel scrolling page by page
        if abs(dy) > 0.1:
    ...
    def apply_layout(self):
        ...
        if columns > 1:
            # For multi-column layout, we use ONLY column-count, not column-width
            # This ensures we always get exactly the number of columns specified
            css = f"""
                html {{
                    margin: 0 !important;
                    padding: 0 !important;
                ...
                body {{
                    ...
                    /* Use viewport height minus margins for column height */
                    height: calc(100vh - {margin * 2}px) !important;
                    
                    /* ONLY use column-count to force exact number of columns */
    ...
    def scroll_viewport(self, direction: int):
        ...
        if self.app.columns == 1:
            # Single column: scroll vertically by page
            page = v_adj.get_page_size()
            current = v_adj.get_value()
            new_val = current + direction * page * 0.9  # 90% overlap for smooth reading
            ...
            # Multi-column: scroll horizontally by one viewport width
            page_size = h_adj.get_page_size()
            current = h_adj.get_value()
            
            # Scroll by approximately one page worth
