...
from gi.repository import Gtk, Adw, Gio, GLib, Pango, GObject, Gdk, GdkPixbuf, PangoCairo
import cairo
...
class EPubViewer(Adw.ApplicationWindow):
    ...
    def _create_font_tab(self):
        """Create the 'Font' sidebar tab widget and wire dropdowns."""
        # Container for the tab content
        vbox = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6, margin_top=8, margin_bottom=8, margin_start=8, margin_end=8)
        
        # Title label
        lbl = Gtk.Label(label="Font", halign=Gtk.Align.START)
        lbl.set_margin_bottom(6)
        vbox.append(lbl)

        # Setup dropdowns on self (win)
        # We pass `self` so setup_* functions store widgets on self
        self.setup_font_dropdown(self)
        vbox.append(self.font_dropdown)

        self.setup_font_size_dropdown(self)
        vbox.append(self.font_size_dropdown)
    ...
    def _add_font_tab(self):
        """Attach the 'Font' tab/page to a sidebar. Call after sidebar exists."""
        page = self._create_font_tab()

        # Try common sidebar attribute names / containers
        tried = []
        for name in ("sidebar", "left_sidebar", "notebook", "stack", "tabview", "sidebar_box"):
            cont = getattr(self, name, None)
            if cont is None:
                continue
            tried.append(name)
            try:
                # Adw.TabView-like: try append or add
                if hasattr(cont, "append"):
                    try:
                        cont.append(page)
                        return
                    except Exception:
                ...
                if hasattr(cont, "add"):
                    try:
                        ...
                        pass
                # Gtk.Stack: add with a name
                if isinstance(cont, Gtk.Stack):
                    cont.add_named(page, "font")
                    return
                # Generic container: append
                if isinstance(cont, Gtk.Box) or isinstance(cont, Gtk.Grid):
                    cont.append(page)
                    return
            except Exception:
                continue

        # fallback: try attaching to a sidebar box attribute named 'sidebar' by creating a Frame
        if hasattr(self, "main_box") and isinstance(self.main_box, Gtk.Box):
            frame = Gtk.Frame(label="Font")
            frame.set_child(page)
            self.main_box.append(frame)
            return

        # If nothing found, print minimal instruction (don't raise)
        print("Add font tab: couldn't find a known sidebar container. Tried:", tried, "- call _create_font_tab() and attach manually.")
    ...
    def set_font_family(self, family):
        """Set font family used for displayed EPUB text and inject CSS."""
        try:
            if not family:
                return
            self.font_family = str(family)
            self._inject_font_css()
        except Exception:
            pass

    def set_font_size_pt(self, size_pt):
        """Set base font size in points (float or int) and inject CSS."""
        try:
            size = float(size_pt)
            if size <= 0:
                return
    ...
    def _inject_font_css(self):
        """Inject or update style tag in WebView to set font + heading scale."""
        try:
            css = (
                ":root {"
                f" --epub-base-font-family: '{self.font_family}';"
                f" --epub-base-font-size: {self.font_size_pt}pt;"
                "}\n"
                ".ebook-content, .ebook-content * {"
                "  font-family: var(--epub-base-font-family) !important;"
                "  font-size: var(--epub-base-font-size) !important;"
                "  line-height: 1.45 !important;"
                "  text-rendering: optimizeLegibility !important;"
                "}\n"
                # heading scale
                "h1 { font-size: calc(var(--epub-base-font-size) * 2) !important; "
                "margin-top: 0.6em !important; margin-bottom: 0.4em !important; }\n"
                "h2 { font-size: calc(var(--epub-base-font-size) * 1.75) !important; "
                "margin-top: 0.6em !important; margin-bottom: 0.35em !important; }\n"
                "h3 { font-size: calc(var(--epub-base-font-size) * 1.5) !important; "
                "margin-top: 0.5em !important; margin-bottom: 0.3em !important; }\n"
                "h4 { font-size: calc(var(--epub-base-font-size) * 1.25) !important; "
                "margin-top: 0.45em !important; margin-bottom: 0.25em !important; }\n"
                "h5 { font-size: calc(var(--epub-base-font-size) * 1.1) !important; "
                "margin-top: 0.4em !important; margin-bottom: 0.2em !important; }\n"
                "h6 { font-size: calc(var(--epub-base-font-size) * 1.0) !important; "
                "margin-top: 0.35em !important; margin-bottom: 0.15em !important; }\n"
                # text style and spacing tweaks
                "strong, b { font-weight: 600 !important; }\n"
                "em, i { font-style: italic !important; }\n"
                "blockquote { margin-left: 1.1em !important; margin-right: 1.1em !important; }\n"
            )

            js = f"""
            (function(){{
                try {{
                    var id = 'user-font-override';
    ...
    # ---- add font sidebar tab (drop-in) ----
    def _create_font_sidebar_widget(self):
        """Return a widget containing font controls (family + size)."""
        ...
        sizes = ["9", "10", "11", "12", "13", "14", "16", "18", "20", "24", "28", "32"]

        box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=8, margin_top=8, margin_bottom=8, margin_start=8, margin_end=8)

        # label + dropdown for family
        lbl_family = Gtk.Label(label="Font family", halign=Gtk.Align.START)
        box.append(lbl_family)

        family_names = [n for n, _ in font_map]
        dd_family = Gtk.DropDown.new_from_strings(family_names)
        # set current selection based on self.font_family if present
        try:
            # choose best match
            current = (self.font_family or "").lower()
            best = 0
            for i, (_, css) in enumerate(font_map):
                if css.lower() in current or current in css.lower() or css.split(",")[0].strip("'\"").lower() == current:
                    best = i
                    break
            dd_family.set_selected(best)
        except Exception:
            pass

        def _on_family_changed(dd):
            idx = dd.get_selected()
            if idx < 0:
                return
            css_value = font_map[idx][1]
            try:
                # call existing setter
                self.set_font_family(css_value)
            except Exception as e:
                print("Failed set_font_family:", e)
        dd_family.connect("notify::selected", lambda w, p: _on_family_changed(w))

        box.append(dd_family)

        # label + dropdown for size
        lbl_size = Gtk.Label(label="Base font size (pt)", halign=Gtk.Align.START)
        box.append(lbl_size)

        dd_size = Gtk.DropDown.new_from_strings(sizes)
        try:
            # pick closest size
            cur = int(round(getattr(self, "font_size_pt", 12)))
            if str(cur) in sizes:
                dd_size.set_selected(sizes.index(str(cur)))
            else:
                # pick default 12
                dd_size.set_selected(sizes.index("12"))
        except Exception:
            pass

        def _on_size_changed(dd):
            idx = dd.get_selected()
            if idx < 0:
                return
            sz = sizes[idx]
            try:
                self.set_font_size_pt(float(sz))
            except Exception as e:
                print("Failed set_font_size_pt:", e)
        dd_size.connect("notify::selected", lambda w, p: _on_size_changed(w))

        box.append(dd_size)

        return box

    def _add_font_sidebar_tab(self):
        """Find a suitable sidebar container and add a font tab / page to it.
        Call this after the app has created its sidebar. If no known sidebar is found,
        it will print a warning so you can attach manually."""
        widget = self._create_font_sidebar_widget()

        # Try a set of common attribute names where a sidebar/page container may live
        candidates = ["sidebar", "left_sidebar", "sidebar_box", "sidebar_stack", "stack", "notebook", "tabview", "paned"]
        for name in candidates:
            cont = getattr(self, name, None)
            if cont is None:
                continue
            # If it's an Adw.TabView or Gtk.Stack or Adw.PreferencesWindow style container, try attaching
            try:
                # Adw.TabView: add a page (tab view uses add or append depending on version)
                if cont.__class__.__name__.endswith("TabView") or cont.__class__.__name__.lower().endswith("tabview"):
                    try:
                        cont.add(widget)
                        return
                    except Exception:
                ...
                # Gtk.Stack-like: add as a named child
                if isinstance(cont, Gtk.Stack):
                    cont.add_named(widget, "font-controls")
                    return

                # Gtk.Box or generic container: add a framed group
                if isinstance(cont, Gtk.Box) or isinstance(cont, Gtk.Grid) or isinstance(cont, Gtk.ScrolledWindow):
                    # put a simple framed group so it looks like a sidebar page
                    frame = Gtk.Frame(label="Fonts")
                    frame.set_child(widget)
                    cont.append(frame)
                    return

                # Adw.PreferencesGroup / Adw.PreferencesPage: append
                if "Preferences" in cont.__class__.__name__ or cont.__class__.__name__.endswith("Page"):
                    try:
                        cont.add(widget)
                        return
                    except Exception:
        ...
        # If we reach here no known container found
        print("Warning: no sidebar container found to attach font controls. Call _add_font_sidebar_tab() with a valid container or attach _create_font_sidebar_widget() manually.")
