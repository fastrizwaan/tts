...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        book_box.set_halign(Gtk.Align.START)
        book_box.set_hexpand(True)
        book_box.set_margin_top(0); book_box.set_margin_bottom(20)
        book_box.set_margin_start(0); book_box.set_margin_end(0)

        self.cover_image = Gtk.Image()
        placeholder_pb = GdkPixbuf.Pixbuf.new(GdkPixbuf.Colorspace.RGB, False, 8, 40, 40)
        ...
        try:
            ...
            self.cover_image.set_halign(Gtk.Align.END)
            self.cover_image.set_size_request(40, 40)
        except Exception:
            pass
        ...
        # Stop reading button on the right of metadata
        self.close_btn = Gtk.Button()  # will be shown only when reading
        self.close_btn.add_css_class("flat")
        self.close_btn.set_tooltip_text("Stop reading")
        try:
            self.close_btn.set_icon_name("process-stop-symbolic")
        except Exception:
            # fallback: set a child image
            self.close_btn.set_child(Gtk.Image.new_from_icon_name("process-stop-symbolic"))
        ...
        # clicking stops reading (save/cleanup and show library)
        self.close_btn.connect("clicked", lambda b: self._stop_reading(self.book_path))
        book_box.append(self.close_btn)
    ...
    # ---- Library ordering / decorate loaded entry ----
    ...
    def _decorate_loaded_card(self, card: Gtk.Widget, entry: dict):
        try:
            try:
                ...
                pass
            badge = Gtk.Label()
            badge.set_markup('<small><b>► Reading</b></small>')
            badge.add_css_class("meta")
            badge.set_halign(Gtk.Align.START)
            badge.set_valign(Gtk.Align.START)
            badge.set_margin_start(6); badge.set_margin_top(4)
            try:
                card.prepend(badge)
            except Exception:
                try:
                    children = list(card.get_children())
                    for c in children:
                        card.remove(c)
                    card.append(badge)
                    for c in children:
    ...
    def _stop_reading(self, path=None):
        ...
        try:
            ...
            try:
                ...
                try: self.stop_btn.set_visible(False)
                except Exception: pass
            except Exception:
                pass
    ...
    def show_library(self):
        ...
        for entry in entries:
            ...
            a = Gtk.Label(); a.add_css_class("author"); a.set_ellipsize(Pango.EllipsizeMode.END); a.set_max_width_chars(18)
            ...
            if self._is_loaded_entry(entry):
                self._decorate_loaded_card(card, entry)
                # prepend a Stop button to close/save/cleanup the current reading session
                try:
                    stop_btn = Gtk.Button(); stop_btn.add_css_class("flat")
                    stop_btn.set_tooltip_text("Stop reading")
                    stop_btn.set_icon_name("process-stop-symbolic")
                    stop_btn.connect("clicked", lambda b, p=path: self._stop_reading(p))
                    # place stop button to left of menu
                    right_box.prepend(stop_btn)
                except Exception:
                    pass
    ...
    def cleanup(self):
        ...
        try:
            ...
            pass
        # reset toggle/search/open visibility for safe state
        try:
            self.content_sidebar_toggle.set_visible(False)
            self.open_btn.set_visible(True)
            self.search_toggle_btn.set_visible(True)
