...
class TTSManager:
    ...
    def _run_js_highlight(self, sid):
        ...
        if not webview:
            return
        # Smooth scroll that only moves when element is not visible
        js = f"""
        (function() {{
            try {{
                ...
                if (isMultiColumn) {{
                    // Multi-column mode - only scroll horizontally if element is not visible
                    ...
                    if (!isVisible) {{
                        // Element not visible - calculate minimal scroll needed
                        var targetScroll = currentScroll;
                        
                        if (rect.left < 0) {{
                            // Element is to the left - scroll left just enough
                            targetScroll = currentScroll + rect.left - 20; // 20px margin
                        }} else if (rect.right > viewportWidth) {{
                            // Element is to the right - scroll right just enough
                            targetScroll = currentScroll + (rect.right - viewportWidth) + 20;
                        }}
                    ...
                    // If visible, do nothing - no jarring snap
                }} else {{
                    // Single column mode - center vertically if not visible
