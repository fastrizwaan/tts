...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # --- NEW: Column controls (minimal) ---
        col_controls = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        col_controls.set_valign(Gtk.Align.CENTER)
        self.columns_combo = Gtk.ComboBoxText()
        for i in range(1, 11):
            self.columns_combo.append_text(str(i))
        self.columns_combo.set_active(self.column_count - 1)
        self.columns_combo.set_tooltip_text("Columns (1-10)")
        self.columns_combo.connect("changed", lambda w: self._on_column_control_changed())
        col_controls.append(self.columns_combo)
        self.use_width_toggle = Gtk.ToggleButton(label="Use width")
        self.use_width_toggle.set_active(self.column_mode_use_width)
        self.use_width_toggle.connect("toggled", lambda w: self._on_column_control_changed())
        col_controls.append(self.use_width_toggle)
        adj = Gtk.Adjustment(value=self.column_width_px, lower=100, upper=500, step_increment=10, page_increment=50)
        self.col_width_spin = Gtk.SpinButton(adjustment=adj, climb_rate=1)
        self.col_width_spin.set_snap_to_ticks(True)
        self.col_width_spin.set_sensitive(self.column_mode_use_width)
        self.col_width_spin.connect("value-changed", lambda w: self._on_column_control_changed())
        self.col_width_spin.set_tooltip_text("Column width in px (100-500)")
        col_controls.append(self.col_width_spin)
        self.content_header.pack_end(col_controls)
    ...
    def _on_column_control_changed(self):
        try:
            try:
                self.column_count = max(1, min(10, int(self.columns_combo.get_active_text() or "1")))
            except Exception:
                self.column_count = 1
            self.column_mode_use_width = bool(self.use_width_toggle.get_active())
            try:
                self.col_width_spin.set_sensitive(self.column_mode_use_width)
                self.column_width_px = int(self.col_width_spin.get_value())
            except Exception:
                pass
            try:
    ...
    def _wrap_html(self, raw_html, base_uri):
        ...
        Wrap EPUB HTML so:
          ...
          - prevents nested column multiplication,
          - runtime-detects effective column count and toggles .single-column on <body>.
        """
        page_css = (self.css_content or "") + "\n" + THEME_INJECTION_CSS

        try:
            # Build column css for .ebook-content depending on mode
            ...
            # Ensure gap and column-fill
            gap_decl = f"column-gap: {self._column_gap}px; -webkit-column-gap: {self._column_gap}px;"
            fill_decl = "column-fill: auto; -webkit-column-fill: auto;"

            col_rules = (
                "/* Reset potential nested column rules from EPUB CSS to avoid N×N behavior */\n"
                ".ebook-content * {\n"
                ...
                "}\n"
                "/* Root: keep horizontal overflow hidden so multi-column doesn't push outside viewport */\n"
                "html, body { height: 100%; min-height: 100%; margin: 0; padding: 0; overflow-x: hidden; }\n"
                "/* Default: treat as multi-column container — height locked to viewport and vertical overflow hidden */\n"
                ".ebook-content {\n"
                ...
                "  column-count: unset !important;\n"
                "}\n"
                "/* Make sure images and block elements don't force extra height horizontally */\n"
        ...
        # JS: determine effective columns at runtime and toggle .single-column on <body>.
        # This handles the width-mode detection too (when col-count is not specified or browser reduces cols).
        js_detect_columns = f"""
        <script>
        (function() {{
          function effectiveColumns(el) {{
            try {{
              ...
              var cc = cs.getPropertyValue('column-count') || cs.getPropertyValue('-webkit-column-count');
              var cw = cs.getPropertyValue('column-width') || cs.getPropertyValue('-webkit-column-width');
              var gap = cs.getPropertyValue('column-gap') || cs.getPropertyValue('-webkit-column-gap') || "{self._column_gap}px";
              cc = parseInt(cc, 10);
              cw = parseFloat(cw);
              gap = parseFloat(gap);
              if (!isNaN(cc) && cc > 0 && cc !== Infinity) return cc;
              if (!isNaN(cw) && cw > 0) {{
                // fallback: compute how many cols fit in the content's logical width
                var available = Math.max(1, el.clientWidth);
                var approx = Math.floor(available / (cw + (gap||0)));
          ...
          function updateMode() {{
            var c = document.querySelector('.ebook-content');
            if (!c) return;
            var cols = effectiveColumns(c);
            if (cols <= 1) document.documentElement.classList.add('single-column');
            else document.documentElement.classList.remove('single-column');
            // optional: ensure vertical scroll position is top when switching to multi-column
            if (cols > 1) window.scrollTo({{ left: 0, top: 0 }});
          }}

          // Run on load and resize (debounced)
          document.addEventListener('DOMContentLoaded', function() {{ updateMode(); }});
          var rTO = null;
          window.addEventListener('resize', function() {{
            if (rTO) clearTimeout(rTO);
            rTO = setTimeout(function() {{ updateMode(); rTO = null;}}, 150);
          }});
          // Also run after some delay to accommodate EPub injected content or fonts
        ...
        head = (
            ...
            f'<meta name="color-scheme" content="light dark"/>{base_tag}'
            f'<style>{page_css}</style>'
            f'{link_intercept_script}'
            f'{js_detect_columns}'
        )

        # Wrap content in .ebook-content
