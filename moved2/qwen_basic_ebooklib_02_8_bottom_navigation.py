#!/usr/bin/env python3
import gi, os, tempfile, traceback, shutil, urllib.parse, json
...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # pending fragment to jump to after load; used by TOC clicks & intercepted links
        self.pending_fragment = None
        # store load-changed handler id so we can disconnect one-shot
        self._load_handler_id = None
        ...
        # TreeStore + TreeView
        # columns: title, href (may include fragment), index
        ...
        try:
            ...
            # keep load-changed connected (we will use handler id for one-shot)
            self._global_load_id = self.webview.connect("load-changed", self._on_load_changed_global)
        except Exception:
            self.WebKit = None
            self.webview = None
    ...
    # minimal global load-changed handler (no-op) to keep signal available;
    # real one-shot handlers are attached/removed in display_page()
    def _on_load_changed_global(self, webview, load_event):
        return
                ...
                if toc_nav:
                    def walk_list(ol, parent_iter=None):
                        for li in ol.find_all("li", recursive=False):
                            ...
                            # store full href (may include fragment) so we can navigate to fragment later
            ...
            if ncx_item:
                ...
                def walk_navpoints(parent, parent_iter=None):
                    for np in parent.find_all("navPoint", recursive=False):
                        ...
                        # content src may include fragment; store full
    ...
    def on_tree_selection_changed(self, selection):
        ...
        if iter_:
            ...
            href = model.get_value(iter_, 1) or ""
            if isinstance(idx, int) and idx >= 0:
                self.current_index = idx
                self.update_navigation()
                # schedule fragment after page load
                if "#" in href:
                    self.pending_fragment = href.split("#", 1)[1]
                else:
                    self.pending_fragment = None
                self.display_page()
    ...
    def on_decide_policy(self, webview, decision, decision_type):
        # Intercept local file navigations (internal links) so we can load them
        # via display_page() (which injects extracted CSS) and then jump to fragment.
        try:
        ...
        if not uri or not uri.startswith("file://") or not self.temp_dir:
            return False

        try:
            # path part, percent-decoded
            path = urllib.parse.urlparse(uri).path
            path = urllib.parse.unquote(path)
            # try to find matching item (endswith handles relative paths)
            target_idx = None
            for i, it in enumerate(self.items):
                name = it.get_name()
                if path.endswith(name) or path.endswith(os.path.basename(name)):
                    target_idx = i
                    break
            ...
            # load through our loader so CSS is applied
            self.current_index = target_idx
            self.update_navigation()
            self.pending_fragment = frag or None
            self.display_page()
    ...
    # one-shot handler: called on load-changed; when FINISHED run fragment jump then disconnect
    def _attach_one_shot_fragment_handler(self, webview):
        try:
            # disconnect previous one-shot if present
            if getattr(self, "_one_shot_id", None):
                try:
                    webview.disconnect(self._one_shot_id)
                except Exception:
                    pass
                self._one_shot_id = None

            def _handler(wv, load_event):
                try:
                    if load_event == self.WebKit.LoadEvent.FINISHED:
                        frag = self.pending_fragment
                        if frag:
                            # prefer element scrollIntoView, fallback to location.hash
                            script = (
                                "(function(){"
                                "var id = %s;"
                                "var el = document.getElementById(id);"
                                "if(el){ el.scrollIntoView(); } else { location.hash = '#'+id; }"
                                "})();"
                            ) % (json.dumps(frag))
                            try:
                                ...
                                try:
                                    wv.run_javascript(script)
                                except Exception:
                                    pass
                        # clear pending and remove this handler
                        self.pending_fragment = None
                        try:
                            wv.disconnect(self._one_shot_id)
                        except Exception:
                            pass
                        self._one_shot_id = None
                except Exception:
            ...
            self._one_shot_id = webview.connect("load-changed", _handler)
        except Exception:
            pass
    ...
    def display_page(self):
        ...
        if self.webview:
            ...
            # attach one-shot handler if fragment is pending
            if self.pending_fragment:
                self._attach_one_shot_fragment_handler(self.webview)
        else:
    ...
    def next_page(self, *_):
        if self.current_index < len(self.items)-1:
            ...
            self.update_navigation()
            # clear any pending fragment when using next/prev
            self.pending_fragment = None
            self.display_page()
    ...
    def cleanup(self):
        ...
        self.treestore.clear()
        self.pending_fragment = None
        # disconnect handlers if any
        try:
            if getattr(self, "_global_load_id", None) and self.webview:
                self.webview.disconnect(self._global_load_id)
        except Exception:
            pass
        try:
            if getattr(self, "_one_shot_id", None) and self.webview:
                self.webview.disconnect(self._one_shot_id)
        except Exception:
            pass
