...
# -------------------------
class TTSEngine(Gtk.Box):
    """Full TTS engine with Kokoro + GStreamer synthesis + built-in GTK4 controls"""
    def __init__(self, webview_getter, base_temp_dir=None):
        ...
        # --- kokoro / gstreamer init ---
        self._init_kokoro()
        self._init_gstreamer()

        GLib.timeout_add(400, self._update_ui_state)
    ...
    def _build_ui(self):
        ...
        self.prev_btn = btn("media-skip-backward-symbolic", "Previous sentence", self.prev_sentence)
        self.play_btn = btn("media-playback-start-symbolic", "Play current chapter", self.play)
        self.pause_btn = btn("media-playback-pause-symbolic", "Pause/Resume", self.toggle_pause)
        self.stop_btn = btn("media-playback-stop-symbolic", "Stop", self.stop)
        self.next_btn = btn("media-skip-forward-symbolic", "Next sentence", self.next_sentence)
    ...
    def _init_kokoro(self):
        if not TTS_AVAILABLE or not Kokoro:
            return
        try:
    ...
    # Connect to viewer
    # =============================================================
    def connect_to_viewer(self, highlight_callback, finished_callback, sentence_provider):
        self._tts_highlight_callback = highlight_callback
        self._tts_finished_callback = finished_callback
        self._sentence_provider = sentence_provider

    # =============================================================
    # Core logic (identical to your original)
    # =============================================================
    def is_playing(self): return bool(self.is_playing_flag) and not self.paused
    def is_paused(self): return bool(self.paused)
    ...
    def split_sentences(self, text):
        ...
        abbr = r"(Mr|Mrs|Ms|Dr|Prof|Sr|Jr|St|Mt|vs|etc|Fig|Eq|Dept|No|pp|Rev|Lt|Col|Gen|Sgt|Capt|Sen|Rep|Gov|Pres|Ave|Rd|Blvd|Jan|Feb|Mar|Apr|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)\."
        protected = re.sub(abbr, lambda m: m.group(0).replace('.', '∯'), text)
    ...
    def synthesize_sentence(self, sentence, voice, speed, lang):
        ...
        try:
            ...
            tmp = _tempfile.NamedTemporaryFile(prefix="tts_", suffix=".wav", delete=False, dir=base)
            tmp_name = tmp.name; tmp.close()
            import soundfile as sf; sf.write(tmp_name, samples, sample_rate)
            return tmp_name
        except Exception as e:
            print(f"[error] Synthesis error: {e}")
            return None

    # caching/timer identical
    def _cancel_delayed_timer(self):
        with self._delayed_timer_lock:
            if self._delayed_timer:
                try: self._delayed_timer.cancel()
        ...
        def timer_cb():
            try:
                ...
                with self._audio_lock:
                    if self._audio_files.get(idx): return
                if idx != self._current_play_index: return
                af = self.synthesize_sentence(
                    self._tts_sentences[idx], self._tts_voice, self._tts_speed, self._tts_lang)
                if af:
                    with self._audio_lock:
                        self._audio_files[idx] = af
            except Exception as e:
                print("[error] delayed synthesis:", e)
            finally:
                with self._delayed_timer_lock:
                    self._delayed_timer = None
        t = threading.Timer(delay, timer_cb)
        with self._delayed_timer_lock: self._delayed_timer = t
    ...
    # =============================================================
    def play(self):
        if not hasattr(self, "_sentence_provider"):
            print("[TTS] no sentence provider attached")
            return
        sents = self._sentence_provider() or []
        if not sents: return
        self.stop(); time.sleep(0.05)
        self.should_stop = False
        self._tts_sentences = [s["text"] for s in sents]
        self._tts_sids = [s["sid"] for s in sents]
        self._current_play_index = 0
    ...
    def _tts_thread(self):
        try:
            ...
            for idx in range(total):
                ...
                if not af:
                    af = self.synthesize_sentence(self._tts_sentences[idx], self._tts_voice, self._tts_speed, self._tts_lang)
                    if af: self._audio_files[idx] = af
                if not af: continue
                if self.player:
                    self.player.set_property("uri", f"file://{af}")
                    self.player.set_state(Gst.State.PLAYING)
                    self.playback_finished = False
                while not self.playback_finished and not self.should_stop:
                    time.sleep(0.05)
                if self.player: self.player.set_state(Gst.State.NULL)
                try: os.remove(af)
                except Exception: pass
            self.is_playing_flag = False
            self._finish_playback()
        except Exception as e:
            print("[error] TTS thread:", e)
            self._finish_playback()
    ...
    def stop(self):
        ...
        try:
            with self._audio_lock:
                for p in list(self._audio_files.values()):
    ...
    def _update_ui_state(self):
        ...
        has_sents = bool(self._tts_sentences)
        try:
            self.play_btn.set_sensitive(True)
            self.stop_btn.set_sensitive(playing or has_sents)
            self.pause_btn.set_sensitive(playing or has_sents)
