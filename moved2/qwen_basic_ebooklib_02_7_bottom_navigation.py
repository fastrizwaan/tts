    ...
    def on_tree_selection_changed(self, selection):
        ...
        if iter_:
            ...
            if isinstance(idx, int) and idx >= 0:
                ...
                self.update_navigation()
                # display_page will load the target document; if href contains a fragment (#id)
                # we apply the fragment after load using JS so the same-item anchor works.
                frag = ""
                if "#" in href:
                    frag = href.split("#", 1)[1]
                self.display_page()
                if frag and self.webview:
                    try:
                        script = f"location.hash = {json.dumps('#' + frag)};"
                        try:
