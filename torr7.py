...
class MainWindow(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # Add categories (torrfetch doesn't filter by category directly, but we can keep this for UI consistency)
        categories = [
            ("All", "all"),
            ("Audio", "audio"),
            ("Video", "video"),
    ...
    def format_size(self, size_str):
        ...
        # torrfetch returns sizes like "2.2 GB" or "1024 MB"
        # We'll keep the original string for now
        return size_str
    ...
    def perform_search(self, query):
        ...
        try:
            ...
            if TORRFETCH_AVAILABLE:
                print("Using torrfetch...")
                # Use torrfetch
                # Note: torrfetch doesn't support category filtering, so we just search
                print("Calling torrfetch.search_torrents...")
                results = torrfetch.search_torrents(query, mode="parallel")
                print(f"torrfetch returned {len(results)} results")
                
                for i, result in enumerate(results):
                    print(f"Processing result {i+1}: {result.get('title', 'Unknown')}")
                    # Extract size in bytes from the size string
                    size_bytes = self.parse_size_to_bytes(result.get("size", "0"))
                    
                    all_results.append({
                        "title": result.get("title", "Unknown"),
                        "size": str(size_bytes),
                        "date": result.get("uploaded", "Unknown"),
                        "seeders": str(result.get("seeders", 0)),
                        "leechers": str(result.get("leechers", 0)),
                        "magnet": result.get("magnet", ""),
                        "source": result.get("source", "Unknown")
                    })
                    print(f"  Added: {result.get('title', 'Unknown')}")
            else:
                print("torrfetch not available, using fallback...")
            ...
            print(f"Final results after sorting and limiting: {len(self.full_results)}")
    ...
    def parse_size_to_bytes(self, size_str):
        """Convert size string like '2.2 GB' or '1024 MB' to bytes"""
        try:
    ...
    def update_results(self, results):
        print(f"Updating UI with {len(results)} results")
        self.list_store.remove_all()
        for item in results:
