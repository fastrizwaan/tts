...
class DSLRenderer:
    """GoldenDict-style HTML renderer for DSL lines (robust stack parser)."""
    ...
    def _open_tag(self, tag: str, param: str = None) -> str:
        if tag.startswith("m") and len(tag) > 1 and tag[1:].isdigit():
            ...
            return f"<span class='lang' data-lang='{html.escape(param or '')}'>"
        elif tag == "<<":
    ...
    def _render_line(self, line: str, headword: str) -> str:
        ...
        def process_text(txt: str):
            ...
            # Convert literal POS markers like <n>, <m>, <adj>, etc.
            def pos_marker(match):
                return f"<span class='pos-tag'>⟨{html.escape(match.group(1))}⟩</span>"
            txt_esc = re.sub(r"&lt;([a-zA-Z0-9\-]+)&gt;", pos_marker, txt_esc)
