    ...
    def build_html(self, results):
        ...
        for word, dict_data in results[:100]:
            for dname, defs, color in dict_data:
                ...
                in_list = False
                current_li_content = []
                needs_new_list = False
                last_number_seen = 0 
                for i, d in enumerate(defs):
                    # Extract content from [mX] to check if it's numbered
                    m_content = re.search(r'\[m\d+\](.*?)\[/m\]', d.strip(), re.DOTALL)
                    if m_content:
                        inner = m_content.group(1).strip()

                        # --- Normalize text for comparison (remove tags, spaces) ---
                        inner_plain = re.sub(r'\[/?[a-z]+\]', '', inner, flags=re.IGNORECASE)
                        inner_compact = re.sub(r'\s+', '', inner_plain)
                        head_compact = re.sub(r'\s+', '', word)

                        # --- Detect lemma headers ---
                        is_lemma_header = ('~' in inner) or inner_compact.lower().startswith(head_compact.lower())

                        # --- Detect part-of-speech header ("■ adjective", etc.) ---
                        is_pos_header = bool(
                            re.match(r'^■\s*(\[c\s+[^\]]+\])?(adjective|noun|verb|adverb|interjection|preposition)',
                                     inner, re.IGNORECASE)
                        )

                        if is_lemma_header:
                            # Lemma headers reset numbering fully
                            if in_list:
                            ...
                            defs_html += f"<div class='standalone pos-header'>{self.render_dsl_text(d, headword=word)}</div>"
                            continue


                            defs_html += f"<div class='standalone pos-header'>{self.render_dsl_text(d, headword=word)}</div>"
                            needs_new_list = True  # <-- next list starts fresh at 1
                            continue




                        # --- Normal numbered or bulleted meanings ---
                        is_numbered_m = (
                            re.match(r'^\d+[\.\)》]\s', inner) or
                            re.match(r'^\[([biu])\]\d+[\.\)》]\[/\1\]', inner)
                        )
                    else:
                        is_numbered_m = False
                    ...
                    # Extract a visible number if present (e.g., "6》")
                    num_match = re.match(r'^\D*(\d+)[\.\)》]', d.strip())
                    current_number = int(num_match.group(1)) if num_match else None

                    # Determine type
                    is_numbered = bool(current_number)
                    is_header_restart = current_number == 1  # "1》" → start new numbered section

                    if is_header_restart:
                        # Close any active numbered list before starting new one
                        if in_list:
                            if current_li_content:
                                defs_html += "<li>" + "".join(current_li_content) + "</li>"
                                current_li_content = []
                        ...
                        last_number_seen = 0  # reset numbering baseline

                    if is_numbered:
                        # Close previous <li> if continuing in same list
                        if current_li_content:
                            defs_html += "<li>" + "".join(current_li_content) + "</li>"
                            current_li_content = []

                        # Start new <ol> if not open
                        if not in_list:
                            start_attr = f" start='{current_number or 1}'"
                            defs_html += f"<ol{start_attr}>"
                            in_list = True

                        last_number_seen = current_number or (last_number_seen + 1)
                        current_li_content.append(self.render_dsl_text(d, is_main=True, headword=word))

                    else:
                        # Sub-item or non-numbered line
                        if in_list:
                            current_li_content.append(f"<div class='sub-item'>{self.render_dsl_text(d, headword=word)}</div>")
                        else:
                            defs_html += f"<div class='standalone'>{self.render_dsl_text(d, headword=word)}</div>"
        ...
        """
        script = ""
        return f"<html><head><meta charset='utf-8'><style>{css}</style></head><body>{body}{script}</body></html>"
