...
class MainWindow(Adw.ApplicationWindow):
    ...
    def on_search(self, entry):
        ...
        #  Word
        #    > Dictionary Name
        #          ─── [separator color] Name [separator color] ───
        #          definition…
        # ------------------------------------------------------------

        for word, dict_data in results[:100]:

            # --- Top-level word block ---
            word_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6)
            word_box.set_margin_top(16)
            word_box.set_margin_start(12)
            word_box.set_margin_end(12)
            self.listbox.append(word_box)

            word_lbl = Gtk.Label(label=word)
            word_lbl.add_css_class("title-2")
            word_lbl.set_wrap(True)
            word_lbl.set_wrap_mode(Gtk.WrapMode.WORD_CHAR)
            word_lbl.set_xalign(0)
            word_box.append(word_lbl)

            # --- Dictionary expanders under this word ---
            for dname, defs, color in dict_data:

                expander = Adw.ExpanderRow()
                expander.set_title(dname)

                # Optional: color code the expander title
                if color != "default":
                    expander.add_css_class(f"dictname-{color}")
                ...
                # --------------------------------------
                # Colored separator row INSIDE expander
                # --------------------------------------
                ...
                if color != "default":
                    name_lbl.add_css_class(f"dictname-{color}")
                ...
                if clean_defs:
                    def_lbl = Gtk.Label(label="\n".join(clean_defs))
                    def_lbl.set_wrap(True)
                    def_lbl.set_xalign(0)
                    def_lbl.set_selectable(True)
                    inner.append(def_lbl)

                # Put padded inner box inside expander
                expander.add_row(inner)

                # Add the expander under the word block
                word_box.append(expander)
