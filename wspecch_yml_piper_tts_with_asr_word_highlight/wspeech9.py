...
class TTSApp(Adw.Application):
    def __init__(self, **kwargs):
        ...
        self.asr_model = whisper.load_model("small", device="cuda" if torch.cuda.is_available() else "cpu")
    ...
    def generate_and_play(self, text):
        try:
            ...
            print("ASR Output (words with timing):")
            for i, timing in enumerate(self.word_timings):
                print(f"  {i}: '{timing.word}' ({timing.start_time:.2f}s - {timing.end_time:.2f}s)")
    ...
    def update_highlight(self):
        try:
            ...
            if current_word_idx >= 0 and current_word_idx < len(self.word_timings):
                ...
                # Find all word boundaries in the text (including various quotes and apostrophes)
                ...
                if current_word_idx < len(matches):
                    ...
                    # Print debug info
                    highlighted_word = text_content[word_start:word_end]
                    asr_word = self.word_timings[current_word_idx].word
                    print(f"Highlighting: '{highlighted_word}' (ASR: '{asr_word}', Time: {current_time:.2f}s)")
...
if __name__ == "__main__":
    app = TTSApp(application_id='io.github.fastrizwaan.pipertts_asr')
    app.run(None)
