...
class EpubViewer(Adw.ApplicationWindow):
    ...
    def on_key_pressed(self, controller, keyval, keycode, state):
        ...
        # In single column mode, allow normal vertical scrolling for arrow keys
        if self.is_single_column_mode():
            # Only handle Page Up/Down in single column mode, let arrows work normally
            if keyval == 65365:  # Page Up
                js_code = "window.scrollBy({ top: -window.innerHeight * 0.9, behavior: 'smooth' });"
                ...
                self.webview.evaluate_javascript(js_code, -1, None, None, None, self._on_js_result, None)
                return True
            # Let arrow keys work normally for vertical scrolling
            return False
    ...
    def process_chapter_content(self, content, item):
        ...
        if apply_columns:
            ...
            # Single column CSS - vertical scroll
            body_style = f"""
            margin: 0;
            padding: {self.column_padding}px;
            font-family: 'Cantarell', sans-serif;
    ...
    def on_prev_page(self, button):
        ...
        # In single column mode, use vertical scrolling
        if self.is_single_column_mode():
            js_code = "window.scrollBy({ top: -window.innerHeight * 0.9, behavior: 'smooth' });"
            self.webview.evaluate_javascript(js_code, -1, None, None, None, self._on_js_result, None)
            return
