...
class EpubViewer(Adw.ApplicationWindow):
    ...
    def _refresh_buttons_based_on_adjustment(self):
        ...
        if max_pos <= 5:
            ...
            self.prev_page_btn.set_sensitive(current > 0)
            self.next_page_btn.set_sensitive(current < max_pos - 5)
    ...
    # Page navigation uses step = actual_column_width + column_padding
    ...
    def update_page_info(self):
        ...
        current = self.h_adjustment.get_value()
        max_pos = max(0, self.h_adjustment.get_upper() - self.h_adjustment.get_page_size())
        total = max(1, int(math.ceil((max_pos + step) / step)))
        cur_idx = int(current / step) + 1
        if cur_idx < 1: cur_idx = 1
        if cur_idx > total: cur_idx = total
        self.page_info.set_text(f"{cur_idx}/{total}")
        self.prev_page_btn.set_sensitive(current > 0)
        self.next_page_btn.set_sensitive(current < max_pos - 5)
