...
class ViewerWindow(Adw.ApplicationWindow):
    def __init__(self, **kwargs):
        ...
        self.sidebar_btn.connect("clicked", self.toggle_sidebar)
    ...
    def populate_sidebar_toc(self):
        ...
        print("DEBUG: Populating sidebar TOC")
        ...
        def on_sidebar_row_activated(listbox, row):
            ...
            print(f"DEBUG: Sidebar row activated - index: {row_index}")
            if 0 <= row_index < len(self._flat_toc):
                title, href, depth = self._flat_toc[row_index]
                print(f"DEBUG: Sidebar TOC row activated - Title: '{title}', Href: '{href}'")
                GLib.idle_add(lambda: self.navigate_to_toc_item(href))
            else:
                print(f"DEBUG: Invalid sidebar row index: {row_index}")
        ...
        print(f"DEBUG: Added {len(self._flat_toc)} items to sidebar TOC")
