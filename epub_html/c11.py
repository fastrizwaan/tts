...
class EpubViewer(Adw.ApplicationWindow):
    ...
    def on_key_pressed(self, controller, keyval, keycode, state):
        if not self.current_book or not self.h_adjustment:
            return False
        step = int(self.actual_column_width + self.column_padding)
        ...
        if keyval in (65361, 65365):  # Left or PageUp -> step back
            ...
            new_pos = min(max_pos, current_pos + step)
    ...
    def snap_to_nearest_step(self):
        if not self.current_book or not self.h_adjustment:
            self.snap_timeout_id = None
            return False
        step = max(1, int(self.actual_column_width + self.column_padding))
        current_pos = self.h_adjustment.get_value()
        idx = round(current_pos / step)
        target = idx * step
        max_pos = max(0, self.h_adjustment.get_upper() - self.h_adjustment.get_page_size())
        target = max(0, min(target, max_pos))
        if abs(current_pos - target) > 5:
    ...
    def on_prev_page(self, button):
        ...
        self.calculate_column_dimensions()
        step = int(self.actual_column_width + self.column_gap)
        
        # Use JavaScript to scroll the page content
        js_code = f"""
        (function() {{
            var currentScroll = window.pageXOffset || document.documentElement.scrollLeft;
            var newScroll = Math.max(0, currentScroll - {step});
            window.scrollTo({{
                left: newScroll,
                behavior: 'smooth'
            }});
            
            // Update scroll info after animation
            setTimeout(function() {{
                window.webkit.messageHandlers.scrollUpdate.postMessage({{
    ...
    def on_next_page(self, button):
        ...
        (function() {{
            var currentScroll = window.pageXOffset || document.documentElement.scrollLeft;
            var maxScroll = Math.max(0, document.documentElement.scrollWidth - document.documentElement.clientWidth);
            var newScroll = Math.min(maxScroll, currentScroll + {step});
            window.scrollTo({{
                left: newScroll,
                behavior: 'smooth'
            }});
