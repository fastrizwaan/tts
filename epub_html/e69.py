    ...
    def _on_app_set_columns(self, action, param):
        ...
        if win and hasattr(win, "apply_column_layout"):
            try:
                ...
                n = 0
            win.apply_column_layout(n, pixel_width=None)
    ...
    def _on_app_set_pixel_columns(self, action, param):
        ...
        if win and hasattr(win, "apply_column_layout"):
            try:
                ...
                pixels = 0
            win.apply_column_layout(0, pixel_width=pixels)
...
class ViewerWindow(Adw.ApplicationWindow):
    def __init__(self, **kwargs):
        ...
        self._current_column_config = {'type': 'count', 'value': 1}
        ...
        # Column layout button (MenuButton)
        ...
        # Create column menu with both count-based and pixel-based options
    ...
    def _on_page_previous(self, *args):
        """Handle previous page navigation with snapping"""
        if self._column_mode:
            ...
            (function() {
                ...
                const currentPage = Math.floor(currentScroll / viewportWidth);
                const targetPage = Math.max(0, currentPage - 1);
                const targetScroll = targetPage * viewportWidth;
            ...
            # Regular back navigation for non-column mode
    ...
    def _on_page_next(self, *args):
        """Handle next page navigation with snapping"""
        if self._column_mode:
            js_code = """
            (function() {
                const wrapper = document.getElementById('__viewer_column_wrapper');
                ...
                const targetScroll = Math.min(targetPage * viewportWidth, maxScroll);
            ...
            # Regular forward navigation for non-column mode
            js_code = "window.history.forward();"
    ...
    def apply_column_layout(self, columns=0, pixel_width=None):
        """Apply column layout with improved snapping and pixel-based columns"""
        if pixel_width:
            self._current_column_config = {'type': 'pixel', 'value': pixel_width}
            self._column_mode = True
        else:
            cols_int = int(columns) if columns else 0
            self._current_column_config = {'type': 'count', 'value': cols_int}
            self._column_mode = cols_int > 1
        ...
        (function() {
            ...
            if (config.type === 'count' && config.value <= 1) {
            ...
            let cols, columnWidth, columnGap = 30;
            
            if (config.type === 'pixel') {
                // Pixel-based columns
                columnWidth = config.value;
                cols = Math.floor((vw - 80) / (columnWidth + columnGap)); // Account for padding
                cols = Math.max(1, cols);
            } else {
                // Count-based columns
                cols = config.value;
                columnWidth = Math.floor((vw - 80 - (cols - 1) * columnGap) / cols);
            }
            ...
            // Set wrapper width to accommodate all columns + gaps + padding
            const totalWidth = (columnWidth * cols) + ((cols - 1) * columnGap) + 80; // +80 for padding
            wrapper.style.width = totalWidth + 'px';

            // Enhanced wheel handling for smooth page turning
            function handleWheel(event) {
                if (config.type === 'count' && config.value <= 1 || event.shiftKey) {
                    return;
                }
                ...
                if (Math.abs(event.deltaY) > Math.abs(event.deltaX)) {
                    ...
                    // Snap to viewport-width pages
                    const currentScroll = wrapper.scrollLeft;
                    const viewportWidth = window.innerWidth;
                    const currentPage = Math.round(currentScroll / viewportWidth);
                    const scrollDirection = event.deltaY > 0 ? 1 : -1;
                    const targetPage = Math.max(0, currentPage + scrollDirection);
                    const maxPages = Math.ceil(wrapper.scrollWidth / viewportWidth);
                    const finalPage = Math.min(targetPage, maxPages - 1);
                    
                    wrapper.scrollTo({
                        left: finalPage * viewportWidth,
                        behavior: 'smooth'
                    });
                }
            }
            ...
            function handleKeydown(event) {
                if ((config.type === 'count' && config.value <= 1)) return;
            ...
            function handleResize() {
                ...
                if (config.type === 'pixel') {
                    ...
                    newCols = Math.floor((newVw - 80) / (newColumnWidth + columnGap));
                    newCols = Math.max(1, newCols);
                } else {
                    newCols = config.value;
                    newColumnWidth = Math.floor((newVw - 80 - (newCols - 1) * columnGap) / newCols);
                }
                
                const newTotalWidth = (newColumnWidth * newCols) + ((newCols - 1) * columnGap) + 80;
                ...
                const currentPage = Math.round(wrapper.scrollLeft / vw);
                wrapper.scrollLeft = currentPage * newVw;
            }
        ...
        """
        js_code = js_template.replace("__CONFIG__", json.dumps(self._current_column_config))
        try:
            self.webview.evaluate_javascript(js_code, -1, None, None, None, None, None)
    ...
    def _update_nav_buttons(self, *a):
        try:
            if self._column_mode:
                # In column mode, always enable navigation buttons for page turning
                self.back_btn.set_sensitive(True)
                self.forward_btn.set_sensitive(True)
            else:
                # In regular mode, use browser navigation
