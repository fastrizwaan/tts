...
class ViewerWindow(Adw.ApplicationWindow):
    def __init__(self, **kwargs):
        ...
        self._flat_toc = []
        self._spine_hrefs = []  # Add this to track spine order
    ...
    def navigate_to_toc_item(self, href):
        ...
        if chapter_index is not None:
            ...
            if fragment:
                ...
                print("DEBUG: No fragment in fallback, cannot navigate")
    ...
    def on_toc_clicked(self, btn):
        print("DEBUG: TOC button clicked")
        print(f"DEBUG: _flat_toc has {len(self._flat_toc)} items")
        ...
        def on_row_activated(listbox, row):
            # Get the row index to find the corresponding TOC item
            row_index = row.get_index()
            print(f"DEBUG: ListBox row activated - index: {row_index}")
            if 0 <= row_index < len(self._flat_toc):
                title, href, depth = self._flat_toc[row_index]
                print(f"DEBUG: TOC row activated - Title: '{title}', Href: '{href}'")
                dlg.close()
                # Use the new navigation method for concatenated documents
                print("DEBUG: Calling navigate_to_toc_item via GLib.idle_add")
        ...
        for title, href, depth in self._flat_toc:
            print(f"DEBUG: Adding TOC item - Title: '{title}', Href: '{href}', Depth: {depth}")
        ...
        # Connect to the listbox row-activated signal instead of individual row signals
        listbox.connect("row-activated", on_row_activated)
        print("DEBUG: Connected row-activated signal to listbox")
