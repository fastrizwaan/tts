...
class ViewerWindow(Adw.ApplicationWindow):
    ...
    def apply_column_layout(self, columns):
        ...
        (function() {
            ...
            if (wrapper.__col_pager_bound) {
                wrapper.removeEventListener('wheel', wrapper.__col_pager_bound, {passive:false});
                window.removeEventListener('keydown', wrapper.__col_key_bound);
                window.removeEventListener('resize', wrapper.__col_resize_bound);
                wrapper.__col_pager_bound = null;
                wrapper.__col_key_bound = null;
                wrapper.__col_resize_bound = null;
            }

            // Remove old column anchors
            let oldAnchors = wrapper.querySelectorAll('.__col_anchor');
            ...
            // Use CSS columns - set column-width instead of column-count for better control
            const columnWidth = Math.floor((vw - (cols - 1) * 20) / cols); // 20px gap between columns
            ...
            wrapper.style.MozColumnBreakInside = 'avoid';
            wrapper.style.pageBreakInside = 'avoid';
            wrapper.style.breakInside = 'avoid';
            
            // Set wrapper width to accommodate all columns + gaps
            const totalWidth = (columnWidth * cols) + ((cols - 1) * 20);
            wrapper.style.width = totalWidth + 'px';
            
            // Add invisible scroll targets for each column
            for (let i = 0; i < cols; i++) {
                const anchor = document.createElement('div');
                anchor.className = '__col_anchor';
                anchor.style.cssText = `
                    position: absolute;
                    top: 0;
                    left: ${i * (columnWidth + 20)}px;
            ...
            // Column navigation function
            function navigateToColumn(columnIndex) {
                if (isScrolling) return;
                
                const targetIndex = Math.max(0, Math.min(cols - 1, columnIndex));
                const scrollTarget = targetIndex * (columnWidth + 20);
            ...
            // Wheel event handler for column navigation
            let wheelTimeout;
            const onWheel = (e) => {
                // Only handle if the event target is within our wrapper
                ...
                wheelTimeout = setTimeout(() => {
                    ...
                    if (Math.abs(delta) > 10) {
                        ...
                        navigateToColumn(currentColumn + direction);
                    }
                }, 50);
            };
            ...
            const onResize = () => {
                ...
                const newColumnWidth = Math.floor((newVw - (cols - 1) * 20) / cols);
                const newTotalWidth = (newColumnWidth * cols) + ((cols - 1) * 20);
                ...
                const anchors = wrapper.querySelectorAll('.__col_anchor');
                anchors.forEach((anchor, i) => {
                    anchor.style.left = (i * (newColumnWidth + 20)) + 'px';
                    anchor.style.width = newColumnWidth + 'px';
                });
                
                // Maintain current column position
                const newScrollTarget = currentColumn * (newColumnWidth + 20);
                wrapper.scrollLeft = newScrollTarget;
            };

            // Scroll handler to track current column
            const onScroll = () => {
                if (isScrolling) return;
                
                const scrollLeft = wrapper.scrollLeft;
                const newColumn = Math.round(scrollLeft / (columnWidth + 20));
                currentColumn = Math.max(0, Math.min(cols - 1, newColumn));
            };

            // Bind event listeners
            wrapper.__col_pager_bound = onWheel;
            wrapper.__col_key_bound = onKey;
            wrapper.__col_resize_bound = onResize;
            wrapper.__col_scroll_bound = onScroll;
