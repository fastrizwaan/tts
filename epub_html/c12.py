...
class EpubViewer(Adw.ApplicationWindow):
    ...
    def __init__(self, app):
        ...
        self.desired_column_width = 200
        # add small widths requested by user
        self.column_gap = 60
        self.column_padding = 60
    ...
    def setup_ui(self):
        ...
        width_menu = Gio.Menu()
        for w in (100,150,200,300,350,400,450,500):
    ...
    def on_prev_page(self, button):
        ...
        # Use JavaScript to scroll the page content with proper column snapping
        js_code = f"""
        (function() {{
            var columnWidth = {column_width};
            var columnGap = {column_gap};
            ...
            // Calculate how many complete columns fit in the viewport
            var columnsPerView = Math.floor(viewportWidth / stepSize);
            if (columnsPerView < 1) columnsPerView = 1;
            
            // Calculate the scroll step as one viewport width
            var scrollStep = columnsPerView * stepSize;
            ...
            console.log('Prev: current=' + currentScroll + ', currentCol=' + currentColumn + ', targetCol=' + targetColumn + ', newScroll=' + newScroll);
            ...
            // Ensure exact positioning after smooth scroll
            setTimeout(function() {{
                window.scrollTo({{
                    left: newScroll,
                    behavior: 'auto'
    ...
    def on_next_page(self, button):
        ...
        (function() {{
            ...
            console.log('Next: current=' + currentScroll + ', currentCol=' + currentColumn + ', targetCol=' + targetColumn + ', newScroll=' + newScroll + ', max=' + maxScroll);
