...
class ViewerWindow(Adw.ApplicationWindow):
    ...
    def apply_column_layout(self, columns):
        js_code = f"""
        (function() {{
            // Try to find a selected container; if none, use body
            ...
            if (sel && sel.rangeCount) {{
                ...
                while (node) {{
                    const nn = node.nodeName ? node.nodeName.toLowerCase() : '';
                    if (nn === 'div' || nn === 'p' || nn === 'article' || nn === 'section') {{ container = node; break; }}
