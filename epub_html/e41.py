        ...
        if chapter_index is not None:
            if fragment:
                ...
                (function() {{
                    ...
                    if (targetEl) {{ targetEl.scrollIntoView({{behavior:'smooth', block:'start'}}); return 'ok'; }}
                    let el = document.querySelector('[name="{fragment}"]');
                    if (el) {{ el.scrollIntoView({{behavior:'smooth', block:'start'}}); return 'ok_name'; }}
                    let ch = document.getElementById('chapter_{chapter_index}');
                    if (ch) {{ ch.scrollIntoView({{behavior:'smooth', block:'start'}}); return 'ch_ok'; }}
                    return 'not_found';
                }})();
                """
            else:
                js_code = f"document.getElementById('chapter_{chapter_index}') && document.getElementById('chapter_{chapter_index}').scrollIntoView({{behavior:'smooth', block:'start'}});"
            try:
                self.webview.evaluate_javascript(js_code, -1, None, None, None, None, None)
            except Exception:
                pass
        else:
            # fallback: try to load file directly (if exists)
