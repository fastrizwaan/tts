...
class EpubViewer(Adw.ApplicationWindow):
    ...
    def process_chapter_content(self, content, item):
        ...
        .column-container {{
            ...
            box-sizing: border-box;
            /* Ensure the container maintains its width with padding */
            min-width: 100%;
            /* Add invisible content at the end to maintain padding */
        }}
        
        /* Invisible spacer to ensure right padding is maintained */
        .column-container::after {{
            content: '';
            display: inline-block;
            width: {self.column_padding}px;
            height: 1px;
            margin-left: -{self.column_padding}px;
        }}
        ...
        return f"""<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">{css_styles}</head><body><div class="column-container">{body_content}</div>{script}</body></html>"""
