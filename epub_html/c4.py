...
class EpubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # Column settings - will be calculated dynamically
        self.column_count = 2
        self.base_column_width = 400  # Base width, will be adjusted
        self.column_gap = 40
        self.column_padding = 20
        self.actual_column_width = self.base_column_width  # Calculated width
    ...
    def setup_ui(self):
        ...
        # Chapter navigation and column navigation buttons
        ...
        # Connect scroll events and keyboard navigation
        self.setup_navigation()
        
    def setup_navigation(self):
        """Setup navigation and scrolling for columns"""
        # Get horizontal adjustment for scrolling control
        self.h_adjustment = self.scrolled_window.get_hadjustment()
        
        # Connect to adjustment changes to update column info
        self.h_adjustment.connect("value-changed", self.on_scroll_position_changed)
        
        # Add key controller to the window for keyboard navigation
        key_controller = Gtk.EventControllerKey()
        key_controller.connect("key-pressed", self.on_key_pressed)
        self.add_controller(key_controller)
        
        # Add scroll controller for mouse/touchpad scrolling with snapping
        ...
        # Timer for delayed column snapping
        self.snap_timeout_id = None
        
    def on_scroll_position_changed(self, adjustment):
        """Handle scroll position changes to update UI"""
        self.update_column_info()
        
    def on_key_pressed(self, controller, keyval, keycode, state):
        """Handle keyboard navigation for column movement"""
        ...
        if keyval == 65361:  # Left Arrow
            ...
            max_pos = self.h_adjustment.get_upper() - self.h_adjustment.get_page_size()
            ...
            new_pos = self.h_adjustment.get_upper() - self.h_adjustment.get_page_size()
        else:
            return False
    ...
    def on_scroll_event(self, controller, dx, dy):
        """Handle mouse/touchpad scroll events"""
        # Cancel any pending snap
        if self.snap_timeout_id:
            GLib.source_remove(self.snap_timeout_id)
            
        # Allow the scroll to happen first, then snap after a delay
    ...
    def snap_to_nearest_column(self):
        ...
        # Find the nearest column position
    ...
    def smooth_scroll_to(self, target_pos):
        ...
        # Simple smooth scrolling using GLib timeout
        current_pos = self.h_adjustment.get_value()
        ...
        def animation_frame():
            nonlocal step_count
            result = animate_step(step_count)
            step_count += 1
            return result
            
        GLib.timeout_add(16, animation_frame)  # ~60fps
    ...
    def process_chapter_content(self, content, item):
        ...
        <script>
        // Simple script to handle focus and prevent default key behaviors that might interfere
        document.addEventListener('DOMContentLoaded', function() {{
            // Prevent arrow key scrolling in the document since GTK handles it
            ...
            // Make body focusable but don't steal focus from GTK
    ...
    def update_column_info(self):
        """Update column position indicator"""
        if not self.current_book:
            self.column_info.set_text("--/--")
            self.prev_column_btn.set_sensitive(False)
            self.next_column_btn.set_sensitive(False)
        ...
        self.next_column_btn.set_sensitive(current_pos < max_pos - 5)  # 5px tolerance    
        
    def calculate_column_dimensions(self):
        """Calculate optimal column dimensions based on window size"""
        # Get current window size
        width = self.get_allocated_width()
        height = self.get_allocated_height()
        
        if width <= 0 or height <= 0:
            # Use default if window not allocated yet
            width = 1200
            height = 800
        ...
        # Ensure we don't exceed window width
    ...
    def on_window_resize(self, window, pspec):
        """Handle window resize to recalculate columns"""
        self.calculate_column_dimensions()
        if self.current_book and self.chapters:
            # Reload current chapter with new dimensions
            GLib.timeout_add(100, self._delayed_reload)  # Small delay to avoid rapid reloads
            
    def _delayed_reload(self):
        """Delayed reload to avoid multiple rapid reloads during resize"""
        self.extract_chapters()
        self.load_chapter()
        return False        # Connect to window resize events to recalculate columns
