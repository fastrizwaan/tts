...
from gi.repository import Gtk, Adw, WebKit, Gio, GLib, Pango, GLib as _GLib
...
class EpubViewer(Adw.ApplicationWindow):
    ...
    def setup_ui(self):
        ...
        # Prev page (previous) and next page buttons
    ...
    def on_key_pressed(self, controller, keyval, keycode, state):
        if not self.current_book or not self.h_adjustment:
            return False
        # Page navigation: PageUp/PageDown, Left/Right treated as page moves
    ...
    def update_navigation(self):
        ...
        if self.h_adjustment:
            try:
                ...
                pass
            self.h_adjustment.connect("value-changed", self.on_scroll_position_changed)
        # refresh column/page buttons based on current adjustment
        self._refresh_buttons_based_on_adjustment()
        self.update_column_info()

    def _refresh_buttons_based_on_adjustment(self):
        """Enable/disable page buttons only when there's scrollable horizontal range."""
        if not self.h_adjustment:
            self.prev_column_btn.set_sensitive(False)
            self.next_column_btn.set_sensitive(False)
            return
    ...
    # ---- Page navigation: move by viewport/page size ----
    ...
    def update_column_info(self):
        """Show current page/total pages (uses viewport/page size)."""
        if not self.current_book or not self.h_adjustment:
            self.column_info.set_text("--/--")
            self.prev_column_btn.set_sensitive(False)
            self.next_column_btn.set_sensitive(False)
    ...
    def calculate_column_dimensions(self):
        ...
        column_area_width = max(0, available_width - total_gap_width)
