...
class Renderer:
    ...
    def draw(self, cr, alloc, buf, scroll_line, scroll_x, sel_s, sel_e):
        ...
        for ln in range(scroll_line, min(scroll_line + max_vis, total_lines)):
            ...
            # Draw selection background (including empty lines)
            if sel_range:
                ...
                if start_l <= ln <= end_l:
                    ...
                    if ln == start_l and ln == end_l:
                        ...
                        sel_start_x = self.ln_width + (start_c * self.char_w) - scroll_x
                        # For empty lines or end of line, extend to reasonable width
                        sel_end_x = self.ln_width + (max(len(text), start_c + 1) * self.char_w) - scroll_x
                    elif ln == end_l:
                        sel_start_x = self.ln_width - scroll_x
                        sel_end_x = self.ln_width + (end_c * self.char_w) - scroll_x
                    else:
                        # Middle line - select entire line width
                        sel_start_x = self.ln_width - scroll_x
                        # For empty lines, show at least some selection width
                        sel_end_x = self.ln_width + (max(len(text), 1) * self.char_w) - scroll_x
...
class EditorWindow(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        self.set_title("UltraEditor v3.7 — Editable + Selection")
...
class UltraEditorApp(Adw.Application):
    def __init__(self):
        super().__init__(application_id="dev.ultraeditor.v37")
