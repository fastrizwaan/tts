...
TAB_CSS = """
/* ===== Chrome-style tabs with crisp wide separators ===== */

.chrome-tab {
    min-height: 32px;
    padding: 0 8px;

    /* Wide separator: 10px spacing + 1px thin line */
    padding-left: 10px;
    border-left: 1px solid alpha(@window_fg_color, 0.15);

    border-radius: 6px 6px 0 0;
}

/* First tab has no separator */
.chrome-tab:first-child {
    padding-left: 8px;       /* restore normal padding */
    border-left: none;
}

/* Hover removes separator */
.chrome-tab:hover {
    background-color: alpha(@window_fg_color, 0.1);
    border-left-color: transparent;
}

/* Active tab removes separator */
.chrome-tab.active {
...
/* Hide NEXT tab's separator when current tab hovered */
.chrome-tab:hover + .chrome-tab {
    border-left-color: transparent;
}

/* Hide NEXT tab's separator when current tab active */
...
class ChromeTab(Gtk.Box):
    ...
    def __init__(self, title="Untitled", closeable=True):
        ...
        self.tab_content.add_controller(motion_controller)
    ...
    def _on_hover_enter(self, controller, x, y):
        """Handle mouse entering tab"""
        self._is_hovered = True
        if self.tab_bar:
            self.tab_bar._update_separators()

    def _on_hover_leave(self, controller):
        """Handle mouse leaving tab"""
        self._is_hovered = False
        if self.tab_bar:
            self.tab_bar._update_separators()
...
class ChromeTabBar(Gtk.Box):
    ...
    def _update_separators(self):
        """Update separator transparency based on active tab and hover state"""
        for i, separator in enumerate(self.separators):
            left_index = i
            right_index = i + 1

            left_tab = self.tabs[left_index]
            right_tab = self.tabs[right_index]

            should_hide = (
                left_index == self.active_tab_index or
                right_index == self.active_tab_index or
                left_tab.is_hovered() or
                right_tab.is_hovered()
            )

            if should_hide:
                separator.add_css_class("faded")
            else:
                separator.remove_css_class("faded")
...
class DemoApp(Adw.Application):
    ...
    def on_activate(self, app):
        ...
        css_provider.load_from_data(TAB_CSS.encode())
