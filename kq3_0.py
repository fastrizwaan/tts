...
class MainWindow(Adw.ApplicationWindow):
    ...
    def on_prev_clicked(self, button):
        ...
        self.play_specific_sentence(target_index)
    ...
    def play_specific_sentence(self, sentence_index):
        """Play a specific sentence without stopping generation"""
        # Stop current playback but keep generation running
        self.stop_current_playback()
        
        # Update current sentence index
        self.app.current_sentence_index = sentence_index
        
        # Update UI controls
        self.play_split_button.set_sensitive(False)
        self.pause_button.set_sensitive(True)
        self.stop_button.set_sensitive(True)
        self.update_navigation_buttons(sentence_index)
        ...
        # Check if this sentence is already generated
        if sentence_index in self.app.generated_sentences:
            # Play the already generated sentence
            sentence_info = self.app.generated_sentences[sentence_index]
            # Start a new playback thread for this specific sentence
            self.app.playback_thread = threading.Thread(
                target=self.play_single_sentence,
                args=(sentence_info,),
                daemon=True
            )
            self.app.playback_thread.start()
        else:
            # If not generated yet, we need to wait or generate it
            # For now, we'll create a new playback thread that will wait for the sentence
            self.app.playback_thread = threading.Thread(
                target=self.wait_and_play_sentence,
                args=(sentence_index,),
                daemon=True
            )
            self.app.playback_thread.start()

    def wait_and_play_sentence(self, sentence_index):
        """Wait for a specific sentence to be generated and then play it"""
        ...
        while time.time() - start_time < max_wait_time:
            ...
            if sentence_index in self.app.generated_sentences:
                sentence_info = self.app.generated_sentences[sentence_index]
                self.play_single_sentence(sentence_info)
                return
                
            time.sleep(0.1)
            
        # If we timed out, try to generate it directly
        if not self.app.should_stop and not self.app.is_shutting_down:
            self.generate_and_play_sentence_directly(sentence_index)

    def generate_and_play_sentence_directly(self, sentence_index):
        """Generate and play a specific sentence directly"""
        try:
            if sentence_index < len(self.app.sentences):
                sentence = self.app.sentences[sentence_index]
                
                # Get current voice, speed, and language settings
                ...
                sentence_info = {
                    ...
                    'start_pos': self.app.sentence_positions[sentence_index][0] if sentence_index < len(self.app.sentence_positions) else 0,
                    'end_pos': self.app.sentence_positions[sentence_index][1] if sentence_index < len(self.app.sentence_positions) else 0
                }
                
                # Store in generated sentences
                self.app.generated_sentences[sentence_index] = sentence_info
                
                # Play the sentence
                self.play_single_sentence(sentence_info)
                
        except Exception as e:
            print(f"Error generating and playing sentence {sentence_index}: {e}")
            GLib.idle_add(self.on_playback_finished)

    def play_single_sentence(self, sentence_info):
    ...
    def start_playback_from_position(self, start_index):
        ...
        self.app.generated_sentences = {}  # Reset generated sentences cache
    ...
    def stop_current_playback(self):
        """Stop current playback without stopping generation"""
        # Set stop flags
        self.app.should_stop = True
        self.app.is_playing = False
        self.app.is_paused = False
        
        # Terminate current audio process
        ...
        try:
            for i, sentence in enumerate(self.app.sentences[start_index:], start_index):
                ...
                try:
                    ...
                    # Store in generated sentences cache
                    self.app.generated_sentences[i] = sentence_info
                    ...
                    if not self.app.should_stop and not self.app.is_shutting_down:
                        ...
                        try:
                            ...
                            # Remove from generated sentences
                            if i in self.app.generated_sentences:
                                del self.app.generated_sentences[i]
                        except:
                            pass
                        break
