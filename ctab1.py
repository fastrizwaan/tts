...
class DemoApp(Adw.Application):
    ...
    def on_activate(self, app):
        ...
        t1.connect("activate-requested", self._on_tab_activated)
        t1.connect("close-requested", self._on_tab_closed)
        self.tabbar.add_tab(t1)
        self.tabbar.set_tab_active(t1)

        t2 = ChromeTab("سلام دنیا — اردو ٹیب")
        t2.set_modified(True)
        t2.connect("activate-requested", self._on_tab_activated)
        t2.connect("close-requested", self._on_tab_closed)
        self.tabbar.add_tab(t2)

        t3 = ChromeTab("a-very-long-file-name-to-test-ellipsize.txt")
        t3.connect("activate-requested", self._on_tab_activated)
        t3.connect("close-requested", self._on_tab_closed)
