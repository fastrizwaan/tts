...
class DictionaryManager:
    ...
    def search(self, q, exact_only=True, max_preview=100, with_defs=False):
        ...
        Tiered, case-insensitive search:
          0. Exact (O(1))
          1. Prefix (startswith)
          2. Proximity (query appears early)
          3. Substring (anywhere)
             + phrase-level: ' <q> ' inside multiword entries
        """
        q = q.strip()
        if not q:
            return []
        ...
        phrase_pattern = re.compile(rf"\b{re.escape(q_lower)}\b", re.IGNORECASE)
        ...
        for dict_path, idx_data in self.index_cache.items():
            ...
            for e in idx_data.get("entries", []):
                ...
                # --- Tier 2: Proximity (query near start) ---
                ...
                if m:
                    ...
                    # Include both embedded matches and space-bounded ones (phrases)
                    if pattern.search(key) or phrase_pattern.search(key):
                        defs = self._read_entry(fpath, e, enc) if with_defs else []
        ...
        # Sort within tiers for predictability
