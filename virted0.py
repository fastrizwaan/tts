...
class _VTVLayout(Gtk.LayoutManager):
    def measure(self, widget, orientation, for_size):
        # Minimal measure required for layout managers
        if orientation == Gtk.Orientation.VERTICAL:
            return (1, -1, -1, -1)  # minimum, natural, etc.
        else:
            return (1, -1, -1, -1)

    def allocate(self, widget, width, height, baseline):
        # Store the allocated height in the widget
        widget._allocated_height = height
...
class VirtualTextView(Gtk.DrawingArea):
    ...
    def _on_key_pressed(self, controller, keyval, keycode, state):
        ...
        if key == "Up":
            ...
            visible_lines = max(1, self._allocated_height // self.line_height)
    ...
    def _ensure_cursor_visible(self):
        # Try to lazy-connect vadj if not present
        if not self.vadj:
            ...
            if isinstance(parent, Gtk.ScrolledWindow):
                ...
                if self.vadj:
                    try:
                        self._vadj_handler_id = self.vadj.connect("value-changed", self._adj_changed)
                    except Exception:
                        pass
            if not self.vadj:
                return
    ...
    def on_map(self, *args):
        ...
        # Grab the scroller's adjustment early
        self.vadj = parent.get_vadjustment()
        if self.vadj:
            # Keep a single connection
            try:
                self.vadj.disconnect(self._vadj_handler_id)
            except Exception:
                pass
            self._vadj_handler_id = self.vadj.connect("value-changed", self._adj_changed)
        ...
        total_height = max(1, estimated_lines) * self.line_height

        # Set a sensible request now so layout is aware; then force a layout pass
        self.set_size_request(-1, total_height)
        parent.queue_resize()          # force the ScrolledWindow to re-evaluate scrollbars/layout

        # Some values (page_size) need the widget to be laid out — update them in idle
        def finish_scroll_setup():
            if not self.vadj:
                self.vadj = parent.get_vadjustment()
                if not self.vadj:
                    return True  # try again

            # Use allocated height for page size (safer than get_height())
            alloc_height = self._allocated_height or self.get_height() or 1

            self.vadj.set_upper(total_height)
            self.vadj.set_page_size(alloc_height)
            self.vadj.set_step_increment(self.line_height * 3)
            self.vadj.set_page_increment(max(1, alloc_height))
            return False

        GLib.idle_add(finish_scroll_setup)
    ...
    def load_file(self, path):
        ...
        def setup_scroll():
            if self.view.vadj:
                ...
                self.view.set_size_request(-1, total_height)
