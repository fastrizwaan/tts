...
class EpubViewer(Adw.ApplicationWindow):
    ...
    def process_chapter_content(self, content, item_path, resources_map=None):
        """
        Inject sentence spans for many block-level tags while preserving inline HTML.
        Accepts a precomputed resources_map (basename->rel path).
        """
        self.calculate_column_dimensions()
        ...
        # Include local jszip/epub.js if copied to resources
        resources_dir_rel = 'resources'
        js_includes = ""
        try:
            res_root = os.path.join(self.temp_dir, resources_dir_rel)
            if os.path.exists(os.path.join(res_root, LOCAL_JSZIP.name)):
                js_includes += f'<script src="{resources_dir_rel}/{LOCAL_JSZIP.name}"></script>\n'
            if os.path.exists(os.path.join(res_root, LOCAL_EPUBJS.name)):
        ...
        # Wrap bare top-level text nodes into <p>
        try:
            body_content = re.sub(
                r'(?<=^|>)(\s*[^<\s][^<]*?)(?=<|$)',
        ...
        # resource rewrites (images/css) using resources_map for robust resolution
        def repl_src(m):
            orig = m.group(1)
            name = os.path.basename(orig)
            # prefer resources_map basename -> rel path
            if resources_map and name in resources_map:
                return f'src="{resources_map[name]}"'
            # fallback: resolve relative to original item_path
        ...
        def repl_href(m):
            ...
            name = os.path.basename(orig)
            if resources_map and name in resources_map:
                return f'href="{resources_map[name]}"'
            candidate = os.path.normpath(os.path.join(os.path.dirname(item_path), orig))
            if os.path.exists(candidate):
                rel = os.path.relpath(candidate, self.temp_dir)
                return f'href="{rel}"'
        ...
        # TARGET_TAGS for sentence wrapping
        TARGET_TAGS = [
