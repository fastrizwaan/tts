...
class DictionaryIndexer:
    ...
    def build_index(self, on_progress=None):
        ...
        ngram_index = {}  # NEW for substring search
            ...
            while True:
                ...
                while True:
                    ...
                    if word and word not in offsets_seen:
                        ...
                        # --- Build multi-length n-gram index for substring acceleration ---
                        lower = key
                        max_n = 4  # up to 4-letter n-grams
...
class DictionaryManager:
    ...
    def search(self, q, exact_only=True, max_preview=100, with_defs=False):
        ...
        Fast tiered search:
          ...
          2. Phrase
          3. Proximity
          4. Substring (via prebuilt n-gram index if available)
        """
        q = q.strip()
        if not q:
            return []
        ...
        for dict_path, idx_data in self.index_cache.items():
            ...
            # --- Try prebuilt n-gram index ---
            ngram_path = Path(idx_data["file"]).with_suffix(".ngrams.json")
            ngrams = None
            ...
            if ngrams:
                sub = q_lower[:min(len(q_lower), 4)]  # use 1-4 chars dynamically

                # direct hit first
                if sub in ngrams:
                    candidate_words.update(ngrams[sub])
                # fallback: fuzzy partials
                for k in ngrams.keys():
                    if sub in k:
                        candidate_words.update(ngrams[k])
            else:
                # fallback to slow linear scan only if no n-gram index
                for e in idx_data.get("entries", []):
                    key = e.get("key") or e.get("word", "").lower()
                    if pattern.search(key):
                        candidate_words.add(e["word"])

            # now find corresponding entries for those candidates
            for e in idx_data.get("entries", []):
                word = e["word"]
                key = e.get("key") or word.lower()
                if word not in candidate_words or key in seen:
                    continue

                tier = 4
                if key.startswith(q_lower):
                    tier = 1
                elif f" {q_lower} " in key or key.endswith(f" {q_lower}") or key.startswith(f"{q_lower} "):
                    tier = 2
