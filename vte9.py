...
class FindReplaceBar(Gtk.Box):
    ...
    def _collect_matches(self, pattern):
        ...
        for i, line in enumerate(self.buffer.lines):
            for m in pattern.finditer(line):
                self.matches.append((i, m.start(), m.end()))
    ...
    def _goto_match(self, index):
        ...
        line, start, end = self.matches[index]
    ...
    def on_replace_one(self, *a):
        ...
        if not pattern:
            return
        line, start, end = self.matches[self.current_index]
        text = self.buffer.get_line(line)
        replacement = self.replace_entry.get_text()
        new_text = text[:start] + replacement + text[end:]
        self.buffer.set_line(line, new_text)
        self.textview.queue_draw()
        self.on_find_next()
    ...
    def on_replace_all(self, *a):
        ...
        def worker():
            ...
            GLib.idle_add(self.textview.queue_draw)
...
class TextEditorWindow(Adw.ApplicationWindow):
    def __init__(self, **kwargs):
        ...
        def toggle_find_bar(button):
            ...
            if visible:
                # Focus Find entry automatically when shown
        ...
        self.main_box.append(self.find_revealer)       # search bar below toolbar
