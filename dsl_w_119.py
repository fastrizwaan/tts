...
class DictionaryManager:
    ...
    def search(self, q, exact_only=False, max_preview=100, with_defs=False):
        ...
        Optimized: collects per-tier buckets to reduce sorting time.
        Tiers:
          0. ^WORD$           exact
          1. ^WORD[a-zA-Z]    compound
          2. ^WORD\s          phrase start
          3. \sWORD\s         middle
          4. \sWORD$          ends
          5. .*WORD.*         substring
        """
        ...
        # Create per-tier word groups (avoids global sort)
        tier_groups = [defaultdict(list) for _ in range(len(patterns))]
        seen_words = set()
        ...
        for dict_path, idx_data in self.index_cache.items():
            ...
            for e in entries:
                ...
                if key in seen_words:
                    continue
                tier = match_tier(key)
                if tier is None or (exact_only and tier > 0):
                    continue

                seen_words.add(key)
                tier_groups[tier][key].append((dict_name, e, fpath, enc))

        # Merge results tier by tier with local sorting
        results = []
        for tier in range(len(tier_groups)):
            for key in sorted(tier_groups[tier].keys()):
                group_entries = []
                for dict_name, e, fpath, enc in tier_groups[tier][key]:
                    defs = self._read_entry(fpath, e, enc) if with_defs else []
                    group_entries.append((dict_name, defs, "default"))
                results.append((tier_groups[tier][key][0][1]["word"], group_entries))
