...
class MainWindow(Adw.ApplicationWindow):
    ...
    def on_pause_clicked(self, button):
        ...
        # Terminate current audio process if playing
        if self.app.current_process:
            try:
                self.app.current_process.terminate()
                self.app.current_process.wait(timeout=1)
            except:
                try:
    ...
    def play_sentence_sync(self, sentence_info):
        ...
        try:
            ...
            while word_index < len(word_timings) and not self.app.should_stop and self.app.is_playing:
            ...
            # Wait for audio to finish (if not stopped)
