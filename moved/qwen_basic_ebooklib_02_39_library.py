...
# Changes: removed "open another ebook" dialog, hide open button when book loaded,
# show back icon when navigating to library with a loaded book, move menu to content header,
# add library search revealer + search button in content header (filters title/author/path).
...
LIB_COVER_W, LIB_COVER_H = 200, 300   # saved large cover
LIB_DISPLAY_W, LIB_DISPLAY_H = 200, 300  # displayed in FlowBox
...
# --- add this CSS once (near other CssProviders) ---
_LIBRARY_CSS = b"""
.library-grid {
  padding: 12px;
}
...
/* Dark mode - GTK applies .dark class automatically */
.dark .library-card {
...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # header inside sidebar: only library button + title (menu moved to content header)
        ...
        # Book cover + metadata: HORIZONTAL
        ...
        self.book_title.set_max_width_chars(18)
        ...
        # bottom tabs in sidebar (TOC/ann/bookmarks)
        ...
        # show/hide sidebar button in content header (or back icon)
        ...
        # Open button (content header) - keep reference to hide/show
        ...
        self.content_title_label.set_wrap(True)
        self.content_header.set_title_widget(self.content_title_label)

        # search button (toggles library search revealer) - placed before menu on pack_end
        ...
        self.library_search_entry.connect("search-changed", lambda e: self._on_library_search_changed(e.get_text()))
        ...
        # add header and (below it) the search_revealer as top bars of toolbar
        ...
        # reader content box (saved for easy swapping with library)
        ...
        # responsive Breakpoint usage; fallback to size-allocate
        try:
            bp = Adw.Breakpoint()
            try:
        ...
        # start with library visible and disable toggle (no TOC yet)
    ...
    def _on_library_search_changed(self, text):
        ...
        # simply rebuild library view with filter
        self.show_library()

    # ---------- Library UI ----------
    def on_library_clicked(self, *_):
        # behave same as show_library
        self.show_library()

    def show_library(self):
        # Build WrapBox grid with covers + titles
        wrap = Adw.WrapBox()
        wrap.set_align(Gtk.Align.FILL)
        wrap.set_pack_direction(Gtk.Orientation.HORIZONTAL)
        wrap.set_child_spacing(10)
        wrap.set_line_spacing(10)
        wrap.add_css_class("library-grid")

        wrap.set_margin_start(0); wrap.set_margin_end(0)
        wrap.set_margin_top(0); wrap.set_margin_bottom(0)
        ...
        # Open button visibility: hidden if book loaded; visible otherwise
        try:
            if getattr(self, "book", None):
        ...
        # empty-state: show message if no books
        ...
        for entry in entries:
            ...
            if cover and os.path.exists(cover):
                try:
                    ...
                    pb = GdkPixbuf.Pixbuf.new(GdkPixbuf.Colorspace.RGB, False, 8, 180, 200)
                    pb.fill(0xddddddff)
            ...
            prog_lbl = Gtk.Label(label=f"{int(progress*100)}%")
            ...
            def _remove_entry(btn, p=path, coverp=cover):
                try:
                    # compact confirmation using Adw.MessageDialog
            ...
            # Make card clickable with GestureClick
            gesture = Gtk.GestureClick.new()

            def _on_click(_gesture, _n, _x, _y, p=path, resume_idx=idx):
                if p and os.path.exists(p):
                    # immediate replace: save progress and cleanup previous (no dialog)
                    try:
                    ...
                    try:
                        ...
                        pass
                    # show reader content and load new book
    ...
    def _on_sidebar_toggle(self, btn):
        ...
        if isinstance(child, Gtk.Image):
            try:
                child.set_from_icon_name("sidebar-show-symbolic")
            except Exception:
                pass
    ...
    def on_file_opened(self, dialog, result):
        try:
            ...
            if f:
                target = f.get_path()
                # directly save progress, cleanup and open new (no dialog)
                try:
                    self._save_progress_for_library()
                except Exception:
                    pass
                try:
                    ...
                    pass
                # hide open button and enable sidebar toggle for loaded book
                try:
                    self.open_btn.set_visible(False)
                except Exception:
                    pass
                self._enable_sidebar_toggle()
                self.load_epub(target)
        except GLib.Error:
            pass

    # helper to enable sidebar toggle and set proper state
    def _enable_sidebar_toggle(self):
        try:
            self.content_sidebar_toggle.set_sensitive(True)
            self._sidebar_is_back = False
            self._sidebar_img.set_from_icon_name("sidebar-show-symbolic")
    ...
    def load_epub(self, path, resume=False, resume_index=None):
        try:
            ...
            try:
                self._enable_sidebar_toggle()
                # hide Open button while a book is loaded (user requested library as entrypoint)
                try:
                    self.open_btn.set_visible(False)
                except Exception:
                    pass
            ...
            # cleanup previous temporary extraction
            try: self.cleanup()
            except Exception: pass
            ...
            # fallback unzip original epub to preserve layout (if necessary)
                        ...
                        try:
                            ...
                            for src in srcs:
                                ...
                                for v in (candidate_rel, os.path.basename(candidate_rel)):
                                    if v in extracted_paths:
                                        cover_path_to_use = os.path.join(self.temp_dir, v); break
                                    if v in image_names:
                                        cover_item_obj = image_names[v]; break
    ...
    def on_decide_policy(self, webview, decision, decision_type):
        ...
        if decision_type == self.WebKit.PolicyDecisionType.NAVIGATION_ACTION:
            try:
                ...
                print(f"Error getting URI from decision: {e}")
                return False

            if not uri:
                return False

            if uri in ("", "about:blank", "file://"):
                return False
    ...
    def display_page(self, fragment=None):
        ...
        try:
            ...
            try:
                ...
                pass
            # save progress for library entry
            self._save_progress_for_library()
        except Exception as e:
    ...
    def cleanup(self):
        ...
        if self.webview:
            try:
                blank = self._wrap_html("", "")
                self.webview.load_html(blank, "")
            except Exception: pass
        ...
        try:
            ...
            if not getattr(self, "book", None):
                self.content_sidebar_toggle.set_sensitive(False)
            # restore Open button visibility when there's no loaded book
            try:
                self.open_btn.set_visible(True)
            except Exception:
                pass
    ...
    def _update_library_entry(self):
        ...
        if cover_src and os.path.exists(cover_src):
            try:
                # deterministic filename based on book path
                ...
                # load original pixbuf (if loader fails, fallback to simple copy)
                try:
                    pix = GdkPixbuf.Pixbuf.new_from_file(cover_src)
                    # scale preserving aspect to LIB_COVER_W x LIB_COVER_H maximum
                    scaled = pix.scale_simple(LIB_COVER_W, LIB_COVER_H, GdkPixbuf.InterpType.BILINEAR)
                    # save scaled image
                    scaled.savev(cover_dst, ext.replace(".", ""), [], [])
                except Exception:
                    # fallback: raw copy (if original is fine)
    ...
    def resume_book(self, *_):
        try:
            ...
            try:
                if getattr(self, "toc_root_store", None) and self.toc_root_store.get_n_items() > 0:
                    # restore sidebar button to normal toggle and show sidebar
                    self._sidebar_is_back = False
                    self._sidebar_img.set_from_icon_name("sidebar-show-symbolic")
                    self.content_sidebar_toggle.set_tooltip_text("Show/Hide sidebar")
                    ...
                    # If no TOC, keep sidebar hidden and enable toggle (user can still toggle)
                    self._sidebar_is_back = False
                    self._sidebar_img.set_from_icon_name("sidebar-show-symbolic")
                    self.content_sidebar_toggle.set_tooltip_text("Show/Hide sidebar")
                    self.content_sidebar_toggle.set_sensitive(True)
