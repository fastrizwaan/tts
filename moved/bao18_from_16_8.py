...
class Win(Adw.ApplicationWindow):
    ...
    def _build_foliate_toc(self, parent_box, nodes, level=0):
        ...
        print(f"\n🏗️  Building TOC at level {level}, {len(nodes)} nodes")
        for idx, node in enumerate(nodes):
            ...
            print(f"  Node[{idx}]: title='{title[:40]}', href='{href}', children={len(children)}")

            if children:
                print(f"    Creating expander for '{title[:30]}'")
                ...
                # Add chevron as a prefix widget
                ...
                try:
                    header_row.add_prefix(chev)
                except Exception as e:
                    print(f"    ⚠️  Could not add prefix: {e}")
                ...
                def _make_toggle(href_local, revealer_local, chev_local):
                    def _toggle_and_nav():
                        print(f"\n🖱️  Expander clicked! href='{href_local}'")
                        try:
                            ...
                            if href_local:
                                print(f"   Calling _on_toc_clicked with href='{href_local}'")
                                self._on_toc_clicked(None, href_local)
                                self._set_active(href_local)
                            else:
                                print(f"   ⚠️  No href for this expander, skipping navigation")
                        except Exception as e:
                ...
                try:
                    handler_id = header_row.connect("activated", lambda w, fn=toggle_fn: fn())
                    print(f"    ✅ Connected 'activated' signal for expander (handler_id={handler_id})")
                except Exception as e:
                    print(f"    ❌ Failed to connect activated signal: {e}")

                try:
                    gesture = Gtk.GestureClick.new()
                    def on_expander_click(g, n_press, x, y, fn=toggle_fn):
                        print(f"\n🖱️  GESTURE Click on expander!")
                        fn()
                    gesture.connect("pressed", on_expander_click)
                    header_row.add_controller(gesture)
                    print(f"    ✅ Added gesture click controller for expander (on header_row)")
                except Exception as e:
                    print(f"    ❌ Failed to add gesture: {e}")
                ...
                if href:
                    ...
                    print(f"    ✅ Added to row_map: '{href}'")
            else:
                print(f"    Creating leaf for '{title[:30]}'")
                row = Adw.ActionRow()
                row.set_title(safe_title)
                row.set_activatable(True)
                
                # Create a callback function with proper closure
                def make_leaf_callback(h):
                    def callback(w):
                        print(f"\n🖱️  Leaf clicked! href='{h}'")
                        print(f"   Widget: {w}")
                        print(f"   Calling _on_toc_clicked with href='{h}'")
                        self._on_toc_clicked(w, h)
                        self._set_active(h)
                    return callback
                
                leaf_callback = make_leaf_callback(href)
                handler_id = row.connect("activated", leaf_callback)
                print(f"    ✅ Connected 'activated' signal (handler_id={handler_id}) for leaf with href='{href}'")
                
                # Also add a click gesture as backup
                try:
                    gesture = Gtk.GestureClick.new()
                    def on_click(g, n_press, x, y, h=href):
                        print(f"\n🖱️  GESTURE Click detected! href='{h}'")
                        self._on_toc_clicked(None, h)
                        self._set_active(h)
                    gesture.connect("pressed", on_click)
                    row.add_controller(gesture)
                    print(f"    ✅ Added gesture click controller as backup")
                except Exception as e:
                    print(f"    ⚠️  Could not add gesture: {e}")
    ...
    def _on_toc_clicked(self, widget, href):
        ...
        print(f"\n🔍 === TOC CLICK HANDLER CALLED ===")
        print(f"   Widget: {widget}")
        print(f"   Href received: '{href}'")
        print(f"   Href type: {type(href)}")
        
        if not self.book:
            print(f"   ❌ ERROR: No book loaded!")
            return
            
        if not href:
            print(f"   ❌ ERROR: No href provided!")
            return
        ...
        print(f"\n🔎 Starting search strategies...")
        print(f"   Total items in item_map: {len(self.item_map)}")
        
        # Strategy 1: Direct lookup from item_map
        print(f"\n1️⃣  Strategy 1: Direct lookup for '{href_path}'")
        if href_path in self.item_map:
            item = self.item_map[href_path]
            print(f"   ✅ Found via direct lookup")
        else:
            print(f"   ❌ Not found in direct lookup")
        
        # Strategy 2: Try with common EPUB base directories
        if not item:
            print(f"\n2️⃣  Strategy 2: Trying with base directories...")
            ...
            for base in common_bases:
                ...
                if test_path in self.item_map:
                    ...
                    break
            if not item:
                print(f"   ❌ Not found with any base directory")
        
        # Strategy 3: Try basename match
        if not item:
            href_basename = os.path.basename(href_path)
            print(f"\n3️⃣  Strategy 3: Trying basename match for '{href_basename}'")
            for item_name, item_obj in self.item_map.items():
                if os.path.basename(item_name) == href_basename:
                    ...
                    break
            if not item:
                print(f"   ❌ Not found via basename match")
        
        # Strategy 4: Search spine
        if not item:
            print(f"\n4️⃣  Strategy 4: Searching spine ({len(self.spine)} items)...")
            for spine_entry in self.spine:
                ...
                if (href_path == spine_href or 
                    ...
                    break
            if not item:
                print(f"   ❌ Not found in spine")
        
        # Strategy 5: Partial path matching (suffix match)
        if not item:
            print(f"\n5️⃣  Strategy 5: Trying partial path matching...")
            href_parts = href_path.split('/')
            print(f"   Href parts: {href_parts}")
            for item_name, item_obj in self.item_map.items():
                ...
                if len(href_parts) <= len(item_parts):
                    if item_parts[-len(href_parts):] == href_parts:
                        ...
                        break
            if not item:
                print(f"   ❌ Not found via partial match")
        
        # Strategy 6: Case-insensitive match
        if not item:
            print(f"\n6️⃣  Strategy 6: Trying case-insensitive match...")
            ...
            for item_name, item_obj in self.item_map.items():
                if (item_name.lower() == href_lower or 
                    ...
                    break
            if not item:
                print(f"   ❌ Not found via case-insensitive match")
        ...
        if item:
            ...
            print(f"   Is ITEM_DOCUMENT: {item.get_type() == ITEM_DOCUMENT}")
        
        if item and item.get_type() == ITEM_DOCUMENT:
            try:
                print(f"\n✅ Loading content from: {item.get_name()}")
                html_text = item.get_content().decode("utf-8", errors="ignore")
                print(f"   Content length: {len(html_text)} bytes")
                processed_content = self._process_epub_content(html_text, item)
                print(f"   Processed content length: {len(processed_content)} bytes")
                self.original_html_content = processed_content
                print(f"   Calling apply_column_layout...")
                self.apply_column_layout(self.current_columns - 1)
                print(f"   ✅ Content loaded successfully!")

                # Jump to fragment if present
                if fragment:
                    print(f"   🔗 Jumping to fragment: '{fragment}'")
                    js = f"""
                    setTimeout(function() {{
                        const el = document.getElementById("{fragment}") || 
                                   document.querySelector("[name='{fragment}']") ||
                                   document.querySelector("a[id='{fragment}']");
                        if (el) {{
            ...
            error_msg = f"TOC target '{href}' not found in book."
            print(f"\n❌ {error_msg}")
            print(f"\n📋 Available items in item_map:")
            for idx, name in enumerate(list(self.item_map.keys())[:10]):
                print(f"  {idx+1}. {name}")
            if len(self.item_map) > 10:
                print(f"  ... and {len(self.item_map) - 10} more")
