...
class EPubViewer(Adw.ApplicationWindow):
    ...
    def _toc_on_setup(self, factory, list_item):
        # wrapper ensures ListView row always has a box child (prevents "box != NULL")
        wrapper = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=0)

        row = Adw.ActionRow()
        row.set_activatable(True)

        # per-row disclosure image (collapsed by default); may be hidden for leaves in bind
        disc = Gtk.Image.new_from_icon_name("pan-end-symbolic")
        row.add_prefix(disc)
        row.set_title("")  # populated in bind
        wrapper.append(row)

        # container for nested ListView (children)
        ...
        def _activate_item():
            ...
            # toggle if it has children (and navigate to parent's target)
            if item.children.get_n_items() > 0:
                ...
                # ALSO navigate to parent target (index preferred)
                href = item.href or ""
        ...
        # connect both keyboard activation and mouse presses
        try:
            row.connect("activated", lambda _r: _activate_item())
        except Exception:
            pass
        g = Gtk.GestureClick()
        g.connect("pressed", lambda gesture, n_press, x, y: _activate_item())
    ...
    def _toc_on_bind(self, factory, list_item):
        ...
        row = getattr(list_item, "_row", None)
        disc = getattr(list_item, "_disc", None)
        nested = getattr(list_item, "_nested", None)
        if row is None or disc is None or nested is None:
            self._toc_on_setup(factory, list_item)
            row = list_item._row; disc = list_item._disc; nested = list_item._nested
        ...
        # show disclosure only for items that actually have children
        disc.set_visible(has_children)
        if has_children:
            disc.set_from_icon_name("pan-down-symbolic" if nested.get_visible() else "pan-end-symbolic")
        else:
            # ensure no icon for leaves
            disc.set_from_icon_name(None)

        # build nested ListView once; child rows are Adw.ActionRow too (recursive style)
        if has_children and not getattr(list_item, "_nested_view", None):
            def nsetup(f, li):
                # child action row (use ActionRow for uniform look)
                ch_wrapper = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=0)
                ch_row = Adw.ActionRow()
                ch_row.set_activatable(True)

                # per-child disc (may be hidden for leaf children)
                ch_disc = Gtk.Image.new_from_icon_name("pan-end-symbolic")
                ch_row.add_prefix(ch_disc)
                ch_row.set_title("")  # set in bind
                ch_wrapper.append(ch_row)
                ...
                ch_wrapper.append(ch_nested)

                def _activate_child():
                    it = li.get_item()
                    if not it:
                        return
                    # if child has its own children -> toggle
                    if it.children.get_n_items() > 0:
                        ...
                        if gv:
                            gv.set_visible(vis)
                        # also navigate to child's target if present
                        href = it.href or ""
                ...
                # signals for keyboard and mouse
                try:
                    ch_row.connect("activated", lambda _r: _activate_child())
                except Exception:
                    pass
                gch = Gtk.GestureClick()
                gch.connect("pressed", lambda gesture, n_press, x, y: _activate_child())
                ch_row.add_controller(gch)

                li.set_child(ch_wrapper)
                # stash widgets for bind
                li._row = ch_row
                li._disc = ch_disc
            ...
            def nbind(f, li):
                it = li.get_item()
                ch_row = getattr(li, "_row", None)
                ch_disc = getattr(li, "_disc", None)
                ch_nested = getattr(li, "_nested", None)
                if not it or ch_row is None or ch_disc is None or ch_nested is None:
                    return
                has_kids = it.children.get_n_items() > 0
                ch_row.set_title(it.title or "")
                ch_disc.set_visible(has_kids)
                if has_kids:
                ...
                # if grandchildren exist and not created yet, create nested ListView recursively
                if has_kids and not getattr(li, "_nested_view", None):
                    # create sub-factory that reuses the same nsetup/nbind (recursion)
                    sub_factory = Gtk.SignalListItemFactory()
                    sub_factory.connect("setup", nsetup)
                    sub_factory.connect("bind", nbind)
            ...
            nfactory.connect("setup", nsetup)
            nfactory.connect("bind", nbind)
            ...
            # ensure nested view follows nested box visibility
            nested_view.set_visible(nested.get_visible())

        # keep nested view visibility in sync when re-bound
        nv = getattr(list_item, "_nested_view", None)
        if nv:
