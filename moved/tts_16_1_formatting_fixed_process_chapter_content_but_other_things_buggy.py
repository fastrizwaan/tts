...
Uses epub (zip) extraction via stdlib (zipfile) instead of ebooklib / BeautifulSoup.
Preserves original app features: columns (fixed/width), single-column scrolling,
page navigation, mouse scroll snapping, PageUp/PageDown/Home/End, TTS (Kokoro),
and keeps local epub.min.js/jszip.min.js available in extracted resources.

Place local jszip.min.js and epub.min.js next to this script if you want them copied.
"""
import os, json, tempfile, shutil, re, urllib.parse, signal, sys, math, threading, queue, subprocess, uuid, time, pathlib, hashlib, multiprocessing, zipfile
...
# --- TTS Manager (unchanged features, robust) ---
class TTSManager:
    """
...
class EpubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # epub state
        self.current_book = None            # set to dict on load
        self.chapters = []
    ...
    def on_key_pressed(self, controller, keyval, keycode, state):
        ...
        # Multi-column navigation preserved
    ...
    # -----------------------
    # EPUB handling (zip-based, preserving features)
    # -----------------------
    ...
    def load_epub(self, filepath):
        try:
            ...
            # set current_book (keeps UI enabling logic intact)
            self.current_book = {'path': filepath, 'title': os.path.basename(filepath)}
    ...
    def _parse_opf_and_build_lists(self):
        ...
        root = tree.getroot()
        manifest = {}
        # robust namespace-agnostic search
    ...
    def extract_chapters(self):
        ...
        if not self.manifest or not self.spine:
            return
        # build available resources map (basename -> relative path from temp_dir)
        resources_map = {}
        for root, dirs, files in os.walk(self.temp_dir):
            for fn in files:
                # skip processed chapter outputs to avoid loops
                if fn.endswith('.processed.html'):
                    continue
                rel = os.path.relpath(os.path.join(root, fn), self.temp_dir)
                resources_map.setdefault(fn, rel)
        ...
        for item_id in self.spine:
            ...
            # accept xhtml/html
            if path and os.path.exists(path) and ('application/xhtml+xml' in media_type or path.lower().endswith(('.xhtml', '.html', '.htm'))):
                try:
                ...
                processed_content = self.process_chapter_content(content, path, resources_map)
    ...
    def process_chapter_content(self, content, item, resources_map=None):
        """
        Minimal non-destructive processing for chapter HTML pages.

        - Detects XHTML vs HTML and emits XHTML-safe <base /> if needed.
        - Injects a tiny, harmless CSS block that:
            * preserves heading weight when authors used odd classes
            * exposes theme variables (fallbacks only)
            * defines .tts-highlight appearance (uses CSS variables so dark-mode user script wins)
        - DOES NOT change body column rules, heights, margins, or set explicit colors so
          your external JS (column layout) and user-content theme injection keep working.
        """
        try:
            # If using epub.js externally (rendering through epub.js), skip processing
            if getattr(self, "use_epubjs", False):
                return content

            # ---- detect XHTML-ish documents ----
            is_xhtml = False
            if re.search(r'^\s*<\?xml\b', content[:200], flags=re.IGNORECASE):
                is_xhtml = True
            elif re.search(r'<html\b[^>]*\bxmlns=["\']http://www.w3.org/1999/xhtml', content, flags=re.IGNORECASE):
                is_xhtml = True

            # ---- base href so relative resources resolve ----
            try:
                base_dir = os.path.dirname(os.path.abspath(item))
                base_href = GLib.filename_to_uri(base_dir + os.sep)
            except Exception:
                base_href = ""

            base_tag = f'<base href="{base_href}" />\n' if is_xhtml else f'<base href="{base_href}">\n'

            # ---- CSS: minimal, non-destructive ----
            # NOTE: we intentionally avoid touching body, column-* or background/color.
            css = r"""
    /* injected: preserve heading weight and avoid breaks (non-destructive) */
    [class^="heading"], [class*=" heading_"], [class*="heading-"], [class*="heading_"] {
    ...
    /* ensure actual headings keep strong weight */
    h1,h2,h3,h4,h5,h6 { font-weight: bold !important; line-height: 1.15 !important; }

    /* Theme variables fallback (do not override existing variables) */
    :root {
        --viewer-bg: unset;
        --viewer-text: unset;
        --tts-highlight-bg: rgba(255,235,59,0.95);
        --tts-highlight-text: #000000;
    }

    /* Highlight style uses variables so theme/user scripts can override them */
    .tts-highlight {
        background: var(--tts-highlight-bg, rgba(255,235,59,0.95)) !important;
        color: var(--tts-highlight-text, #000) !important;
    ...
    """
            ...
            # Wrap CSS in CDATA for XHTML to avoid XML issues.
            if is_xhtml:
                style_block = f'<style type="text/css" data-app="injected-heading-preserve">/*<![CDATA[*/\n{css}\n/*]]>*/</style>\n'
            else:
                style_block = f'<style data-app="injected-heading-preserve">\n{css}\n</style>\n'

            head_injection = base_tag + style_block

            # ----- find insertion point -----
            lower = content.lower()
            idx_head_close = lower.find("</head>")
            if idx_head_close != -1:
                # Insert before </head>
                insert_at = idx_head_close
                new_content = content[:insert_at] + head_injection + content[insert_at:]
                return new_content

            # If there is an open <head> tag, insert after it
            m_head_open = re.search(r"<head\b[^>]*>", content, flags=re.IGNORECASE)
            if m_head_open:
                insert_at = m_head_open.end()
                new_content = content[:insert_at] + "\n" + head_injection + content[insert_at:]
                return new_content

            # If there's an <html> tag, create a <head> after it
            m_html_open = re.search(r"<html\b[^>]*>", content, flags=re.IGNORECASE)
            if m_html_open:
                insert_at = m_html_open.end()
                head_wrapper = "\n<head>\n" + head_injection + "</head>\n"
                new_content = content[:insert_at] + head_wrapper + content[insert_at:]
                return new_content

            # No html/head present -> do a minimal safe wrapper (preserve xml prolog if present)
            xml_prolog = ""
            m_xml = re.match(r'^\s*(<\?xml[^>]*\?>\s*)', content, flags=re.IGNORECASE)
            rest = content
            if m_xml:
                xml_prolog = m_xml.group(1)
                rest = content[len(xml_prolog):]

            if is_xhtml:
                wrapped = (
                    f'{xml_prolog}<!doctype html>\n'
            ...
            print(f"[warn] process_chapter_content exception: {e}")
    ...
    def load_chapter(self):
        ...
        # initial layout/default columns only once unless user set columns
    ...
    def _on_page_info_result(self, webview, result, user_data):
        # best-effort: try to read result, otherwise fall back to chapter index display
        try:
            # result may be a WebKitJavascriptResult; attempt to get string via API if available
            try:
                jsval = result.get_js_value()  # may or may not exist depending on binding
                s = str(jsval.to_string()) if jsval is not None else None
            except Exception:
                # fallback: show simple "X/Y" using chapter index
                s = None
        except Exception:
            s = None
        if s:
            try:
                self.page_info.set_text(s)
                return
            except Exception:
                pass
        # fallback: show chapter-position-ish info
        if self.current_book and self.chapters:
            self.page_info.set_text(f"{self.current_chapter+1}/{len(self.chapters)}")
        else:
            self.page_info.set_text("--/--")
