...
class Win(Adw.ApplicationWindow):
    ...
    def _populate_epub_ui(self, toc_nodes):
        ...
        try:
            if self.book:
                ...
                spine = [i for i in self.book.get_items() if i.get_type() == 9]  # 9 is ITEM_DOCUMENT
                if spine:
    ...
    def _on_toc_clicked(self, widget, href):
        ...
        for item in self.book.get_items():
            ...
            # match by tail or full href (handles subfolders)
            if ihref.endswith(target_path) or os.path.basename(ihref) == os.path.basename(target_path):
                try:
                    html_text = item.get_content().decode("utf-8", errors="ignore")
                    processed_content = self._process_epub_content(html_text, item)

                    # our CSS and images are already embedded; use a neutral base
                    self.webview.load_html(processed_content, "about:blank")

                    # if there's a fragment, scroll to it after load
                    if frag:
                        def _after_load(_wv, ev):
                            if ev == WebKit.LoadEvent.FINISHED:
                                js = f'''
                                    (function(){{
                                        var el = document.getElementById({json.dumps(frag)});
                                        if(!el) el = document.querySelector({json.dumps(f'#{frag}')});
                                        if(el) el.scrollIntoView({{"block":"start","inline":"nearest","behavior":"auto"}});
                                    }})();
                                '''
                                try:
                                    _wv.disconnect(handler_id)
                                except Exception:
                                    pass
                                _wv.evaluate_javascript(js, -1, None, None, None, None, None)

                        import json  # stdlib
                        handler_id = self.webview.connect("load-changed", _after_load)

                    # keep your active-row highlighting behavior
