...
class Win(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # Add event controller for keyboard events
    ...
    def apply_column_layout(self, selected_column_index):
        ...
        # Add JavaScript for snapping scroll positions
        js_script = f"""
        ...
        function snapScroll() {{
            ...
            // Calculate which column we're currently on
            const currentColumn = Math.round(currentScroll / colWidth);
            
            // Calculate target scroll position
            const targetScroll = currentColumn * colWidth;
            
            // Only scroll if we're not already at the target (with tolerance)
        ...
        document.addEventListener('wheel', function(e) {{
            if (e.deltaX !== 0) return; // Already horizontal scroll
            ...
            // Perform horizontal scroll to snapped position
            window.scrollTo(targetScroll, window.scrollY);
        ...
        document.addEventListener('keydown', function(e) {{
            ...
            switch(e.key) {{
                ...
                case 'PageUp':
                    e.preventDefault();
                    const targetLeft = Math.round((currentScroll - colWidth) / colWidth) * colWidth;
                    window.scrollTo(targetLeft, window.scrollY);
                    break;
                case 'ArrowRight':
                case 'PageDown':
                    e.preventDefault();
                    const targetRight = Math.round((currentScroll + colWidth) / colWidth) * colWidth;
                    window.scrollTo(targetRight, window.scrollY);
                    break;
                case 'Home':
                    e.preventDefault();
                    window.scrollTo(0, 0);
        ...
        if '<body>' in original_html.lower() and '</body>' in original_html.lower():
            ...
            # Find the end tag after the start position
    ...
    def on_size_changed(self, *args):
        ...
        GLib.timeout_add(100, lambda: self.update_column_count_in_js())
        GLib.idle_add(self.apply_column_layout, self.current_columns - 1)
    
    def update_column_count_in_js(self):
        js_code = f"window.currentColumnCount = {self.current_columns};"
        self.webview.run_javascript(js_code, None, None, None)
        return False  # Don't repeat

    def on_key_pressed(self, controller, keyval, keycode, state):
        # Handle keyboard navigation - now handled in JavaScript
        # This is a fallback in case JavaScript doesn't work
        ...
        if keyval in [Gdk.KEY_Left, Gdk.KEY_Right, Gdk.KEY_Page_Up, Gdk.KEY_Page_Down, Gdk.KEY_Home, Gdk.KEY_End]:
            if self.current_columns <= 1:
                return False  # No horizontal scrolling needed for single column

            # The actual scrolling is handled in JavaScript
            # This is just to indicate the event was handled
            return True
