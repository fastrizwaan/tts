#!/usr/bin/env python3
# epubviewer_foliate_expander_style_ellipsize.py
...
/* expander header style: height like a button + rounded */
.toc-expander-row {
  min-height: 36px;
  border-radius: 10px;
  margin-right: 6px;
...
/* leaf rows slightly shorter but aligned */
.toc-leaf {
  min-height: 36px;
  border-radius: 10px;
  margin-right: 6px;
...
/* chevron larger */
.toc-chev { margin-left: 6px; margin-right: 8px; }
...
class EPubViewerWindow(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # connect minimal responsive hooks
    ...
    # TOC builder — ensure right-side ellipsize for long names
    def _build_foliate_toc(self, parent_box, nodes, level=0):
        ...
        for node in nodes:
            ...
            # escape markup chars so GTK/Pango won't treat text as markup
            ...
            if children:
                ...
                # allow header_box to constrain label width so ellipsize works
                try:
                ...
                lbl.set_xalign(0)
                lbl.set_wrap(False)  # important: single line
                lbl.set_ellipsize(Pango.EllipsizeMode.END)
                lbl.set_hexpand(True)
                # safe optional char width cap
                try:
                ...
                child_container.set_margin_start(indent_px + 12)
                ...
                row = Adw.ActionRow()
                # leaf: use a safe label as child to avoid markup parsing
                lbl = Gtk.Label()
                lbl.set_text(safe_title)
                lbl.set_xalign(0)
                lbl.set_wrap(False)
    ...
    # small error dialog (keeps simple API)
