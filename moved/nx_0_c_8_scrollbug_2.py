...
class EPUBWindow(Adw.ApplicationWindow):
    ...
    def on_webview_load_changed(self, _webview, event):
        if event == WebKit.LoadEvent.COMMITTED:
            # Apply layout immediately on commit to prevent flicker
            self.apply_layout()
        elif event == WebKit.LoadEvent.FINISHED:
            # Reapply and reset scroll position
            self.apply_layout()
            GLib.timeout_add(50, self.reset_scroll_position)
    ...
    def load_prev_spine_item(self):
        ...
        if prev >= 0:
            ...
            it = self.app.book.get_item_with_id(item_id)
            if it:
                # Set flag to scroll to end after loading
