...
class EPubViewer(Adw.ApplicationWindow):
    ...
    def is_single_column_mode(self):
        """Return True if currently displaying a single column."""
        try:
            if not getattr(self, "column_mode_use_width", False) and getattr(self, "column_count", 1) <= 1:
                return True
            if getattr(self, "column_mode_use_width", False):
                width = self.get_allocated_width() or 1200
                available = max(100, width - (2 * getattr(self, "_column_gap", 32)))
                if getattr(self, "column_width_px", 400) >= (available - getattr(self, "_column_gap", 32)):
                    return True
            return False
        except Exception:
            return True
    ...
    def snap_to_nearest_step(self):
        if not self.book or self.is_single_column_mode():
            self.snap_timeout_id = None
            return False
        self.calculate_column_dimensions()
        js_code = f"""
        (function(){{
            var cw={int(self.actual_column_width)},cg={int(getattr(self, "_column_gap", 32))};
            var s=cw+cg,cur=window.pageXOffset||document.documentElement.scrollLeft;
            var n=Math.round(cur/s),t=n*s,m=Math.max(0,document.documentElement.scrollWidth-(window.innerWidth||document.documentElement.clientWidth));
            t=Math.max(0,Math.min(t,m));
            if(Math.abs(cur-t)>5)window.scrollTo({{left:t,behavior:'smooth'}});
        }})();
        """
    ...
    def calculate_column_dimensions(self):
        ...
        if not getattr(self, "column_mode_use_width", False):  # fixed mode
            ...
            self.column_width_px = cw
        else:
            desired = int(getattr(self, "column_width_px", 400))
            self.column_width_px = max(50, min(desired, available))
    ...
    def on_scroll_event(self, controller, dx, dy):
        ...
        if abs(dx) > 0.1 or abs(dy) > 0.1:
            ...
            fixed_mode = not getattr(self, "column_mode_use_width", False)
            column_gap = int(getattr(self, "_column_gap", 32))
            if scroll_left:
                if fixed_mode:
                    column_width = int(self.column_width_px)
                    js_code = f"""
                    (function(){{
                        var w={column_width},g={column_gap},s=w+g;
                    ...
                    (function(){{
                        ...
                        var step=cw+g;
                        var col=Math.max(0,Math.round(cur/step)-1);
                        window.scrollTo({{left:col*step,behavior:'smooth'}});
                    }})();"""
            elif scroll_right:
                if fixed_mode:
                    column_width = int(self.column_width_px)
                    ...
                    (function(){{
                        ...
                        var cw=(avail-total)/n;
                        var step=cw+g;
                        var col=Math.round(cur/step)+1;
                        var tgt=Math.min(max,col*step);
                        window.scrollTo({{left:tgt,behavior:'smooth'}});
                    }})();"""
            else:
                return False
    ...
    def on_key_pressed(self, controller, keyval, keycode, state):
        ...
        if self.is_single_column_mode():
            single_map = {
                ...
                (function(){
                  ...
                  var target=Math.max(0,s-lines*lh);
                ...
                (function(){
                  ...
                  if(!lh||isNaN(lh)){var fs=parseFloat(cs.fontSize)||16;lh=fs*1.2;}
                  var lines=Math.max(1,Math.floor(h/lh));
                  var target=s+lines*lh;
                  var max=Math.max(0,d.scrollHeight-h);
                  if(target>max)target=max;
                  window.scrollTo({top:target,behavior:'smooth'});
                })();""",
                Gdk.KEY_Home: "window.scrollTo({top:0,behavior:'smooth'});",
                Gdk.KEY_End:  "window.scrollTo({top:document.documentElement.scrollHeight,behavior:'smooth'});"
            }
            js = single_map.get(keyval)
            if js:
        ...
        if fixed_mode:
            ...
            (function(){{
              ...
              var tgt=Math.max(0,col-per);
              window.scrollTo({{left:tgt*s,behavior:'smooth'}});
              setTimeout(()=>window.scrollTo({{left:tgt*s,behavior:'auto'}}),400);
            }})();"""
            js_right = f"""
            (function(){{
              var w={column_width},g={column_gap},s=w+g;
        ...
        if keyval in (Gdk.KEY_Left, 65361, Gdk.KEY_Page_Up, 65365):
            self.webview.evaluate_javascript(js_left, -1, None, None, None, None, None)
            return True
        elif keyval in (Gdk.KEY_Right, 65363, Gdk.KEY_Page_Down, 65366):
            self.webview.evaluate_javascript(js_right, -1, None, None, None, None, None)
            return True
    ...
    def setup_navigation(self):
        self.h_adjustment = self.scrolled.get_hadjustment()
        ...
        self.scrolled.add_controller(scroll_controller2)
        self.snap_timeout_id = None
