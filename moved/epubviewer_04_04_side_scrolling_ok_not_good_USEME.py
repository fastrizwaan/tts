...
class EPubViewer(Adw.ApplicationWindow):
    ...
    def _webview_horizontal_margins(self):
        """Return horizontal margin (left + right) around WebView content."""
        try:
            # If you have style or padding vars, use them; else, just return a constant
            return getattr(self, "page_margin_left", 20) + getattr(self, "page_margin_right", 20)
        except Exception:
            return 40
    ...
    def calculate_column_dimensions(self):
        """Compute actual column width based on mode (fixed-count or width-based)."""
        try:
            alloc = self.get_allocation()
            width, height = alloc.width or 1200, alloc.height or 800
            gap = getattr(self, "_column_gap", 32)
            padding = 0
            available = max(100, width - 2 * padding - 2 * gap)
            ...
            if not getattr(self, "column_mode_use_width", False):
                ...
                cw = max(50, min(int(getattr(self, "column_width_px", 400)), available))
                self.actual_column_width = cw
        except Exception as e:
            print("calculate_column_dimensions error:", e)
    ...
    def on_scroll_event(self, controller, dx, dy):
        ...
        # determine if we use fixed column count or width-based
        is_fixed = not getattr(self, "column_mode_use_width", False)
        ...
        if abs(dx) > 0.1 or abs(dy) > 0.1:
            ...
            if scroll_left:
                if is_fixed:
                    column_width = int(getattr(self, "actual_column_width", 400))
                    column_gap = int(getattr(self, "_column_gap", 32))
                    js_code = f"""
                    (function(){{
                        var s={column_width}+{column_gap};
                        var cur=Math.round((window.pageXOffset||document.documentElement.scrollLeft)/s)-1;
                        window.scrollTo({{left:Math.max(0,cur*s),behavior:'smooth'}});
                    }})();
                    """
                else:
                    desired_width = int(getattr(self, "column_width_px", 400))
                    column_gap = int(getattr(self, "_column_gap", 32))
                    js_code = f"""
                    (function(){{
                        var s={desired_width}+{column_gap};
                        var cur=Math.round((window.pageXOffset||document.documentElement.scrollLeft)/s)-1;
                        window.scrollTo({{left:Math.max(0,cur*s),behavior:'smooth'}});
                    }})();
                    """
                ...
                if is_fixed:
                    ...
                    (function(){{
                        var s={column_width}+{column_gap};
                        var cur=Math.round((window.pageXOffset||document.documentElement.scrollLeft)/s)+1;
                        var maxScroll=Math.max(0,document.documentElement.scrollWidth-window.innerWidth);
                        window.scrollTo({{left:Math.min(maxScroll,cur*s),behavior:'smooth'}});
                    }})();
                    """
                else:
                    desired_width = int(getattr(self, "column_width_px", 400))
    ...
    def on_scroll_position_changed(self, adjustment):
        """Called when horizontal adjustment (scroll) changes."""
        try:
            # placeholder: could update progress, buttons, or info
            pass
        except Exception:
            pass

    def on_key_pressed(self, controller, keyval, keycode, state):
        """Handle basic keyboard column navigation."""
        try:
            # Page Left / Right keys
            if keyval in (Gdk.KEY_Left, Gdk.KEY_Page_Up):
                js = "(function(){window.scrollBy({left:-window.innerWidth,behavior:'smooth'});})();"
                ...
                js = "(function(){var m=Math.max(0,document.documentElement.scrollWidth-window.innerWidth);window.scrollTo({left:m,behavior:'smooth'});})();"
                self.webview.evaluate_javascript(js, -1, None, None, None, None, None)
                return True
        except Exception:
            pass
