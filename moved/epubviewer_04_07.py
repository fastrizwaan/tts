...
###################
    ...
    def _wrap_html(self, raw_html, base_uri):
        ...
        This version ensures user font/size/line-height/justification and column-gap
        are injected after EPUB CSS so they persist when we rewrap the page.
        """
        ...
        try:
            ...
            # allow external controls to have set self._column_gap
            ...
            # Build user overrides (font/size/line-height/justify) and append AFTER EPUB CSS
            extra = ""
            try:
                fam = getattr(self, "user_font_family", None)
                sz  = getattr(self, "user_font_size", None)
                # user_font_size stored as plain number string (e.g. "14") or "14pt" in some places;
                if sz:
                    try:
                        # normalize to 'Npt' if numeric
                        if isinstance(sz, (int, float)): sz_val = f"{sz}pt"
                        else:
                            s = str(sz)
                ...
                justify = getattr(self, "user_justify", None)  # expected: 'none' | 'full' | 'hyphen'
            ...
            # fallback to the plain page_css if anything fails
            page_css = (self.css_content or "") + "\n" + THEME_INJECTION_CSS
        ...
        (function() {
          ...
          function onKey(e) {
            ...
            if (e.code === 'PageDown') {
              ...
              e.preventDefault();
              const step = columnStep(cont);
              window.scrollTo({ left: cont.scrollWidth - step, top: 0, behavior: 'smooth' });
            }
          }

          function onResize() {
            try { snapToNearestColumn(); } catch(e){}
          }
