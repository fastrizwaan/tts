...
class Win(Adw.ApplicationWindow):
    ...
    def load_epub(self, path):
        ...
        if not toc_nodes:
            toc_nodes = []
            try:
                docs = [i for i in self.book.get_items() if i.get_type() == epub.ITEM_DOCUMENT] # Fixed constant access
    ...
    def _populate_epub_ui(self, toc_nodes):
        ...
        try:
            if self.book:
                # Get all document items (HTML/XHTML content) - Fixed constant access
    ...
    def _process_epub_content(self, html_content, item):
        ...
        css_content = ""
        try:
            # Get all CSS/style items (type 3 is ITEM_STYLE) - Fixed constant access
            for css_item in self.book.get_items():
                if css_item.get_type() == epub.ITEM_STYLE:
                    try:
        ...
        try:
            # Get all image items (type 2 is ITEM_IMAGE) - Fixed constant access
            for img_item in self.book.get_items():
                if img_item.get_type() == epub.ITEM_IMAGE:
                    try:
        ...
        for item in self.book.get_items():
            ...
            if item_filename == target_filename:
                try:
                    ...
                    # Store the new content for column layout
                    self.original_html_content = processed_content
                    # Apply the column layout with the new content
                    self.apply_column_layout(self.current_columns - 1)
                except Exception as e:
