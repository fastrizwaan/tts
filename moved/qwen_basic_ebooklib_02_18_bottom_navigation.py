...
# --- CSS: rounded action-row highlight (scoped to epub-sidebar) ---
# do NOT set sidebar background so theme paints it
_css = """
/* scope only rows inside our sidebar */
.epub-sidebar .adw-action-row {
  margin: 4px 6px;           /* space so rounded corners are visible */
  padding: 6px;              /* inner spacing */
  border-radius: 8px;        /* rounded corners for the highlight band */
  background-color: transparent; /* keep theme default except hover/selected */
  -gtk-icon-transform: none;
}

/* hover: subtle rounded band */
.epub-sidebar .adw-action-row:hover {
  background-color: rgba(0,0,0,0.06);
}

/* persistent selection: slightly stronger */
...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # mapping TocItem -> ActionRow widget for selection styling
        self._toc_actrows = {}
        ...
        # keep theme sidebar surface and also scope our css
        ...
        # do not add extra css that would override theme; keep default look
        ...
        # keep scrolled window default styling so theme sidebar color remains
    ...
    def _toc_on_bind(self, factory, list_item):
        ...
        # cleanup previous binding mapping
        ...
        # register mapping so we can style selection later
        try:
            self._toc_actrows[item] = actrow
            # ensure previously selected css class isn't left lingered
        ...
        if has_children and not getattr(list_item, "_nested_view", None):
            ...
            def child_bind(f, li):
                ...
                # register child actionrow for selection styling
                try:
    ...
    def _set_toc_selected(self, toc_item):
        # clear existing selection and set selected css class for target row
        try:
    ...
    def cleanup(self):
        ...
        # Clear the TOC model and actrow mapping
        try:
