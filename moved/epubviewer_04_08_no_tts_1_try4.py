...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # Header
        bookmarks_header = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        bookmarks_label = Gtk.Label(label="Bookmarks")
        bookmarks_label.add_css_class("title-3")
        bookmarks_label.set_halign(Gtk.Align.START)
        bookmarks_label.set_hexpand(True)
        bookmarks_header.append(bookmarks_label)

        # Clear all bookmarks button
        clear_all_btn = Gtk.Button(icon_name="edit-clear-all-symbolic")
        clear_all_btn.add_css_class("flat")
        clear_all_btn.set_tooltip_text("Clear all bookmarks for this book")
        clear_all_btn.connect("clicked", self._on_clear_all_bookmarks)
        bookmarks_header.append(clear_all_btn)

        bookmarks_box.append(bookmarks_header)
        bookmarks_box.append(Gtk.Separator(orientation=Gtk.Orientation.HORIZONTAL))
        ...
        # Bookmarks list box
        self.bookmarks_listbox = Gtk.ListBox()
        self.bookmarks_listbox.add_css_class("navigation-sidebar")
        self.bookmarks_listbox.set_selection_mode(Gtk.SelectionMode.NONE)
        self.bookmarks_listbox.connect("row-activated", self._on_bookmark_row_activated)
        bookmarks_scroll.set_child(self.bookmarks_listbox)

        bookmarks_box.append(bookmarks_scroll)

        # Add to side stack
        self.side_stack.add_titled(bookmarks_box, "bookmarks", "Bookmarks")
        # ============================================================================
        # END OF BOOKMARKS TAB
        # ============================================================================        
        ...
        # Bookmark toggle button (add after columns menu button)
        self.bookmark_btn = Gtk.ToggleButton()
        self.bookmark_btn.set_icon_name("bookmark-outline-symbolic")
        self.bookmark_btn.set_tooltip_text("Add Bookmark")
        self.bookmark_btn.connect("toggled", self.toggle_bookmark)
        header.pack_end(self.bookmark_btn)
        ...
        self.prev_chapter_btn = Gtk.Button(icon_name="go-first-symbolic")
        ...
        self.prev_chapter_btn.connect("clicked", self.prev_chapter)
        bottom_bar.append(self.prev_chapter_btn)
        ...
        # Next chapter button
        self.next_chapter_btn = Gtk.Button(icon_name="go-last-symbolic")
        ...
        self.next_chapter_btn.connect("clicked", self.next_chapter)
        bottom_bar.append(self.next_chapter_btn)
    ...
    def _on_scroll_boundary_received(self, content_manager, js_result):
        ...
        try:
            ...
            # Check if current position matches any bookmark
            GLib.idle_add(self._check_current_bookmark)
...
###################
    ...
    ########### bookmark list management methods
    def _refresh_bookmarks_list(self):
        """Refresh the bookmarks list display"""
        try:
            # Clear existing
            while True:
                row = self.bookmarks_listbox.get_row_at_index(0)
                if row:
                    self.bookmarks_listbox.remove(row)
                else:
                    break
            
            if not self.book_path or self.book_path not in self.bookmarks:
                # Show empty state
                empty_label = Gtk.Label(label="No bookmarks yet")
                empty_label.add_css_class("dim-label")
                empty_label.set_margin_top(20)
                empty_label.set_margin_bottom(20)
                self.bookmarks_listbox.append(empty_label)
                return
            
            # Sort bookmarks by item_index and scroll position
            book_bookmarks = sorted(
                self.bookmarks[self.book_path],
                key=lambda x: (x.get('item_index', 0), x.get('scroll_top', 0), x.get('scroll_left', 0))
            )
            ...
            print(f"Error refreshing bookmarks list: {e}")

    def _create_bookmark_row(self, bookmark):
        """Create a row widget for a bookmark"""
        row = Gtk.ListBoxRow()
        row.bookmark_data = bookmark
        ...
        # Chapter title
        chapter_label = Gtk.Label(label=bookmark.get('chapter_title', 'Unknown'))
        chapter_label.set_halign(Gtk.Align.START)
        chapter_label.set_wrap(True)
        chapter_label.set_wrap_mode(Pango.WrapMode.WORD_CHAR)
        chapter_label.set_max_width_chars(30)
        chapter_label.add_css_class("heading")
        info_box.append(chapter_label)
        
        # Preview text
        viewport_text = bookmark.get('viewport_text', '')
        if viewport_text:
            preview = viewport_text[:50] + ('...' if len(viewport_text) > 50 else '')
        ...
        # Timestamp
        import time
        timestamp = bookmark.get('timestamp', 0)
        time_str = time.strftime('%b %d, %Y %H:%M', time.localtime(timestamp))
        ...
        delete_btn.connect("clicked", lambda b: self._on_delete_bookmark(bookmark.get('id')))
        box.append(delete_btn)
        
        row.set_child(box)
        return row

    def _on_bookmark_row_activated(self, listbox, row):
        """Handle bookmark row click - restore bookmark"""
        try:
            if hasattr(row, 'bookmark_data'):
                self._restore_bookmark(row.bookmark_data)
        except Exception as e:
            print(f"Error activating bookmark: {e}")

    def _on_delete_bookmark(self, bookmark_id):
        """Handle delete button click"""
        self._remove_bookmark(bookmark_id)

    def _on_clear_all_bookmarks(self, button):
        ...
        dialog.set_heading("Clear All Bookmarks?")
        dialog.set_body("This will remove all bookmarks for this book. This action cannot be undone.")
        ...
        def on_response(dialog, response):
            if response == "clear":
                if self.book_path in self.bookmarks:
                    del self.bookmarks[self.book_path]
                    self._save_bookmarks()
                    self.current_bookmark_id = None
                    self._update_bookmark_button_icon()
                    self._refresh_bookmarks_list()
    ...
    def display_page(self, index=None, fragment=None):
        ...
        GLib.timeout_add(600, lambda: (self._check_current_bookmark(), False)[1])
        return True
    ...
    # ============================================================================
    # BOOKMARKS - Save and restore exact viewport positions
    # ============================================================================

    def _init_bookmarks(self):
        """Initialize bookmarks storage"""
        self.bookmarks = {}  # {book_path: [bookmark_dict, ...]}
        self.current_bookmark_id = None  # ID of bookmark at current location
        self._load_bookmarks()

    def _load_bookmarks(self):
        """Load bookmarks from file"""
        try:
            bookmarks_file = os.path.join(GLib.get_user_data_dir(), "epub-viewer", "bookmarks.json")
            if os.path.exists(bookmarks_file):
                with open(bookmarks_file, 'r', encoding='utf-8') as f:
                    self.bookmarks = json.load(f)
                print(f"✓ Loaded bookmarks from {bookmarks_file}")
        except Exception as e:
    ...
    def _save_bookmarks(self):
        ...
        try:
            data_dir = os.path.join(GLib.get_user_data_dir(), "epub-viewer")
            os.makedirs(data_dir, exist_ok=True)
            bookmarks_file = os.path.join(data_dir, "bookmarks.json")
            with open(bookmarks_file, 'w', encoding='utf-8') as f:
                json.dump(self.bookmarks, f, indent=2, ensure_ascii=False)
            print(f"✓ Saved bookmarks to {bookmarks_file}")
        except Exception as e:
            print(f"Error saving bookmarks: {e}")

    def _get_viewport_text(self, callback):
        """Extract text visible at top of viewport (for bookmark matching)"""
        if not self.webview:
            callback(None)
            return
        ...
        (function(){
            try {
                ...
                // Get text at viewport start
                var x, y;
                if(actualColCount === 1) {
                    // Single column: top-left of viewport
                    x = 50;
                    y = container.scrollTop + 50;
                } else {
                    // Multi-column: top-left of current column
                    x = container.scrollLeft + 50;
                    y = 50;
                }
                ...
                // Get text from this element and nearby
                var textContent = '';
                ...
                // Return first ~100 chars as identifier
                return textContent.trim().substring(0, 100);
            } catch(e) {
                console.log('[Bookmark] Error getting viewport text:', e);
                return null;
            }
        })();
        """
        ...
        def on_result(webview, result, user_data):
            try:
                ...
                if js_result:
                    ...
                    callback(text if text != "null" else None)
                else:
                    callback(None)
            except Exception as e:
                print(f"Error getting viewport text: {e}")
                callback(None)
    ...
    def _get_scroll_position(self, callback):
        """Get current scroll position"""
        if not self.webview:
            callback({"scrollLeft": 0, "scrollTop": 0, "colCount": 1})
            return
        ...
        (function(){
            try {
                var container = document.querySelector('.ebook-content');
                if(!container) return {scrollLeft: 0, scrollTop: 0, colCount: 1};
                ...
                return {scrollLeft: 0, scrollTop: 0, colCount: 1};
            }
        })();
        """
    ...
    def _check_current_bookmark(self):
        """Check if current location is bookmarked and update button state"""
        if not self.book_path or not hasattr(self, 'bookmark_btn'):
            return
        
        def on_position_received(position):
            def on_text_received(viewport_text):
                # Check if any bookmark matches current location
                book_bookmarks = self.bookmarks.get(self.book_path, [])
                self.current_bookmark_id = None
                
                for bookmark in book_bookmarks:
                    if bookmark.get('item_index') == self.current_index:
                        # Same chapter, check if scroll position is close (within 50px)
                        scroll_diff_x = abs(bookmark.get('scroll_left', 0) - position.get('scrollLeft', 0))
                        scroll_diff_y = abs(bookmark.get('scroll_top', 0) - position.get('scrollTop', 0))
                        
                        if scroll_diff_x < 50 and scroll_diff_y < 50:
                            self.current_bookmark_id = bookmark.get('id')
                            break
                
                # Update button icon
                GLib.idle_add(self._update_bookmark_button_icon)
            
            self._get_viewport_text(on_text_received)
        
        self._get_scroll_position(on_position_received)

    def _update_bookmark_button_icon(self):
        """Update bookmark button icon based on current state"""
        if not hasattr(self, 'bookmark_btn'):
            return
        
        is_bookmarked = bool(self.current_bookmark_id)
        
        # Block signal to prevent recursion
        self.bookmark_btn.handler_block_by_func(self.toggle_bookmark)
        
        if is_bookmarked:
            self.bookmark_btn.set_icon_name("bookmark-filled-symbolic")
            self.bookmark_btn.set_tooltip_text("Remove Bookmark")
            self.bookmark_btn.set_active(True)
        else:
            self.bookmark_btn.set_icon_name("bookmark-outline-symbolic")
            self.bookmark_btn.set_tooltip_text("Add Bookmark")
            self.bookmark_btn.set_active(False)
        
        # Unblock signal
        self.bookmark_btn.handler_unblock_by_func(self.toggle_bookmark)
    ...
    def toggle_bookmark(self, button):
        ...
        was_active = bool(self.current_bookmark_id)
        
        if was_active:
            # Remove existing bookmark
            self._remove_bookmark(self.current_bookmark_id)
    ...
    def _add_bookmark(self):
        ...
        def on_position_received(position):
            def on_text_received(viewport_text):
                ...
                try:
                    if hasattr(self, 'toc_root_store'):
                        current_toc = self._find_current_toc_item()
                        if current_toc:
                            chapter_title = current_toc.title
                except Exception:
                    pass
                ...
                bookmark = {
                    ...
                    'scroll_left': position.get('scrollLeft', 0),
                    'scroll_top': position.get('scrollTop', 0),
                    'col_count': position.get('colCount', 1),
                    'viewport_text': viewport_text or '',
                ...
                self.bookmarks[self.book_path].append(bookmark)
                self._save_bookmarks()
                
                self.current_bookmark_id = bookmark_id
                self._update_bookmark_button_icon()
                self._refresh_bookmarks_list()
                
                print(f"✓ Bookmark added: {chapter_title[:30]}")
    ...
    def _remove_bookmark(self, bookmark_id):
        ...
        self.bookmarks[self.book_path] = [
            bm for bm in self.bookmarks[self.book_path]
            if bm.get('id') != bookmark_id
        ]
        
        if not self.bookmarks[self.book_path]:
            del self.bookmarks[self.book_path]
    ...
    def _restore_bookmark(self, bookmark):
        """Restore a bookmark - navigate and scroll to exact position"""
        try:
            item_index = bookmark.get('item_index', 0)
            scroll_left = bookmark.get('scroll_left', 0)
            scroll_top = bookmark.get('scroll_top', 0)
            viewport_text = bookmark.get('viewport_text', '')
            
            print(f"📖 Restoring bookmark: {bookmark.get('chapter_title', 'Unknown')[:30]}")
                ...
                try:
                    ...
                    (function(){{
                        try {{
                            ...
                            console.log('[Bookmark] Scrolled to: left=' + {scroll_left} + ', top=' + {scroll_top});
                            
                            // Highlight the viewport text if available
                            var searchText = {json.dumps(viewport_text[:50])};
                            if(searchText) {{
                                setTimeout(function() {{
                                    ...
                                    var fullText = textNodes.map(n => n.nodeValue).join(' ');
                                    var idx = fullText.indexOf(searchText.substring(0, 30));
                                    
                                    if(idx !== -1) {{
                                        // Find the node containing this text
                                        var charCount = 0;
                                        for(var i = 0; i < textNodes.length; i++) {{
                                            var nodeText = textNodes[i].nodeValue;
                                            if(charCount + nodeText.length > idx) {{
                                                // Found the node
                                                ...
                                                if(parent) {{
                                                    ...
                                                    console.log('[Bookmark] Highlighted text');
                                                }}
                                                break;
                                            }}
                            ...
                            console.log('[Bookmark] Error restoring:', e);
                        }}
                    }})();
                    """
            ...
            GLib.timeout_add(500, restore_position)
            
        except Exception as e:
            print(f"Error restoring bookmark: {e}")
    # ---- Navigation ----
    # ============================================================================
    # NAVIGATION - Pages (scroll columns) and Chapters (TOC items)
    # ============================================================================

    def update_navigation(self):
        """Update button states for both page and chapter navigation"""
        # Page navigation (always enabled if book is loaded)
        has_book = bool(getattr(self, "book", None) and getattr(self, "items", None))
        try:
            self.prev_btn.set_sensitive(has_book)
            self.next_btn.set_sensitive(has_book)
        except Exception:
            pass
        
        # Chapter navigation (enabled only if TOC exists and has items)
        has_toc = False
        current_toc_item = None
        try:
            if hasattr(self, "toc_root_store") and self.toc_root_store:
                has_toc = self.toc_root_store.get_n_items() > 0
                if has_toc:
                    current_toc_item = self._find_current_toc_item()
        except Exception:
            pass
        
        # Enable/disable chapter buttons
        try:
            if hasattr(self, "prev_chapter_btn"):
                can_go_prev = has_toc and self._has_previous_toc_item(current_toc_item)
                self.prev_chapter_btn.set_sensitive(can_go_prev)
            if hasattr(self, "next_chapter_btn"):
                can_go_next = has_toc and self._has_next_toc_item(current_toc_item)
                self.next_chapter_btn.set_sensitive(can_go_next)
        except Exception:
            pass

    def next_page(self, button):
        """Scroll forward (like PageDown) - scrolls columns in multi-column mode"""
        if not self.webview:
            return
        ...
        (function(){
            ...
            if(actualColCount > 1) {
                ...
                var currentCol = Math.round(container.scrollLeft / pageWidth);
                var targetCol = currentCol + actualColCount;
                var maxScroll = container.scrollWidth - container.clientWidth;
                var target = Math.min(maxScroll, targetCol * pageWidth);
    ...
    def prev_page(self, button):
        """Scroll backward (like PageUp) - scrolls columns in multi-column mode"""
        if not self.webview:
            return
        ...
        (function(){
            ...
            if(actualColCount > 1) {
                ...
                var currentCol = Math.round(container.scrollLeft / pageWidth);
                var targetCol = Math.max(0, currentCol - actualColCount);
                var target = targetCol * pageWidth;
    ...
    # Chapter navigation helper methods
    def _find_current_toc_item(self):
        """Find the TOC item corresponding to current_index"""
        if not hasattr(self, "toc_root_store") or not self.toc_root_store:
            return None
        
        def search_toc(store):
            for i in range(store.get_n_items()):
                item = store.get_item(i)
                if isinstance(item.index, int) and item.index == self.current_index:
                    return item
                if item.children and item.children.get_n_items() > 0:
                    child_result = search_toc(item.children)
                    if child_result:
                        return child_result
            return None
        
        return search_toc(self.toc_root_store)

    def _get_all_toc_items_flat(self):
        """Get a flat list of all TOC items in reading order"""
        if not hasattr(self, "toc_root_store") or not self.toc_root_store:
            return []
        ...
        def collect_items(store):
            for i in range(store.get_n_items()):
                ...
                if isinstance(item.index, int) and item.index >= 0:
                    items.append(item)
                if item.children and item.children.get_n_items() > 0:
                    collect_items(item.children)
        
        collect_items(self.toc_root_store)
        items.sort(key=lambda x: x.index)
        return items

    def _has_next_toc_item(self, current_item):
        """Check if there's a next chapter in TOC"""
        toc_items = self._get_all_toc_items_flat()
        if not toc_items or not current_item:
            return len(toc_items) > 0
        
        try:
            current_idx = toc_items.index(current_item)
            return current_idx < len(toc_items) - 1
        except ValueError:
            return False

    def _has_previous_toc_item(self, current_item):
        """Check if there's a previous chapter in TOC"""
        toc_items = self._get_all_toc_items_flat()
        if not toc_items or not current_item:
            return False
    ...
    def next_chapter(self, button):
        """Navigate to next chapter/section in TOC"""
        toc_items = self._get_all_toc_items_flat()
        if not toc_items:
            return
        ...
        if current_item:
            try:
                current_idx = toc_items.index(current_item)
                if current_idx < len(toc_items) - 1:
                    next_item = toc_items[current_idx + 1]
                    print(f"📖 Next chapter: {next_item.title}")
                    self.current_index = next_item.index
                    self.update_navigation()
                    self.display_page()
                    self._save_progress_for_library()
                    self._set_toc_selected(next_item)
            except (ValueError, IndexError) as e:
                print(f"Error navigating to next chapter: {e}")
        else:
            if toc_items:
                first_item = toc_items[0]
                print(f"📖 Jumping to first chapter: {first_item.title}")
                self.current_index = first_item.index
                self.update_navigation()
                self.display_page()
                self._save_progress_for_library()
                self._set_toc_selected(first_item)

    def prev_chapter(self, button):
        """Navigate to previous chapter/section in TOC"""
        toc_items = self._get_all_toc_items_flat()
        if not toc_items:
            return
        ...
        if current_item:
            try:
                current_idx = toc_items.index(current_item)
                if current_idx > 0:
                    prev_item = toc_items[current_idx - 1]
                    print(f"📖 Previous chapter: {prev_item.title}")
                    self.current_index = prev_item.index
                    self.update_navigation()
                    self.display_page()
                    self._save_progress_for_library()
                    self._set_toc_selected(prev_item)
            except (ValueError, IndexError) as e:
                print(f"Error navigating to previous chapter: {e}")
    # ---- CSS extraction ----
