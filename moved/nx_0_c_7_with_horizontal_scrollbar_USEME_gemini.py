...
# Setting environment variable for compositing mode
...
class EPUBWindow(Adw.ApplicationWindow):
    def __init__(self, **kwargs):
        ...
        # FIX: Variable to store and manage the scroll controller
    ...
    def setup_webview(self):
        ...
        # Keep V-adjustment connection for single-column mode
    ...
    def on_scroll_event(self, controller, dx, dy):
        # Only use vertical scrolling if in single-column mode
        if not (self.app.columns > 1 or not self.app.use_fixed_columns):
            return False # Let vertical scroll happen naturally in vertical mode

        # If in column mode, capture any vertical or horizontal scroll attempts and translate to a "page" turn.
        if abs(dx) > 0.1:
    ...
    def on_webview_load_changed(self, webview, load_event):
        if load_event == WebKit.LoadEvent.FINISHED:
            self.apply_layout()
            # Defer scroll reset slightly to allow layout to fully apply
    ...
    def reset_scroll_position(self):
        ...
        # For horizontal/column mode, reset horizontal scroll via JS
        if self.app.columns > 1 or not self.app.use_fixed_columns:
            ...
            """
            try:
                self.webview.evaluate_javascript(js_reset, -1, None, None, None)
            except:
                pass
        
        # We need to explicitly refresh the button state after a new chapter loads
    ...
    def apply_layout(self):
        ...
        if is_column_mode:
            if use_fixed:
                column_props = f"column-count: {columns} !important;"
            else:
                column_props = f"column-width: {col_width}px !important;"

            # Corrected brace escaping in f-string for JS/CSS template
            css = f"""
                html {{
                    margin: 0 !important;
                    padding: 0 !important;
                    height: 100vh !important; /* Use viewport height for column-fill auto */
                    width: 100% !important;
                }}
                ...
                body {{
                    ...
                    /* Key change: Enable horizontal scrolling on body */
                    overflow-x: scroll !important;
                    overflow-y: hidden !important;
                    
                    /* Scroll snapping for column alignment */
            ...
            css = f"""
                html {{
                    ...
                    padding: 0 !important;
                    width: 100% !important;
                    height: 100% !important; /* Full height content in single column */
                }}
        ...
        # Corrected brace escaping in f-string for JS template
        js_inject = f"""
        (function() {{
            ...
            // Force scroll behavior on the body element
            const body = document.body;
            if (body) {{
                if ({is_column_mode}) {{
                    // Set scroll behavior in JS for better control and snapping
                    body.style.scrollSnapType = 'x mandatory';
        ...
        try:
            # We only need the one call signature, handling the result/error is optional
            self.webview.evaluate_javascript(js_inject, -1, None, None, None)
        except Exception as e:
            print(f"Error injecting CSS/JS: {e}", file=sys.stderr)


        if is_column_mode:
            # Hide GTK horizontal scrollbar, rely on WebKit's body overflow
            self.scrolled_window.set_policy(Gtk.PolicyType.NEVER, Gtk.PolicyType.NEVER)
            
            # FIX: Remove old controller via reference
            if self.scroll_controller:
                self.scrolled_window.remove_controller(self.scroll_controller)
                self.scroll_controller = None

            # Re-add scroll controller to capture scroll/swipe gestures for page-turn
            self.scroll_controller = Gtk.EventControllerScroll()
            self.scroll_controller.set_flags(Gtk.EventControllerScrollFlags.VERTICAL | Gtk.EventControllerScrollFlags.HORIZONTAL)
            self.scroll_controller.connect("scroll", self.on_scroll_event)
            self.scrolled_window.add_controller(self.scroll_controller)
            
        else:
            # Single-column vertical mode: only show vertical scrollbar
            self.scrolled_window.set_policy(Gtk.PolicyType.NEVER, Gtk.PolicyType.AUTOMATIC)
            
            # FIX: Remove scroll controller when switching back to vertical mode
            if self.scroll_controller:
                self.scrolled_window.remove_controller(self.scroll_controller)
                self.scroll_controller = None

        
        # Update buttons after layout change
    ...
    def scroll_viewport(self, direction: int):
        ...
        if is_column_mode:
            # Use JS to scroll the body element horizontally
            js_code = f"""
            (function() {{
                const body = document.scrollingElement || document.body;
                ...
                // Calculate the nearest page alignment
                let curPage = Math.round(cur / vw);
                let targetPage = curPage + ({direction});
                
                let target = targetPage * vw;

                target = Math.max(0, Math.min(target, maxScroll));

                body.scrollTo({{ left: target, behavior: 'smooth' }}); 
                
                // Return data for boundary check
                return JSON.stringify({{ cur, target, maxScroll, vw }});
            }})();
            """

            # FIX: Correct callback signature (webview, task)
            def process_result(webview, task):
                try:
                    # FIX: Use evaluate_javascript_finish to get WebKit.JavascriptResult
                    result = webview.evaluate_javascript_finish(task)
                    
                    # FIX: Get the GLib.Value from the JavascriptResult
                    js_val = result.get_js_value() 
                    if not js_val: return

                    import json
                    # FIX: Correctly convert the GLib.Value to a string for parsing
                    data = json.loads(js_val.to_string())
                    target = data.get("target", 0)
                    # Correct key from JS is 'maxScroll'
                    max_scroll = data.get("maxScroll", 0) 

                    # Chapter boundary handoff (only at true edges)
                    if direction > 0 and target >= max_scroll - 2:
                        # Give smooth scroll time to complete before loading next chapter
                        GLib.timeout_add(300, self.load_next_spine_item)
                    elif direction < 0 and target <= 1:
                        # Give smooth scroll time to complete before loading previous chapter
                        GLib.timeout_add(300, self.load_prev_spine_item)

                except Exception as e:
                    print(f"scroll eval error: {e}", file=sys.stderr)

                # Update buttons immediately based on direction, then again after a short delay
                self.update_nav_buttons()
                GLib.timeout_add(400, self.update_nav_buttons)


            def run_js():
                # FIX: Use proper asynchronous signature (None for CancellationToken, callback)
            ...
            # Vertical single-column mode - unchanged, still uses GTK adjustment
            ...
            # Use a short timeout to prevent rapid firing from event controllers
            GLib.timeout_add(50, lambda: v_adj.set_value(target_scroll))
    ...
    def load_prev_spine_item(self):
        ...
        if prev_index >= 0:
            ...
            if prev_item:
                ...
                # Scroll to the end of the newly loaded page to simulate "previous page"
    ...
    def scroll_to_end_of_page(self):
        ...
        if self.app.columns > 1 or not self.app.use_fixed_columns:
            # For multi-column layouts: scroll to the end horizontally using JS
            js_code = """
            (function() {
                const body = document.scrollingElement || document.body;
                const vw = window.innerWidth;
                const maxScroll = body.scrollWidth - vw;
                // Snap target to the last page boundary
                const lastPageScroll = Math.floor(maxScroll / vw) * vw;
            ...
            # For single column: scroll to the end vertically
            viewport_height = v_adj.get_page_size()
            max_scroll = v_adj.get_upper() - viewport_height
    ...
    def update_nav_buttons(self, *args):
        ...
        if is_column_mode:
            # Column mode navigation state is determined by JavaScript values
            js_code = """
            (function() {
                const body = document.scrollingElement || document.body;
            ...
            def process_result(webview, task):
                try:
                    ...
                    # Fuzzy comparison for page edges
                    is_first_page = current <= 5
                    is_last_page = current >= max_pos - 5 
            ...
            # Single column layout - vertical scrolling using GTK adjustment
            v_adj = self.scrolled_window.get_vadjustment()
