...
class EpubViewer(Adw.ApplicationWindow):
    ...
    def setup_ui(self):
        ...
        for i in range(1, 11):  # 1 to 10 columns
    ...
    def load_epub_viewer(self):
        ...
        # Only use spread:auto for exactly 2 columns, otherwise none
        spread_mode = "auto" if self.column_count == 2 else "none"
    ...
    <body>
        ...
        <script>
            var COLUMN_COUNT = {self.column_count};
            
            // Generate column CSS dynamically based on column count
            var IFRAME_STYLES = `
                * {{ transition: none !important; -webkit-transition: none !important; animation: none !important; }}
                ...
                a {{ text-decoration: underline; color: #6598eb !important; }}
                .tts-highlight {{
                    background: rgba(255, 215, 0, 0.9) !important;
                    box-shadow: 0 0 0 2px rgba(255, 215, 0, 0.75) !important;
                    color: #000 !important;
                ...
                @media (prefers-color-scheme: light) {{
                    body, html {{ color: #111 !important; background: #fafafa !important; }}
                    a {{ color: #0b66ff !important; }}
                }}
            `;
            
            // Add multi-column CSS if more than 2 columns
            if (COLUMN_COUNT > 1) {{
                IFRAME_STYLES += `
                    body {{
                        ...
                        column-width: auto;
                        column-rule: 1px solid rgba(128, 128, 128, 0.3) !important;
                    }}
                    h1, h2, h3, h4, h5, h6 {{
            ...
            // Intercept iframe creation to inject styles IMMEDIATELY
                    ...
                    mutation.addedNodes.forEach(function(node) {{
                        if (node.tagName === 'IFRAME') {{
                            ...
                            setTimeout(function() {{
                                try {{
                                    var doc = node.contentDocument;
                                    if (doc && !doc.getElementById('__tts_injected_styles__')) {{
                                        var s = doc.createElement('style');
                                        s.id = '__tts_injected_styles__';
                                        s.textContent = IFRAME_STYLES;
                                        var head = doc.head || doc.documentElement;
            ...
            rendition.on('rendered', function(section) {{
                setTimeout(function() {{
                    try {{ injectTTSSpans(section); }} catch(e){{}}
            ...
            function injectTTSSpans(section) {{
                try {{
                    ...
                    // Ensure styles exist with column CSS
                    if (!doc.getElementById('__tts_injected_styles__')) {{
                        var s = doc.createElement('style');
                        s.id = '__tts_injected_styles__';
                        s.textContent = IFRAME_STYLES;
                    ...
                    TARGET_TAGS.forEach(function(tag) {{
                        ...
                        Array.from(elements).forEach(function(el) {{
                            if (el.querySelector('[data-tts-id]') || el.closest('[data-tts-id]')) return;
                            var text = getDirectTextContent(el);
                            if (!text.trim()) return;
                            var sentences = splitSentences(text);
                            if (sentences.length) wrapSentencesInElement(el, sentences);
                        }});
                    }});
                    ...
                    doc.addEventListener('click', function(ev) {{
                        ...
                        ev.preventDefault();
                        try {{ rendition.display(href); }} catch(e) {{}}
                    }}, true);
                }} catch(e) {{}}
            }}
            ...
            window.prevChapter = function() {{
                ...
                if (loc) {{
                    var ch = book.spine.get(loc.start.href);
                    if (ch) {{
                        var idx = book.spine.spineItems.indexOf(ch);
                        if (idx > 0) rendition.display(idx - 1);
                    }}
                }}
            }};
            window.nextChapter = function() {{
                ...
                if (loc) {{
                    ...
                    if (ch) {{
                        var idx = book.spine.spineItems.indexOf(ch);
                        if (idx < book.spine.length - 1) rendition.display(idx + 1);
                    }}
                }}
            }};
            ...
            window.getCurrentChapterSentences = function() {{
                ...
                try {{
                    var doc = iframeDoc();
                    if (!doc) return sentences;
                    doc.querySelectorAll('[data-tts-id]').forEach(function(span) {{
