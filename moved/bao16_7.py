...
class Win(Adw.ApplicationWindow):
    ...
    def load_epub(self, path):
            ...
            # Extract all items to temp directory for proper resource loading
            import tempfile
            import zipfile
            ...
            # Get document items in spine order
    ...
    def _on_toc_clicked(self, widget, href):
        ...
        for item in self.book.get_items():
            ...
            if href_path in (item.file_name, os.path.basename(item.file_name)):
                try:
                    html_text = item.get_content().decode("utf-8", errors="ignore")
                    ...
                    if fragment:
                        ...
                        """
                        GLib.timeout_add(400, lambda: self.webview.evaluate_javascript(js, -1, None, None, None))
                except Exception as e:
