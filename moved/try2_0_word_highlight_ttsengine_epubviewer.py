...
# -------------------------
# -------------------------
# COMPLETE TTSEngine class with word-by-word highlighting
# Replace your existing TTSEngine class with this entire code block
# -------------------------
...
class TTSEngine:
    def __init__(self, webview_getter, base_temp_dir=None, kokoro_model_path=None, voices_bin_path=None):
        ...
        # NEW: word tracking for word-by-word highlighting
        self._current_word_index = 0
        self._word_timer = None
        self._word_timer_lock = threading.Lock()
    ...
    def _cancel_word_timer(self):
        """Cancel any active word highlighting timer"""
        with self._word_timer_lock:
            if self._word_timer:
                try:
                    self._word_timer.cancel()
                except Exception:
                    pass
                self._word_timer = None
    ...
    def _get_audio_duration(self, audio_file):
        ...
        try:
            ...
            try:
                ...
                return max(0.5, size / 32000.0)
            except Exception:
                return 0.0

    def _highlight_words_progressively(self, sentence_idx, sentence_text, audio_duration):
        """Highlight words one by one as audio plays"""
        if not self._tts_highlight_callback:
            return
        ...
        def highlight_next_word(word_idx):
            if self.should_stop or self.paused or self._current_play_index != sentence_idx:
                return False
            ...
            try:
                GLib.idle_add(
                    ...
                    sentence_idx,
                    {
                        "sid": self._tts_sids[sentence_idx] if sentence_idx < len(self._tts_sids) else sentence_idx,
                        "text": sentence_text,
                        "word_index": word_idx,
                ...
                print(f"[TTS] Error highlighting word {word_idx}: {e}")
            
            # Schedule next word
            if word_idx < len(words) - 1 and not self.should_stop:
                with self._word_timer_lock:
                    self._word_timer = threading.Timer(
                        time_per_word,
                        lambda: highlight_next_word(word_idx + 1)
                    )
        ...
        def tts_thread():
            try:
                ...
                while self._current_play_index < len(self._tts_sentences) and not self.should_stop:
                    ...
                    # Initial sentence highlight (word 0)
                        ...
                        try:
                            GLib.idle_add(
                                ...
                                idx,
                                {
                                    ...
                                    "word_index": 0,
                                    "total_words": len(self._tts_sentences[idx].split())
                                }
                            )
                        except Exception:
                            pass
                    ...
                    # NEW: Start word-by-word highlighting before playing audio
                    audio_duration = self._get_audio_duration(audio_file)
                    if audio_duration > 0:
                        self._highlight_words_progressively(
                            idx,
    ...
    def next_sentence(self):
        ...
        with self._audio_lock:
            ...
            idx = self._current_play_index
        if self._tts_highlight_callback:
            GLib.idle_add(self._tts_highlight_callback, idx, {"sid": self._tts_sids[idx], "text": self._tts_sentences[idx], "word_index": 0, "total_words": len(self._tts_sentences[idx].split())})
        try:
            if self.player:
                self.player.set_state(Gst.State.NULL)
        except Exception:
...
###################
    ...
    def _ensure_sentence_wrapping_and_start(self, sentences, auto_start=True):
        """
        Wrap sentences AND words into the webview DOM for TTS highlighting.
        """
        try:
            if not getattr(self, "webview", None):
                if auto_start:
            ...
            # SENTENCE WRAPPING (existing code)
            js = """
            (function(){
              try {
                ...
                console.log('[TTS-DBG] applied sentence wraps=', applied);
              } catch(e){
            ...
            try:
                ...
                print("[TTS] sentence wrapper injection error:", e)

            # NEW: WORD WRAPPING - Add after sentence wrapping completes
            js_word_wrap = """
            (function() {
                try {
                    console.log('[TTS] Starting word wrapping');
                    
                    // Wait a bit for sentence wrapping to complete
                    setTimeout(function() {
                        var sentences = document.querySelectorAll('.tts-sentence');
                        console.log('[TTS] Found ' + sentences.length + ' sentences to wrap words in');
                        
                        sentences.forEach(function(sentenceSpan, sentenceIdx) {
                            var sid = sentenceSpan.dataset.sid;
                            var text = sentenceSpan.textContent;
                            ...
                            words.forEach(function(word, wordIdx) {
                                ...
                                wordSpan.dataset.sid = sid;
                                wordSpan.dataset.wid = wordIdx;
                        ...
                        console.log('[TTS] Word wrapping complete');
                    }, 150);  // Short delay to ensure sentence spans are ready
                    
                } catch(e) {
                    console.log('[TTS] Word wrapping error:', e);
                }
            })();
            """

            try:
                # Execute word wrapping shortly after sentence wrapping
                GLib.timeout_add(200, lambda: (
                    self.webview.evaluate_javascript(js_word_wrap, -1, None, None, None, None, None)
                    if self.webview else None,
                    False
                )[1])
            except Exception:
                try:
                    GLib.timeout_add(200, lambda: (
                        self.webview.run_javascript(js_word_wrap, None, None, None)
                        if self.webview else None,
                        False
                    )[1])
                except Exception:
            ...
            if auto_start:
                # Start TTS after wrapping completes
                GLib.timeout_add(400, lambda: (self.tts.speak_sentences_list(
                    sentences,
                    highlight_callback=self._on_tts_highlight,
                    finished_callback=self._on_tts_finished
                ), False)[1])
    ...
    def _on_tts_highlight(self, idx, meta):
        """Enhanced highlighting with word-level support"""
        if not self.webview:
            return
        ...
        try:
            ...
            if isinstance(meta, dict):
                sid = meta.get("sid", None)
                word_index = meta.get("word_index", -1)
                total_words = meta.get("total_words", 0)
            
            if sid is None:
                sid = idx
            
            # Clear all highlights if stopping
            if sid is None or (isinstance(sid, int) and sid < 0):
                js_clear = """(function(){
                    document.querySelectorAll('.tts-highlight').forEach(e => e.classList.remove('tts-highlight'));
                    document.querySelectorAll('.tts-word-active').forEach(e => e.classList.remove('tts-word-active'));
                    console.log('[TTS] Cleared all highlights');
                })();"""
                try:
                    self.webview.evaluate_javascript(js_clear, -1, None, None, None, None, None)
                except Exception:
            ...
            # Highlight sentence and current word
            js = f"""(function(){{
                try {{
                    // Clear previous word highlights
                    document.querySelectorAll('.tts-word-active').forEach(e => 
                        e.classList.remove('tts-word-active')
                    );
                    ...
                    const sentences = document.querySelectorAll('[data-sid="{sid}"]');
                    sentences.forEach(sent => {{
                        if (sent.classList.contains('tts-sentence')) {{
                            sent.classList.add('tts-highlight');
                            
                            // Highlight current word within sentence
                            if ({word_index} >= 0) {{
                                const words = sent.querySelectorAll('.tts-word');
                                if (words[{word_index}]) {{
                                    words[{word_index}].classList.add('tts-word-active');
                                    
                                    // Scroll word into view if needed
                                    const wordRect = words[{word_index}].getBoundingClientRect();
                                    const container = document.querySelector('.ebook-content');
                                    if (container) {{
                                        const containerRect = container.getBoundingClientRect();
                                        if (wordRect.left < containerRect.left || 
                    ...
                    console.log('[TTS] Highlighted sentence ' + {sid} + ', word ' + {word_index});
                }} catch(err) {{
                    console.log('[TTS] Highlight error:', err);
                }}
            }})();"""
