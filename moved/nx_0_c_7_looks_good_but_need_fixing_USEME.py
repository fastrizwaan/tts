...
class EPUBWindow(Adw.ApplicationWindow):
    ...
    def scroll_viewport(self, direction: int):
        ...
        if horizontal_mode:
            # PAGE = viewport width; with the CSS above, exactly N columns fill the viewport.
            js_code = f"""
            (function() {{
                const body = document.scrollingElement || document.body;
                const vw = window.innerWidth;                   // page size
                const cur = body.scrollLeft;
                const maxScroll = body.scrollWidth - vw;

                // next target is one full viewport step
            ...
            def process_result(result):
                try:
                    ...
                    # final hard snap after animation for pixel-perfect alignment
                    snap_js = f"""
                    (function() {{
                        const body = document.scrollingElement || document.body;
                        const vw = {vw};
            ...
            # vertical single-column mode unchanged
