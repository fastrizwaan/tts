#!/usr/bin/env python3
"""Complete EPUB & HTML Viewer - save this as epub_viewer.py"""
...
class Win(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        for i in range(1, 11): string_list.append(f"{i} Columns")
        ...
        self.columns_combo.set_selected(1)
        GLib.idle_add(lambda: self.apply_column_layout(1))
            ...
            if file:
                def load():
                    try:
                        ...
                        GLib.idle_add(lambda: (self.split.set_show_sidebar(False), self.apply_column_layout(self.current_columns - 1)))
    ...
    def load_epub(self, path):
        ...
        if not toc_nodes:
            ...
            if raw:
                # ... [keep your existing recurse_item function] ...
                def recurse(it):
    ...
    def _on_toc_clicked(self, widget, href):
        ...
        print(f"[DEBUG] TOC clicked: {href}")  # Debug output

        # Split into path and fragment (anchor)
        href_path, _, fragment = href.partition("#")
        href_path = href_path.lstrip("./")
        
        # Try to find the item by comparing names
        target_item = None
        
        # First, try exact matches in item_map
        if href_path in self.item_map:
            target_item = self.item_map[href_path]
            print(f"[DEBUG] Found in item_map (exact): {href_path}")
        
        # Try basename match in item_map
        if not target_item:
            for name, item in self.item_map.items():
                if os.path.basename(name) == os.path.basename(href_path):
                    target_item = item
                    print(f"[DEBUG] Found in item_map (basename): {name}")
                    break
        
        # Try all items with various matching strategies
        if not target_item:
            for item in self.items:
                ...
                if (href_path == item_name or 
                    ...
                    target_item = item
                    print(f"[DEBUG] Found by name matching: {item_name}")
                    break
    ...
    def apply_column_layout(self, selected_column_index, restore_position=False):
        """Apply column layout to content"""
    ...
    # And in the App class, add this method:
