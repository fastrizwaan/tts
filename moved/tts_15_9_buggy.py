...
class EpubViewer(Adw.ApplicationWindow):
    ...
    def process_chapter_content(self, content, item):
        ...
        # Allowed inline tags to preserve inside sentence spans
        ALLOWED_INLINE_TAGS = set(["b","strong","i","em","u","span","sup","sub","a","code","abbr","cite","mark","small"])

        def rebuild_block_preserve_html(block):
            """
            Preserve inline formatting by mapping normalized plain text back to slices of the original innerHTML.
            Each sentence becomes a <span data-tts-id="...">...HTML preserved...</span>
            """
            orig_html = "".join(str(ch) for ch in block.contents)
            if not orig_html or not orig_html.strip():
                return

            # Get normalized plain text (single spaces)
            plain = BeautifulSoup(orig_html, "html.parser").get_text(" ", strip=False)
            normalized_plain = re.sub(r'\s+', ' ', plain.strip())
            if not normalized_plain:
                return
            sentences = split_sentences(normalized_plain)
            if not sentences:
                return

            # Build mapping from each normalized character position -> (start_html_index, end_html_index)
            # Tokenize orig_html into tags and text
            parts = list(re.finditer(r'(<[^>]+>|[^<]+)', orig_html))
            char_map = []  # list of (html_start, html_end) for each normalized char
            norm_pos = 0
            ...
            for m in parts:
                ...
                end_idx = m.end()
                if token.startswith("<"):
                    # tags don't correspond to characters
                    continue
                # token is text - build runs of whitespace / non-whitespace to normalize
                runs = list(re.finditer(r'(\s+|[^\s]+)', token))
                for run in runs:
                    run_text = run.group(0)
                    run_start = start_idx + run.start()
                    run_end = start_idx + run.end()
                    if run_text.isspace():
                        # normalized to a single space char
                        # map one normalized char to full whitespace range
                        char_map.append((run_start, run_end))
                        norm_pos += 1
                    else:
                        # non-whitespace - map each character individually
                        # map each character to its corresponding html index range (single char)
                        for i_ch in range(len(run_text)):
                            ch_start = run_start + i_ch
                            ch_end = ch_start + 1
                            char_map.append((ch_start, ch_end))
                            norm_pos += 1

            # If char_map length differs from len(normalized_plain), try a fallback:
            if len(char_map) < len(normalized_plain):
                # fallback: map as much as possible, then every missing char map to end
                diff = len(normalized_plain) - len(char_map)
                last_end = len(orig_html)
                for _ in range(diff):
                    char_map.append((last_end, last_end))
            elif len(char_map) > len(normalized_plain):
                # truncate
                char_map = char_map[:len(normalized_plain)]

            # Now for each sentence find the corresponding html slice
            out_fragments = []
            pos = 0
            for sent in sentences:
                ...
                if L == 0:
                    continue
                start_char_index = pos
                end_char_index = pos + L - 1  # inclusive
                if start_char_index < 0: start_char_index = 0
                if end_char_index >= len(char_map):
                    end_char_index = len(char_map) - 1
                # compute html slice bounds from char_map
                html_starts = [char_map[i][0] for i in range(start_char_index, end_char_index+1) if i < len(char_map)]
                html_ends = [char_map[i][1] for i in range(start_char_index, end_char_index+1) if i < len(char_map)]
                if not html_starts or not html_ends:
                    # fallback to entire block
                    frag_html = orig_html.strip()
                else:
                    slice_start = min(html_starts)
                    slice_end = max(html_ends)
                    # expand slice to include surrounding tags that might enclose this slice
                    # move slice_start left to include any opening tags that start before slice_start and are not closed
                    # and move slice_end right to include their closing tags.
                    # Simple heuristic: extend to nearest '<' on left and nearest '>' on right to avoid breaking tags.
                    # (This keeps tags intact in most practical cases.)
                    # clamp
                    slice_start = max(0, slice_start)
                    slice_end = min(len(orig_html), slice_end)
                    # extend left to tag boundary
                    left_lt = orig_html.rfind('<', 0, slice_start)
                    left_gt_after = orig_html.rfind('>', 0, slice_start)
                    if left_lt != -1 and (left_gt_after < left_lt):
                        slice_start = left_lt
                    # extend right to tag boundary
                    right_gt = orig_html.find('>', slice_end)
                    right_lt_before = orig_html.find('<', slice_end)
                    if right_gt != -1 and (right_lt_before == -1 or right_gt < right_lt_before):
                        slice_end = right_gt + 1
                    frag_html = orig_html[slice_start:slice_end]

                # append fragment
                out_fragments.append((sent_norm, frag_html))
                pos += L
                # consume one space if present between sentences in normalized_plain
                # If next char exists and is space, advance pos by 1
                if pos < len(normalized_plain) and normalized_plain[pos] == ' ':
                    pos += 1

            # Replace block contents with constructed spans preserving HTML fragments
            new_html = ""
            from bs4 import BeautifulSoup as BS
            for sent_text, frag_html in out_fragments:
                sid = stable_id_for_text(sent_text)
                # Build span with fragment's inner HTML. Use BS to ensure valid HTML.
                # We'll create a span and then set its inner contents by parsing frag_html
                span_soup = BS("", "html.parser")
                span_tag = span_soup.new_tag("span")
                span_tag.attrs["data-tts-id"] = sid
                # Insert frag_html parsed so tags are real nodes
                try:
                    inner = BS(frag_html, "html.parser")
                    for node in inner.contents:
                        span_tag.append(node)
                except Exception:
                    # fallback to text
                    span_tag.string = sent_text
                new_html += str(span_tag) + " "
            # replace the block contents in the original soup
            try:
                block.clear()
                # parse new_html into soup fragment and insert children
                frag = BeautifulSoup(new_html, "html.parser")
                for child in frag.body.contents if frag.body else frag.contents:
                    block.append(child)
            except Exception:
                # worst-case: leave block unchanged
                pass

        # Process each block once (no recursion into inline <i>/<b>)
        for tag in TARGET_TAGS:
            for elem in soup.find_all(tag):
                rebuild_block_preserve_html(elem)
    ...
    def extract_chapters(self):
        """
        Build self.chapters from the book's navigation (nav / toc) when available.
        Each chapter entry: { 'id': item_id, 'title': title, 'file': path, 'item': item, 'href': href }
        Also builds self._href_to_file map for href -> local file path (used for fragment navigation).
        """
        self.chapters = []
        self._href_to_file = {}
        if not self.current_book:
        ...
        # ensure resources extracted so links can point to resources/<name>
        self.extract_resources()

        # Build a mapping of item href (relative) -> (item, item_id)
        href_map = {}
        for book_item in self.current_book.get_items():
            iid = getattr(book_item, "id", None)
            try:
                href = getattr(book_item, "href", None) or book_item.get_name()
            except Exception:
                href = None
            if href:
                href_map[href] = (book_item, iid)

        # Helper to write item content to temp file (if not already)
        def write_item_to_temp(item, item_id, href_hint=None):
            try:
                content = item.get_content().decode('utf-8')
            except Exception:
                content = item.get_content().decode('utf-8', 'ignore')
            outfn = f"{item_id}.html" if item_id else (os.path.basename(href_hint) if href_hint else f"{uuid.uuid4().hex}.html")
            outpath = os.path.join(self.temp_dir, outfn)
            processed = self.process_chapter_content(content, item)
            with open(outpath, 'w', encoding='utf-8') as f:
                f.write(processed)
            # store map key as the href without fragment
            key = href_hint.split('#')[0] if href_hint else (getattr(item, 'href', '') or outfn)
            self._href_to_file[key] = outpath
            return outpath

        # Attempt to use the book TOC / nav if present
        toc = []
        try:
            raw_toc = self.current_book.get_toc()  # may be nested list
            toc = self._flatten_toc(raw_toc)
        except Exception:
            toc = []

        # If we have a proper toc, use it to build chapters list (preserving order)
        if toc:
            used_items = set()
            for entry in toc:
                href = entry.get('href') or entry.get('src') or entry.get('link') or ''
                title = entry.get('title') or entry.get('name') or "Untitled"
                # split fragment if any
                href_no_frag = href.split('#')[0] if href else ''
                # find matching item in href_map (prefer exact match)
                item_tuple = href_map.get(href_no_frag)
                if item_tuple:
                    item, iid = item_tuple
                    # write file if not already
                    if href_no_frag not in self._href_to_file:
                        file_path = write_item_to_temp(item, iid, href_no_frag)
                    else:
                        file_path = self._href_to_file[href_no_frag]
                    self.chapters.append({
                        'id': iid,
                        'title': title,
                        'file': file_path,
                        'item': item,
                        'href': href  # keep original href (may include fragment)
                    })
                    used_items.add(iid)
                else:
                    # Fallback: create a synthetic html that links to the href (external or missing)
                    synthetic_path = os.path.join(self.temp_dir, f"toc_link_{uuid.uuid4().hex[:8]}.html")
                    link_target = href
                    html = f"<!doctype html><html><head><meta charset='utf-8'><title>{_html.escape(title)}</title></head><body><a href='{_html.escape(link_target)}'>{_html.escape(title)}</a></body></html>"
            ...
            # Ensure spine items not present in toc are appended after
            spine_items = [s[0] for s in self.current_book.spine]
            for item_id in spine_items:
                if any(c.get('id') == item_id for c in self.chapters):
                    continue
                # find item object
                item = None
                for bi in self.current_book.get_items():
                    if getattr(bi, 'id', None) == item_id:
                        item = bi
                        break
                if item and getattr(item, 'media_type', '') == 'application/xhtml+xml':
                    file_path = write_item_to_temp(item, item_id, getattr(item, 'href', item_id))
                    self.chapters.append({
                        'id': item_id,
                        'title': self.extract_title(item.get_content().decode('utf-8', 'ignore')),
                        'file': file_path,
                        'item': item,
                        'href': getattr(item, 'href', item_id)
                    })
        else:
            # No TOC: fallback to spine order
            spine_items = [s[0] for s in self.current_book.spine]
            for item_id in spine_items:
                item = None
                for bi in self.current_book.get_items():
    ...
    def _flatten_toc(self, raw_toc):
        """
        Normalize ebooklib's get_toc() result into a flat list of dicts {title, href}.
        raw_toc can be a list of Link objects, tuples, or nested lists.
        """
        out = []
        def walk(node):
            if not node:
                return
            # node may be a Link-like object
            try:
                # ebooklib Link has .title and .href
                href = getattr(node, 'href', None)
                title = getattr(node, 'title', None)
                if href or title:
                    out.append({'title': title or 'Untitled', 'href': href or ''})
                    return
            except Exception:
                pass
            # node may be a tuple (Link, [children])
            if isinstance(node, (list, tuple)):
                # if first element is Link-like
                first = node[0]
                rest = node[1] if len(node) > 1 else None
                try:
                    ...
                    # fallback if first is a primitive
                    pass
                # children may be in second element if iterable
                if rest:
        ...
        # top-level might be a list of entries
        for entry in raw_toc:
            walk(entry)
        # normalize href/title fields
        normalized = []
        for e in out:
            h = e.get('href') or e.get('link') or e.get('src') or ''
            t = e.get('title') or e.get('name') or 'Untitled'
            normalized.append({'href': h, 'title': t})
        return normalized

    def _populate_toc_list(self):
        """Fill the sidebar ListBox with chapter rows. Supports fragment links."""
        # clear existing
        child = self.toc_list.get_first_child()
        while child:
            next_child = child.get_next_sibling()
        ...
        for idx, ch in enumerate(self.chapters):
            ...
            row.set_activatable(True)
            row.set_child(label)
            # store index and href on row for activation
            row._chapter_index = idx
            row._chapter_href = ch.get('href')  # may include fragment
            row._chapter_file = ch.get('file')
            self.toc_list.append(row)

    def _on_toc_row_activated(self, listbox, row):
        # handle href with fragment (e.g., OEBPS/chapter1.html#section5)
        if not hasattr(row, "_chapter_index"):
            return
        idx = row._chapter_index
        href = getattr(row, "_chapter_href", None)
        file = getattr(row, "_chapter_file", None)
        if href:
            # if href includes '#', split and restore scroll after load
            parts = href.split('#', 1)
            href_no_frag = parts[0]
            frag = parts[1] if len(parts) > 1 else None
            # Prefer mapping in self._href_to_file if available
            target_file = None
            if href_no_frag in getattr(self, "_href_to_file", {}):
                target_file = self._href_to_file[href_no_frag]
            elif file and os.path.exists(file):
                target_file = file
            else:
                # try to find by basename
                bn = os.path.basename(href_no_frag)
                target_file = self._href_to_file.get(bn) or file

            if target_file and os.path.exists(target_file):
                # set current_chapter to idx if that chapter corresponds to file, else set to idx
                self.current_chapter = idx
                # load chapter and optionally navigate to fragment after load
                self.webview.load_uri(GLib.filename_to_uri(target_file))
                # store fragment restore
                if frag:
                    def do_fragment_restore():
                        js = f"location.hash = '#{frag}'; var el = document.getElementById('{frag}'); if(el) el.scrollIntoView();"
                        self.webview.evaluate_javascript(js, -1, None, None, None, None, None)
                        return False
                    GLib.timeout_add(350, do_fragment_restore)
                chapter_info = f"Chapter {self.current_chapter + 1} of {len(self.chapters)}: {self.chapters[self.current_chapter].get('title')}"
                self.chapter_label.set_text(chapter_info)
                GLib.timeout_add(300, self.update_navigation)
                return
        # fallback: use index-based load
        if 0 <= idx < len(self.chapters):
            self.current_chapter = idx
            self.load_chapter()
            GLib.timeout_add(300, self.update_navigation)
