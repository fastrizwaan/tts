...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # bottom tabs (Adw.ToggleGroup + Gtk.ToggleButtons with icons)
        ...
        def make_icon_tab(icon_name, tooltip, name):
            ...
            def on_toggled(btn, nm=name):
                if btn.get_active():
                    # deactivate other toggle buttons in the same tabs_box
                    for sibling in tabs_box.get_children():
                        if sibling is not btn and isinstance(sibling, Gtk.ToggleButton):
        ...
        # add show/hide sidebar button in content header
        ...
        self.content_header.pack_end(self.sidebar_toggle_btn)
        ...
        try:
            ...
            try:
                ...
                if cov:
                    ...
                    # placeholder already set in __init__; keep placeholder (but ensure scaled)
                    placeholder_pb = GdkPixbuf.Pixbuf.new(GdkPixbuf.Colorspace.RGB, False, 8, COVER_W, COVER_H)
                    placeholder_pb.fill(0xddddddff)
                    placeholder_tex = Gdk.Texture.new_for_pixbuf(placeholder_pb)
                    self.cover_image.set_from_paintable(placeholder_tex)
