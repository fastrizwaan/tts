...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # pending fragment to jump after load
        ...
        try:
            ...
            # keep a global no-op handler so connect/disconnect behavior is safe
            self._global_load_id = self.webview.connect("load-changed", lambda *a: None)
        except Exception:
            self.WebKit = None
            self.webview = None
                ...
                if toc_nav:
                    def walk_list(ol, parent_iter=None):
                        for li in ol.find_all("li", recursive=False):
                            ...
                            # store full href (keep fragment if present)
    ...
    def on_tree_selection_changed(self, selection):
        ...
        if iter_:
            ...
            if isinstance(idx, int) and idx >= 0:
                ...
                self.update_navigation()
                # if href has fragment, schedule it
                if "#" in href:
    ...
    def on_tree_gesture_pressed(self, gesture, n_press, x, y):
        ...
        if cell_x is not None and cell_x < 22:
            ...
            if iter_:
                ...
                href = self.treestore.get_value(iter_, 1) or ""
                if isinstance(idx, int) and idx >= 0:
                    self.current_index = idx
                    self.update_navigation()
                    if "#" in href:
    ...
    def handle_internal_link(self, uri):
        # parse fragment and base
        try:
            parsed = urllib.parse.urlparse(uri)
            base = urllib.parse.unquote(parsed.path)
    ...
    def display_page(self):
        ...
        if self.webview:
            ...
            # attach one-shot if fragment pending
    ...
    def go_to_chapter(self, href):
        """Navigate to href: prefer internal loader (CSS + fragment handling) else fallback to rendition.js."""
        if not href:
            return
        # split fragment
        ...
        def href_to_index(h):
            if not h:
                return -1
            h0 = h.split("#")[0]
            candidates = [h0, os.path.basename(h0)]
            try:
                uq = urllib.parse.unquote(h0)
                if uq != h0:
        ...
        # fallback: try epub.js rendition.display(href) and inject CSS
        try:
            if self.webview and self.css_content:
                js_inject = (
                    "(function(){"
                    "var s = document.getElementById('epubcss-custom');"
                    "if(!s){ s = document.createElement('style'); s.id='epubcss-custom'; document.head.appendChild(s); }"
                    "s.innerHTML = %s;"
                    "})();"
                ) % (json.dumps(self.css_content))
                try:
                    self.webview.run_javascript(js_inject, None, None, None)
                except TypeError:
                    try:
                        self.webview.run_javascript(js_inject)
                    except Exception:
                        pass
            js_display = f"if(window.rendition) window.rendition.display({json.dumps(href)});"
            try:
                self.webview.run_javascript(js_display, None, None, None)
            except TypeError:
                try:
                    self.webview.run_javascript(js_display)
                except Exception:
                    pass
        except Exception:
            pass
    ...
    def cleanup(self):
        ...
        try:
            ...
            pass
        self.pending_fragment = None
        # disconnect one-shot/global handlers if present
        try:
