...
class EPUBWindow(Adw.ApplicationWindow):
    ...
    def scroll_to_end_of_page(self):
        ...
        Behavior:
        - Multi-column: Scrolls horizontally to the rightmost column
        - Single-column: Scrolls vertically to the bottom
        """
        h_adj = self.scrolled_window.get_hadjustment()
        v_adj = self.scrolled_window.get_vadjustment()
        
        if self.app.columns > 1 or not self.app.use_fixed_columns:
            # Multi-column: scroll to rightmost column
            ...
            (function() {{
                window.scrollTo({{
                    left: {target_scroll},
                    behavior: 'auto' // Immediate jump, no animation
                }});
            }})();
            """
            self.webview.evaluate_javascript(js_code, -1, None, None, None)
        else:
            # Single-column: scroll to bottom
            viewport_height = v_adj.get_page_size()
            max_scroll = v_adj.get_upper() - viewport_height
