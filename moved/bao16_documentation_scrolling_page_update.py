...
class Win(Adw.ApplicationWindow):
    ...
    def on_scroll_event_received(self, content_manager, js_result):
        ...
        try:
            ...
            icons = {
                'wheel': 'ðŸ”ƒ ',  # Wheel now represents single column scroll
                'wheel-y': '↕️  ',
                'arrow-left': '⬅️  ',
                'arrow-right': '➡️ ',
                'page-up': 'PgUp',
    ...
    def apply_column_layout(self, selected_column_index, restore_position=False):
        ...
        4. Mouse wheel column navigation (single column scroll)
        ...
        # break-inside: auto: Allows text flow in text elements
        # ===================================================================
        css = f"""
        <style>
            body {{
        ...
        // ===============================================================
        // MOUSE WHEEL NAVIGATION (SCROLLS 1 COLUMN)
        // ===============================================================
        // PURPOSE: Navigate one column per wheel event (Requirement 1)
        // 
        // SINGLE COLUMN MODE:
        ...
        document.addEventListener('wheel', function(e) {{
            ...
            const colWidth = getColumnWidth();
            if (colWidth <= 0) return;
            // Calculate scroll direction and distance (always 1 column)
            const scrollDir = e.deltaY > 0 ? 1 : -1; // 1 for right, -1 for left
            const scrollDist = scrollDir * colWidth; // Distance is always 1 column width
            const target = Math.round(
                (window.scrollX + scrollDist) / colWidth
            ) * colWidth; // Snap target to column boundary
            // Apply smooth scroll animation
            smoothScrollTo(target, window.scrollY);
            // Report the event to Python
            sendScrollEvent('wheel'); // Use 'wheel' for single column scroll
        ...
        // - Arrow Left/Right: Move one column (Requirement 2 - kept as is)
        // - Page Up/Down: Move N columns (N = current number of columns) (Requirement 3)
        ...
        document.addEventListener('keydown', function(e) {{
            ...
            if (window.currentColumnCount === 1) {{
                ...
                switch (e.key) {{
                    case 'ArrowLeft': 
                        e.preventDefault(); 
                        x = Math.max(0, x - colWidth); // Move 1 column left
                        type = 'arrow-left'; 
                        break;
                    case 'ArrowRight': 
                        e.preventDefault(); 
                        x = Math.min(maxScrollX, x + colWidth); // Move 1 column right
                        type = 'arrow-right'; 
                        break;
                    // --- MODIFIED: Page Up/Down now scrolls N columns (N = currentColumnCount) ---
                    case 'PageUp': 
                        e.preventDefault(); 
                        x = Math.max(0, x - (colWidth * window.currentColumnCount)); // Move N columns left
                        type = 'page-up'; 
                        break;
                    case 'PageDown': 
                        e.preventDefault(); 
                        x = Math.min(maxScrollX, x + (colWidth * window.currentColumnCount)); // Move N columns right
                        type = 'page-down'; 
                        break;
                    // --- END MODIFIED ---
                    case 'Home': 
