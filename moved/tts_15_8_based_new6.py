...
class TTSManager:
    ...
    def _run_js_highlight(self, sid):
        ...
        if not webview:
            return
        # Snap to column/page when element is not visible
        js = f"""
        (function() {{
            try {{
                ...
                if (isMultiColumn) {{
                    // Multi-column mode - snap to column if element is not visible or near edge
                    ...
                    // Check if element is near the right edge (within 20% of viewport width from right)
                    var distanceFromRight = viewportWidth - rect.right;
                    var isNearRightEdge = isVisible && distanceFromRight < (viewportWidth * 0.2);
                    
                    if (!isVisible || isNearRightEdge) {{
                        // Element not visible OR near right edge - snap to the column containing it
                        var elementLeft = currentScroll + rect.left;
                        
                        // Snap to the leftmost edge of the column
                        var targetScroll = elementLeft - 20; // 20px margin from left
                    ...
                    // Single column mode - scroll to top if not visible or near bottom
                    var isVerticallyVisible = rect.top >= 0 && rect.bottom <= viewportHeight;
                    
                    // Check if element is near the bottom edge (within 20% of viewport height from bottom)
                    var distanceFromBottom = viewportHeight - rect.bottom;
                    var isNearBottomEdge = isVerticallyVisible && distanceFromBottom < (viewportHeight * 0.2);
                    
                    if (!isVerticallyVisible || isNearBottomEdge) {{
                        // Scroll element to the top of viewport
                        var currentScrollY = window.pageYOffset || document.documentElement.scrollTop;
                        var elementTop = currentScrollY + rect.top;
                        var targetScrollY = elementTop - 20; // 20px margin from top
...
class EpubViewer(Adw.ApplicationWindow):
    ...
    def _on_sidebar_toggle_complete(self):
        ...
        if self.current_book and self.chapters:
            ...
            """
            self.webview.evaluate_javascript(js_code, -1, None, None, None, self._on_sidebar_scroll_info, None)
        return False
