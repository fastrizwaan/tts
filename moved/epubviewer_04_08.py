...
###################
    ...
    def _on_sidebar_toggle(self, btn):
        try:
            ...
            # Trigger webview resize after sidebar animation completes
            if self.webview:
                GLib.timeout_add(350, self._trigger_webview_resize)
        except Exception:
            pass

    def _trigger_webview_resize(self):
        """Force webview to recalculate layout and snap to column after sidebar toggle."""
        try:
            if self.webview:
                js = """
                (function() {
    ...
    def _escape_js_for_injection(js_code):
        """Escape JavaScript code for safe injection into another JS string."""
    ...
    def _wrap_html(self, raw_html, base_uri):
        ...
        try:
            ...
            col_rules = (
                ...
                "}\n"
                "/* Snap points for columns */\n"
                ".ebook-content > * {\n"
                "  scroll-snap-align: start !important;\n"
                "  scroll-snap-stop: always !important;\n"
                "}\n"
                "/* Single column mode with vertical scrolling */\n"
        ...
        # Improved column navigation JavaScript with viewport resize detection
        js_detect_columns = """<script>
        (function() {
            const COLUMN_GAP = __GAP__;
            ...
            function getColumnWidth() {
                ...
                const gap = parseFloat(style.columnGap) || COLUMN_GAP;
            ...
            function saveScrollPosition() {
                ...
                if (maxScroll > 0) {
                    currentScrollRatio = content.scrollLeft / maxScroll;
                } else {
            ...
            function restoreScrollPosition() {
                ...
                const maxScroll = content.scrollWidth - content.clientWidth;
                const targetScroll = maxScroll * currentScrollRatio;
                
                content.scrollLeft = targetScroll;
                
                // Snap after restoration
                setTimeout(() => {
                    snapToNearestColumn();
                }, 50);
            }
            ...
            function snapToNearestColumn() {
                ...
                const content = document.querySelector('.ebook-content');
                if (!content || isSingleColumnMode()) return;
                ...
                const targetScroll = nearestColumn * columnWidth;
            ...
            function handleWheel(e) {
                ...
                // Check if we're scrolling vertically with content that fits
                ...
                if (hasVerticalScroll && Math.abs(e.deltaY) > Math.abs(e.deltaX)) {
                    // Allow vertical scrolling if content overflows vertically
                    return;
                }
                ...
                // Convert vertical scroll to horizontal for column navigation
                const delta = e.deltaY || e.deltaX;
                const direction = delta > 0 ? 1 : -1;
                
                // Small deltas = scroll smoothly, large deltas = jump column
                const threshold = 50;
                if (Math.abs(delta) < threshold) {
                    // Smooth pixel scrolling for trackpad
                    ...
                    // Jump to next/prev column for mouse wheel
                    const currentColumn = Math.round(content.scrollLeft / columnWidth);
            ...
            function handleScroll() {
                if (isSnapping || isSingleColumnMode()) return;
            ...
            function handleKeyboard(e) {
                ...
                if (direction !== 0) {
                    ...
                    isSnapping = true;
                    content.scrollTo({
                        left: Math.max(0, Math.min(targetScroll, content.scrollWidth - content.clientWidth)),
                        behavior: 'smooth'
                    });
            ...
            function handleResize() {
                ...
                if (Math.abs(currentWidth - lastViewportWidth) > 10) {
                    ...
                    // Wait for layout to stabilize, then restore position
                    clearTimeout(snapTimeout);
                    snapTimeout = setTimeout(() => {
            ...
            function init() {
                ...
                if (content) {
                    ...
                    lastViewportWidth = content.clientWidth;
                    
                    // Initial snap
                    setTimeout(snapToNearestColumn, 100);
                }
            }
            ...
            // Listen for viewport resize (including sidebar toggle)
            let resizeTimeout;
            ...
            // Also listen for visibility change (when sidebar is toggled)
        ...
        </script>""".replace("__GAP__", str(getattr(self, "_column_gap", 24)))
