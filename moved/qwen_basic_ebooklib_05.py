...
class EPubViewer(Adw.ApplicationWindow):
    ...
    def on_decide_policy(self, webview, decision, decision_type):
        ...
        if uri.startswith("file://") and self.temp_dir:
            ...
            # Extract fragment before checking path
            fragment = ''
            if '#' in local_path:
            ...
            if local_path.startswith(self.temp_dir):
                # This is an internal link - prevent default navigation
                try:
                    decision.ignore()
                except Exception:
                    pass
                # Handle it as chapter navigation
                return self.handle_internal_link(uri)

        return False

    def handle_internal_link(self, uri):
        """Enhanced link handler that navigates between chapters like next/prev buttons"""
        frag = ''
        ...
        # Find the matching item in our document list
        target_item = self.find_item_by_path(internal)
        
        if target_item:
            target_name = target_item.get_name()
            # Find index and navigate to it (just like next/prev buttons)
            for i, it in enumerate(self.items):
                if it.get_name() == target_name:
                    ...
                    # Display the page (this will re-render with proper content)
    ...
    def load_epub(self, path):
        try:
            ...
            # Create temp directory and extract all content
    ...
    def build_item_map(self):
        """Build a comprehensive mapping of all possible path variations to items"""
        self.item_map = {}
        for item in self.items:
            name = item.get_name()
            if not name:
    ...
    def resolve_resource_path(self, src, item_dir):
        ...
        # Try to resolve relative to current item directory
    ...
    def display_page(self, jump_fragment=''):
        ...
        for link in soup.find_all('a', href=True):
            ...
            # Handle same-page fragment links
            if href.startswith('#'):
                continue
            ...
            if target:
                # Try to resolve relative to current item
...
"""
        if self.webview:
            base_uri = f"file://{os.path.join(self.temp_dir, item_dir)}/"
            try:
