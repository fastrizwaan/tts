...
###################
    ...
    def _on_sidebar_toggle(self, btn):
        ...
        The browser automatically fires a resize event when the sidebar animates,
        and the JavaScript resize handler maintains the current column position.
        No need to call display_page().
        """
        try:
            ...
            # The JavaScript resize handler (with 400ms delay) handles everything
            print(f"🔄 Sidebar toggled - letting resize handler maintain position")
    ...
    def _wrap_html(self, raw_html, base_uri):
        ...
        # COMPLETE JAVASCRIPT WITH FIXED SCROLL LISTENER
        ...
        (function() {{
            ...
            // FIXED: Scroll event listener - waits for DOM ready
            function attachScrollListener() {{
                ...
                container.addEventListener('scroll', function() {{
                    console.log('📜 Scroll event fired - scroll: ' + container.scrollLeft.toFixed(0) + 'px left, ' + container.scrollTop.toFixed(0) + 'px top');
                    ...
                    console.log('⏱️ Starting 3-second timer to save position...');
                    savePositionTimer = setTimeout(() => {{
                        console.log('⏰ 3 seconds elapsed, saving position now...');
                        const pos = saveReadingPosition();
                        if (pos) {{
                            console.log('💾 Auto-saved reading position after scroll');
                        }} else {{
            ...
            window.addEventListener('resize', function() {{
                ...
                resizeTimer = setTimeout(function() {{
                    ...
                    if (window.currentReadingPosition && window.currentReadingPosition.text) {{
                        setTimeout(() => {{
                            console.log('✨ Highlighting saved position after resize');
                            highlightReadingPosition(3000);
                        }}, 100);
                    }} else {{
                        console.log('⚠️ No saved position to highlight (scroll first)');
                    }}
                }}, 400);
            }});
    ...
    def _update_column_css_via_js(self):
        """Update column CSS via JavaScript - highlights saved position."""
        if not getattr(self, "webview", None):
            return
