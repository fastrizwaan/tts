...
class Win(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        self.original_html_content = "<h1>Welcome</h1><p>Select an HTML file or EPUB to view.</p>" # Moved here for consistency
    ...
    def load_epub(self, path):
        ...
        if not toc_nodes:
            toc_nodes = []
            try:
                # Use the manually defined constant
    ...
    def _populate_epub_ui(self, toc_nodes):
        ...
        try:
            if self.book:
                # Get all document items (HTML/XHTML content) - Use manually defined constant
                spine = [i for i in self.book.get_items() if i.get_type() == ITEM_DOCUMENT]
                if spine:
                    ...
                    # Store the original content for column layout
                    self.original_html_content = processed_content
                    # Apply column layout immediately after processing
                    self.apply_column_layout(self.current_columns - 1)
                    loaded = True
        except Exception as e:
        ...
        if not loaded:
            ...
            self.apply_column_layout(self.current_columns - 1) # Still apply layout, even if error content
    ...
    def _build_foliate_toc(self, parent_box, nodes, level=0):
        ...
        for node in nodes:
            ...
            if children:
                ...
                def _make_leaf_callback(href_local):
                    def _leaf_click(_widget):
                        self._on_toc_clicked(_widget, href_local)
                        self._set_active(href_local)
                    return _leaf_click

                row.connect("activated", _make_leaf_callback(href))
    ...
    def _process_epub_content(self, html_content, item):
        ...
        # Resolve base path for current item (e.g. "Text/" or "OEBPS/")
        ...
        try:
            for img_item in self.book.get_items():
                if img_item.get_type() == ITEM_IMAGE:
                    ...
                    if img_file_name.lower().endswith(".png"):
                        mime_type = "image/png"
                    elif img_file_name.lower().endswith(".gif"):
                        mime_type = "image/gif"
                    elif img_file_name.lower().endswith(".svg"):
                    ...
                    # Patterns for relative path resolution
                    rel_path = os.path.relpath(img_file_name, base_path)
                    patterns = {
                        img_file_name,
                        os.path.basename(img_file_name),
                        f"./{os.path.basename(img_file_name)}",
                        rel_path,
                        rel_path.replace("\\", "/"),
                    }
    ...
    def _on_toc_clicked(self, widget, href):
        ...
        candidates = {href_path, os.path.basename(href_path), href_path.replace("\\", "/")}

        # Find matching EPUB document item
        target_item = None
        for item in self.book.get_items():
            if item.get_type() != ITEM_DOCUMENT:
                continue
            fn = item.file_name.replace("\\", "/")
            if fn in candidates or os.path.basename(fn) in candidates or href_path in fn:
                target_item = item
                break

        if not target_item:
            print(f"[TOC] No match for: {href}")
            self.show_error_dialog(f"Target not found: {href}")
            return

        try:
            html_text = target_item.get_content().decode("utf-8", errors="ignore")
            processed = self._process_epub_content(html_text, target_item)
            self.original_html_content = processed
            self.apply_column_layout(self.current_columns - 1)

            if fragment:
                # Retry until element exists (renders may lag)
                js_scroll = f"""
                function tryScrollToFragment(id) {{
                ...
                (function retryScroll(){{
                    if(!tryScrollToFragment("{fragment}")) {{
                        setTimeout(retryScroll, 300);
                    }}
                }})();
                """
                GLib.timeout_add(500, lambda: self.webview.evaluate_javascript(js_scroll, -1, None, None, None))
        except Exception as e:
