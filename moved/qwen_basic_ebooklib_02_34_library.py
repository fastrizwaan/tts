...
class EPubViewer(Adw.ApplicationWindow):
    ...
    def show_library(self):
        ...
        # Remove or comment out: wrap.set_natural_line_length(1200)
        wrap.set_child_spacing(12)
        wrap.set_line_spacing(12)
       # wrap.add_css_class("library-grid")

        wrap.set_margin_start(20); wrap.set_margin_end(20)
        wrap.set_margin_top(20); wrap.set_margin_bottom(20)
        ...
        for entry in list(reversed(self.library)):  # most recent first
            ...
            card.set_size_request(160, 200)
            card.set_hexpand(False)
            card.set_vexpand(False)

            # cover image (centered)
            img = Gtk.Image()
            img.set_size_request(180, 200)
            if cover and os.path.exists(cover):
                try:
                    pix = GdkPixbuf.Pixbuf.new_from_file_at_scale(cover, 140, 200, True)
                    tex = Gdk.Texture.new_for_pixbuf(pix)
                    img.set_from_paintable(tex)
                except Exception:
                    pb = GdkPixbuf.Pixbuf.new(GdkPixbuf.Colorspace.RGB, False, 8, 140, 200)
                    pb.fill(0xddddddff)
                ...
                pb.fill(0xddddddff)
                img.set_from_paintable(Gdk.Texture.new_for_pixbuf(pb))
            img.add_css_class("rounded-icon")
