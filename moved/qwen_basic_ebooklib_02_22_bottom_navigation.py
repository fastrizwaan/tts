...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # left: show/hide sidebar button (moved to left as requested)
        self.sidebar_toggle_btn = Gtk.Button(icon_name="sidebar-show-symbolic")
        self.sidebar_toggle_btn.set_tooltip_text("Show/Hide sidebar")
        ...
        # add action handler (Application.create_action exists)
        app.create_action("about", lambda a, p: self.show_error("EPUB Viewer — minimal menu"))

        # pin/unpin toggle -- use view-pin-symbolic
        ...
        # Book image + metadata area (LARGER cover)
        book_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
        book_box.set_margin_top(8)
        book_box.set_margin_bottom(8)
        book_box.set_margin_start(8)
        book_box.set_margin_end(8)

        # placeholder large cover using paintable texture (no undefined p)
        ...
        # bottom tabs (icon buttons) — keep own list to manage exclusivity
        ...
        def make_icon_tab(icon_name, tooltip, name):
            ...
            def on_toggled(btn, nm=name):
                if btn.get_active():
                    # deactivate others using our tracked list
                    for sib, sib_name in list(self._tab_buttons):
                        if sib is not btn:
                            try:
        ...
        # content header previously had show/hide on right; moved to sidebar header per request
    ...
    def _on_pin_toggled(self, btn):
        ...
        # pinned => disable overlay (persistent sidebar)
        try:
            # prefer explicit overlay control if available
            if hasattr(self.split, "set_overlay"):
                self.split.set_overlay(not pinned)
        except Exception:
            pass
        # show sidebar when pinned, hide when unpinned (overlay will control behavior)
        try:
    ...
    def load_epub(self, path):
        try:
            ...
            # try to load a cover image if present (prefer filenames with 'cover', else first image)
            try:
                cov = None
                ...
                if cov:
                    ...
                    # load scaled pixbuf and present as paintable texture (GTK4-friendly)
                    ...
                    # ensure placeholder remains at correct size
                    placeholder_pb = GdkPixbuf.Pixbuf.new(GdkPixbuf.Colorspace.RGB, False, 8, COVER_W, COVER_H)
                    placeholder_pb.fill(0xddddddff)
                    placeholder_tex = Gdk.Texture.new_for_pixbuf(placeholder_pb)
                    self.cover_image.set_from_paintable(placeholder_tex)
