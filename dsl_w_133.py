...
class DictionaryManager:
    ...
    def search(self, q, exact_only=True, max_preview=300, with_defs=False):
        ...
        for dict_path, idx_data in self.index_cache.items():
            ...
            # Build word->entry lookup for fast access
            word_to_entry = {e["word"]: e for e in idx_data.get("entries", [])}
            ...
            for word in candidate_words:
                ...
                # Look up the entry for this candidate
                e = word_to_entry.get(word)
                if not e:
                    continue
                    
                seen.add(word.lower())
                key = e.get("key") or word.lower()
                ...
                if norm_key.lower().startswith(q_lower):
                    ...
                    tier = 3
                candidate_hits.append((tier, word, dict_name, e, idx_data))
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def _search_worker_loop(self):
        """Runs in a background thread; handles both exact and substring phases."""
        while True:
            ...
            def async_subsearch(local_search_id=search_id, local_query=query, local_exact_words=exact_words):
                ...
                filtered = [(w, data) for w, data in results2 if w.lower() not in local_exact_words]

                if (
                    not self._stop_flag.is_set()
                    and local_search_id == self._search_id
                    and local_query.lower() == self._active_query
                ):
                    if filtered:
                        print(f"[DEBUG] Appending {len(filtered)} n-gram results for '{local_query}'")
                        GLib.idle_add(self._on_search_results, local_query, filtered, False)
                    else:
                        print(f"[DEBUG] No new n-gram results for '{local_query}'")
                    GLib.idle_add(self.spinner.stop)
                else:
                    print(f"[DEBUG] Discarded stale async results for '{local_query}'")
    ...
    def _start_search_thread(self, query):
        ...
        # Drain queue to prevent stale backlog
        try:
        ...
        # --- Single authoritative search_id increment ---
        self._search_id += 1
        ...
        # Put the work on the background queue
