...
class DictionaryManager:
    ...
    # ------------------------------------------------------------
    # ------------------------------------------------------------
    def search(self, q, exact_only=True, max_preview=10, with_defs=False):
        r"""
        Unified case-insensitive search grouped by word:
          - Runs exact, compound, and substring matches in one pass.
          - Preserves tier priority: exact → compound → substring.
          - Avoids multiple passes or extra threads.
        """
        q = q.strip()
        if not q:
            return []
        ...
        q_esc = re.escape(q)
        exact_pat = re.compile(rf"^{q_esc}$", re.IGNORECASE)
        compound_pat = re.compile(rf"^{q_esc}[a-zA-Z]", re.IGNORECASE)
        substring_pat = re.compile(rf".*{q_esc}.*", re.IGNORECASE)

        def match_tier(word):
            """Return numeric tier: lower = higher priority."""
            if exact_pat.fullmatch(word):
                return 0
            if compound_pat.match(word):
                return 1
            if not exact_only and substring_pat.search(word):
                return 2
        ...
        # Sort by tier (exact first) and alphabetically
