...
class DictionaryIndexer:
    ...
    def build_index(self, on_progress=None):
        """Build JSON index for .dsl or .dsl.dz dictionary (UTF-8, case-insensitive)."""
        import struct
        ...
        with open_func(self.path, "rb") as fb:
            ...
            fb.seek(0)
            print(f"[DEBUG] Detected encoding for {self.path.name}: {encoding}")
            ...
            while True:
                ...
                while True:
                    # Detect newline based on encoding
                    ...
                    # --- Skip headers, comments, or junk ---
                    # 1. Empty or commented lines
                    if not line or line.lstrip().startswith("#"):
                        continue
                    # 2. Lines starting with space, tab, or hyphen are definitions or separators
                    if line[0].isspace() or line[0] == "-":
                        continue
                    # 3. Safety: ignore any lingering #NAME-style entries
                ...
                # Yield to GTK main loop for UI responsiveness
...
class DictionaryManager:
    ...
    def load_dictionary(self, path, parent_window=None, *args):
        """Load a DSL or DSL.DZ dictionary efficiently."""
        ...
        # Build index if missing/outdated
        if rebuild_index:
            transient_target = (
        ...
        print(f"[INFO] Loaded dictionary: {dnname} ({len(idx_data['entries'])} entries)")

    # ------------------------------------------------------------
    # ------------------------------------------------------------
    def search(self, q, exact_only=True, max_preview=100, with_defs=False):
        r"""
        Unified, tiered case-insensitive search grouped by word:
          0. ^WORD$           exact
          1. ^WORD[a-zA-Z]    compound
          2. ^WORD\s          phrase starting with word
          3. \sWORD\s         word in the middle
          4. \sWORD$          ends with word
          5. .*WORD.*         loose substring (broadest)
        All tiers are evaluated in one pass for efficiency.
        """
        q = q.strip()
        if not q:
            return []
        ...
        q_esc = re.escape(q)
        patterns = [
            #re.compile(rf"^{q_esc}$", re.IGNORECASE),         # 0 exact
            #re.compile(rf"^{q_esc}[a-zA-Z]", re.IGNORECASE),  # 1 compound
            re.compile(rf".*{q_esc}.*", re.IGNORECASE),  # 1 compound            
            
        ]

        # Determine which pattern tier a given key belongs to
        def match_tier(word):
            for tier, pat in enumerate(patterns):
                if pat.search(word):
                    return tier
            return None
        ...
        # Traverse all loaded dictionary indexes once
        for dict_path, idx_data in self.index_cache.items():
            ...
            enc = idx_data.get("encoding", "utf-8")
            fpath = Path(idx_data["file"])
            ...
            for e in entries:
                ...
                # If exact_only, only allow tier 0
                #if exact_only and tier > 0:
                #    continue

                word_groups.setdefault((tier, key), []).append((dict_name, e, fpath, enc))

        # Sort by tier (0→5) and then alphabetically
        for (tier, key) in sorted(word_groups.keys(), key=lambda k: (k[0], k[1])):
            seen_words.add(key)
            group_entries = []
            for dict_name, e, fpath, enc in word_groups[(tier, key)]:
                defs = self._read_entry(fpath, e, enc) if with_defs else []
                group_entries.append((dict_name, defs, "default"))
            results.append((word_groups[(tier, key)][0][1]["word"], group_entries))

            # Stop early when enough results collected
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def _search_worker_loop(self):
        """Worker: exact + prefix definitions first, then loose See-also suggestions."""
        while True:
            query = self._search_queue.get()
            if query is None:
                break
            query = query.strip()
            if not query:
            ...
            self._active_query = query.lower()

            # --- Phase 1: exact + prefix (definitions) ---
            tier1 = self.dict_manager.search(query, exact_only=False, with_defs=True)
            tier1 = [
                (w, d)
                for (w, d) in tier1
                if re.match(rf"^{re.escape(query)}([a-zA-Z]*$)", w, re.IGNORECASE)
            ]
            if not self._stop_flag.is_set():
                GLib.idle_add(self._on_search_results, query, tier1, True)

            # --- Phase 2: see-also suggestions (no definitions) ---
            tier2 = self.dict_manager.search(query, exact_only=False, with_defs=False)
            seealso = []
            for word, _ in tier2:
                if (
                    not re.match(rf"^{re.escape(query)}([a-zA-Z]*$)", word, re.IGNORECASE)
                    and query.lower() in word.lower()
                ):
                    seealso.append(word)
            if not self._stop_flag.is_set():
                GLib.idle_add(self._on_see_also_ready, query, seealso)
    ...
    def on_search(self, entry):
        """Debounced instant search with threaded backend."""
        ...
        self._search_timeout_id = GLib.timeout_add(0, delayed_start)

    # ------------------------------------------------------------
    def _start_search_thread(self, query):
        """Queue new search requests to the worker thread."""
        ...
        self._search_queue.put(query)

    def _on_see_also_ready(self, query, words):
        """Append a 'See also' section with headwords only."""
        ...
        links = " · ".join(
            f'<a href="dict://{html.escape(w)}" class="dict-link">{html.escape(w)}</a>'
            for w in unique
        )

        section_html = f"""
        <div style='margin-top:1.2em; padding-top:0.6em; border-top:1px solid #6663;'>
          <div style='opacity:0.8; font-size:0.9em; margin-bottom:0.4em;'>
            See also:
          </div>
          <div style='line-height:1.6; flex-wrap:wrap;'>{links}</div>
        </div>
        """

        # Inject after existing results
        if self._last_html:
            html_doc = self.build_html(self._last_html)
            final_html = html_doc.replace("</body>", section_html + "</body>")
            self.webview.load_html(final_html, "file:///")


    def _on_suggestions_ready(self, query, suggest_results):
        """Display suggestion words side by side below exact results."""
        if query.strip().lower() != getattr(self, "_active_query", ""):
            return
        ...
        # Collect unique headwords only (ignore duplicates)
        seen = set()
        suggestions = []
        for word, _ in suggest_results:
            lw = word.lower()
            if lw not in seen:
                seen.add(lw)
                suggestions.append(word)
            if len(suggestions) >= 30:  # reasonable limit
                break

        # Build horizontal list of clickable suggestions
        links = " ".join(
            f'<a href="dict://{html.escape(w)}" class="dict-link">{html.escape(w)}</a>'
            for w in suggestions
        )
        section_html = f"""
        <div style='margin-top:1.2em; padding-top:0.8em; border-top:1px solid #6663;'>
            <div style='opacity:0.8; font-size:0.9em; margin-bottom:0.4em;'>
                Suggestions:
            </div>
            <div style='display:flex; flex-wrap:wrap; gap:0.6em; line-height:1.6;'>
                {links}
            </div>
        </div>
        """

        # Inject suggestions into existing HTML
        if self._last_html:
            base_html = self.build_html(self._last_html)
            final_html = base_html.replace("</body>", section_html + "</body>")
            self.webview.load_html(final_html, "file:///")


    # ------------------------------------------------------------
    def _on_search_results(self, query, results, replace=True):
        """Render results in WebView; can either replace or append."""
        # Discard results from stale queries
        if query.strip().lower() != getattr(self, "_active_query", ""):
            return
        ...
        if replace:
            ...
            combined = list(self._last_html or []) + results
            html = self.build_html(combined)
            self.webview.load_html(html, "file:///")
            self._last_html = combined
