...
class MultiColumnTextView(Gtk.Box):
    ...
    def update_columns(self):
        ...
        # Create new columns based on width
        for i in range(3):  # Start with 3 columns
            scrolled = Gtk.ScrolledWindow()
            scrolled.set_policy(Gtk.PolicyType.NEVER, Gtk.PolicyType.AUTOMATIC)
            scrolled.set_size_request(self.column_width, -1)
    ...
    def set_content(self, text_buffer):
        ...
        # For simplicity, split by paragraphs
        ...
        for i, col_buffer in enumerate(self.text_buffers):
            ...
            end_idx = start_idx + paras_per_col if i < col_count - 1 else len(paragraphs)
            col_text = '\n\n'.join(paragraphs[start_idx:end_idx])
            col_buffer.set_text(col_text)
...
class HTMLTextViewApp(Adw.Application):
    ...
    def on_activate(self, app):
        ...
        self.multi_column_view.update_columns()
