...
class EPubViewer(Adw.ApplicationWindow):
    ...
    def _wrap_html(self, raw_html, base_uri):
        """
        Wrap raw_html with <head> containing our extracted CSS + theme css and proper <base>.
        Ensures every loaded HTML uses self.css_content + THEME_INJECTION_CSS.
        Also injects JavaScript to intercept ALL link clicks.
        """
        page_css = (self.css_content or "") + "\n" + THEME_INJECTION_CSS
        
        # Critical: JavaScript to intercept ALL link clicks and prevent default navigation
        # This ensures on_decide_policy is called for every link click
        ...
        (function() {
            ...
            // CRITICAL: Intercept ALL link clicks
            // We must prevent default navigation and let Python handle it
            function interceptLinks() {
                document.addEventListener('click', function(e) {
                    ...
                    if (target && target.tagName === 'A' && target.href) {
                        ...
                        console.log('[js] intercepted link click:', href);
                        
                        // Prevent default navigation
                        e.preventDefault();
                        e.stopPropagation();
                        
                        // Try to navigate via window.location (this WILL trigger on_decide_policy)
                        try {
    ...
    def on_decide_policy(self, webview, decision, decision_type):
        """
        Intercept ALL navigation to ensure CSS is applied to every page.
        This is critical for links within content pages (like TOC pages).
        """
        if not self.WebKit:
            return False
        ...
        if decision_type == self.WebKit.PolicyDecisionType.NAVIGATION_ACTION:
            ...
            # Ignore empty/blank URIs
            if uri in ("", "about:blank", "file://"):
                return False
            
            print(f"[navigation] intercepted: {uri}")
            
            # Block external links for security
            if uri.startswith("http://") or uri.startswith("https://"):
                print(f"[navigation] blocking external link: {uri}")
                try:
                    decision.ignore()
                except Exception:
                    pass
            ...
            # Intercept ALL file:// links to apply CSS
            if uri.startswith("file://"):
                # Check if this is a navigation we initiated (to prevent loops)
                # We detect this by checking if it's the exact same content we just loaded
                current_uri = webview.get_uri()
                if current_uri and current_uri == uri:
                    # This is a page we just loaded via load_html, allow it
                    print(f"[navigation] allowing our own load: {uri}")
                    return False
                
                # This is a user-clicked link or initial load - handle it with CSS injection
                print(f"[navigation] injecting CSS for: {uri}")
    ...
    def handle_internal_link(self, uri):
        """
        Enhanced internal link handling with proper CSS injection for ALL content types.
        - If link points to a document in spine -> display_page
        - If link points to an extracted resource (image/html) -> load with CSS wrapper
        - If a matching TocItem exists, select it and sync current_index
        ...
        if toc_match:
            if isinstance(toc_match.index, int) and toc_match.index >= 0:
                ...
                # TOC refers to non-spine item - resolve and load with CSS
        ...
        # Try to find the file in extracted paths
        possible_paths = []
        if self.temp_dir:
    ...
    def _load_file_with_css(self, file_path, fragment=None):
        """
        Load any file (HTML, image, etc.) with proper CSS injection and dark mode support.
        Returns True if successfully handled.
        """
        if not os.path.exists(file_path):
            return False
        
        # Ensure CSS is extracted before wrapping
        ...
        # Handle images - wrap in styled HTML
        ...
        # Handle HTML/XHTML - read and wrap with CSS
        if ext in (".html", ".xhtml", ".htm"):
            try:
                with open(file_path, "r", encoding="utf-8", errors="ignore") as fh:
                    content = fh.read()
                
                # Parse to extract body content (avoid double wrapping)
                soup = BeautifulSoup(content, "html.parser")
                
                # Remove any existing style/link tags to avoid conflicts
                for tag in soup.find_all(['style', 'link']):
                    tag.decompose()
                
                body = soup.find("body")
                if body:
                    # Get body content and preserve body attributes
        ...
        # Other file types - not handled
        return False

    def display_page(self, fragment=None):
        """
        Display the current spine page with proper CSS injection and dark mode support.
        """
        ...
        try:
            ...
            # Extract body content to avoid double wrapping
            body = soup.find("body")
            if body:
                body_attrs = ' '.join([f'{k}="{v}"' if isinstance(v, str) else f'{k}="{" ".join(v)}"' 
                                      for k, v in body.attrs.items()])
