...
#        menu_button.set_size_request(16, 20)
    ...
    def _close_split(self, overlay_to_close):
        ...
        # Get the TabRoot (grandparent)
        tab_root = parent.get_parent()
        if not isinstance(tab_root, Gtk.Box):
            print("Error: Paned parent is not TabRoot")
            return
        ...
        # Remove Paned from TabRoot
        tab_root.remove(parent)
        
        # Add the kept child back to TabRoot
        tab_root.append(keep_child)
        
        # Focus the kept editor
        if hasattr(keep_child, '_editor'):
            keep_child._editor.view.grab_focus()
