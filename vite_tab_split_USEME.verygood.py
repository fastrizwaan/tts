...
class Renderer:
    ...
    def get_visual_line_count_for_logical(self, cr, buf, ln, ln_width, viewport_width, allow_approximation=False):
        ...
        if allow_approximation and self.use_fast_approximation:
            ...
            max_width = viewport_width - ln_width
            if max_width <= 0:
                return 1
...
class ChromeTab(Gtk.Box):
    ...
    def _show_context_menu(self, x, y):
        ...
        section1.append_item(add_item("Horizontal Splitter", "win.tab_split_horizontal", idx_str))
...
#        menu_button.set_size_request(16, 20)
    ...
    def _create_editor_overlay(self, editor):
    ...
    def on_tab_move_new_window(self, action, parameter):
        ...
        title = editor.get_title()
        is_modified = editor.view.get_buffer().get_modified() # Check underlying buffer
        
        # Create new window
        app = self.get_application()
        new_window = EditorWindow(application=app)
        new_window.present()
        
        # Add tab in new window (initially empty)
        # We use add_tab() without path to get an empty editor, then populate it
        # This avoids async loading issues and allows transferring unsaved content
        new_editor = new_window.add_tab(None)
        
        # Transfer state
        new_editor.set_text(content)
        new_editor.current_file_path = file_path
        new_editor.set_title(title)
        
        # Update title in new tab
        # We need to find the page/tab for the new editor
        # add_tab returns editor. We can find the page via find_tab_with_file if path exists, 
        # but here we just added it. It's the selected page.
        new_page = new_window.tab_view.get_selected_page()
        if new_page:
            new_page.set_title(title)
            # Find the tab widget to update its title and modified state
            for tab in new_window.tab_bar.tabs:
                if getattr(tab, '_page', None) == new_page:
                    tab.set_title(title)
                    tab.set_modified(is_modified)
                    break
        
        # Mark old editor as not modified to bypass save prompt
        editor.view.get_buffer().set_modified(False)
        # Also update the tab UI to remove the dot (optional but clean)
        for tab in self.tab_bar.tabs:
            if getattr(tab, '_page', None) == page:
    ...
    def _split_view(self, parameter, orientation):
        ...
        # Create overlay for new editor
        new_overlay, new_editor = self._create_editor_overlay(new_editor)
        ...
        # Set position (50/50) - wait for realize or just set generic
        paned.set_position(400) # Arbitrary, maybe calculate?
