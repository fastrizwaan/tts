...
class SearchManager:
    ...
    def _search_yts(self, query):
        ...
        params = {"query_term": query, "limit": 250, "sort_by": "seeds"}
...
class MainWindow(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # Keep row activation for double-click behavior
        ...
        # 2. Info Button (New)
        col_info = Gtk.ColumnViewColumn(title="")
        f_info = Gtk.SignalListItemFactory()
        f_info.connect("setup", self._setup_info_btn)
        f_info.connect("bind", self._bind_info_btn)
        col_info.set_factory(f_info)
        col_info.set_fixed_width(50)
        self.col_view.append_column(col_info)
        ...
        # 6. Download Button (New)
        col_dl = Gtk.ColumnViewColumn(title="")
        f_dl = Gtk.SignalListItemFactory()
        f_dl.connect("setup", self._setup_dl_btn)
        f_dl.connect("bind", self._bind_dl_btn)
        col_dl.set_factory(f_dl)
        col_dl.set_fixed_width(50)
        self.col_view.append_column(col_dl)

        self.scrolled_window = Gtk.ScrolledWindow()
        self.scrolled_window.set_child(self.col_view)
        self.stack.add_named(self.scrolled_window, "results")
    ...
    def _add_column(self, title, prop_name, expand, setup_func, bind_func, width=-1):
        ...
        if prop_name == "size" or prop_name == "seeders":
    ...
    def _scroll_to_top(self):
        """Helper to reset viewport scroll to top."""
    ...
    def apply_filter(self):
        ...
        self._scroll_to_top() # Scroll to top on filter apply
    ...
    # -- Info Button Factory --
    def _setup_info_btn(self, factory, item):
        btn = Gtk.Button(icon_name="info-outline-symbolic")
        btn.add_css_class("flat")
        btn.set_tooltip_text("Show Details")
        # We need to store the item to use in callback, will bind later
        item.set_child(btn)

    def _bind_info_btn(self, factory, item):
        btn = item.get_child()
        obj = item.get_item()
        # Disconnect previous handlers to prevent duplicates if row is recycled
        if hasattr(btn, "handler_id"):
            btn.disconnect(btn.handler_id)
        btn.handler_id = btn.connect("clicked", lambda b: self.show_details(obj))

    # -- Download Button Factory --
    def _setup_dl_btn(self, factory, item):
        btn = Gtk.Button(icon_name="folder-download-symbolic")
        btn.add_css_class("flat")
        btn.set_tooltip_text("Download Magnet")
        item.set_child(btn)

    def _bind_dl_btn(self, factory, item):
        btn = item.get_child()
        obj = item.get_item()
        if hasattr(btn, "handler_id"):
            btn.disconnect(btn.handler_id)
        # On click, launch magnet
        btn.handler_id = btn.connect("clicked", lambda b: (
            Gio.AppInfo.launch_default_for_uri(obj.magnet, None),
            self.toast_overlay.add_toast(Adw.Toast.new("Opening torrent client..."))
        ))
