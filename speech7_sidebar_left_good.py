...
class PiperTTSApp(Adw.Application):
                        ...
                        if sentence_idx < len(self.sentences) and not self.sentences[sentence_idx].processed:
                            ...
                            try:
                                ...
                                if success:
                                    ...
                                    if alignment_data: # Fixed syntax error
    ...
    def on_activate(self, app):
        ...
        # --- Create Sidebar Content (Left Side) ---
...
Try placing your cursor anywhere in the text and selecting "From Cursor" mode to start reading from that position."""
        ...
        self.split_view.set_enable_hide_gesture(True) # Enable swipe to hide
        self.split_view.set_enable_show_gesture(True) # Enable swipe to show
        ...
        # Wrap sidebar content in a NavigationPage for proper styling
        sidebar_page = Adw.NavigationPage.new(sidebar_content, "Sidebar")
        self.split_view.set_sidebar(sidebar_page)

        # Main content area
        self.split_view.set_content(main_content_box)

        # Header bar for main content
        header = Adw.HeaderBar()
        # Add a button to the header to toggle sidebar visibility
        self.sidebar_button = Gtk.ToggleButton()
        self.sidebar_button.set_icon_name("sidebar-show-left-symbolic") # Standard icon for left sidebar
        self.sidebar_button.set_active(True) # Sidebar starts visible
        # Bind the button's active state to the split view's show-sidebar property
        self.sidebar_button.bind_property("active", self.split_view, "show-sidebar", 1) # 1 = G_BINDING_SYNC_CREATE
