...
class EPubViewer(Adw.ApplicationWindow):
    ...
    def _toc_on_setup(self, factory, list_item):
        # create a wrapper box (ensures ListView row always has a real box child)
        box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=0)

        row = Adw.ButtonRow.new()
        row.set_activatable(True)

        # create an inner horizontal box to host the title label (prevents "box != NULL" focus error)
        ...
        # default: no start icon (will be set in bind if item has children)
        row.set_start_icon_name(None)
        ...
        # activation handler (use activated only — Adw.ButtonRow is activatable)
        def on_activated(_row):
            ...
            if item.children.get_n_items() > 0:
                ...
                row.set_start_icon_name("down-small-symbolic" if visible else "right-small-symbolic")
            else:
                href = item.href or ""
    ...
    def _toc_on_bind(self, factory, list_item):
        ...
        lbl = getattr(list_item, "_lbl", None)
        nested = getattr(list_item, "_nested", None)
        row = getattr(list_item, "_row", None)
        if lbl is None or nested is None or row is None:
            # force setup to create widgets
            self._toc_on_setup(factory, list_item)
            lbl = list_item._lbl; nested = list_item._nested; row = list_item._row
        ...
        row.set_start_icon_name("right-small-symbolic" if has_children else None)
        lbl.set_text(item.title or "")

        if has_children and not list_item._nested_view:
            def nsetup(f, li):
                br = Adw.ButtonRow.new()
                br.set_activatable(True)
                ...
                # child activation navigates to the item
                def on_child_activated(_br):
                    it = li.get_item()
                    if not it:
                        return
                    href = it.href or ""
                ...
                br.connect("activated", on_child_activated)
                li.set_child(br)
                li._lbl = l

            def nbind(f, li):
                it = li.get_item()
                li._lbl.set_text(it.title if it else "")
