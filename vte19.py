...
class VirtualTextView(Gtk.DrawingArea):
    ...
    def __init__(self):
        ...
        # Setup drag and drop
        # --- Drag for selection ---
        # 1 — Drag selection FIRST so it gets priority
        ...
        # 2 — Click controller AFTER drag gesture and set to bubble phase
        click_controller = Gtk.GestureClick()
    ...
    def _is_position_in_selection(self, line, col):
        ...
        return True
    # --------------------------------------------------------------------
    # gedit-style drag selection
    #   • DO NOT start selection in drag-begin
    #   • Start selection only on first real movement (drag-update)
    # --------------------------------------------------------------------

    def _on_drag_begin_select(self, gesture, x, y):
        """
        Record the origin, but do NOT begin selection yet (gedit behavior).
        """
        self.in_drag = True
        self._drag_started = False

        # Save the pixel starting point
        ...
        self._drag_origin_pos = (line, col)
    ...
    def _on_drag_update_select(self, gesture, offset_x, offset_y):
        ...
        sx = self._gesture_start_x
        sy = self._gesture_start_y
        cx = sx + offset_x
        cy = sy + offset_y

        DRAG_THRESHOLD = 5

        # Begin selection only after real movement
        if not self._drag_started:
            ...
            # Start selection now
            start_line, start_col = self._drag_origin_pos
            self.has_selection = True
            self.selection_start_line = start_line
            self.selection_start_col = start_col
            self.selection_end_line = start_line
            self.selection_end_col = start_col

        # Update the end of selection
        line, col = self._get_position_from_coords(cx, cy)
        if line == -1:
            return
    ...
    def _on_drag_end_select(self, gesture, offset_x, offset_y):
        ...
        if not self._drag_started:
            # No real drag → treat as click (no selection)
            self.has_selection = False
            self.cursor_visible = True
        else:
            # Update final selection endpoint
            line, col = self._get_position_from_coords(fx, fy)
            if line != -1:
                self.selection_end_line = line
                self.selection_end_col = col
                self.cursor_line = line
    ...
    def _on_drag_prepare(self, drag_source, x, y):
        # If no selection → disable text drag
        if not self.has_selection:
            return None
