...
# Added: Library view (persistent), copies cover images into persistent dir,
# remembers last index/progress and shows grid of recent books on startup.
# Fix: library cover images are now saved as a larger scaled image and shown bigger.
...
# The persistent cover saved size (bigger so library shows large covers)
...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # header inside sidebar (library + search + menu + pin)
        header = Adw.HeaderBar()
        header.add_css_class("flat")

        # create library button (move to sidebar header)
        ...
        self.library_btn.connect("clicked", lambda *_: self.on_library_clicked())
        ...
        # hide sidebar on fresh launch (requirement #1)
        try:
        ...
        # show/hide sidebar button in content header
        ...
        # create open button and put it in content header (swapped, requirement #3)
        ...
        # show library on startup (requirement #1)
        self.show_library()
    ...
    def show_library(self):
        ...
        wrap = Adw.WrapBox()
        wrap.set_align(Gtk.Align.FILL)  # Changed from START
        ...
        # ensure sidebar hidden when showing library (requirements #1 & #2)
        try:
            self.split.set_show_sidebar(False)
        except Exception:
            pass
        ...
        for entry in list(reversed(self.library)):  # most recent first
            ...
            # author + progress + menu in one horizontal line
            ...
            left_box.append(a)
            meta_box.append(left_box)

            # percentage + menu button grouped
            ...
            right_box.append(prog_lbl)

            # small flat menu button (view-more) -> popover with Remove ebook
            ...
            def _remove_entry(_btn, p=path, coverp=cover):
                try:
                    # remove from library list
                    new_lib = [ee for ee in self.library if ee.get("path") != p]
                    self.library = new_lib
                    # delete saved cover if present and belongs to our covers dir
                    try:
            ...
            def _on_click(_gesture, _n, _x, _y, p=path, resume_idx=idx):
                if p and os.path.exists(p):
                    # restore normal reading layout and load
    ...
    def on_library_clicked(self):
        # requirement #2: when EPUB loaded and library clicked -> close EPUB, hide sidebar, show library
        try:
    ...
    def load_epub(self, path, resume=False, resume_index=None):
        try:
            # ensure reader content replaces library immediately when opening a file
            try:
                self.toolbar.set_content(self._reader_content_box)
            except Exception:
                pass
