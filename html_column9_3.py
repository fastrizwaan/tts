...
class ColumnLayoutWindow(Adw.ApplicationWindow):
    def __init__(self, **kwargs):
        ...
        # Track user sidebar state
        self._user_hid_sidebar = False
        toggle_button.connect("toggled", self._on_sidebar_toggle)
        
        # Setup window resize monitoring
        self.connect("notify::default-width", self._on_window_resized)
    
    def _setup_keyboard_navigation(self):
        """Setup keyboard event controller for navigation"""
        key_controller = Gtk.EventControllerKey()
        key_controller.connect("key-pressed", self._on_key_pressed)
        self.add_controller(key_controller)
        
        # Setup scroll controller for mouse wheel
    ...
    def _on_key_pressed(self, controller, keyval, keycode, state):
        ...
        if keyval == 65361:  # Left arrow
            self._navigate_column("prev")
            return True
        elif keyval == 65363:  # Right arrow
            self._navigate_column("next")
            return True
    ...
    def _on_scroll(self, controller, dx, dy):
        """Handle mouse scroll events for column navigation"""
        # Let the JavaScript handle scrolling naturally
        # The snap will happen after scrolling stops
        GLib.timeout_add(150, self._snap_to_current_column)
        return False
    
    def _navigate_column(self, direction):
            ...
            try {{
                if (!window.isSingleColumnMode) {{
                    ...
                    if (metrics && metrics.colCount > 1) {{
                        ...
                        const targetCol = ('{direction}' === 'next') ? 
                            Math.min(currentCol + 1, metrics.maxCol) :
                            Math.max(currentCol - 1, 0);
                        console.log('⌨️ Navigating to column ' + targetCol);
    ...
    def _on_window_resized(self, *args):
        """Handle window resize - browser will fire resize event automatically"""
        # The JavaScript resize handler will automatically maintain column position
        # We just need to trigger it after the animation completes
        GLib.timeout_add(450, self._snap_to_current_column)
        ...
        * {
        ...
        (function() {
            ...
            window.getContainerMetrics = function() {
                ...
                // Calculate total columns in content
                const totalCols = Math.max(1, Math.ceil((scrollWidth - paddingLeft - paddingRight + gap) / (colWidth + gap)));
                const maxCol = Math.max(0, totalCols - colCount);
                
                // Calculate page width (column + gap)
            ...
            window.getCurrentColumnIndex = function() {
                const metrics = getContainerMetrics();
                if (!metrics || window.isSingleColumnMode) return 0;
                
                const scrollLeft = metrics.container.scrollLeft;
                const adjustedScroll = scrollLeft - metrics.paddingLeft;
                const currentCol = Math.round(adjustedScroll / metrics.pageWidth);
                
                return Math.max(0, Math.min(currentCol, metrics.maxCol));
            };
            
            // Scroll to specific column index
            window.scrollToColumnIndex = function(colIndex, smooth = true) {
                const metrics = getContainerMetrics();
                if (!metrics || window.isSingleColumnMode) return;
                
                const targetCol = Math.max(0, Math.min(colIndex, metrics.maxCol));
                const targetScroll = metrics.paddingLeft + (targetCol * metrics.pageWidth);
            ...
            // Scroll event handler for snapping
            let scrollTimeout;
            const container = document.querySelector('.page');
            if (container) {
            ...
            window.checkAndApplyLayout = function() {
                ...
                if (wouldFitCols === 1 && !window.isSingleColumnMode) {
                    ...
                    // Restore column position after switching
                    setTimeout(() => {
                        const metrics = getContainerMetrics();
                        if (metrics && metrics.colCount > 1) {
                            const targetCol = Math.min(currentCol, metrics.maxCol);
                            console.log('🔄 Restoring to column ' + targetCol + ' (max: ' + metrics.maxCol + ')');
                    ...
                    // Maintain column position after property update
                    setTimeout(() => {
                        const metrics = getContainerMetrics();
                        if (metrics && metrics.colCount > 1) {
                            const targetCol = Math.min(currentCol, metrics.maxCol);
            ...
            setTimeout(() => {
                ...
                if (m) {
                    ...
                    // Initial snap in multi-column mode
        ...
        <div class="page">
            ...
            <p><strong>Keyboard Navigation:</strong> Use the Left/Right arrow keys or PageUp/PageDown to navigate between columns. Each press smoothly scrolls to the next or previous column.</p>
            
            <p><strong>Mouse Scrolling:</strong> Scroll with your mouse wheel or trackpad, and the view will automatically snap to the nearest column boundary after you stop scrolling.</p>
            
            <p><strong>Column Snapping:</strong> The layout intelligently snaps to column boundaries, ensuring you're always viewing complete columns rather than partial content between columns.</p>
