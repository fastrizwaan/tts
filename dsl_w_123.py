...
class DictionaryManager:
    ...
    def search(self, q, exact_only=True, max_preview=100, with_defs=False):
        ...
        Tiered, case-insensitive search:
          0. Exact (instant lookup via _exact_map)
          1. Prefix (fast startswith scan)
          2. Substring (fallback regex)
        ...
        # If user only wants exact matches, return now
        if exact_only:
            return results
        ...
        for dict_path, idx_data in self.index_cache.items():
            ...
            for e in idx_data.get("entries", []):
                ...
                if key.startswith(q_lower):
                    ...
                    seen_words.add(key)
                    if len(prefix_hits) + len(results) >= max_preview:
                        break
            if len(prefix_hits) + len(results) >= max_preview:
                break

        results.extend(sorted(prefix_hits, key=lambda x: x[0]))
        ...
        for dict_path, idx_data in self.index_cache.items():
            ...
            for e in idx_data.get("entries", []):
                ...
                if pattern.search(key):
                    ...
                    seen_words.add(key)
                    if len(substring_hits) + len(prefix_hits) + len(results) >= max_preview:
                        break
            if len(substring_hits) + len(prefix_hits) + len(results) >= max_preview:
                break

        results.extend(sorted(substring_hits, key=lambda x: x[0]))
    ...
    def _search_worker_loop(self):
        ...
        while True:
            ...
            if not self._stop_flag.is_set():
                # Immediately show exact matches
                GLib.idle_add(self._on_search_results, query, results, True)

                # Launch substring search in a background thread so UI stays responsive
                def async_subsearch():
