...
Version: 2.3 (Selection & Search Overhaul)
Date: November 2025

Changes in v2.3:
- FIXED: Mouse drag selection now works (refactored event controllers)
- FIXED: Shift+arrow selection now works (added normalization)
- FIXED: Scrollbar drag now works (removed conflicting GestureDrag)
- FIXED: Search no longer freezes (async + limits)
- FIXED: Auto-focus on launch
- ADDED: Match counter in find dialog ("Match X of Y")
- REFACTORED: Complete selection system overhaul
- REFACTORED: Async search operations

Changes in v2.2:
- Fixed scrollbar functionality
- Fixed text selection
- Fixed find next navigation
- Added font family and size settings
- Fixed typing triggering search
...
class VirtualTextBuffer(GObject.Object):
    ...
    def search(self, text, regex=False, case_sensitive=False):
        """Search for text in the buffer - optimized for large files"""
        ...
        # Limit search for very large files to prevent freezing
        max_search_lines = 10000  # Search first 10k lines max
        search_range = min(len(self.lines), max_search_lines)
        ...
        for line_num in range(search_range):
            line = self.lines[line_num]
...
class VirtualTextView(Gtk.DrawingArea):
    ...
    def _setup_event_controllers(self):
        ...
        # Click controller - for cursor positioning and gutter clicks
        ...
        # Motion controller - for drag selection
        ...
        focus.connect('enter', lambda c: self.grab_focus())
        self.add_controller(focus)
    
    def _on_button_released(self, gesture, n_press, x, y):
        """Handle button release - stop drag selection"""
    ...
    def _on_click_pressed(self, gesture, n_press, x, y):
        ...
        line_num = int((y + self.scroll_offset * self.line_height) / self.line_height)
        col = int((x - self.gutter_width + self.scroll_x) / self.char_width) if self.char_width > 0 else 0
...
class SearchDialog(Gtk.Window):
    ...
    def _on_find_next(self, widget):
        ...
        # Run search asynchronously to prevent freezing
        def do_search():
            self.text_view.buffer.search(text, regex, case_sensitive)
            total = len(self.text_view.buffer.search_matches)
            ...
            if match:
                ...
                # Update status
                current = self.text_view.buffer.current_match_index + 1
                self.status_label.set_text(f"Match {current} of {total}")
    ...
    def _on_replace_all(self, widget):
        ...
        # Run replace all asynchronously
        def do_replace_all():
            regex = self.regex_check.get_active()
            case_sensitive = self.case_check.get_active()
...
class TextEditorApp(Adw.Application):
    ...
    def on_activate(self, app):
        ...
        self.window.present()
        # Grab focus on text view after window is shown
        GLib.idle_add(self.window.text_view.grab_focus)
