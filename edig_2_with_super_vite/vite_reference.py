...
class Renderer:
    def __init__(self):
        ...
        # Exact Visual Map (from background calculator)
        self.exact_visual_map = None # array('Q')
        self.exact_total_visual = None

    def apply_exact_visual_map(self, map_array, total):
        """Apply pre-calculated visual map"""
        self.exact_visual_map = map_array
        self.exact_total_visual = total
        # Clear other caches to favor this one
        self.estimated_total_cache = None
        self.total_visual_lines_cache = total
        self.total_visual_lines_locked = True # Lock it!
    ...
    def update_colors_for_theme(self, is_dark=None):
        ...
        if is_dark:
            ...
            self.syntax_colors = {
                ...
                'brackets': hex_to_pango("#d19a66"),     # Orange
                'types': hex_to_pango("#56b6c2"),        # Cyan
                'entity': hex_to_pango("#d19a66"),       # Orange
                'bool_ops': hex_to_pango("#d19a66"),     # Orange
                'brackets': hex_to_pango("#d19a66"),     # Orange
    ...
    def get_total_visual_lines(self, cr, buf, ln_width, viewport_width):
        ...
        if getattr(self, 'total_visual_lines_locked', False) and hasattr(self, 'total_visual_lines_cache') and self.total_visual_lines_cache is not None:
             return self.total_visual_lines_cache
             
        # PRE-CALC CHECK: Use exact map
        if self.exact_total_visual is not None:
             return self.exact_total_visual
    ...
    def logical_to_visual_line(self, cr, buf, logical_line, column, ln_width, viewport_width):
        ...
        # EXACT MAP CHECK
        if self.exact_visual_map is not None:
            if logical_line < len(self.exact_visual_map) - 1:
                start_vis = self.exact_visual_map[logical_line]
                # Recalculate just this line to find column offset
                count = self.get_visual_line_count_for_logical(cr, buf, logical_line, ln_width, viewport_width)
                
                # To be precise about WHICH visual line (0 to count-1) the column falls in:
                # We effectively repeat get_visual_line_count logic but stop at column
                # This is "good enough" - often logical_to_visual just needs the start
                # or we can refine:
                if count > 1:
                    # We need to find which sub-line 'column' is in
                    wrap_points = self.get_wrap_points_for_line(cr, buf, logical_line, ln_width, viewport_width)
                    for i, (s, e) in enumerate(wrap_points):
                        if s <= column <= e:
                            return start_vis + i
                    # Fallback
                    return start_vis + count - 1
                return start_vis
    ...
    def visual_to_logical_line(self, cr, buf, visual_line, ln_width, viewport_width):
        ...
        if self.exact_visual_map is not None:
             import bisect
             # exact_visual_map is sorted, so we can binary search
             idx = bisect.bisect_right(self.exact_visual_map, visual_line) - 1
             if idx < 0: idx = 0
             if idx >= len(self.exact_visual_map) - 1: idx = len(self.exact_visual_map) - 2
             
             logical_line = idx
             start_vis = self.exact_visual_map[idx]
             rel_visual = visual_line - start_vis
             
             # Calculate wrap points to define the specific visual sub-line
             wrap_points = self.get_wrap_points_for_line(cr, buf, logical_line, ln_width, viewport_width)
             
             if 0 <= rel_visual < len(wrap_points):
                 start_col, end_col = wrap_points[rel_visual]
                 return (logical_line, rel_visual, start_col, end_col)
             
             # Fallback
             text = buf.get_line(logical_line)
             return (logical_line, 0, 0, len(text))
    ...
    def draw(self, cr, alloc, buf, scroll_line, scroll_x,
        ...
        for ln in range(scroll_line, total):
            ...
            for vis_idx in range(start_vis_idx, len(wrap_points)):
                ...
                cr.move_to(base_x, y)
                PangoCairo.show_layout(cr, layout)
                layout.set_attributes(None)  # Clear attributes for next use (fixes line number color leak)
...
class VirtualTextView(Gtk.DrawingArea):
    ...
    def __init__(self, buf):
        ...
        # NEW: Visual line tracker for word wrap
        self.tracker = VisualLineTracker()
        self.tracker.attach(buf, self.renderer)
    ...
    # -------------------------------------------------------------------------
    # Unified Scroll Operations (Phase 3)
    # -------------------------------------------------------------------------
    def scroll_to_visual(self, visual_line):
        """Scroll viewport to show visual_line at top.
        
        Unified scroll method - all scroll operations should use this.
        """
        surface = cairo.ImageSurface(cairo.FORMAT_ARGB32, 1, 1)
        cr = cairo.Context(surface)
        total = self.buf.total()
        ln_width = self.renderer.calculate_line_number_width(cr, total)
        viewport_width = self.get_width()
        
        # Update tracker viewport
        self.tracker.set_viewport(viewport_width, ln_width)
        
        # Use tracker for visual to logical conversion
        logical, offset, _, _ = self.tracker.get_logical_for_visual(visual_line, cr)
        
        self.scroll_line = logical
        self.scroll_visual_offset = offset
        self.update_scrollbar()
        self.queue_draw()

    def scroll_to_logical(self, line, col=0, center=False):
        """Scroll viewport to show logical line.
        
        Args:
            line: Logical line number
            col: Column position (for sub-line positioning in wrapped text)
            center: If True, center the line in viewport
        """
        surface = cairo.ImageSurface(cairo.FORMAT_ARGB32, 1, 1)
        cr = cairo.Context(surface)
        total = self.buf.total()
        ...
        # Get visual line for this logical position
        visual = self.tracker.get_visual_for_logical(line, col, cr)
        
        if center:
            visible_lines = self.get_height() // self.renderer.line_h
            visual = max(0, visual - visible_lines // 2)
        
        self.scroll_to_visual(visual)
    ...
    def on_vadj_changed(self, adj):
        ...
        if self.renderer.wrap_enabled:
            ...
            if self.scrollbar_dragging:
                ...
                # EXACT MAP OPTIMIZATION: Use pre-calculated map if available
                if self.renderer.exact_visual_map:
                    import bisect
                    visual_line = int(val)
                    idx = bisect.bisect_right(self.renderer.exact_visual_map, visual_line) - 1
                    if idx < 0: idx = 0
                    if idx >= len(self.renderer.exact_visual_map) - 1: idx = len(self.renderer.exact_visual_map) - 2
                    
                    self.scroll_line = idx
                    start_vis = self.renderer.exact_visual_map[idx]
                    self.scroll_visual_offset = max(0, visual_line - start_vis)
                    self.queue_draw()
                    return
    ...
    def _process_scroll_update(self):
        ...
        # EXACT MAP OPTIMIZATION: If we have the exact map, use it!
        # It's fast (O(logN)) and accurate.
        if self.renderer.exact_visual_map:
            import bisect
            # Find logical line
            idx = bisect.bisect_right(self.renderer.exact_visual_map, visual_line) - 1
            ...
            self.scroll_visual_offset = max(0, rel_visual)
            
            self.pending_scroll_value = None
            self.queue_draw()
            
            # Schedule next update if needed
            if self.pending_scroll_value is not None:
                 return True
            else:
                 self.scroll_update_pending = False
                 return False
    ...
    def on_key(self, c, keyval, keycode, state):
        ...
        if ctrl_pressed and not shift_pressed and not alt_pressed and (name == "z" or name == "Z"):
            ...
            if self.renderer.wrap_enabled:
                ...
                self.renderer.visual_line_map = []
                self.renderer.estimated_total_cache = None
                self.renderer.exact_visual_map = None
                self.renderer.exact_total_visual = None
                self.renderer.total_visual_lines_locked = False
                self.tracker.invalidate_all()
            
            self.update_scrollbar()
            self.update_scrollbar()
            self.keep_cursor_visible(center=False) # Reduced jumpiness
            self.update_im_cursor_location()
            self.update_im_cursor_location()
            self.queue_draw()
            return True
        ...
        try:
            ...
            if text:
                ...
                if self.renderer.wrap_enabled:
                    ...
                    self.renderer.estimated_total_cache = None
                    self.renderer.visual_line_map = []
                    self.renderer.visual_line_anchor = (0, 0) # Reset anchor
                    self.renderer.edits_since_cache_invalidation = 0
    ...
    def keep_cursor_visible(self, center=False):
        """Keep cursor visible by scrolling if necessary.
        
        REFACTORED: Uses VisualLineTracker for unified positioning.
        """
        self.update_matching_brackets()
        ...
        line_h = self.renderer.line_h
        visible_lines = max(1, alloc_h // line_h)
        total_lines = self.buf.total()
        ...
        if self.renderer.wrap_enabled:
            ...
            # Check if tracker has valid map
            if not self.tracker.is_valid():
                # Map unavailable - use simple line-based positioning to avoid estimation errors
                # This just ensures the cursor LINE is visible, not exact visual line
                if cl < self.scroll_line:
                    # Cursor is above - scroll up
                    self.scroll_line = cl
                    self.scroll_visual_offset = 0
                elif cl > self.scroll_line + visible_lines - 1:
                    # Cursor is below - scroll down
                    self.scroll_line = max(0, cl - visible_lines + 1)
                    self.scroll_visual_offset = 0
                # else: cursor line is already in visible range, do nothing
                if self.scroll_x != 0:
                    self.scroll_x = 0
                    self.hadj.set_value(0)
                self.queue_draw()
            ...
            # Get cursor visual position using tracker (map is valid)
            cursor_visual = self.tracker.get_visual_for_logical(cl, cc, cr)
            
            # Get current scroll visual position
            scroll_visual = self.tracker.get_visual_for_logical(self.scroll_line, 0, cr)
            scroll_visual += self.scroll_visual_offset
            
            # Check if centering requested
            if center:
                target_visual = max(0, cursor_visual - visible_lines // 2)
                self.scroll_to_visual(target_visual)
                return
            
            # Check if cursor is above visible area
            if cursor_visual < scroll_visual:
                self.scroll_to_visual(cursor_visual)
                return
            
            # Check if cursor is below visible area
            if cursor_visual >= scroll_visual + visible_lines:
                # Scroll down so cursor is at bottom of viewport
                new_top_visual = cursor_visual - visible_lines + 1
                total_visual = self.tracker.get_total_visual(cr)
                max_scroll = max(0, total_visual - visible_lines)
                new_top_visual = min(new_top_visual, max_scroll)
                self.scroll_to_visual(new_top_visual)
                return
            
            # Cursor already visible, reset horizontal scroll for wrap mode
            if self.scroll_x != 0:
                self.scroll_x = 0
                self.hadj.set_value(0)
            return
...
# -------------------------------------------------------------------------
# VISUAL LINE TRACKER - Centralized visual line management for word wrap
# -------------------------------------------------------------------------
class VisualLineTracker:
    """Manages visual line mapping for word-wrapped text.
    
    Provides:
    - Incremental updates instead of full invalidation
    - Unified API for all scroll/positioning operations
    - Lazy recalculation for performance
    """
    
    def __init__(self):
        self.map = None  # array('Q') - visual start index for each logical line
        self.total = 0   # Total visual lines
        self.dirty_start = None  # First line needing recalc (None = clean)
        self.line_count = 0  # Cached logical line count
        
        # References set by owner
        self.buf = None
        self.renderer = None
        self.viewport_width = 0
        self.ln_width = 0
    
    def attach(self, buf, renderer):
        """Attach buffer and renderer references."""
        self.buf = buf
        self.renderer = renderer
    
    def set_viewport(self, width, ln_width):
        """Update viewport dimensions. Call when window resizes."""
        if width != self.viewport_width or ln_width != self.ln_width:
            self.viewport_width = width
            self.ln_width = ln_width
            self.invalidate_all()
    
    def set_map(self, map_array, total):
        """Set pre-calculated map (from background thread)."""
        import array
        self.map = map_array
        self.total = total
        self.line_count = len(map_array) - 1 if map_array else 0
        self.dirty_start = None  # Clean state
    
    def invalidate_all(self):
        """Mark entire map as dirty. Call on major changes."""
        self.map = None
        self.total = 0
        self.dirty_start = 0
    
    def invalidate_line(self, line):
        """Mark single line as dirty. Call on character edits."""
        if self.map is None:
            return  # Already fully dirty
        if self.dirty_start is None or line < self.dirty_start:
            self.dirty_start = line
    
    def invalidate_from(self, line):
        """Mark line and all downstream as dirty. Call on line insertions/deletions."""
        self.invalidate_line(line)
    
    def is_valid(self):
        """Check if map is valid and complete."""
        return self.map is not None and self.dirty_start is None
    
    def get_total_visual(self, cr=None):
        """Get total visual line count."""
        if self.map is not None and self.dirty_start is None:
            return self.total
        ...
        if self.buf is None:
            return 0
        total_logical = self.buf.total()
        avg = getattr(self.renderer, 'avg_wraps_per_line', 1.2) if self.renderer else 1.2
        return int(total_logical * avg)
    
    def get_visual_for_logical(self, line, col=0, cr=None):
        """Get visual line index for a logical position.
        
        Returns visual line index where this logical line starts,
        plus offset for column within wrapped sub-lines.
        """
        if self.map is not None and self.dirty_start is None:
            # Fast path: use pre-calculated map
            if line < 0:
                return 0
            if line >= len(self.map) - 1:
                return self.total - 1 if self.total > 0 else 0
            
            visual = self.map[line]
            
            # Add sub-line offset based on column
            if col > 0 and self.renderer and self.buf and cr:
                wrap_points = self.renderer.get_wrap_points_for_line(
                    cr, self.buf, line, self.ln_width, self.viewport_width
                )
                for i, (start_col, end_col) in enumerate(wrap_points):
    ...
    def get_logical_for_visual(self, visual, cr=None):
        """Get logical line and offset for a visual line.
        
        Returns (logical_line, visual_offset, start_col, end_col)
        """
        if self.map is not None and self.dirty_start is None:
            # Fast path: binary search on map
            import bisect
            idx = bisect.bisect_right(self.map, visual) - 1
            if idx < 0:
                idx = 0
            if idx >= len(self.map) - 1:
                idx = len(self.map) - 2
            
            logical = idx
            start_vis = self.map[idx]
            rel_visual = visual - start_vis
            
            # Get column range for this sub-line
            if self.renderer and self.buf and cr:
                wrap_points = self.renderer.get_wrap_points_for_line(
                    cr, self.buf, logical, self.ln_width, self.viewport_width
                )
                if 0 <= rel_visual < len(wrap_points):
                    start_col, end_col = wrap_points[rel_visual]
                    return (logical, rel_visual, start_col, end_col)
            
            return (logical, rel_visual, 0, 0)
        
        # Fallback: estimation
        if self.buf is None:
            return (0, 0, 0, 0)
        avg = getattr(self.renderer, 'avg_wraps_per_line', 1.2) if self.renderer else 1.2
        logical = int(visual / avg) if avg > 0 else visual
        logical = max(0, min(logical, self.buf.total() - 1))
        return (logical, 0, 0, 0)


# -------------------------------------------------------------------------
# WORD WRAP CALCULATOR (Threaded)
# -------------------------------------------------------------------------
class WordWrapCalculator(Thread):
    def __init__(self, buffer, renderer, viewport_width, callback_progress, callback_done):
        super().__init__()
        self.buffer = buffer
        self.renderer = renderer
        ...
        # Snapshotted metrics to avoid thread safety issues
        self.total_lines = buffer.total()
        self.line_h = renderer.line_h
        
        # Create a dummy surface/context for metrics (thread-local)
        # We can't reuse the renderer's context safely
        self.surface = cairo.ImageSurface(cairo.FORMAT_ARGB32, 1, 1)
        self.cr = cairo.Context(self.surface)
        
        # Calculate context-dependent metrics once
        self.ln_width = renderer.calculate_line_number_width(self.cr, self.total_lines)
        # self.viewport_width passed in args now
        
        # Pango layout for measurement
        self.layout = PangoCairo.create_layout(self.cr)
        self.layout.set_font_description(renderer.font)
        if hasattr(renderer, 'tab_array') and renderer.tab_array:
            self.layout.set_tabs(renderer.tab_array)
        
        # Calculate safe char width for fast path
        # Use 'W' as worst case
        self.layout.set_text("W", -1)
        ink, logical = self.layout.get_pixel_extents()
        self.max_char_width = logical.width / Pango.SCALE
        self.safe_char_width = self.max_char_width * 1.1 # 10% safety margin
        
        # Available width for text
        self.available_width = max(100, self.viewport_width - self.ln_width - renderer.right_margin_width)
        
        # Safe length: if line is shorter than this char count, it definitely fits in 1 visual line
        self.safe_length = int(self.available_width / self.safe_char_width)

    def run(self):
        import array
        import time
        
        # Store start index for each logical line
        # visual_starts[i] = visual line index where logical line i starts
        # Size: total_lines + 1 (last entry is total visual lines)
        visual_starts = array.array('Q', [0] * (self.total_lines + 1))
        ...
        for i in range(self.total_lines):
            ...
            visual_starts[i] = current_visual
            
            # Retrieve line text
            # NOTE: accessing buffer from thread is usually unsafe if buffer is mutating
            # But line_indexer (mmap) is safe for reading.
            # We assume risk of race with 'edits' dict, but worst case is slightly stale data
            # which cancels anyway on buffer change.
            line_text = self.buffer.get_line(i)
            
            # Fast Path: Checks length
            # Must also check for tabs, because a single tab char '\t' can be very wide (e.g. 8 spaces)
            # but len() counts it as 1.
            if len(line_text) < self.safe_length and '\t' not in line_text:
                count = 1
            else:
                # Slow Path: Pango measurement
                self.layout.set_text(line_text, -1)
                self.layout.set_width(int(self.available_width * Pango.SCALE))
                self.layout.set_wrap(Pango.WrapMode.WORD_CHAR)
                
                # Pango tells us lines
                count = self.layout.get_line_count()
                
            current_visual += count
            
            # Report progress every 1% or 0.5s
            # Report progress every 5000 lines or 1.0s to reduce main thread contention
            if i % 5000 == 0:
                now = time.time()
                # Yield to main thread briefly to prevent GIL starvation
                time.sleep(0.001) 
                
                if now - last_progress_time > 1.0:
                    GLib.idle_add(self.callback_progress, i, self.total_lines)
                    last_progress_time = now

        # Set total
        visual_starts[self.total_lines] = current_visual
        
        if not self.cancelled:
            GLib.idle_add(self.callback_done, visual_starts, current_visual)
...
class EditorWindow(Adw.ApplicationWindow):
    ...
    def _create_editor_overlay(self, editor, add_close_button=False):
        ...
        # ---------------------------------------------------------
        # Word Wrap Progress Bar
        # ---------------------------------------------------------
        editor.wrap_progress_bar = Gtk.ProgressBar()
        editor.wrap_progress_bar.set_hexpand(True)
        editor.wrap_progress_bar.set_valign(Gtk.Align.END)
        editor.wrap_progress_bar.set_margin_bottom(20)
        editor.wrap_progress_bar.set_margin_start(50)
        editor.wrap_progress_bar.set_margin_end(50)
        editor.wrap_progress_bar.set_show_text(True)
        editor.wrap_progress_bar.set_visible(False)
        
        # Initialize calculator state
        editor.wrap_calculator = None
        editor.wrap_calculation_pending = False
        
        overlay.add_overlay(editor.wrap_progress_bar)
        overlay.add_overlay(busy_box) # Spinner also an overlay
    ...
    def apply_settings_to_editor(self, editor):
        ...
        if getattr(editor.view.renderer, 'wrap_enabled', False) != new_wrap:
            ...
            # Start background calculation if enabled
            if new_wrap:
                self.start_wrap_calculation(editor)
            else:
                # Cancel if disabled
                if getattr(editor, 'wrap_calculator', None) and editor.wrap_calculator.is_alive():
                    editor.wrap_calculator.cancel()
                if hasattr(editor, 'wrap_progress_bar'):
                    editor.wrap_progress_bar.set_visible(False)
    ...
    def on_buffer_changed(self, editor):
        ...
        if editor.view.renderer.wrap_enabled:
            # Clear wrap cache for changed lines
            cursor_line = editor.buf.cursor_line
            for ln in range(max(0, cursor_line - 2), min(editor.buf.total(), cursor_line + 3)):
                if ln in editor.view.renderer.wrap_cache:
                    del editor.view.renderer.wrap_cache[ln]
            
            # Cancel calculation if running
            if getattr(editor, 'wrap_calculator', None) and editor.wrap_calculator.is_alive():
                editor.wrap_calculator.cancel()
            
            # Invalidate tracker and renderer map
            editor.view.tracker.invalidate_line(cursor_line)
            editor.view.renderer.exact_visual_map = None
            editor.view.renderer.exact_total_visual = None
    ...
    # -------------------------------------------------------------------------
    # Word Wrap Calculation Helpers
    # -------------------------------------------------------------------------
    def start_wrap_calculation(self, editor=None):
        """Start background word wrap calculation for the given (or current) editor"""
        if not editor:
            editor = self.get_current_editor()
        
        if not editor or not hasattr(editor, 'view') or not editor.view.renderer.wrap_enabled:
            return

        # Cancel existing
        if editor.wrap_calculator and editor.wrap_calculator.is_alive():
            editor.wrap_calculator.cancel()
        
        # Only start if file is large enough to matter? 
        # User requested "perfect", so we do it for any wrapped file that might benefit.
        # But let's skip very small files where estimation is instant/identical.
        total = editor.buf.total()
        if total < 100: 
            return 

        # Show progress
        if hasattr(editor, 'wrap_progress_bar'):
            editor.wrap_progress_bar.set_visible(True)
            editor.wrap_progress_bar.set_fraction(0)
            editor.wrap_progress_bar.set_text("Calculating wraps...")
        
        # Create callbacks bound to this editor
        def cb_prog(curr, tot):
            self.on_wrap_progress(curr, tot, editor)
            
        def cb_done(starts, tot):
            self.on_wrap_done(starts, tot, editor)

        editor.wrap_calculator = WordWrapCalculator(
            editor.buf, 
            editor.view.renderer,
            editor.view.get_width(),
            cb_prog, 
            cb_done
        )
        editor.wrap_calculator.start()

    def on_wrap_progress(self, current, total, editor):
        if editor.wrap_calculator and not editor.wrap_calculator.cancelled:
            frac = current / max(1, total)
            if hasattr(editor, 'wrap_progress_bar'):
                editor.wrap_progress_bar.set_fraction(frac)
                editor.wrap_progress_bar.set_text(f"Wrapping: {int(frac*100)}%")

    def on_wrap_done(self, visual_starts, total_visual, editor):
        if hasattr(editor, 'wrap_progress_bar'):
            editor.wrap_progress_bar.set_visible(False)
        
        if not (editor.wrap_calculator and editor.wrap_calculator.cancelled):
            # Apply the calculated map to both renderer and tracker
            editor.view.renderer.apply_exact_visual_map(visual_starts, total_visual)
            editor.view.tracker.set_map(visual_starts, total_visual)
            editor.view.update_scrollbar()
            editor.view.queue_draw()

    def _restart_wrap_calculation_debounced(self, editor):
        editor.wrap_calculation_pending = False
        if editor.view.renderer.wrap_enabled:
             self.start_wrap_calculation(editor)
        return False
