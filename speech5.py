...
class PiperTTSApp(Adw.Application):
    def __init__(self, **kwargs):
        ...
        # Initialize ASR model for alignment (optional)
        print("Loading ASR model for word alignment...")
        ...
        try:
            ...
            if self.use_cli:
                ...
                with wave.open(output_path, "wb") as wav_file:
                    self.piper_voice.synthesize(text, wav_file, length_scale=1.0 / self.speech_rate)
                return True
    ...
    def on_activate(self, app):
        ...
        info_label = Gtk.Label(label=f"{tts_status} | Device: {self.device}")
    ...
    def process_current_sentence(self):
        ...
        try:
            ...
            # Process alignment based on settings
            if not self.skip_alignment:
                alignment_data = self.process_sentence_alignment(wav_file, sentence.words)
            else:
                waveform, sample_rate = torchaudio.load(wav_file)
    ...
    def process_sentence_alignment(self, wav_file, words):
        """Process alignment for a single sentence"""
       
        # Skip alignment if disabled
        if self.skip_alignment:
            print("Skipping alignment due to settings, using fallback")
            waveform, sample_rate = torchaudio.load(wav_file)
        ...
        try:
            ...
            for c in transcript:
                ...
                if token_id >= emission.size(1):
                    print(f"Token ID {token_id} out of bounds, using fallback")
                    return self.create_fallback_alignment(words, waveform, sample_rate)
               
                tokens.append(token_id)
           
            # Additional safety checks
            if len(tokens) == 0 or emission.size(0) < len(tokens):
                print(f"Invalid dimensions for alignment, using fallback")
                return self.create_fallback_alignment(words, waveform, sample_rate)
    ...
    def get_trellis(self, emission, tokens, blank_id=0):
        ...
        # Fill first row (except first element)
        ...
        try:
            ...
            while j > 0:
                ...
                if j >= len(tokens) or tokens[j] >= emission.size(1):
                    print(f"Invalid token access: j={j}")
                    return []
