...
class MainWindow(Adw.ApplicationWindow):
    def __init__(self, *args, **kwargs):
        ...
        languages = [
            ...
            ("zh", "🇨🇳 Mandarin Chinese"),
            ("es", "🇪🇸 Spanish"),
            ("fr-fr", "🇫🇷 French"),  # Using specific dialect code
            ("hi", "🇮🇳 Hindi"),
            ("it", "🇮🇹 Italian"),
    ...
    def update_voices_for_language(self, lang_code):
        ...
        lang_to_prefix = {
            ...
            "zh": "z",
            "es": "e",
            "fr-fr": "f",  # French with specific dialect code
            "hi": "h",
            "it": "i",
            "pt-br": "p"
        }
        ...
        try:
            for i, sentence in enumerate(self.app.sentences):
                ...
                samples, sample_rate = self.app.kokoro.create(
                    ...
                    voice=voice,
                    speed=speed,
                    lang=lang  # Pass the specific dialect language code
                )
                ...
                self.app.audio_queue.put(sentence_info)
    ...
    def play_audio_sequential(self):
        ...
        try:
            while not self.app.should_stop:
                try:
                    # Get next sentence info from queue (with timeout)
                    sentence_info = self.app.audio_queue.get(timeout=1.0)
