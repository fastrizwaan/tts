...
class TTSApplication(Adw.Application):
    def __init__(self):
        ...
        self.paused_highlight_state = None  # Store highlight position during pause
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def on_play_clicked(self, button):
        ...
        self.app.paused_highlight_state = None  # Reset pause state
    ...
    def on_stop_clicked(self, button):
        ...
        self.app.is_playing = False
        self.app.is_paused = False
        self.app.paused_highlight_state = None  # Clear pause state
        
        # Terminate current audio process immediately
        if self.app.current_process:
            try:
                self.app.current_process.terminate()
                try:
    ...
    def resume_playback(self):
        ...
        # Continue playback from where we left off
        # The playback thread will automatically continue from the paused state
    ...
    def play_audio_sequential(self):
        ...
        sentence_index = 0
        start_from_paused = False
        
        try:
            # Check if we're resuming from paused state
            if self.app.paused_highlight_state:
                sentence_index = self.app.paused_highlight_state.get('sentence_index', 0)
                start_from_paused = True
            
            while not self.app.should_stop:
                sentence_info = None
                
                # If resuming from pause, use the paused sentence info
                if start_from_paused and self.app.paused_highlight_state:
                    sentence_info = self.app.paused_highlight_state.get('sentence_info')
                    start_from_paused = False
                else:
                    ...
                    try:
                        ...
                        if sentence_info is None:  # End signal
                            break
                        # Update sentence index for normal playback
                ...
                completed = self.play_sentence_sync(sentence_info, sentence_index)
                
                # If playback was paused during this sentence, stop processing more sentences
                if self.app.is_paused:
                    break
    ...
    def play_sentence_sync(self, sentence_info, sentence_index):
        ...
        try:
            ...
            # Check if we're resuming from a paused state for this specific sentence
            start_word_index = 0
            if (self.app.paused_highlight_state and 
                self.app.paused_highlight_state.get('sentence_index') == sentence_index):
                # Resume from the paused word position
                start_word_index = self.app.paused_highlight_state.get('word_index', 0)
                # Clear the pause state after using it
            ...
            while word_index < len(word_timings) and not self.app.should_stop:
                ...
                if self.app.is_paused:
                    ...
                    self.app.paused_highlight_state = {
            ...
            # If paused during this sentence, don't mark as completed
            if self.app.is_paused:
                return False
