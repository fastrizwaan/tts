...
class MainWindow(Adw.ApplicationWindow):
    ...
    def search(self, q, max_preview=100, exact_only=False, with_defs=False):
        """
        Multi-tiered, whitespace-aware search.
        ...
        q_esc = re.escape(q)
        seen = set()
        results_by_tier = [[] for _ in range(6)]

        patterns = [
            re.compile(rf"^{q_esc}$", re.IGNORECASE),           # 1. exact
            re.compile(rf"^{q_esc}[ \t]", re.IGNORECASE),       # 2. starts with word + space
            re.compile(rf"[ \t]{q_esc}$", re.IGNORECASE),       # 3. ends with word
            re.compile(rf"^{q_esc}.*", re.IGNORECASE),          # 4. prefix
            re.compile(rf"[ \t]{q_esc}[ \t]", re.IGNORECASE),   # 5. middle word
            re.compile(rf".*{q_esc}.*", re.IGNORECASE),         # 6. loose substring
        ]
        ...
        for path, idx_data in self.index_cache.items():
            ...
            # evaluate all entries once per dictionary
            for e in entries:
                key = e.get("key") or e["word"].lower()
                if key in seen:
                    continue

                for tier, regex in enumerate(patterns):
                    if regex.search(key):
                        seen.add(key)
                        if with_defs:
                            defs = self._read_entry(file_path, e, enc)
                            results_by_tier[tier].append((e["word"], [(dict_name, defs, "default")]))
                        else:
                            results_by_tier[tier].append((e["word"], [(dict_name, [], "default")]))
                        break  # stop at first matching tier

        # Flatten in tier order, preserving logical priority
        results = []
        for tier_list in results_by_tier:
            if tier_list:
                results.extend(tier_list)
            if len(results) >= max_preview:
                break
