...
class DictionaryManager:
    ...
    def load_dictionary(self, path, parent_window=None, *args):
        """Load a DSL or DSL.DZ dictionary efficiently and preload its n-gram index."""
        ...
        # --- Extract dictionary display name ---
        ...
        # ------------------------------------------------------------
        # ✅ Preload n-gram index (for instant substring search)
        # ------------------------------------------------------------
        INDEX_DIR = Path(GLib.get_user_config_dir()) / "dsl-dictionary" / "indexes"
        file_stem = path.name
        ngr_candidates = [
        ...
        ngr_path = next((p for p in ngr_candidates if p.exists()), None)
        ...
        if ngr_path:
            try:
                ...
                with open(ngr_path, "r", encoding="utf-8") as f:
                    ngrams = json.load(f)
                self._ngram_cache[ngr_path] = ngrams
                elapsed = (time.time() - start_time) * 1000
                print(
                    f"[PRELOAD] {dnname}: preloaded n-gram index "
                    f"({len(ngrams)} keys) in {elapsed:.1f} ms"
                )
            except Exception as e:
                print(f"[WARN] Failed to preload n-gram index for {dnname}: {e}")
        else:
            print(f"[INFO] {dnname}: No n-gram index found; searches will fallback")
    ...
    def search(self, q, exact_only=True, max_preview=300, with_defs=False):
        """Optimized search using preloaded n-gram data (instant, memory-based)."""
        import json, re, time
        from pathlib import Path
        ...
        # Return immediately if only exact matches are requested
        if exact_only:
            return [
        ...
        # -----------------------------------------------------------------
        # Phase 2: Substring search using preloaded n-grams
        # -----------------------------------------------------------------
        INDEX_DIR = Path(GLib.get_user_config_dir()) / "dsl-dictionary" / "indexes"
        ...
        for dict_path, idx_data in self.index_cache.items():
            ...
            ngram_path = next((p for p in ngram_candidates if p.exists()), None)
            ngrams = None

            # --- Use preloaded n-grams if available ---
            if ngram_path and ngram_path in self._ngram_cache:
                ngrams = self._ngram_cache[ngram_path]
                print(f"[CACHE] {dict_name}: Using preloaded n-gram index ({len(ngrams)} keys)")
            elif ngram_path:
                try:
                    with open(ngram_path, "r", encoding="utf-8") as f:
                        ngrams = json.load(f)
                    self._ngram_cache[ngram_path] = ngrams
                    print(f"[LOAD] {dict_name}: Loaded n-gram index ({len(ngrams)} keys) on demand")
                except Exception as e:
                    print(f"[WARN] Failed to load n-gram index for {dict_name}: {e}")
            ...
            if ngrams:
                starts_key = "^" + q_lower
                if starts_key in ngrams:
                    tier_candidates[1].update(ngrams[starts_key])

                boundary_keys = [f" {q_lower} ", f" {q_lower}", f"{q_lower} "]
                for key in boundary_keys:
                    if key in ngrams:
                        tier_candidates[2].update(ngrams[key])

                contains_key = q_lower
                if contains_key in ngrams:
                    tier_candidates[3].update(ngrams[contains_key])

                total_candidates = sum(len(t) for t in tier_candidates.values())
                print(
                    f"[DEBUG] {dict_name}: '{q_lower}' → {total_candidates} n-gram candidates "
                    f"(T1={len(tier_candidates[1])}, T2={len(tier_candidates[2])}, T3={len(tier_candidates[3])})"
                )

                word_map = idx_data.get("_word_map", {})
                matched = 0
                max_per_tier = {1: 500, 2: 300, 3: 200}

                for tier in [1, 2, 3]:
                    for word in islice(tier_candidates[tier], max_per_tier[tier]):
                        wl = word.lower()
                        if wl in seen:
                            continue
                        e = word_map.get(wl)
                        ...
                        seen.add(wl)
                        matched += 1
                        candidate_hits.append((tier, e["word"], dict_name, e, idx_data))
                        if len(candidate_hits) >= max_preview * 3:
                            break
                    if len(candidate_hits) >= max_preview * 3:
                        break

                print(f"[DEBUG] {dict_name}: matched {matched} words from n-gram index (no scan)")

            else:
                # --- Fallback substring scan if no n-grams ---
                print(f"[WARN] {dict_name}: No n-gram index found — fallback substring scan")
                pattern = re.compile(re.escape(q_lower), re.IGNORECASE)
                for e in idx_data.get("entries", []):
                    key = e.get("key") or e.get("word", "").lower()
                    if pattern.search(key):
                        word = e["word"]
                        if word.lower().startswith(q_lower):
                            tier_candidates[1].add(word)
                        elif f" {q_lower} " in word.lower():
                            tier_candidates[2].add(word)
                        else:
                            tier_candidates[3].add(word)

                total_candidates = sum(len(t) for t in tier_candidates.values())
                print(f"[DEBUG] {dict_name}: '{q_lower}' → {total_candidates} via fallback scan")

            print(f"[TIMING] {dict_name}: search completed in {(time.time() - start_t)*1000:.1f}ms")
