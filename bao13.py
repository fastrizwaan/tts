...
class Win(Adw.ApplicationWindow):
    ...
    def apply_column_layout(self, selected_column_index, restore_position=False):
        ...
        // Restore viewport position if anchor data is provided
        const restoreAnchorJson = '{anchor_to_restore if anchor_to_restore else ''}';
        if (restoreAnchorJson) {{
            setTimeout(function() {{
                try {{
                    const anchorData = JSON.parse(restoreAnchorJson);
