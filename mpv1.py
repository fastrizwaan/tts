...
# --- Set C locale for libmpv ---
# This must be done BEFORE creating the mpv handle
...
libmpv.mpv_render_context_create.argtypes = [ctypes.POINTER(c_void_p), c_void_p, c_void_p]
libmpv.mpv_render_context_create.restype = c_int

libmpv.mpv_render_context_render.argtypes = [c_void_p, c_void_p]
libmpv_render_context_render.restype = None
...
# --- Define constants (values should match libmpv headers) ---
MPV_RENDER_PARAM_API_TYPE = 1
MPV_RENDER_PARAM_OPENGL_FBO = 257
MPV_RENDER_API_TYPE_OPENGL = b"opengl"
MPV_RENDER_PARAM_INVALID = 0

# --- Define the MpvRenderParam struct correctly ---
...
class VideoPlayerWindow(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # --- Initialize libmpv handle and context ---
    ...
    def on_gl_area_realize(self, area):
        ...
        params_array = (MpvRenderParam * 2)() # 2 elements: api_type, null_terminator
        params_array[0] = api_type_param
        params_array[1] = null_param

        ctx_ptr = ctypes.POINTER(c_void_p)()
        ...
        if res < 0:
            print(f"Failed to create mpv render context: {res}")
            libmpv.mpv_destroy(self.mpv_handle)
            self.mpv_handle = None
            self.status_label.set_text(f"Error: Could not create render context ({res}).")
            return
    ...
    def on_gl_area_render(self, area, context):
        if not self.initialized or not self.mpv_render_ctx:
            ...
            # Clear the GLArea background (optional, depends on desired default)
            # You might use GDK's GL functions or PyOpenGL glClear here if needed
            return False
        ...
        # --- Prepare render params with the current FBO ID ---
        ...
        # Null-terminate the render-specific params list
        null_param = MpvRenderParam()
        null_param.type = MPV_RENDER_PARAM_INVALID
        null_param.data = None
    ...
    def on_open_clicked(self, button):
        # Example: Load a file using mpv command
        # This requires defining mpv_command_string or mpv_command_array via ctypes
        # which is complex. For now, just print.
        print("Open clicked - would load file here using libmpv command API.")
        # Example command: mpv_command_string(self.mpv_handle, b"loadfile /path/to/video.mp4")
        # You would need to define mpv_command_string first:
        # libmpv.mpv_command_string.argtypes = [c_void_p, c_char_p]
        # libmpv.mpv_command_string.restype = c_int
        # libmpv.mpv_command_string(self.mpv_handle, b"loadfile /path/to/video.mp4")
