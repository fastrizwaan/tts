...
class SearchManager:
    ...
    def search(self, query, providers_config):
        ...
        if TORRFETCH_AVAILABLE and providers_config.get("torrfetch", True):
...
class PreferencesWindow(Adw.PreferencesWindow):
    def __init__(self, app, settings):
        ...
        if TORRFETCH_AVAILABLE:
            self._add_switch(group_prov, "Torrfetch (Aggregator)", "torrfetch")
...
class TorrentSearchApp(Adw.Application):
    ...
    def load_settings(self):
        defaults = {
            "last_search": "", 
            "adult_content": False,
            "providers": {"yts": True, "tpb": True, "torrfetch": True}
        }
        try:
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def show_details(self, item):
        ...
        content_box.set_valign(Gtk.Align.CENTER) # Correct way to center vertical alignment
