...
class DictionaryIndexer:
    ...
    def build_index(self, on_progress=None):
        ...
        with open_func(self.path, "rb") as fb:
            ...
            # helper for safe decode
            def decode_bytes(b):
                if encoding in ("utf-16-le", "utf-16-be"):
                    # Ensure even length (UTF-16 chars are 2 bytes)
                    if len(b) % 2:
                        b = b[:-1]
                try:
                ...
                while True:
                    ...
                    if re.match(r"^[^\s#-]", line):
                        ...
                        if word and word not in offsets_seen:
                            entries.append({"word": word, "offset": pos - len(line_bytes)})
...
class DictionaryManager:
    ...
    def load_dictionary(self, path, parent_window=None, *args):
        ...
        Shows progress dialog if a Gtk.Window or Adw.ApplicationWindow is provided.
        """
        import time
        ...
        # --- Check for existing index reuse ---
        rebuild_index = True
        idx_data = None
        if idx_path.exists():
            try:
        ...
        if rebuild_index:
            print(f"[INFO] Building index for {path.name} ...")

            # Safely detect if parent_window is a GTK window
            transient_target = parent_window if isinstance(parent_window, Gtk.Window) else None
            ...
            print(
                ...
                f"with {len(idx_data['entries'])} headwords in {elapsed:.2f}s."
            )

        # --- Cache index for searches ---
        ...
        if not hasattr(self, "loaded_dicts"):
            self.loaded_dicts = {}
    ...
    def search(self, q):
        """
        Fast substring search using on-disk JSON indexes.
        Returns [(word, [(dict_name, defs, color)])].
        """
        q = q.strip().lower()
        if not q:
            return []
        ...
        results = {}
        index_cache = getattr(self, "index_cache", {})
        loaded_dicts = getattr(self, "loaded_dicts", {})

        for path, idx_data in index_cache.items():
            entries = idx_data.get("entries", [])
            encoding = idx_data.get("encoding", "utf-8")
            file_path = Path(idx_data["file"])

            # Friendly dictionary name
            dict_name = None
            for ld in loaded_dicts.values():
                if Path(ld["path"]) == file_path:
                    dict_name = ld["name"]
                    break
            if not dict_name:
            ...
            # --- Substring search ---
            matches = [e for e in entries if q in e["word"].lower()]
            if not matches:
                continue

            for e in matches[:100]:  # limit for UI responsiveness
                defs = self._read_entry(file_path, e, encoding)
                word = e["word"]
                results.setdefault(word, []).append((dict_name, defs, "default"))
        ...
        sorted_res = sorted(results.items(), key=order_key)
        return sorted_res
    ...
    def _read_entry(self, path, entry, encoding="utf-8"):
        """
        Reads one DSL entry by byte offset from file (supports .dsl and .dsl.dz).
        Returns list of definition lines.
        """
        offset = entry["offset"]
        lines = []
        path = Path(path)
        ...
        try:
            ...
            for line in text.splitlines():
                if not line.strip():
                    continue
                # Stop when we hit the next headword (non-indented)
                if not line[0].isspace() and line != entry["word"]:
                    break
                lines.append(line.rstrip())

        except OSError as e:
            print(f"[WARN] Cannot read from {path.name}: {e}")
        except Exception as e:
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def on_search(self, entry):
        """Debounced instant-search handler."""
        ...
        # Empty → show placeholder immediately
        if not q:
            self.show_placeholder("Enter a word")
            return

        # Delay 150ms after last keystroke before running search
        def delayed_search():
            self.perform_search(q)
            self._search_timeout_id = None
            return GLib.SOURCE_REMOVE

        self._search_timeout_id = GLib.timeout_add(150, delayed_search)
