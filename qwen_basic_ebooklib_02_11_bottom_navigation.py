...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        book_box.set_margin_top(0)
        book_box.set_margin_bottom(0)
        book_box.set_margin_start(0)
        book_box.set_margin_end(0)
        ...
        md_v = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=0)
        md_v.set_valign(Gtk.Align.START)
        md_v.set_halign(Gtk.Align.START)
    ...
    def _on_pin_toggled(self, btn):
        ...
        try:
            ...
            # overlay True when NOT pinned; False when pinned
            if hasattr(self.split, "set_overlay"):
                self.split.set_overlay(not pinned)
            # ensure desired visibility: pinned => shown & push content; unpinned => hide immediately and overlay
            if pinned:
                self.split.set_show_sidebar(True)
            else:
                # set overlay and hide immediately
                try:
    ...
    def _on_window_blur(self):
        # hide immediately when unpinned and window loses focus
        try:
    ...
    def _on_sidebar_toggle(self, btn):
        ...
        try:
            # if currently unpinned (overlay mode), toggling should show overlay
            if getattr(self, "pin_btn", None) and not self.pin_btn.get_active():
                # ensure overlay is enabled
                if hasattr(self.split, "set_overlay"):
                    self.split.set_overlay(True)
