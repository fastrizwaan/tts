...
class ColumnLayoutWindow(Adw.ApplicationWindow):
    ...
    def on_margin_changed(self, spin_row, margin_type):
        ...
        if margin_type == "right":
            # For right margin, update the empty div width
            js_code = f"""
                var rightMargin = document.querySelector('.right-margin');
                if (rightMargin) {{
                    rightMargin.style.width = '{value}px';
                    console.log('Updated right margin to {value}px');
                }}
            """
        else:
            # For other margins, update the page element
            js_code = f"""
                var page = document.querySelector('.page');
                if (page) {{
                    page.style.margin{margin_type.capitalize()} = '{value}px';
    ...
    def load_html_content(self):
        ...
        .page {
            ...
            /* Margins - only left and vertical, NO right */
            ...
            /* IMPORTANT: Make it expand to fit all columns */
            width: max-content;
            flex-shrink: 0;
        ...
        /* Empty div inside .page to create right margin space */
        .right-margin {
            display: block;
            width: var(--right-margin, 250px);
            min-height: 100%;
            flex-shrink: 0;
            background: rgba(255, 0, 0, 0.2); /* Temporary: make it visible */
            break-inside: avoid;
        }
        ...
        window.addEventListener('DOMContentLoaded', function() {
            ...
            console.log("Page scroll width:", page.scrollWidth + "px");
            
            if (rightMargin) {
                var rmStyle = window.getComputedStyle(rightMargin);
                console.log("Right margin div found!");
                console.log("  - width:", rmStyle.width);
                console.log("  - height:", rmStyle.height);
                console.log("  - offsetWidth:", rightMargin.offsetWidth + "px");
                console.log("  - offsetHeight:", rightMargin.offsetHeight + "px");
            } else {
                console.log("Right margin div NOT found!");
            }
        });
        </script>
        </head>
        ...
        <div class="scroll-container">
            <div class="content-wrapper" style="
                ...
                <div class="page">
                    ...
                    <!-- Empty div inside columns to create right margin space -->
