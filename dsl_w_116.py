...
class DictionaryManager:
    ...
    # ------------------------------------------------------------
    def search(self, q, exact_only=False, max_preview=100, with_defs=False):
        r"""
        Tiered, case-insensitive search grouped by word:
          - Each lemma shown once, with all dictionaries under it.
          - Words sorted alphabetically inside each tier.
          - Tier order (stops early if results found):
            1. ^WORD$           exact
            2. ^WORD[a-zA-Z]    compounds
            3. ^WORD\s          phrases starting with word
            4. \sWORD\s         word in middle
            5. \sWORD$          ends with word
            6. .*WORD.*         loose substring
        """
        q = q.strip()
        if not q:
            return []
        ...
        results = []
        seen_words = set()
        tier_limit = len(patterns)

        for tier_idx in range(tier_limit):
            pattern = patterns[tier_idx]
            word_groups = {}

            # Collect matches per word, across all dictionaries
            for dict_path, idx_data in self.index_cache.items():
                ...
                for e in entries:
                    ...
                    if pattern.search(key):
                        word_groups.setdefault(key, []).append((dict_name, e, fpath, enc))

            # Now merge grouped words, maintaining alphabetical order
            for key in sorted(word_groups.keys()):
                seen_words.add(key)
                group_entries = []
                for dict_name, e, fpath, enc in word_groups[key]:
                    defs = self._read_entry(fpath, e, enc) if with_defs else []
                    group_entries.append((dict_name, defs, "default"))
                results.append((word_groups[key][0][1]["word"], group_entries))

            # Stop early when enough results are collected
            if len(results) >= max_preview:
                break

            # If we already got results, skip loose substring tier
            if tier_idx == 4 and results:
                break

        # Return grouped results (word + list of dicts)
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def _start_search_thread(self, query):
        """Dual-phase threaded search (exact + substring) with UI-safe updates."""
        query = query.strip()
        if not query:
            return
        ...
        self._current_results = set()
        self._last_html = None

        def worker_exact():
            """Thread 1: exact matches only (quick)."""
            results = self.dict_manager.search(query, exact_only=True, with_defs=True)
            if self._stop_flag.is_set() or query.lower() != self._active_query:
                return
            self._current_results = {word for word, _ in results}
            GLib.idle_add(self._on_search_results, query, results, True)

            # Start phase 2 automatically after phase 1 finishes
            worker_substring()

        def worker_substring():
            """Thread 2: substring matches (slow, background)."""
            results = self.dict_manager.search(query, exact_only=False, with_defs=True)
            if self._stop_flag.is_set() or query.lower() != self._active_query:
                return
            new_results = [r for r in results if r[0] not in self._current_results]
            GLib.idle_add(self._on_search_results, query, new_results, False)
            GLib.idle_add(self.spinner.stop)

        # Run phase 1 (exact) on background thread
        threading.Thread(target=worker_exact, daemon=True).start()
