...
pg_bg_dark = invert_color(pg_bg)
sidebar_bg_dark = invert_color(pg_bg)
txt_fg_dark = invert_color(txt_fg)
...
def _update_gtk_dark_provider(settings, pspec=None):
    global pg_bg, txt_fg  # so _wrap_html() and theme JS see updates

    try:
        display = Gdk.Display.get_default()
        prefer_dark = settings.get_property("gtk-application-prefer-dark-theme")

        # Remove old providers to prevent accumulation
        for provider in (
            _css_light_provider, _css_dark_provider,
            _hover_light_provider, _hover_dark_provider,
            _dark_provider,
        ):
            try:
        ...
        if app:
            ...
            if hasattr(win, "update_webview_theme"):
                win.update_webview_theme()

    except Exception as e:
        print("Theme provider update failed:", e)


# Connect the signal once, then trigger initial state
try:
    ...
    def update_webview_theme(self):
        ...
        def _on_js_finished(webview, result, user_data=None):
            try:
                ...
                print("✅ WebView theme update:", value.unpack() if value else None)
            except Exception as e:
