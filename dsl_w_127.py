...
class DictionaryManager:
    ...
    def search(self, q, exact_only=True, max_preview=100, with_defs=False):
        """
        Tiered, linguistically ranked search:
          0. Exact
          1. Suffix match (ends with query)
          2. Compound/prefix (query at start)
          3. Phrase (query as a word in a multiword expression)
          4. Embedded/internal (query inside another word)
        """
        q = q.strip()
        if not q:
            return []
        ...
        # --- Tier 0: Exact match (instant O(1)) ---
        for dict_path, idx_data in self.index_cache.items():
            ...
            if entry:
                ...
                defs = self._read_entry(fpath, entry, enc) if with_defs else []
                results.append((entry["word"], [(dict_name, defs, "default")]))
                seen.add(q_lower)
        ...
        re_suffix   = re.compile(rf"{re.escape(q_lower)}$", re.IGNORECASE)            # ends with query
        re_prefix   = re.compile(rf"^{re.escape(q_lower)}[a-zA-Z]", re.IGNORECASE)   # starts with query
        re_phrase   = re.compile(rf"\b{re.escape(q_lower)}\b", re.IGNORECASE)        # isolated word (phrase)
        re_embedded = re.compile(rf"[a-zA-Z]{re.escape(q_lower)}[a-zA-Z]", re.IGNORECASE)  # embedded in word

        suffix_hits, prefix_hits, phrase_hits, embedded_hits = [], [], [], []
        ...
        for dict_path, idx_data in self.index_cache.items():
            ...
            for e in idx_data.get("entries", []):
                ...
                # --- Tier 1: ends with query (fresh air, open air) ---
                if re_suffix.search(key):
                    defs = self._read_entry(fpath, e, enc) if with_defs else []
                    suffix_hits.append((e["word"], [(dict_name, defs, "default")]))
                    seen.add(key)
                    continue

                # --- Tier 2: starts with query (airplane, airbag) ---
                if re_prefix.search(key):
                    defs = self._read_entry(fpath, e, enc) if with_defs else []
                    prefix_hits.append((e["word"], [(dict_name, defs, "default")]))
                    seen.add(key)
                    continue

                # --- Tier 3: phrase-level (multiword, contains isolated ' air ') ---
                if " " in key and re_phrase.search(key):
                    defs = self._read_entry(fpath, e, enc) if with_defs else []
                    phrase_hits.append((e["word"], [(dict_name, defs, "default")]))
                    seen.add(key)
                    continue

                # --- Tier 4: embedded (affair, repair, chair) ---
                if re_embedded.search(key):
                    defs = self._read_entry(fpath, e, enc) if with_defs else []
                    embedded_hits.append((e["word"], [(dict_name, defs, "default")]))
                    seen.add(key)

                if len(results) + len(suffix_hits) + len(prefix_hits) + len(phrase_hits) + len(embedded_hits) >= max_preview:
                    break

        # Sort tiers alphabetically
        suffix_hits.sort(key=lambda x: x[0])
        prefix_hits.sort(key=lambda x: x[0])
        phrase_hits.sort(key=lambda x: x[0])
        embedded_hits.sort(key=lambda x: x[0])

        # Merge tiers in linguistic order
        results.extend(suffix_hits)
        results.extend(prefix_hits)
        results.extend(phrase_hits)
        results.extend(embedded_hits)
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def _search_worker_loop(self):
        while True:
            ...
            # Phase 2: pattern matches in background
            def async_subsearch():
                results2 = self.dict_manager.search(query, exact_only=False, with_defs=True)
                if not self._stop_flag.is_set() and search_id == self._search_id:
                    GLib.idle_add(self._on_search_results, query, results2, False)
    ...
    def on_search(self, entry):
        ...
        self._search_timeout_id = GLib.timeout_add(300, delayed_start)
    ...
    def _on_search_results(self, query, results, replace=True):
        ...
        if replace:
            # Phase 1: exact results — load fresh page
            html = self.build_html(results)
            self.webview.load_html(html, "file:///")
            self._last_html = results
            return

        # Phase 2: append new results — no reload
