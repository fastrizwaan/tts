...
class MainWindow(Adw.ApplicationWindow):
    ...
    def build_html(self, results):
        ...
        for word, dict_data in results[:100]:
            for dname, defs, color in dict_data:
                ...
                in_list = False
                for d in defs:
                    rendered = self.render_dsl_text(d)
        ...
        ol, ul { margin: 0.5em 0 0.5em 1.3em; line-height: 1.4; }
        li { margin-bottom: 0.3em; }
        div:not(.entry):not(.header) { margin: 0.2em 0; line-height: 1.4; }
    ...
    def render_dsl_text(self, raw):
        t = raw
        # Only wrap lines starting with "[0-9]. " in <li> tags
