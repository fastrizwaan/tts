...
class DictionaryManager:
    ...
    def search(self, q, exact_only=True, max_preview=100, with_defs=False):
        ...
        Fast tiered search:
          0. Exact (O(1))
          1. Prefix
          2. Phrase / space-bounded
          3. Proximity (near start)
          4. Substring
        Now optimized to delay definition loading.
        """
        q = q.strip()
        if not q:
            return []
        ...
        q_lower = q.lower()
        results, seen = [], set()
        candidate_hits = []  # collect first, fetch defs later
        ...
        if exact_only:
            # Load defs for exact match only
        ...
        # --- Precompile for substring matching ---
