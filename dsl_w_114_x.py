        ...
        with open_func(self.path, "rb") as fb:
            ...
            while True:
                ...
                while True:
                    ...
                    line = decode_bytes(line_bytes).rstrip("\r\n\ufeff\u200b")

                    pos += len(line_bytes)
                    if not line or line.lstrip().startswith("#"):
                        continue

                    # Headword line detection
                    if re.match(r"^[^\s#-]", line):
                        word = line.strip()
                        if word and word not in offsets_seen:
                            # --- New: precomputed lowercase key for case-insensitive search ---
                            key = word.lower()
                            entries.append({
                                "word": word,
                                "key": key,
...
class DictionaryManager:
    ...
    # ------------------------------------------------------------
    def search(self, q, exact_only=False, max_preview=50, with_defs=False):
        """Case-insensitive exact or substring search with optional definitions."""
        q = q.strip().lower()
        if not q:
            return []

        results = []
        for path, idx_data in self.index_cache.items():
            entries = idx_data.get("entries", [])
            dict_name = Path(idx_data["file"]).stem

            # Pretty dictionary name
            ...
            for e in entries:
                ...
                if exact_only and key == q:
                    if with_defs:
                        enc = idx_data.get("encoding", "utf-8")
                        file_path = Path(idx_data["file"])
                        defs = self._read_entry(file_path, e, enc)
                    else:
                        defs = []
                    results.append((e["word"], [(dict_name, defs, "default")]))
                    continue

                # Substring pass (skip exact matches)
                if not exact_only and q in key and key != q:
                    if with_defs:
                        enc = idx_data.get("encoding", "utf-8")
                        file_path = Path(idx_data["file"])
                        defs = self._read_entry(file_path, e, enc)
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def search(self, q, max_preview=100, exact_only=False, with_defs=False):
        r"""
        Sequential tier search - stops when results found.
        Tier ordering:
          1. ^WORD$              - exact (air)
          2. ^WORD[a-zA-Z]       - compound words (airborne, airplane)
          3. ^WORD\s             - phrases (air bag, air force)
          4. \sWORD\s            - middle (bag of air)
          5. \sWORD$             - ends (change of air)
          6. .*WORD.*            - substring (fair, chair - only if exact_only=False)
        """
        q = q.strip()
        if not q:
            return []
        ...
        tiers = [
            ...
            re.compile(rf"^{q_esc}\s", re.IGNORECASE),             # 2. phrase
            re.compile(rf"\s{q_esc}\s", re.IGNORECASE),            # 3. middle
            re.compile(rf"\s{q_esc}$", re.IGNORECASE),             # 4. ends
            re.compile(rf".*{q_esc}.*", re.IGNORECASE),            # 5. substring
        ]
        
        # If exact_only, only check first 3 tiers
        max_tier = 2 if exact_only else 5
        ...
        for tier_idx in range(max_tier + 1):
            ...
            for path, idx_data in self.index_cache.items():
                ...
                # Check entries for this tier only
                for e in entries:
                    key = e.get("key") or e["word"].lower()
                    if key in seen:
                        continue
                    ...
                    if pattern.search(key):
                        ...
                        if with_defs:
                            defs = self._read_entry(file_path, e, enc)
                            tier_results.append((e["word"], [(dict_name, defs, "default")]))
                        else:
                            tier_results.append((e["word"], [(dict_name, [], "default")]))
            ...
            # Don't check tier 5 (substring) if we already have results
            if tier_idx == 4 and results:
                break
        
        return results[:max_preview]
    ...
    def _start_search_thread(self, query):
        """Dual-phase: instant lemma list + background definitions."""
        # Cancel any older running thread
        if hasattr(self, "_search_thread") and self._search_thread.is_alive():
            self._abort_search = True

        # --- assign new active query token ---
        self._active_query = query.strip().lower()

        # Phase 1: exact matches only (instant)
        preview = self.dict_manager.search(query, exact_only=True, with_defs=True)
        self._current_results = {word for word, _ in preview}
        GLib.idle_add(self._on_search_results, query, preview, True)

        # Phase 2: substring matches (background)
        self._abort_search = False
        self.spinner.start()

        def worker(local_query=self._active_query):
            import time
            time.sleep(0.25)  # short delay to avoid race during rapid typing
            all_results = self.dict_manager.search(local_query, exact_only=False, with_defs=True)
            if self._abort_search or local_query != self._active_query:
                return  # discard stale thread
            new_results = [r for r in all_results if r[0] not in self._current_results]
            GLib.idle_add(self._on_search_results, query, new_results, False)
