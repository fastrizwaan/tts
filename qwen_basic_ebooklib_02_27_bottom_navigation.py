...
class EPubViewer(Adw.ApplicationWindow):
    ...
    def _find_cover_via_opf(self, extracted_paths, image_names, image_basenames):
        """
        Debug-friendly OPF-first cover detection:
        - search all *.opf under temp_dir (recursive)
        - print which OPF files checked and which lines contain 'cover'
        - try (in order): <meta name="cover" content="ID"> -> manifest item href,
                          manifest item with properties="cover-image",
                          manifest item href containing 'cover' + image ext,
                          first manifest image (fallback)
        - resolve href relative to the OPF file and match against extracted_paths / image maps.
        Returns (absolute_path_on_disk_or_None, ebooklib_image_item_or_None)
        """
