...
class DSLRenderer:
    """GoldenDict-style HTML renderer for DSL lines (stateful stack parser)."""
    ...
    TAG_MAP = {
        ...
        "ref":      ("<a class='dict-link'>", "</a>"),
        "s":        ("<span class='media'>", "</span>"),
        "lang":     ("<span class='lang'>", "</span>"),
        "star":     ("<span class='full-translation'>", "</span>"),
    ...
    def _open_tag(self, tag: str, param: str = None) -> str:
        if tag.startswith("m"):
            ...
            return f"<span class='lang' data-lang='{param or ''}'>"
        elif tag == "<<":
    ...
    def _render_line(self, line: str, headword: str) -> str:
        ...
        def push(tag):
            ...
            stack.append(name)
            html.append(self._open_tag(name, param))

        def pop(tag):
            if not stack:
                return
            top = stack.pop()
            html.append(self._close_tag(top))

        def flush_text(txt):
            if not txt:
                return

            # Convert literal POS-like tags <n>, <m>, <adj>, etc. to visible Unicode brackets
            # These are *not* HTML tags, they must be preserved visually.
            def pos_marker(match):
                content = match.group(1)
                return f"<span class='pos-tag'>⟨{GLib.markup_escape_text(content)}⟩</span>"
            txt = re.sub(r"<([a-zA-Z0-9\-]+)>", pos_marker, txt)
    ...
    def render_entry(self, headword, defs):
        parts = [self._render_line(line, headword) for line in defs]
        return "\n".join(parts)
