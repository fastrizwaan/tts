 ...
 # Shim for legacy external access
    ...
    def on_vadj_changed(self, adj):
        ...
        if self.mapper.enabled:
            ...
            ratio = val / float(total_vis)
            target_log = int(ratio * self.buf.total())
            
            self.scroll_line = max(0, min(target_log, self.buf.total() - 1))
            self.scroll_visual_offset = 0 
            self.queue_draw()
        else:
            # Logical line scrolling
            self.scroll_line = int(val)
            self.queue_draw()
    ...
    def update_scrollbar(self):
        ...
        try:
            ...
            if self.mapper.enabled:
                ...
                self.vadj.set_upper(total_vis)
                self.vadj.set_page_size(visible_rows)
                self.vadj.set_step_increment(1)
                self.vadj.set_page_increment(visible_rows)
                
                # Estimate current visual position
                curr_ratio = self.scroll_line / max(1, total_lines)
                curr_val = curr_ratio * total_vis
