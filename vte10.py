...
class FindReplaceBar(Gtk.Box):
    ...
    def on_replace_one(self, *a):
        ...
        # Update edit buffer if this line is currently being edited
        if self.textview.editing and self.textview.edit_line == line:
            self.textview.edit_text = new_text
            # Adjust cursor position if needed
            len_diff = len(new_text) - len(text)
            if self.textview.edit_cursor_pos >= end:
                self.textview.edit_cursor_pos += len_diff
            elif self.textview.edit_cursor_pos > start:
                self.textview.edit_cursor_pos = start + (end - start) + len_diff
            ...
            if has_newline:
                ...
                if new_text != full_text:
                    ...
                    def on_complete():
                        ...
                        # Update edit buffer if needed
                        if self.textview.editing:
                            if self.textview.edit_line < len(new_lines):
                                self.textview.edit_text = new_lines[self.textview.edit_line]
                                self.textview.edit_cursor_pos = min(
                                    self.textview.edit_cursor_pos,
                                    len(self.textview.edit_text)
                                )
                            else:
                                # Edited line no longer exists
                                self.textview.editing = False
                ...
                def on_complete():
                    if modified_lines:
                        ...
                        # Update edit buffer if the current line was modified
                        if self.textview.editing and self.textview.edit_line in modified_lines:
                            self.textview.edit_text = self.buffer.lines[self.textview.edit_line]
                            # Keep cursor at end or adjust as needed
                            self.textview.edit_cursor_pos = min(
                                self.textview.edit_cursor_pos, 
                                len(self.textview.edit_text)
                            )
