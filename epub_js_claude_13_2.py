...
class EpubViewerWindow(Adw.ApplicationWindow):
    def __init__(self, application, file_path=None):
        ...
        self.setup_column_selector(header_bar)
        self.setup_margin_controls(header_bar)
    ...
    def setup_column_selector(self, header_bar):
        ...
        # Create column selector box
        column_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        header_bar.pack_end(column_box)
        
        # Label
        column_label = Gtk.Label(label="Columns:")
        column_box.append(column_label)
        
        # Dropdown for column selection
        self.column_dropdown = Gtk.DropDown()
        
        # Create string list with column options
        column_options = Gtk.StringList()
        
        # Add column count options (1-10)
        for i in range(1, 11):
            column_options.append(f"{i} Column{'s' if i > 1 else ''}")
        
        # Add pixel-based options (100px - 500px)
        for px in range(100, 501, 50):
            column_options.append(f"{px}px width")
        
        self.column_dropdown.set_model(column_options)
        self.column_dropdown.set_selected(0)  # Default: 1 column
        self.column_dropdown.connect("notify::selected", self.on_column_changed)
        
        column_box.append(self.column_dropdown)
        
        # Store current column setting
        self.current_column_count = 1
        self.current_column_width = None

    def on_column_changed(self, dropdown, _):
        """Handle column selection change"""
        selected_idx = dropdown.get_selected()
        
        # Determine column setting from index
        if selected_idx < 10:  # Column count (1-10)
            self.current_column_count = selected_idx + 1
            self.current_column_width = None
            print(f"[Column] Setting to {self.current_column_count} columns")
        else:  # Pixel-based width
            px_index = selected_idx - 10
            self.current_column_width = 100 + (px_index * 50)
            self.current_column_count = None
            print(f"[Column] Setting to {self.current_column_width}px width")
    ...
    def apply_columns_after_load(self):
        ...
        (function() {{
            try {{
                ...
                if (columnCount !== null) {{
                    // Column count mode - columns flow side by side horizontally
                    style.textContent = `
                        html {{
                    ...
                    // Fixed pixel width mode - columns flow side by side
                    style.textContent = `
                        html {{
                            overflow-x: auto !important;
                            overflow-y: hidden !important;
    ...
    def setup_margin_controls(self, header_bar):
        """Setup margin controls for the epub viewer"""
        # Create margins box
        margins_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        header_bar.pack_end(margins_box)
        
        # Label
        margins_label = Gtk.Label(label="Margins:")
        margins_box.append(margins_label)
        
        # Create adjustment and spin button for each margin
        # Top margin
        self.margin_top_adj = Gtk.Adjustment(value=0, lower=0, upper=200, step_increment=5, page_increment=10)
        self.margin_top_spin = Gtk.SpinButton(adjustment=self.margin_top_adj, climb_rate=1, digits=0)
        self.margin_top_spin.set_tooltip_text("Top margin (px)")
        self.margin_top_spin.set_width_chars(4)
        self.margin_top_spin.connect("value-changed", self.on_margin_changed)
        
        top_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=2)
        top_box.append(Gtk.Label(label="T:"))
        top_box.append(self.margin_top_spin)
        margins_box.append(top_box)
        
        # Bottom margin
        self.margin_bottom_adj = Gtk.Adjustment(value=0, lower=0, upper=200, step_increment=5, page_increment=10)
        self.margin_bottom_spin = Gtk.SpinButton(adjustment=self.margin_bottom_adj, climb_rate=1, digits=0)
        self.margin_bottom_spin.set_tooltip_text("Bottom margin (px)")
        self.margin_bottom_spin.set_width_chars(4)
        self.margin_bottom_spin.connect("value-changed", self.on_margin_changed)
        
        bottom_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=2)
        bottom_box.append(Gtk.Label(label="B:"))
        bottom_box.append(self.margin_bottom_spin)
        margins_box.append(bottom_box)
        
        # Left margin
        self.margin_left_adj = Gtk.Adjustment(value=0, lower=0, upper=500, step_increment=5, page_increment=10)
        self.margin_left_spin = Gtk.SpinButton(adjustment=self.margin_left_adj, climb_rate=1, digits=0)
        self.margin_left_spin.set_tooltip_text("Left margin (px)")
        self.margin_left_spin.set_width_chars(4)
        self.margin_left_spin.connect("value-changed", self.on_margin_changed)
        
        left_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=2)
        left_box.append(Gtk.Label(label="L:"))
        left_box.append(self.margin_left_spin)
        margins_box.append(left_box)
        
        # Right margin
        self.margin_right_adj = Gtk.Adjustment(value=0, lower=0, upper=500, step_increment=5, page_increment=10)
        self.margin_right_spin = Gtk.SpinButton(adjustment=self.margin_right_adj, climb_rate=1, digits=0)
        self.margin_right_spin.set_tooltip_text("Right margin (px)")
        self.margin_right_spin.set_width_chars(4)
        self.margin_right_spin.connect("value-changed", self.on_margin_changed)
        
        right_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=2)
        right_box.append(Gtk.Label(label="R:"))
        right_box.append(self.margin_right_spin)
        margins_box.append(right_box)
    
    def on_margin_changed(self, spin_button):
        """Handle margin value changes and apply them"""
        top = int(self.margin_top_spin.get_value())
        bottom = int(self.margin_bottom_spin.get_value())
        left = int(self.margin_left_spin.get_value())
        right = int(self.margin_right_spin.get_value())
        
        print(f"[Margins] Updating to T:{top} B:{bottom} L:{left} R:{right}")
        
        # Apply margins via JavaScript
        js_code = f"if (window.updateMargins) {{ window.updateMargins({top}, {bottom}, {left}, {right}); }}"
    ...
    def _delayed_column_apply(self):
        """Delayed reapplication of column styles"""
        if not hasattr(self, 'current_column_count'):
            return False
        ...
        (function() {{
            ...
            if (style) {{
                ...
                console.log('[Column] Delayed reapply complete');
            }}
        }})();
        """
    ...
    def on_nav_changed(self, manager, message):
        try:
            ...
            # Reapply column settings after navigation
            GLib.timeout_add(100, self.apply_columns_after_load)
            GLib.timeout_add(300, self.apply_columns_after_load)
                    ...
                    function highlightInElement(element) {{
                        ...
                        for (var i = 0; i < textNodes.length; i++) {{
                            ...
                            if (currentPos < actualEndPos && nodeEnd > actualStartPos) {{
                                ...
                                var bg = getComputedStyle(doc.documentElement).getPropertyValue('--tts-highlight-bg') || 'rgba(255,235,59,0.95)';
                                var fg = getComputedStyle(doc.documentElement).getPropertyValue('--tts-highlight-text') || '#000000';
...
<head>
  ...
  <style>
    ...
    html, body {{ height: 100%; margin: 0; padding: 0; background: var(--viewer-bg); color: var(--viewer-text); overflow: hidden; }}
    #viewer {{ 
      width: 100%; 
      height: 100%; 
    ...
    #viewer {{
      ...
      scrollbar-color: rgba(128, 128, 128, 0.5) var(--viewer-bg);
    }}
    /* Ensure epub.js iframe shows horizontal scrollbar for columns */
    #viewer iframe {{
      width: 100% !important;
      height: 100% !important;
      border: none;
    ...
    /* Make epub container scrollable horizontally */
    .epub-container {{
      overflow-x: auto !important;
      overflow-y: auto !important;
      width: 100% !important;
  ...
  <script>
    (function(){{
      ...
      try {{
        ...
        // Enable horizontal scrolling for columns
          ...
          try {{
            ...
            if (iframe) {{
              ...
              if (iframe.contentDocument) {{
                ...
                // Enable overflow scrolling in iframe html and body
                if (iframeHtml) {{
                ...
                // Add scrollbar styling to iframe
            ...
            console.error('Error setting iframe scrolling:', e);
          }}
        }});
        ...
        window.updateMargins = function(top, bottom, left, right) {{
          var viewer = document.getElementById('viewer');
          if (viewer) {{
            viewer.style.paddingTop = top + 'px';
            viewer.style.paddingBottom = bottom + 'px';
            viewer.style.paddingLeft = left + 'px';
            viewer.style.paddingRight = right + 'px';
            console.log("Updated margins:", top, bottom, left, right);
          }}
        }};
      }} catch (e) {{
