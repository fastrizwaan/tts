            ...
            for vis_idx in range(start_vis_idx, len(wrap_points)):
                ...
                if sel_start_ln != -1:
                    ...
                    if ln > sel_start_ln and ln < sel_end_ln:
                        ...
                        # Selection starts and ends on same logical line
                        # Check if selection overlaps with this wrap segment
                        if sel_end_col <= col_start or sel_start_col >= col_end:
                            # Selection doesn't overlap this wrap segment - skip drawing
                            s_col = -1
                        else:
                        ...
                        # Selection starts on this line - check if it overlaps this wrap segment
                        if sel_start_col >= col_end:
                            # Selection starts after this wrap segment - skip
                            s_col = -1
                        elif sel_start_col < col_end or (sel_start_col == col_start and vis_idx == len(wrap_points) - 1) or (sel_start_col == col_end and vis_idx == len(wrap_points) - 1):
                            s_col = max(0, sel_start_col - col_start)
                            e_col = len(text_segment)
                            if vis_idx == len(wrap_points) - 1:
                                e_col += 1
                    elif ln == sel_end_ln:
                        # Selection ends on this line - check if it overlaps this wrap segment
                        if sel_end_col <= col_start:
                            # Selection ends before this wrap segment - skip
                            s_col = -1
                        elif sel_end_col > col_start:
                            s_col = 0
