...
/* MODIFIED/NEW CSS RULES: 
 * Hide close button when the tab has the 'compact' class,
 * overriding the :hover rule.
 */
.chrome-tab.compact .chrome-tab-close-button,
.chrome-tab.compact:hover .chrome-tab-close-button {
    opacity: 0; 
}
...
class ChromeTab(Gtk.Box):
    ...
    # NEW: Override size_allocate to check width and toggle 'compact' class
    def size_allocate(self, allocation):
        """Called by Gtk when the widget is allocated its final size."""
        # 1. Must call the parent class's allocation method first
        super().size_allocate(allocation)
        
        # 2. Logic to check width
        MIN_WIDTH_FOR_CLOSE_BUTTON = 100 
        
        if self._closeable and self.close_button:
            if allocation.width < MIN_WIDTH_FOR_CLOSE_BUTTON:
...
class ChromeTabBar(Gtk.Box):
    ...
    __gsignals__ = {
        ...
        'add-tab-requested': (GObject.SignalFlags.RUN_FIRST, None, ()), # NEW SIGNAL
    }
    ...
    def __init__(self):
        ...
        # Initial separator (always kept at index 0)
        first_sep = Gtk.Box()
        ...
        # NEW: Add Tab Button
        self.add_tab_button = Gtk.Button()
        self.add_tab_button.set_icon_name("list-add-symbolic")
        self.add_tab_button.add_css_class("flat")
        self.add_tab_button.set_size_request(32, 32)
        # Crucial: Don't let it expand, so tabs push it to the end
        self.add_tab_button.set_hexpand(False) 
        self.add_tab_button.set_halign(Gtk.Align.START)
        self.add_tab_button.connect('clicked', self._on_add_tab_clicked)
        
        # Append the button to the main Gtk.Box
        self.append(self.add_tab_button)
    ...
    def _on_add_tab_clicked(self, button):
        """Emits a signal for the application to create a new tab."""
        self.emit('add-tab-requested')

    def add_tab(self, tab):
        idx = len(self.tabs)

        # Before adding the tab, remove the add button temporarily (if it exists)
        if self.add_tab_button.get_parent():
            self.remove(self.add_tab_button)
        ...
        # Re-append the add button at the very end
        self.append(self.add_tab_button)
    ...
    def remove_tab(self, tab):
        ...
        # Ensure the last tab is active if one exists
        if self.tabs and tab.has_css_class("active"):
             self.set_tab_active(self.tabs[-1])
    ...
    def _show_drop_indicator(self, position):
        ...
        if position == 0:
            ...
            if len(self.separators) > len(self.tabs):
                # Anchor to the separator just before the add button
...
class DemoApp(Adw.Application):
    ...
    def on_activate(self, app):
        ...
        # NEW: Connect the custom signal from the tab bar's integrated button
        self.tabbar.connect("add-tab-requested", lambda tb: self._add_tab())
        ...
        # REMOVED: The old, external 'Add Tab' button
