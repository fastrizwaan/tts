...
from gi.repository import Gtk, Adw, Gio, GLib, Pango, PangoCairo, Gdk, cairo
import cairo
...
# ============= LineIndex =============
# Replace the entire LineIndex class with this corrected version:

class LineIndex:
    def __init__(self, mapped_file, chunk=500_000):
        self.mf = mapped_file
        self.chunk = chunk
        self.newlines = []
        self.finished = False
    ...
    def _find_newlines_in_block(self, block, base_offset):
        ...
        if self.encoding in ('utf-16-le', 'utf-16-be'):
            ...
            while True:
                ...
                if nl == -1:
                    break
                # Ensure we're at a valid UTF-16 boundary (even offset for LE, odd for BE)
                if self.encoding == 'utf-16-le' and nl % 2 != 0:
                    # Skip if not aligned
                    start = nl + 1
                    continue
                elif self.encoding == 'utf-16-be' and nl % 2 != 1:
                    # Skip if not aligned
                    start = nl + 1
                    continue
        ...
        for _ in range(max_lines):
            ...
            if nl_pos == -1:
                ...
                if pos + chunk_size >= self.mf.size:
                    # Last line
                    try:
                        text = chunk.decode(self.encoding, "replace")
                    except:
                ...
                try:
                    text = line_data.decode(self.encoding, "replace")
                except:
                    text = line_data.decode("utf-8", "replace")
                lines.append((text, pos, pos + nl_pos))
                pos += nl_pos + (2 if self.encoding in ('utf-16-le', 'utf-16-be') else 1)
...
class VirtualTextView(Gtk.DrawingArea):
    ...
    def _compute_metrics(self):
        # Tiny surface just for measuring text
        ...
        layout.set_text("M\nM")
        _, logical = layout.get_pixel_extents()
        self.line_height = logical.height // 2
    ...
    def on_map(self, *args):
        self._compute_metrics()      # correct metrics, fixes overlap
...
class EditorWindow(Adw.ApplicationWindow):
    ...
    def _on_index_update(self):
        ...
        # Compute real discovered total lines
        actual_lines = self.buf.line_count()
        total = actual_lines * self.view.line_height

        # Always grow height ASAP
        if total > self.view.content_height:
            self.view.set_content_height(total)

            # absolutely required: keep vadjustment upper in sync
            if self.view.vadj:
        ...
        if self.idx.finished:
            ...
            self.status_label.set_text(
                f"{os.path.basename(self.mf.path)} - "
                f"{self.buf.line_count():,} lines "
                f"({size_mb:.1f} MB, {self.mf.encoding}){dirty_str}"
            )
        else:
            progress_pct = (self.idx.indexed_up_to / self.mf.size) * 100 if self.mf.size else 0
            self.status_label.set_text(
