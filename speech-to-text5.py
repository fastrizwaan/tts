...
class LiveTranscriberApp(Adw.Application):
    def __init__(self):
        ...
        # AI Engines
        self.recognizer_online = None  # Zipformer
        self.recognizer_offline = None # SenseVoice
        self.recognizer_parakeet = None # Parakeet
        self.whisper_model = None      # Whisper
    ...
    def do_activate(self):
        ...
        window.set_title("Ultimate 4-in-1 Real-time STT")
        ...
        self.engine_dropdown = Gtk.DropDown.new_from_strings([
            ...
            "NVIDIA Parakeet TDT (High-Speed Chunked)"
        ])
    ...
    def load_engine(self, idx):
        if idx == 0:
            ...
            self.text_buffer.set_text("Switching to NVIDIA Parakeet TDT...")
            threading.Thread(target=self.prepare_sherpa_model, args=("parakeet",), daemon=True).start()
    ...
    def download_sherpa_worker(self, model_key, url, folder_name):
        ...
        try:
            ...
            self.prepare_sherpa_model(model_key) 
        except Exception as e:
    ...
    def init_sherpa_engine(self, model_key, model_dir):
        try:
            if model_key == "zipformer":
                ...
                if not model_path.exists(): model_path = model_dir / "model.onnx"
                    
                self.recognizer_offline = sherpa_onnx.OfflineRecognizer.from_sense_voice(
                    model=str(model_path), tokens=str(model_dir / "tokens.txt"), num_threads=2, use_itn=True
                )
    ...
    def process_audio_loop(self):
        ...
        elif idx == 3: self.run_parakeet()

    # ENGINE 1: SHERPA ZIPFORMER (Online)
    ...
    # ENGINE 2: ALIBABA SENSEVOICE (Chunked)
    def run_sensevoice(self):
        buffer = np.array([], dtype=np.float32)
        CHUNK_SEC = 1.0 # 1-second chunks
        while self.is_recording:
            try:
    ...
    # ENGINE 4: NVIDIA PARAKEET TDT (Chunked)
    def run_parakeet(self):
        buffer = np.array([], dtype=np.float32)
        CHUNK_SEC = 1.5  # Parakeet thrives on slightly larger chunks than SenseVoice
        while self.is_recording:
            try:
                buffer = np.concatenate((buffer, self.audio_queue.get(timeout=0.2)))
            except queue.Empty: continue
            ...
            if len(buffer) >= SAMPLE_RATE * CHUNK_SEC:
                if np.sqrt(np.mean(buffer**2)) > 0.005: 
                    stream = self.recognizer_parakeet.create_stream()
                    stream.accept_waveform(SAMPLE_RATE, buffer)
                    self.recognizer_parakeet.decode_stream(stream)
