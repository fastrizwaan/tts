...
class MainWindow(Adw.ApplicationWindow):
    ...
    def build_html(self, results):
        ...
        for word, dict_data in results[:100]:
            for dname, defs, color in dict_data:
                ...
                for i, d in enumerate(defs):
                    # Check if this is a main definition starting with [m1]
                    is_m1_def = d.strip().startswith('[m1]')
                    # Check if it's a numbered definition without [m] tags
                    is_numbered = re.match(r'^\d+\.\s', d.strip())
                    
                    if is_m1_def or is_numbered:
                        ...
                        # This is a sub-item (example, synonym, etc.) - could be [m2] or other
                        if in_list:
        ...
        ol { margin: 0.5em 0; padding-left: 2em; line-height: 1.6; }
        li { margin-bottom: 0.8em; }
        .sub-item { margin-left: 1em; margin-top: 0.3em; line-height: 1.4; }
