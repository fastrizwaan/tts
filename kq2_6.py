...
class TTSApplication(Adw.Application):
    ...
    def shutdown_application(self):
        ...
        # Get the main window and stop playback
        win = self.props.active_window
        if win and hasattr(win, 'completely_reset_playback'):
            win.completely_reset_playback()
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def highlight_word(self, start_offset, end_offset):
        ...
        try:
            if start_offset >= 0 and end_offset >= 0 and not self.app.should_stop and not self.app.is_shutting_down:
