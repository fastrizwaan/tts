...
class DictionaryManager:
    ...
    def search(self, q, exact_only=True, max_preview=100, with_defs=False):
        ...
        q_esc = re.escape(q)
        #patterns = [
        #    re.compile(rf"^{q_esc}$", re.IGNORECASE),      # 0 exact match
        #    re.compile(rf"^{q_esc}\w*$", re.IGNORECASE), # 1 single word containing query
         #   re.compile(rf"^\w*{q_esc}\w*$", re.IGNORECASE) # 1 single word containing            
        #]
        # Build tiered patterns: exact, then 1-char prefix, 2-char prefix, 3+ prefix
        #patterns = [
        #    re.compile(r"^{}$".format(q_esc), re.I),                # 0: exact match
        #    re.compile(r"^[A-Za-z]{}[\w-]*$".format(q_esc), re.I),  # 1: bands, hands, sands
         #   re.compile(r"^[A-Za-z]{{2}}{}[\w-]*$".format(q_esc), re.I),  # 2: glands, stands
        #    re.compile(r"^[A-Za-z]{{3,}}{}[\w-]*$".format(q_esc), re.I), # 3+: badlands, grandstand
        #]
        patterns = [re.compile(q_esc, re.IGNORECASE)]



        # Determine which pattern tier a given key belongs to
        def match_tier(word):
            m = re.search(q_esc, word, re.IGNORECASE)
            if not m:
                return None
            pos = m.start()
            if pos == 0:
                return 0  # starts with the query, best match
            elif pos == 1:
                return 1  # one char before
            elif pos == 2:
                return 2
            else:
                return 3  # anywhere later
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def _search_worker_loop(self):
        """Dedicated background worker for queued searches."""
        while True:
            ...
            if not self._stop_flag.is_set():
                ...
                # Run substring phase only if user hasn’t typed again
