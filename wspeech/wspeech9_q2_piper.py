    ...
    def update_highlight(self):
        try:
            ...
            if current_word_idx >= 0:
                ...
                # Find the word in the text buffer to apply highlighting
                word = self.word_timings[current_word_idx].word
                # Simple case-insensitive search
                text_lower = text.lower()
                word_lower = word.lower()
                
                # Find all occurrences of the word to handle duplicates
                word_start_idx = -1
                for i, timing_info in enumerate(self.word_timings):
                    if timing_info.word.lower() == word_lower:
                        if i == current_word_idx:
                            # Find this specific occurrence
                            search_start = 0
                            for j in range(i):
                                prev_word = self.word_timings[j].word.lower()
                                prev_pos = text_lower.find(prev_word, search_start)
                                if prev_pos != -1:
                                    search_start = prev_pos + len(prev_word)
                            word_start_idx = text_lower.find(word_lower, search_start)
                            break
                
                if word_start_idx != -1:
                    word_end_idx = word_start_idx + len(word)
                    s_iter = buffer.get_iter_at_offset(word_start_idx)
                    e_iter = buffer.get_iter_at_offset(word_end_idx)
