...
class TTSApp(Adw.Application):
    def __init__(self, **kwargs):
        ...
        # **OPTIMIZED**: Use event-driven processing instead of continuous polling
        self.processing_queue = queue.Queue()
        self.processing_thread = None
        self.processing_active = False
        self.processing_event = threading.Event()  # NEW: Event-driven processing
        ...
        self.word_highlight_timer = None  # **NEW**: Separate timer for word highlighting
        self.playback_start_time = None
        self.highlight_tag = None
        self.is_playing = False
        
        # **NEW**: Word highlighting optimization
        self.current_highlighted_word = -1
        self.highlight_refresh_rate = 50  # ms - adjustable for performance
    ...
    def on_activate(self, app):
        ...
        self.window.set_title("WhisperSpeech TTS - Optimized Word Highlighting")
        ...
        perf_button = Gtk.Button(label="⚙️ Performance")
        perf_button.connect("clicked", self.show_performance_settings)
        button_box.append(perf_button)
        ...
        # Set up highlight tags with better styling
        buffer = self.textview.get_buffer()
        
        # **IMPROVED**: Better word highlighting with multiple colors
        rgba_word = Gdk.RGBA()
        rgba_word.parse("rgba(255, 255, 0, 0.7)")  # Bright yellow for current word
        ...
        rgba_sentence.parse("rgba(135, 206, 250, 0.3)")  # Light blue for current sentence
        self.sentence_tag = buffer.create_tag("sentence", 
                                            background_rgba=rgba_sentence)
        
        # **NEW**: Upcoming word preview
        rgba_next = Gdk.RGBA()
        rgba_next.parse("rgba(144, 238, 144, 0.4)")  # Light green for next word
    ...
    def show_performance_settings(self, button):
        ...
        # Memory management
        memory_label = Gtk.Label(label="Memory Management:")
        memory_label.set_halign(Gtk.Align.START)
        memory_label.add_css_class("heading")
        content.append(memory_label)
        
        cleanup_button = Gtk.Button(label="🧹 Clean CUDA Cache")
        cleanup_button.connect("clicked", lambda b: self.cleanup_cuda_cache())
        content.append(cleanup_button)
        
        # Alignment settings
        alignment_check = Gtk.CheckButton(label="Enable word alignment (may be slow)")
        alignment_check.set_active(not self.skip_alignment)
        content.append(alignment_check)
        ...
        apply_button.connect("clicked", lambda b: self.apply_performance_settings(
            ...
            not alignment_check.get_active(),
            dialog
        ))
    ...
    def apply_performance_settings(self, refresh_rate, skip_alignment, dialog):
        """Apply performance settings"""
        self.highlight_refresh_rate = refresh_rate
        self.skip_alignment = skip_alignment
        dialog.close()
        self.update_status(f"Settings applied: refresh {refresh_rate}ms, alignment {'disabled' if skip_alignment else 'enabled'}")

    def cleanup_cuda_cache(self):
        """Clean CUDA memory cache"""
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
            self.update_status("CUDA cache cleared")
        else:
            self.update_status("No CUDA device available")
    ...
    def on_speak(self, button):
        ...
        # **OPTIMIZED**: Start event-driven background processing
    ...
    def background_processor(self):
        """**OPTIMIZED**: Event-driven background processor - no continuous polling"""
        while self.processing_active:
            try:
                    ...
                    try:
                        ...
                        if sentence_idx < len(self.sentences) and not self.sentences[sentence_idx].processed:
                            ...
                            try:
                                ...
                                # **OPTIMIZED**: Only do alignment if not skipped
                                if not self.skip_alignment:
                                    alignment_data = self.process_sentence_alignment(wav_file, sentence.words)
                                else:
                                    # Use simple fallback alignment
                                    waveform, sample_rate = torchaudio.load(wav_file)
                                    alignment_data = self.create_fallback_alignment(sentence.words, waveform, sample_rate)
                                ...
                                if alignment_data:
                                    ...
                                    print(f"Background processing failed for sentence {sentence_idx + 1}")
                            ...
                            # **OPTIMIZED**: Clean CUDA cache periodically
                            if sentence_idx % 2 == 0 and self.device.type == 'cuda':
                                torch.cuda.empty_cache()
                ...
                if processed_any:
                    self.processing_event.clear()  # Reset event after processing
                else:
                    # No work to do, wait for next event
                    self.processing_event.clear()
    ...
    def process_current_sentence(self):
        ...
        try:
            ...
            # **OPTIMIZED**: Process alignment based on settings
            if not self.skip_alignment:
                alignment_data = self.process_sentence_alignment(wav_file, sentence.words)
            else:
                waveform, sample_rate = torchaudio.load(wav_file)
    ...
    def start_sentence_playback(self, sentence):
        ...
        try:
            ...
            # **OPTIMIZED**: Start precise word highlighting with configurable refresh rate
    ...
    def update_word_highlight(self, sentence):
        """**IMPROVED**: More efficient and precise word highlighting"""
        ...
        # **OPTIMIZED**: Only update highlighting if word changed
        if current_word_idx != self.current_highlighted_word:
            ...
            if current_word_idx >= 0 and current_word_idx < len(sentence.word_starts):
                ...
                # **NEW**: Auto-scroll to current word
                self.textview.scroll_to_iter(s_iter, 0.0, False, 0.0, 0.5)
            
            # **NEW**: Highlight next word preview
    ...
    def process_sentence_alignment(self, wav_file, words):
        ...
        if self.skip_alignment:
            print("Skipping alignment due to settings or previous errors, using fallback")
            waveform, sample_rate = torchaudio.load(wav_file)
        ...
        try:
            ...
            try:
                with torch.inference_mode():
                    ...
                    waveform_device = waveform.to(self.device, non_blocking=True)
                    emissions, _ = self.asr_model(waveform_device)
                    emissions = torch.log_softmax(emissions, dim=-1)
                    
                    # Move back to CPU immediately to free GPU memory
                    emissions = emissions.cpu()
                    del waveform_device  # Explicit cleanup
                    
            except RuntimeError as e:
                if "CUDA" in str(e) or "assert" in str(e).lower() or "out of memory" in str(e).lower():
                    print(f"CUDA/Memory error in ASR model, switching to CPU: {e}")
    ...
    # **OPTIMIZED**: Alignment methods with better memory management
    def get_trellis(self, emission, tokens, blank_id=0):
        """Create trellis with better bounds checking and memory efficiency"""
        ...
        # **OPTIMIZED**: Use more memory-efficient data types
        trellis = torch.full((num_frame, num_tokens), -float("inf"), dtype=torch.float32)
        ...
        # Fill the rest of the trellis with batched operations where possible
        for t in range(1, num_frame):
    ...
    def backtrack(self, trellis, emission, tokens, blank_id=0):
        """Backtrack with better error handling and memory efficiency"""
        try:
    ...
    def merge_repeats(self, path, transcript):
        """Merge repeated segments - optimized version"""
        i1, i2 = 0, 0
        segments = []
        while i1 < len(path):
    ...
    def merge_words(self, segments, separator="|"):
        """Merge segments into words - optimized version"""
        words = []
        i1, i2 = 0, 0
...
if __name__ == "__main__":
    app = TTSApp(application_id='com.example.optimized_ttsapp')
    app.run(None) current playback
