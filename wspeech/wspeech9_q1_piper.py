    ...
    def update_highlight(self):
        try:
            ...
            if current_word_idx >= 0:
                ...
                word_start_idx = text.lower().find(word.lower(), 0)
                
                # If not found from the beginning, try to find it after previous highlights
                if word_start_idx == -1:
                    for i in range(current_word_idx):
                        prev_word = self.word_timings[i].word
                        prev_start = text.lower().find(prev_word.lower(), 0)
                        if prev_start != -1:
                            word_start_idx = text.lower().find(word.lower(), prev_start + len(prev_word))
                            if word_start_idx != -1:
                                break
