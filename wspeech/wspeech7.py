...
class TTSApp(Adw.Application):
    ...
    def process_alignment(self, wav_file, words):
        try:
            ...
            if not clean_words:
                print("No clean words found - falling back to simple timing")
            ...
            while len(word_start_times) < len(words):
                if word_start_times:
                    word_start_times.append(word_end_times[-1])
                    word_end_times.append(word_end_times[-1] + 0.5)
                else:
            ...
            print(f"Alignment successful: {len(word_start_times)} timings for {len(words)} words")
...
if __name__ == "__main__":
    app = TTSApp(application_id='net.mkiol.SpeechNote')
    app.run(None)
