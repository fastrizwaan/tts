    ...
    def _run_js_highlight(self, sid):
        ...
        (function() {{
            try {{
                ...
                if (isMultiColumn) {{
                    // Multi-column mode - snap to column if element is not visible
                    ...
                    // Check if element is partially cut off at the right edge
                    var isPartiallyCutOff = rect.left >= 0 && rect.left < viewportWidth && rect.right > viewportWidth;
                    
                    if (!isVisible || isPartiallyCutOff) {{
                        // Element not visible OR partially cut off - snap to the column containing it
                    ...
                    // Don't proactively scroll - let the next sentence trigger the scroll when it's highlighted
                }} else {{
                    // Single column mode - scroll to top if not visible
                    var isVerticallyVisible = rect.top >= 0 && rect.bottom <= viewportHeight;
                    
                    // Check if element is partially cut off at the bottom
