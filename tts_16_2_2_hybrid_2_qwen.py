...
def writable_path(name):
    """Get a writable path for temporary files, preferring app-specific cache."""
    app_cache_dir = os.path.expanduser("~/.var/app/io.github.fastrizwaan.tts/cache")
    os.makedirs(app_cache_dir, exist_ok=True)
    return os.path.join(app_cache_dir, name)
...
class TTSEngine:
    ...
    def highlight_sentence(self, idx):
        """Highlight the sentence with given index in the WebView"""
        try:
            ...
            if idx < 0:
                ...
                (function() {
                    ...
                    if (iframe && iframe.contentDocument) {
                        var highlighted = iframe.contentDocument.querySelectorAll('.tts-highlight');
                ...
                # Clear previous highlights and highlight new one
                js_highlight = f"""
                (function() {{
                    var iframe = document.querySelector('iframe');
                    if (iframe && iframe.contentDocument) {{
                        ...
                        // Find the element with the matching data-tts-id
                        var targetSpan = iframe.contentDocument.querySelector('[data-tts-id="{self.get_sentence_id(idx)}"]');
                        if (targetSpan) {{
                            // Ensure it's visible by scrolling
                            targetSpan.scrollIntoView({{block: 'center', behavior: 'smooth'}});
                            // Add highlight class
                            targetSpan.classList.add('tts-highlight');
                        }}
                    }}
                }})();
                """
    ...
    def get_sentence_id(self, idx):
        """Get the stable ID for the sentence at index idx"""
        if 0 <= idx < len(self._tts_sentences):
            sentence = self._tts_sentences[idx]
            hash_val = 0
            for char in sentence:
                hash_val = ((hash_val << 5) - hash_val) + ord(char)
                hash_val = hash_val & hash_val
            return abs(hash_val).to_bytes(8, byteorder='big').hex()[:12]
        return ""
    ...
    def is_playing(self):
        """Return True if TTS is actively playing (not paused)"""
        return self.is_playing and not self.is_paused

    def is_paused(self):
        """Return True if TTS is paused"""
        return self.is_paused

    def reapply_highlight_after_reload(self):
        """Re-apply highlight after a page reload if TTS is active"""
        try:
            if self.is_playing and self._current_play_index >= 0:
                self.highlight_sentence(self._current_play_index)
        except Exception:
            pass
    ...
    def on_epub_message(self, content_manager, js_result):
        ...
        try:
            ...
            if data.get("type") == "chapterLoaded":
                ...
                self.tts_play_btn.set_sensitive(True)
