...
class LiveTranscriberApp(Adw.Application):
    ...
    def capture_audio_loop(self):
        ...
        # SAFELY close the PyAudio instance only after the loop has naturally finished
        try:
    ...
    def run_whisper_processor(self):
        """Sliding window approach for Whisper.cpp chunked buffering with Silence Filtering."""
        whisper_buffer = np.array([], dtype=np.float32)
        CHUNK_SECONDS = 2.0  
        SILENCE_THRESHOLD = 0.005  # RMS volume threshold (adjust if your mic is very quiet/loud)
        ...
        while self.is_recording:
            ...
            # If we hit the 2-second mark, process the chunk
            if len(whisper_buffer) >= SAMPLE_RATE * CHUNK_SECONDS:
                
                # 1. Volume Energy Gate: Calculate Root Mean Square (RMS) to check loudness
                rms_volume = np.sqrt(np.mean(whisper_buffer**2))
                
                # Only transcribe if the audio is louder than background noise
                if rms_volume > SILENCE_THRESHOLD:
                    segments = self.whisper_model.transcribe(whisper_buffer)
                    text = " ".join([seg.text for seg in segments]).strip()
                    
                    # 2. Hallucination Filter: Strip [BLANK_AUDIO], (Silence), and common ghost phrases
                    text = re.sub(r'\[.*?\]|\(.*?\)', '', text)  # Removes anything in brackets/parentheses
                    
                    # Hard-remove known specific Whisper quirks
                    ghost_phrases = ["ignore it. Okay.", "ignore it.", "Okay.", "Thank you.", "Thank you", "you"]
                    for phrase in ghost_phrases:
                        # Only remove if it's the exact phrase floating alone
