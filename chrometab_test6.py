...
class ChromeTab(Gtk.Box):
    ...
    def __init__(self, title="Untitled", closeable=True):
        ...
        # Create separator at the end (will be shown/hidden by tab bar)
        self.separator = Gtk.Separator(orientation=Gtk.Orientation.VERTICAL)
        self.separator.add_css_class("tab-separator")
...
class ChromeTabBar(Gtk.Box):
    ...
    def reorder_tab(self, old_index: int, new_index: int):
        ...
        # Reorder in UI
        self.remove(tab)
        if new_index >= len(self.tabs) - 1:
            self.append(tab)
        else:
            self.insert_child_after(tab, self.tabs[new_index - 1] if new_index > 0 else None)
        
        # Update active index
        if self.active_tab_index == old_index:
            self.active_tab_index = new_index
        
        # Update separators after reordering
        self._update_separators()
    ...
    def _update_separators(self):
        ...
        for i, tab in enumerate(self.tabs):
            ...
            # 1. It's the last tab
            # 2. The tab is active
            # 3. The next tab is active (separator is on the right side)
            
            if i == len(self.tabs) - 1:
                # Last tab never shows separator
                tab.separator.set_visible(False)
            elif i == self.active_tab_index:
                # Active tab never shows separator
                tab.separator.set_visible(False)
            elif i + 1 == self.active_tab_index:
                # Tab before active tab doesn't show separator
                tab.separator.set_visible(False)
            else:
                # All other inactive tabs show separator
                tab.separator.set_visible(True)
