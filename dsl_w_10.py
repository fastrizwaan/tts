...
class MainWindow(Adw.ApplicationWindow):
    ...
    def extract_list_marker(self, text):
        """Extract the list marker symbol from [mX] tags"""
        # Check for patterns like [m1]1》, [m2]2), [m3]3., etc.
        match = re.match(r'\[m\d+\](?:\[([biu])\])?\d+([\.\)》])(?:\[/\1\])?', text.strip())
        if match:
            marker = match.group(2) if match.group(2) else '.'
            return marker
    ...
    def build_html(self, results):
            ...
            for dname, defs, color in dict_data:
                ...
                for i, d in enumerate(defs):
                    ...
                    if m_content:
                        ...
                        # Check if the content inside [mX] starts with a list marker
                        is_numbered_m = (re.match(r'^\d+[\.\)》]\s', inner) or 
                                        re.match(r'^\[([biu])\]\d+[\.\)》]\[/\1\]', inner) or
                                        '■' in inner[:10])
                        if is_numbered_m:
                            # Extract the marker symbol
                            list_marker = self.extract_list_marker(d)
                    else:
                        is_numbered_m = False
                    ...
                    if is_numbered:
                        marker_match = re.match(r'^\d+([\.\)》])', d.strip())
                        if marker_match:
        ...
        ol.custom-list[data-marker="》"] li::before { content: counter(item) '》 '; position: absolute; left: -2em; }
        ol.custom-list[data-marker=")"] li::before { content: counter(item) ') '; position: absolute; left: -2em; }
        ol.custom-list[data-marker="."] li::before { content: counter(item) '. '; position: absolute; left: -2em; }
        ol.custom-list[data-marker="■"] li::before { content: '■ '; position: absolute; left: -2em; }
    ...
    def render_dsl_text(self, raw, is_main=False, headword=""):
        ...
        # Remove list markers if it's a main definition (we'll add it via CSS)
        if is_main:
            t = re.sub(r'^\d+[\.\)》]\s+', '', t)
