...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        self.actual_column_width = 400  # default safe value
    ...
    # REPLACE is_single_column_mode with:
    def is_single_column_mode(self):
        try:
            if not getattr(self, "column_mode_use_width", False):
                return getattr(self, "column_count", 1) <= 1
            alloc = self.get_allocation()
            width = alloc.width or 1200
            gap = int(getattr(self, "_column_gap", 32))
            available = max(100, width - (2 * gap))
            return int(getattr(self, "column_width_px", 400)) >= (available - gap)
        except Exception:
            return True
    ...
    def calculate_column_dimensions(self):
        alloc = self.get_allocation()
        width, height = alloc.width or 1200, alloc.height or 800
        if width <= 0 or height <= 0:
            width, height = 1200, 800
        available = max(100, width - (2 * getattr(self, "column_padding", 0)) - self._webview_horizontal_margins())
        if not getattr(self, "column_mode_use_width", False):  # fixed mode
            cols = max(1, int(getattr(self, "column_count", 1)))
            total_gap = (cols - 1) * getattr(self, "_column_gap", 32)
            cw = max(50, (available - total_gap) // cols)
            self.actual_column_width = cw
        else:
            desired = int(getattr(self, "column_width_px", 400))
            self.actual_column_width = max(50, min(desired, available))

    # REPLACE on_scroll_event with the reference-accurate logic:
    def on_scroll_event(self, controller, dx, dy):
        ...
        if abs(dx) > 0.02 or abs(dy) > 0.02:  # lower threshold so touchpads/wheels work
            scroll_left  = dx > 0.02 or dy < -0.02
            scroll_right = dx < -0.02 or dy > 0.02
            ...
            if not getattr(self, "column_mode_use_width", False):
                ...
                cw = int(getattr(self, "actual_column_width", 400))
                if scroll_left:
                    js = f"""
                    (function(){{
                        var w={cw}, g={gap}, s=w+g;
                        var cur=window.pageXOffset||document.documentElement.scrollLeft;
                        var col=Math.max(0, Math.round(cur/s)-1);
                        window.scrollTo({{left:col*s, behavior:'smooth'}});
                    }})();"""
                else:
                    js = f"""
                    (function(){{
                        ...
                        var max=Math.max(0, document.documentElement.scrollWidth - vw);
                        var col=Math.round(cur/s)+1;
                        var tgt=Math.min(max, col*s);
                        window.scrollTo({{left:tgt, behavior:'smooth'}});
                    }})();"""
            else:
                # width-based columns (recompute actual step from viewport)
                desired = int(getattr(self, "column_width_px", 400))
                js = f"""
                (function(){{
                    var desired={desired}, g={gap};
                    var vw=window.innerWidth||document.documentElement.clientWidth;
                    var cur=window.pageXOffset||document.documentElement.scrollLeft;
                    var avail=vw-(2*{getattr(self, 'column_padding', 0)}+{margin_total});
                    var n=Math.floor(avail/(desired+g))||1;
                    var total=(n-1)*g;
                    var cw=(avail-total)/n;
                    var step=cw+g;
                    var col=Math.round(cur/step){' - 1' if scroll_left else ' + 1'};
                    var max=Math.max(0, document.documentElement.scrollWidth - vw);
                    var tgt=Math.{ 'max(0, col*step)' if scroll_left else 'min(max, col*step)' };
                    window.scrollTo({{left:tgt, behavior:'smooth'}});
                }})();"""

            self.webview.evaluate_javascript(js, -1, None, None, None, None, None)
            if getattr(self, "snap_timeout_id", None):
    ...
    def snap_to_nearest_step(self):
        """Snap horizontal scroll to nearest column boundary."""
        try:
            if not getattr(self, "book", None) or (hasattr(self, "is_single_column_mode") and self.is_single_column_mode()):
                self.snap_timeout_id = None
                return False
            if not hasattr(self, "actual_column_width"):
                self.actual_column_width = 400
            self.calculate_column_dimensions()
            step = int(getattr(self, "actual_column_width", 400)) + int(getattr(self, "_column_gap", 32))

            js = f"""
            (function(){{
              const de=document.documentElement;
              const se=document.scrollingElement||de;
              const vw=window.innerWidth||de.clientWidth||0;
              const cur=se.scrollLeft||0;
              const max=Math.max(0,(se.scrollWidth||de.scrollWidth||0)-vw);
              const s={step};
              let n=Math.round(cur/s);
              let tgt=n*s;
              if (tgt<0) tgt=0;
              if (tgt>max) tgt=max;
              if (Math.abs(cur-tgt)>5) {{
                window.scrollTo({{left:tgt, behavior:'smooth'}});
              }}
              return {{cur:cur, tgt:tgt, max:max, step:s}};
            }})();
            """
            cb = getattr(self, "_on_js_result", None)
            print(f"[nav] snap request: step={step}")
            self.webview.evaluate_javascript(js, -1, None, None, None, cb, None) if cb else \
                self.webview.evaluate_javascript(js, -1, None, None, None, None, None)
        except Exception as e:
            print("[nav] snap error:", e)
        finally:
            self.snap_timeout_id = None
        return False
    ...
    def on_scroll_position_changed(self, adjustment):
        try:
            if hasattr(self, "update_page_info"):
                self.update_page_info()
            if hasattr(self, "_refresh_buttons_based_on_adjustment"):
    ...
    def on_key_pressed(self, controller, keyval, keycode, state):
        ...
        if self.is_single_column_mode():
            if keyval == Gdk.KEY_Page_Up:
                ...
                (function(){
                    ...
                    var h=d.clientHeight,s=window.pageYOffset||d.scrollTop;
                    var cs=window.getComputedStyle(b);
                    var lh=parseFloat(cs.lineHeight);
                    if(!lh||isNaN(lh)){var fs=parseFloat(cs.fontSize)||16;lh=fs*1.2;}
                    var lines=Math.max(1,Math.floor(h/lh));
                    var t=Math.max(0,s-lines*lh);
                ...
                (function(){
                    ...
                    var lines=Math.max(1,Math.floor(h/lh));
                    var t=s+lines*lh;
                    var max=Math.max(0,d.scrollHeight-h);
                    if(t>max)t=max;
                    window.scrollTo({top:t,behavior:'smooth'});
                })();"""
                ...
                js = "window.scrollTo({top:document.documentElement.scrollHeight,behavior:'smooth'});"
            else:
                return False
            self.webview.evaluate_javascript(js, -1, None, None, None, None, None)
            return True
        ...
        # --- multi-column mode ---
        margin_total = self._webview_horizontal_margins()
        column_gap = int(getattr(self, "_column_gap", 32))

        if not getattr(self, "column_mode_use_width", False):
            cw = int(self.actual_column_width)
            js_left = f"""
            (function(){{
                var w={cw},g={column_gap},s=w+g;
                var cur=window.pageXOffset||document.documentElement.scrollLeft;
                var vw=window.innerWidth||document.documentElement.clientWidth;
                var per=Math.floor(vw/s)||1;
                var col=Math.round(cur/s);
                var tgt=Math.max(0,(col-per)*s);
                window.scrollTo({{left:tgt,behavior:'smooth'}});
                setTimeout(()=>window.scrollTo({{left:tgt,behavior:'auto'}}),400);
            }})();"""
            js_right = f"""
            (function(){{
                var w={cw},g={column_gap},s=w+g;
                ...
                var per=Math.floor(vw/s)||1;
                var col=Math.round(cur/s);
                var tgt=Math.min(max,(col+per)*s);
                window.scrollTo({{left:tgt,behavior:'smooth'}});
                setTimeout(()=>window.scrollTo({{left:tgt,behavior:'auto'}}),400);
            }})();"""
        else:
            ...
            js_left = f"""
            (function(){{
                var w={desired},g={column_gap};
                var vw=window.innerWidth||document.documentElement.clientWidth;
                var cur=window.pageXOffset||document.documentElement.scrollLeft;
                var avail=vw-(2*{getattr(self,'column_padding',0)}+{margin_total});
                var n=Math.floor(avail/(w+g))||1;
                var total=(n-1)*g;
                var cw=(avail-total)/n;
                var step=cw+g;
                var col=Math.round(cur/step);
                var tgt=Math.max(0,(col-n)*step);
                window.scrollTo({{left:tgt,behavior:'smooth'}});
                setTimeout(()=>window.scrollTo({{left:tgt,behavior:'auto'}}),400);
            }})();"""
            js_right = f"""
            (function(){{
                ...
                var step=cw+g;
                var col=Math.round(cur/step);
                var tgt=Math.min(max,(col+n)*step);
                window.scrollTo({{left:tgt,behavior:'smooth'}});
                setTimeout(()=>window.scrollTo({{left:tgt,behavior:'auto'}}),400);
            }})();"""

        # alias PageUp/PageDown to same column logic
        if keyval in (Gdk.KEY_Page_Up, 65365):
            print("[nav] PgUp → trigger Left column step")
            return self.on_key_pressed(controller, Gdk.KEY_Left, keycode, state)
        elif keyval in (Gdk.KEY_Page_Down, 65366):
            print("[nav] PgDn → trigger Right column step")
            return self.on_key_pressed(controller, Gdk.KEY_Right, keycode, state)

        elif keyval == Gdk.KEY_Home:
            self.webview.evaluate_javascript("window.scrollTo({left:0,behavior:'smooth'});", -1, None, None, None, None, None)
            return True
        elif keyval == Gdk.KEY_End:
            js = "(function(){var m=document.documentElement.scrollWidth-window.innerWidth;window.scrollTo({left:m,behavior:'smooth'});})();"
            self.webview.evaluate_javascript(js, -1, None, None, None, None, None)
            return True
        return False


    def _step_column(self, direction: int):
        """Step one column horizontally. direction: -1 = left/up, +1 = right/down"""
        try:
            if not getattr(self, "book", None):
                print("[nav] _step_column aborted: no book")
                return False
            ...
            (function(){{
              ...
              const max=Math.max(0,(se.scrollWidth||de.scrollWidth||0)-vw);
              const s={step};
              const dir={1 if direction>0 else -1};
              let tgt=cur + dir*s;
              if (tgt<0) tgt=0;
              if (tgt>max) tgt=max;
              window.scrollTo({{left:tgt, behavior:'smooth'}});
              return {{cur:cur, tgt:tgt, max:max, step:s, dir:dir}};
            }})();
            """
            # use your callback if you have it; else just fire-and-forget
            cb = getattr(self, "_on_js_result", None)
            print(f"[nav] step request: dir={direction}, step={step}")
            self.webview.evaluate_javascript(js, -1, None, None, None, cb, None) if cb else \
                self.webview.evaluate_javascript(js, -1, None, None, None, None, None)
            return True
        except Exception as e:
            print("[nav] _step_column error:", e)
            return False

    def setup_navigation(self):
        """Attach unified key + scroll handlers to the webview (gtk4-safe)."""
        print("[nav] setup_navigation initialized")

        # ---- key controller ----
        self.key_controller = Gtk.EventControllerKey.new()

        def _on_webview_key_pressed(controller, keyval, keycode, state):
            name = Gdk.keyval_name(keyval)
            print(f"[nav] key pressed: {name} ({keyval}) state={state}")

            ctrl  = bool(state & Gdk.ModifierType.CONTROL_MASK)
            shift = bool(state & Gdk.ModifierType.SHIFT_MASK)
            alt   = bool(state & Gdk.ModifierType.ALT_MASK)

            # normal navigation (no modifiers)
            # in your on_key_pressed (or the key handler you attached to webview), after your debug prints:
            if keyval in (Gdk.KEY_Left, 65361):
                return self._step_column(-1)
            elif keyval in (Gdk.KEY_Right, 65363):
                return self._step_column(+1)
            elif keyval in (Gdk.KEY_Page_Up, 65365):
                return self._step_column(-1)
            elif keyval in (Gdk.KEY_Page_Down, 65366):
                return self._step_column(+1)
            elif keyval in (Gdk.KEY_Home, 65360):
                self.webview.evaluate_javascript("window.scrollTo({left:0,behavior:'smooth'});", -1, None, None, None, None, None)
                return True
            elif keyval in (Gdk.KEY_End, 65367):
                self.webview.evaluate_javascript("(function(){const se=document.scrollingElement||document.documentElement;const vw=window.innerWidth||document.documentElement.clientWidth;const max=Math.max(0,(se.scrollWidth||document.documentElement.scrollWidth)-vw);window.scrollTo({left:max,behavior:'smooth'});})();", -1, None, None, None, None, None)
                return True

            # allow Ctrl+Scroll etc. to propagate
            return False

        self.key_controller.connect("key-pressed", _on_webview_key_pressed)
        self.webview.set_can_focus(True)
        self.webview.add_controller(self.key_controller)

        # ---- scroll controller ----
        def _on_scroll(controller, dx, dy):
            direction = "scroll-down" if dy < 0 else "scroll-up" if dy > 0 else "horizontal"
            print(f"[nav] scroll detected: dx={dx:.2f}, dy={dy:.2f}, direction={direction}")
            if abs(dx) > abs(dy):  # horizontal wheel
                self._step_column(-1 if dx > 0 else +1)
            elif abs(dy) > 0.05:   # vertical wheel
                self._step_column(+1 if dy > 0 else -1)
            return True

        flags = Gtk.EventControllerScrollFlags.BOTH_AXES
        self.scroll_controller = Gtk.EventControllerScroll.new(flags)
        self.scroll_controller.connect("scroll", _on_scroll)
        self.webview.add_controller(self.scroll_controller)

        print("[nav] setup_navigation complete")
    ...
    def _wrap_html(self, raw_html, base_uri):
        ...
        try:
            ...
            # IMPORTANT: append col_rules AFTER the EPUB CSS so our overrides win.
            page_css_base = (self.css_content or "") + "\n" + THEME_INJECTION_CSS
            page_css_final = page_css_base + "\n" + col_rules

            page_css = page_css_final
