...
class EPUBWindow(Adw.ApplicationWindow):
    ...
    def scroll_viewport(self, direction: int):
        """Scroll one full column or page, snapping precisely to CSS column widths."""
        ...
        if horizontal_mode:
            # --- Multi-column page-turn (accurate snapping) ---
            js_code = f"""
            (function() {{
                let body = document.scrollingElement || document.body;
                let vw = window.innerWidth;
                let maxScroll = body.scrollWidth - vw;
                let cur = body.scrollLeft;

                // Read computed column width + gap
                let style = window.getComputedStyle(document.body);
                let colWidth = parseFloat(style.getPropertyValue('column-width')) || 0;
                let colCount = parseInt(style.getPropertyValue('column-count')) || 1;
                let colGap = parseFloat(style.getPropertyValue('column-gap')) || 0;
                ...
                // Estimate effective page width (actual visible area per column)
                let effectiveWidth = (colWidth > 0 ? colWidth : vw) + colGap;
                // Add margins/padding if present
                effectiveWidth = effectiveWidth + paddingLeft + paddingRight;

                // Compute next target position
                let rawTarget = cur + ({direction}) * effectiveWidth;
                let target = Math.round(rawTarget / effectiveWidth) * effectiveWidth;
                ...
                return JSON.stringify({{
                    cur, target, maxScroll,
                    effectiveWidth, colWidth, colGap, marginLeft, marginRight
                }});
            }})();
            """
            ...
            def process_result(result):
                try:
                    ...
                    effectiveWidth = data.get("effectiveWidth", 0)
                    ...
                    (function() {{
                        let body = document.scrollingElement || document.body;
                        let cur = body.scrollLeft;
                        let snapped = Math.round(cur / {effectiveWidth}) * {effectiveWidth};
                        snapped = Math.max(0, Math.min(snapped, body.scrollWidth - window.innerWidth));
    ...
    def scroll_to_end_of_page(self):
        ...
        if self.app.columns > 1 or not self.app.use_fixed_columns:
            # For multi-column layouts: scroll to the end horizontally
    ...
    def update_nav_buttons(self, *args):
        ...
        # Determine if we're using horizontal scrolling (multi-column) or vertical scrolling (single column)
        if self.app.columns > 1 or not self.app.use_fixed_columns:
            # Multi-column layout - horizontal scrolling
            h_adj = self.scrolled_window.get_hadjustment()
