import gi
import math
import cairo  # Direct Cairo import for PDF creation
...
class PDFEditor(Adw.Application):
    def __init__(self):
        ...
        # Store right-click position for context menu
    ...
    def setup_buttons(self):
        ...
        open_button.set_tooltip_text("Open PDF")
        ...
        save_button.set_tooltip_text("Save PDF")
        save_button.connect("clicked", self.on_save)
        self.header.pack_end(save_button)
        
        # Add elements
        add_text_button = Gtk.Button()
        add_text_button.set_icon_name("insert-text-symbolic")
        add_text_button.set_tooltip_text("Add Text")
        add_text_button.connect("clicked", self.on_add_text)
        self.header.pack_start(add_text_button)
        
        add_image_button = Gtk.Button()
        add_image_button.set_icon_name("insert-image-symbolic")
        add_image_button.set_tooltip_text("Add Image")
        ...
        rotate_left.set_icon_name("object-rotate-left-symbolic")
        rotate_left.set_tooltip_text("Rotate Page Left")
        rotate_left.connect("clicked", lambda b: self.rotate(-90))
        self.header.pack_start(rotate_left)
        
        rotate_right = Gtk.Button()
        rotate_right.set_icon_name("object-rotate-right-symbolic")
        rotate_right.set_tooltip_text("Rotate Page Right")
        rotate_right.connect("clicked", lambda b: self.rotate(90))
        self.header.pack_start(rotate_right)
        
        # Selection rotation
        rot_sel_left = Gtk.Button(label="↺")
        rot_sel_left.set_tooltip_text("Rotate Selected Left")
        rot_sel_left.connect("clicked", lambda b: self.rotate_selected(-90))
        self.header.pack_start(rot_sel_left)
        
        rot_sel_right = Gtk.Button(label="↻")
        rot_sel_right.set_tooltip_text("Rotate Selected Right")
        rot_sel_right.connect("clicked", lambda b: self.rotate_selected(90))
        self.header.pack_start(rot_sel_right)
        
        # Page navigation
        prev_page = Gtk.Button()
        prev_page.set_icon_name("go-previous-symbolic")
        prev_page.set_tooltip_text("Previous Page")
        prev_page.connect("clicked", self.on_prev_page)
        self.header.pack_start(prev_page)
        
        next_page = Gtk.Button()
        next_page.set_icon_name("go-next-symbolic")
        next_page.set_tooltip_text("Next Page")
        next_page.connect("clicked", self.on_next_page)
        self.header.pack_start(next_page)
        
        # Text editing controls - compact
        self.text_content_entry = Gtk.Entry()
        self.text_content_entry.set_placeholder_text("Text")
        self.text_content_entry.set_size_request(100, -1)
        ...
        self.font_size_spin.set_size_request(60, -1)
        ...
        self.font_family_entry.set_size_request(80, -1)
        self.font_family_entry.connect("changed", self.on_font_family_changed)
        self.header.pack_start(self.font_family_entry)
        
        self.bold_toggle = Gtk.ToggleButton(label="B")
        self.bold_toggle.set_tooltip_text("Bold")
        ...
        self.letter_spacing_spin.set_size_request(60, -1)
        self.letter_spacing_spin.set_tooltip_text("Letter Spacing")
        ...
        self.image_width_spin.set_size_request(70, -1)
        self.image_width_spin.set_tooltip_text("Image Width")
        ...
        self.image_height_spin.set_size_request(70, -1)
        self.image_height_spin.set_tooltip_text("Image Height")
        self.image_height_spin.connect("value-changed", self.on_image_height_changed)
        self.header.pack_start(self.image_height_spin)
        
        self.aspect_lock_toggle = Gtk.ToggleButton(label="🔒")
        self.aspect_lock_toggle.set_active(True)
        self.aspect_lock_toggle.set_tooltip_text("Lock Aspect Ratio")
    ...
    def setup_gestures(self):
        ...
        right_click_gesture.set_button(Gdk.BUTTON_SECONDARY)  # Right mouse button
        right_click_gesture.connect("pressed", self.on_right_click)
        self.drawing_area.add_controller(right_click_gesture)
    ...
    def create_context_menu(self):
        ...
        # Create menu model
        menu_model = Gio.Menu()
        menu_model.append("Insert Text Here", "app.insert_text_here")
        menu_model.append("Insert Image Here", "app.insert_image_here")
        
        menu.set_menu_model(menu_model)
        menu.set_parent(self.drawing_area)
        
        # Create actions
        insert_text_action = Gio.SimpleAction.new("insert_text_here", None)
        insert_text_action.connect("activate", self.on_context_insert_text)
        self.add_action(insert_text_action)
        
        insert_image_action = Gio.SimpleAction.new("insert_image_here", None)
        insert_image_action.connect("activate", self.on_context_insert_image)
    ...
    def on_right_click(self, gesture, n_press, x, y):
        ...
        # Store the click position in document coordinates
    ...
    def on_context_insert_text(self, action, parameter):
    ...
    def on_context_insert_image(self, action, parameter):
        """Insert image at the right-click position"""
        dialog = Gtk.FileDialog()
        dialog.open(self.win, None, self.on_context_add_image_finish)

    def on_context_add_image_finish(self, dialog, result):
        """Handle image selection for context menu insertion"""
        try:
            ...
            if file:
                ...
                # Use the stored context menu position
                img = OverlayImage(self.context_menu_x, self.context_menu_y, path, width, height)
                overlays.append(img)
                self.drawing_area.queue_draw()
    ...
    def on_click(self, gesture, n_press, x, y):
        ...
        if self.selected_overlay:
            ...
            self.drag_offset_x = adjusted_x - self.selected_overlay.x
            self.drag_offset_y = adjusted_y - self.selected_overlay.y
    ...
    def on_drag_begin(self, gesture, start_x, start_y):
        if self.selected_overlay:
            ...
            self.drag_start_overlay_x = self.selected_overlay.x
            self.drag_start_overlay_y = self.selected_overlay.y

    def on_drag_update(self, gesture, dx, dy):
        if self.selected_overlay and self.is_dragging:
            # Apply the delta directly to the overlay position with scaling
            # This makes dragging much smoother and more predictable
            scaled_dx = dx / self.scale
            scaled_dy = dy / self.scale
            
            # Apply rotation compensation if page is rotated
            if self.rotation != 0:
                rad = -self.rotation * (math.pi / 180)
                rot_dx = scaled_dx * math.cos(rad) - scaled_dy * math.sin(rad)
                rot_dy = scaled_dx * math.sin(rad) + scaled_dy * math.cos(rad)
                scaled_dx, scaled_dy = rot_dx, rot_dy
            
            # Update position incrementally from the starting position
            self.selected_overlay.x = self.drag_start_overlay_x + scaled_dx
            self.selected_overlay.y = self.drag_start_overlay_y + scaled_dy
    ...
    def save_pdf(self, path):
        ...
        # Get first page size for initial surface creation
        first_page = self.pdf_document.get_page(0)
        w, h = first_page.get_size()
        
        # Create PDF surface using direct Cairo
        ...
        for i in range(self.pdf_document.get_n_pages()):
            ...
            surface.set_size(page_w, page_h)
            ...
            # Move to next page (except for the last page)
            if i < self.pdf_document.get_n_pages() - 1:
                surface.show_page()
